...
class TwoFactorTest extends TestCase
{
    ...
    public function test_a_not_required_account_is_not_challenged_even_if_still_enrolled(): void
    {
        config(['mfa.enforced' => false]);
        // Force-enrolled earlier, but "required" is now off — no code prompt.
        $user = User::factory()->create([
            'password' => Hash::make('Sup3rSecret-Pass'),
    ...
    public function test_toggling_required_off_clears_a_stale_enrolment(): void
    {
        ...
        $this->assertTrue($target->hasTwoFactorEnabled());

        $this->actingAs($admin)->putJson("/api/users/{$target->id}", [
            'name' => $target->name, 'email' => $target->email,
            'role' => $target->role, 'is_active' => true, 'mfa_required' => false,
        ])->assertOk();
