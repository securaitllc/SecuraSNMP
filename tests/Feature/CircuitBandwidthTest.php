...
class CircuitBandwidthTest extends TestCase
{
    ...
    public function test_a_circuit_with_no_contract_speed_is_still_measured(): void
    {
        // A contract is the DENOMINATOR, not the measurement. Seven Massey circuits
        // carry no contract speed, and refusing to show their throughput hid a
        // number already collected from the port's own counters. The percentage is
        // what needs a contract — and it says so by being absent.
        $bytes = (int) (12 * 1_000_000 / 8 * 300);
        $circuit = $this->scenario(0, 0, $bytes, 0);
        $circuit->forceFill(['contract_down_mbps' => null, 'contract_up_mbps' => null])->save();

        $bw = (new CircuitBandwidth)->for($circuit->fresh());

        $this->assertTrue($bw['mapped']);
        $this->assertFalse($bw['contract_known']);
        $this->assertSame(12.0, $bw['down_mbps']);
        $this->assertNull($bw['down_pct'], 'no contract means no share-of-contract — not a fabricated one');
        $this->assertNull($bw['reason']);
    }
