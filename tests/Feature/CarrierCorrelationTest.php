...
class CarrierCorrelationTest extends TestCase
{
    ...
    public function test_a_hop_the_healthy_circuits_also_cross_cleanly_is_not_a_suspect(): void
    {
        // 18 September: ae31-527.bar3.Orlando1.Level3.net was named by 18 of 21
        // degraded Lumen circuits — and it was hop 5, at 0% loss, on the 141 that
        // were fine, because every probe leaves the same building and crosses it.
        // Counting only the sick circuits made the door everyone walks through look
        // like the fault, and it was quoted at a carrier whose counters were clean.
        $this->inPrefix('Lumen', '64.159.0.0', 3, 47, 90);
        $this->inPrefix('Lumen', '64.159.0.0', 4, 0);
        ...
        $this->assertFalse($hop['discriminating'], 'shared transit is not a suspect');
    }

    public function test_a_hop_that_is_lossy_for_the_healthy_too_still_counts(): void
    {
        // If the shared hop really is dropping, the healthy circuits crossing it see
        // loss there as well — and then it IS the answer.
        $this->inPrefix('Lumen', '64.159.0.0', 3, 47, 90);
