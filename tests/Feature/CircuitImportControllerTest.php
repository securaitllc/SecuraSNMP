...
class CircuitImportControllerTest extends TestCase
{
    ...
    public function test_unmatched_site_is_parked_under_the_holding_site(): void
    {
        $admin = User::factory()->admin()->create();
        ...
        $res->assertJsonPath('created_count', 0)->assertJsonPath('parked_count', 1);
        $res->assertJsonPath('parked_site.name', 'Unassigned Imports');

        // The circuit exists, monitoring, attached to the holding site — ready to
        // be reassigned to its real site.
        $holding = \App\Models\Site::firstWhere('name', 'Unassigned Imports');
        $this->assertNotNull($holding);
        $this->assertDatabaseHas('circuits', ['site_id' => $holding->id, 'monitored_ip' => '10.9.9.1', 'monitoring_enabled' => true]);
    }

    public function test_dry_run_parks_nothing_and_creates_no_holding_site(): void
    {
        $admin = User::factory()->admin()->create();

        $this->actingAs($admin)->postJson('/api/circuits/import', [
            'circuits' => [['site' => 'TX9999-SC777', 'ip' => '10.9.9.1', 'interface' => 'wan0']],
            'dry_run' => true,
        ])->assertOk()->assertJsonPath('parked_count', 1);
        $this->assertDatabaseMissing('sites', ['name' => 'Unassigned Imports']);
