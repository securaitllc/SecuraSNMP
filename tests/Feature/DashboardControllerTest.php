...
class DashboardControllerTest extends TestCase
{
    ...
    {
        // A SNMP-only "ec:…:Tunnel" rollup alarm has no per-tunnel row to list, so it
        // must NOT inflate tunnels_down (that produced a card reading "1" whose
        // drill-down was empty). It still shows in the alarms count.
        $site = Site::factory()->create();
        ...
        $res->assertJsonPath('counts.tunnels_down', 0);            // no listable tunnel
        $res->assertJsonPath('counts.active_alarms', 1);          // still an alarm

        // No SSH down tunnels → no type=tunnel entry, so count and content agree.
        $tunnelEntries = collect($res->json('alerts'))->flatMap(fn ($a) => $a['type'] === 'tunnel' ? [$a] : ($a['members'] ?? []))
            ->where('type', 'tunnel');
        $this->assertCount(0, $tunnelEntries);

        // Add a real SSH-down tunnel → count becomes 1 AND it's listable.
