...
class DashboardControllerTest extends TestCase
{
    ...
    public function test_a_manually_cleared_tunnel_drops_off_the_dashboard_but_a_live_one_stays(): void
    {
        // H1 stuck-bell fix: a tunnel still oper-down (peer decommissioned) whose alert
        // the NOC hand-cleared must fall out of the bell + tunnels_down, exactly like a
        // cleared circuit — while a genuinely-down tunnel with an open alert stays.
        $site = Site::factory()->create();
        $device = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect']);

        $cleared = \App\Models\Tunnel::factory()->create(['device_id' => $device->id, 'status' => 'down']);
        \App\Models\TunnelAlert::factory()->create(['tunnel_id' => $cleared->id, 'started_at' => now()->subHour(), 'ended_at' => now(), 'cleared_manually' => true]);

        $live = \App\Models\Tunnel::factory()->create(['device_id' => $device->id, 'status' => 'down']);
        \App\Models\TunnelAlert::factory()->create(['tunnel_id' => $live->id, 'started_at' => now()->subMinutes(5), 'ended_at' => null]);

        $viewer = User::factory()->create();
        $response = $this->actingAs($viewer)->getJson('/api/dashboard');

        $response->assertOk();
        $response->assertJsonPath('counts.tunnels_down', 1); // only the live one
        $tunnelAlerts = collect($response->json('alerts'))
            ->flatMap(fn ($a) => $a['type'] === 'incident' ? $a['members'] : [$a])
            ->where('type', 'tunnel');
        $this->assertCount(1, $tunnelAlerts, 'the cleared tunnel is gone, the live one remains');
    }

    public function test_interface_down_severity_follows_the_alert_access_is_warning_uplink_is_critical(): void
    {
        // X.733: a down access port is non-service-affecting (warning); a down
        // uplink/trunk takes a segment off the network (critical). The dashboard
        // must carry each alert's own severity, not blanket-critical every port.
        $device = Device::factory()->create(['site_id' => Site::factory()->create()->id]);
        $access = DeviceInterface::factory()->create(['device_id' => $device->id, 'if_name' => 'ge-0/0/5', 'status' => 'down', 'admin_status' => 'up', 'alarm_suppressed' => false]);
        $uplink = DeviceInterface::factory()->create(['device_id' => $device->id, 'if_name' => 'ge-0/0/0', 'status' => 'down', 'admin_status' => 'up', 'alarm_suppressed' => false]);
        \App\Models\InterfaceAlert::factory()->create(['device_interface_id' => $access->id, 'severity' => 'warning', 'ended_at' => null]);
        \App\Models\InterfaceAlert::factory()->create(['device_interface_id' => $uplink->id, 'severity' => 'critical', 'ended_at' => null]);
        $viewer = User::factory()->create();

        $alerts = collect($this->actingAs($viewer)->getJson('/api/dashboard')->json('alerts'))
            ->flatMap(fn ($a) => $a['type'] === 'incident' ? $a['members'] : [$a])
            ->where('type', 'interface')->keyBy('title');

        $this->assertSame('warning', $alerts["{$device->name} · ge-0/0/5"]['severity']);
        $this->assertSame('critical', $alerts["{$device->name} · ge-0/0/0"]['severity']);
    }
    ...
    public function test_tunnel_down_severity_reflects_wan_health(): void
    {
        // X.733: a tunnel-down is DEGRADED (warning) while the appliance still has a
        // surviving WAN path (SD-WAN reroutes), and service-affecting (critical) only
        // when it is fully isolated — no up tunnel and no up next-hop.
        $site = Site::factory()->create();

        $degraded = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'EDGE-DEGRADED']);
        Tunnel::factory()->create(['device_id' => $degraded->id, 'status' => 'down']);
        \App\Models\DeviceNextHop::create(['device_id' => $degraded->id, 'ip_address' => '10.0.0.1', 'interface' => 'wan0', 'status' => 'up']);

        $isolated = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'EDGE-ISOLATED']);
        Tunnel::factory()->create(['device_id' => $isolated->id, 'status' => 'down']);
        \App\Models\DeviceNextHop::create(['device_id' => $isolated->id, 'ip_address' => '10.0.0.2', 'interface' => 'wan0', 'status' => 'down']);

        $viewer = User::factory()->create();
        $tunnels = collect($this->actingAs($viewer)->getJson('/api/dashboard')->json('alerts'))
            ->flatMap(fn ($a) => $a['type'] === 'incident' ? ($a['members'] ?? []) : [$a])
            ->where('type', 'tunnel')->keyBy('device_name');

        $this->assertSame('warning', $tunnels['EDGE-DEGRADED']['severity']);
        $this->assertSame('critical', $tunnels['EDGE-ISOLATED']['severity']);
    }
    ...
    public function test_active_alerts_unify_every_alerting_resource(): void
    {
        ...
        // The bell badge counts the CORRELATED incidents (2), matching the list below.
        $response->assertJsonPath('counts.active_incidents', 2);
    ...
    public function test_a_flat_interface_alarm_carries_ack_clear_mute_actions(): void
    {
        // The dashboard flat list gets the same in-place NOC actions as the By-ISP view,
        // so an operator can ack/clear/mute without hopping to another surface.
        $admin = User::factory()->create(['role' => 'admin']);
        $device = Device::factory()->create();
        $iface = DeviceInterface::factory()->create(['device_id' => $device->id, 'if_name' => 'ge-0/0/28', 'status' => 'down', 'admin_status' => 'up', 'alarm_suppressed' => false]);
        $ia = \App\Models\InterfaceAlert::factory()->create(['device_interface_id' => $iface->id, 'ended_at' => null]);

        $alerts = collect($this->actingAs($admin)->getJson('/api/dashboard')->assertOk()->json('alerts'));
        $row = $alerts->firstWhere('type', 'interface');

        $this->assertNotNull($row);
        $this->assertSame("/api/interface-alerts/{$ia->id}/acknowledge", $row['actions']['ack']['url']);
        $this->assertSame("/api/interface-alerts/{$ia->id}/clear", $row['actions']['clear']['url']);
        $this->assertSame("/api/interfaces/{$iface->id}/suppress", $row['actions']['mute']['url']);
        $this->assertSame('ge-0/0/28', $row['if_name']);
    }

    public function test_an_incident_headline_carries_a_single_ports_deep_link_from_its_members(): void
    {
        // A device with an interface-down PLUS another alarm correlates into one incident.
        // The headline must still carry the one port's if_name so a click deep-links to it
        // instead of only opening the device (the grouping used to drop it).
        $admin = User::factory()->create(['role' => 'admin']);
        $device = Device::factory()->create();
        $iface = DeviceInterface::factory()->create(['device_id' => $device->id, 'if_name' => 'ge-0/0/9', 'status' => 'down', 'admin_status' => 'up', 'alarm_suppressed' => false]);
        \App\Models\InterfaceAlert::factory()->create(['device_interface_id' => $iface->id, 'ended_at' => null]);
        DeviceAlarm::factory()->create(['device_id' => $device->id, 'alarm_id' => 'ec:1:System', 'description' => 'NTP', 'severity' => 'warning', 'cleared_at' => null]);

        $alerts = collect($this->actingAs($admin)->getJson('/api/dashboard')->assertOk()->json('alerts'));
        $incident = $alerts->firstWhere('type', 'incident');

        $this->assertNotNull($incident, 'the two same-device alarms correlate into one incident');
        $this->assertSame('ge-0/0/9', $incident['if_name'], 'the single port deep-link survives the grouping');
    }

    public function test_a_hub_tunnel_alarm_for_a_down_spoke_is_not_shown_or_counted_on_the_dashboard(): void
    {
        // #063 spoke transport is down; #893 hub alarms "to_<spoke>" as a symptom. The
        // dashboard must attribute it to the spoke — the hub tunnel alarm must NOT appear
        // as a #893 incident nor inflate the bell (active_incidents).
        $admin = User::factory()->create(['role' => 'admin']);
        ...
        // Spoke transport genuinely down (its own alarm — stays visible under #063).
        ...
        // Hub symptom — note the SC63 vs SC063 zero-padding the resolver must bridge.
        DeviceAlarm::factory()->create(['device_id' => $hub->id, 'alarm_id' => 'ec:65537:to_LA0001-SC63_DIA2-Broadband1',
            'description' => 'Tunnel state is Down — to_LA0001-SC63_DIA2-Broadband1', 'severity' => 'warning', 'cleared_at' => null]);

        $body = $this->actingAs($admin)->getJson('/api/dashboard')->assertOk()->json();
        $alerts = collect($body['alerts']);

        $this->assertNull($alerts->firstWhere('site_id', $hubSite->id), 'the hub must not show the spoke-caused tunnel alarm');
        $this->assertNotNull($alerts->firstWhere('site_id', $spokeSite->id), 'the spoke keeps its own transport alarm');
    }
}
