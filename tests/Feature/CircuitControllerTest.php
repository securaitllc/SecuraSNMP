...
class CircuitControllerTest extends TestCase
{
    ...
    public function test_a_field_dispatch_eta_and_note_can_be_logged_and_cleared_on_the_circuit(): void
    {
        $analyst = \App\Models\User::factory()->create(['role' => 'analyst']);
        $circuit = \App\Models\Circuit::factory()->create();

        // Circuit-level, so no open outage is required or created (unlike the per-outage dispatch).
        $this->actingAs($analyst)->postJson("/api/circuits/{$circuit->id}/isp-dispatch", [
            'dispatch_at' => '2026-08-26T18:30:00Z', 'dispatch_note' => 'ISP tech ETA',
        ])->assertOk()->assertJsonPath('dispatch_note', 'ISP tech ETA');
        $this->assertNotNull($circuit->fresh()->dispatch_at);
        $this->assertSame(0, $circuit->alerts()->count());

        // Clearing the ETA also drops the note (a note with no ETA is meaningless).
        $this->actingAs($analyst)->postJson("/api/circuits/{$circuit->id}/isp-dispatch", ['dispatch_at' => null, 'dispatch_note' => 'stale'])->assertOk();
        $this->assertNull($circuit->fresh()->dispatch_at);
        $this->assertNull($circuit->fresh()->dispatch_note);

        // Viewer cannot set it.
        $viewer = \App\Models\User::factory()->create(['role' => 'viewer']);
        $this->actingAs($viewer)->postJson("/api/circuits/{$circuit->id}/isp-dispatch", ['dispatch_at' => '2026-08-26T18:30:00Z'])->assertForbidden();
    }
}
