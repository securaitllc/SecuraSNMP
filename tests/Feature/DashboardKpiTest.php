...
class DashboardKpiTest extends TestCase
{
    ...
    public function test_counts_carry_the_denominators_the_headline_numbers_need(): void
    {
        ...
        // Both devices HAVE been polled — the test is about the unreachable alarm,
        // not about a stopped poller. Without a reading they would now count as
        // unmeasured, which is a different (and equally real) condition.
        foreach ([$up, $down] as $d) {
            DeviceMetricHistory::create([
                'device_id' => $d->id, 'recorded_at' => now(), 'response_time_ms' => 3.1,
            ]);
        }
        Circuit::factory()->create([
            'site_id' => $site->id, 'status' => 'up',
    ...
    public function test_a_mapped_wan_port_totals_only_the_wan_traffic(): void
    {
        $site = Site::factory()->create();
        ...
        $this->assertSame(1000, $traffic['out_total']);
    }
}
