...
class SiteLeaseTest extends TestCase
{
    ...
    public function test_a_massey_owned_site_still_keeps_its_lease_end(): void
    {
        ...
        $site = Site::factory()->create(['occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addMonths(3)->toDateString()]);
    ...
    {
        $site = Site::factory()->create(['occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addDays(45)->toDateString()]);

        $this->assertSame(45, $site->daysToLeaseEnd());
        $this->assertSame('warning', $site->leaseStatus());                                   // ≤90d
        $this->assertSame('notice', $site->fresh()->fill(['lease_end_date' => OpsTime::now()->addDays(150)])->leaseStatus()); // ≤180d
        $this->assertSame('ok', $site->fresh()->fill(['lease_end_date' => OpsTime::now()->addDays(400)])->leaseStatus());
        $this->assertSame('expired', $site->fresh()->fill(['lease_end_date' => OpsTime::now()->subDay()])->leaseStatus());
    }
    ...
    public function test_owned_sites_are_included_in_the_expiring_scope(): void
    {
        ...
        $owned = Site::factory()->create(['occupancy' => 'owned', 'lease_end_date' => OpsTime::now()->subYear()->toDateString()]);
        $leased = Site::factory()->create(['occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addDays(30)->toDateString()]);
        Site::factory()->create(['occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addYears(5)->toDateString()]);
    ...
    public function test_the_site_leases_report_ranks_soonest_first_and_states_the_decision(): void
    {
        ...
        $urgent = Site::factory()->create(['name' => 'Urgent', 'occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addDays(20)->toDateString()]);
        Circuit::factory()->create(['site_id' => $urgent->id, 'contract_end_date' => OpsTime::now()->addYears(2)->toDateString()]);
        // Lease comfortably out, but a contract that runs past it.
        $overrun = Site::factory()->create(['name' => 'Overrun', 'occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addDays(300)->toDateString()]);
        Circuit::factory()->create(['site_id' => $overrun->id, 'contract_end_date' => OpsTime::now()->addYears(3)->toDateString()]);
        // Lease fine, contract ends inside it — the genuinely safe case.
        $safe = Site::factory()->create(['name' => 'Safe', 'occupancy' => 'owned', 'lease_end_date' => OpsTime::now()->addYears(4)->toDateString()]);
        Circuit::factory()->create(['site_id' => $safe->id, 'contract_end_date' => OpsTime::now()->addYears(1)->toDateString()]);
        // No circuits at all — nothing was compared, so nothing may be called safe.
        Site::factory()->create(['name' => 'No Contracts', 'occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addYears(3)->toDateString()]);
    ...
    public function test_urgent_rows_carry_a_severity_tone_for_the_table_to_paint(): void
    {
        ...
        Site::factory()->create(['occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->subDay()->toDateString()]);
    ...
    public function test_the_recommendation_colour_matches_what_the_sentence_says(): void
    {
        ...
        $site = Site::factory()->create(['occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->subDays(5)->toDateString()]);
        Circuit::factory()->create(['site_id' => $site->id, 'contract_end_date' => OpsTime::now()->addYears(1)->toDateString()]);
    ...
    public function test_a_report_never_calls_a_decision_safe_on_absent_data(): void
    {
        ...
        Site::factory()->create(['name' => 'Bare', 'occupancy' => 'leased', 'lease_end_date' => OpsTime::now()->addDays(4)->toDateString()]);
