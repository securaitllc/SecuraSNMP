...
class TunnelQualityAlarmTest extends TestCase
{
    ...
    public function test_an_alert_names_the_site_it_belongs_to(): void
    {
        // "Lumen — DSLTL18-23703944" identifies the circuit, not the location. An
        // operator triaging the list should not have to look the site up.
        $site = \App\Models\Site::factory()->create(['name' => 'TX0009-SC147']);
        $device = Device::factory()->create(['site_id' => $site->id]);
        DeviceAlarm::create([
            'device_id' => $device->id,
            'alarm_id' => 'ec:327686:to_AZURE-PRI_BulkApps',
            'description' => self::LATENCY,
            'severity' => 'warning',
            'first_seen_at' => now(),
            'active_on_device' => true,
        ]);
        ...
        $response->assertOk();
        $alert = collect($response->json('alerts'))->firstWhere('type', 'tunnel-quality');
        $this->assertSame('TX0009-SC147', $alert['site_name']);
    }
}
