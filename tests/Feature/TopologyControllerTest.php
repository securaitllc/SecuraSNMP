...
class TopologyControllerTest extends TestCase
{
    ...
    public function test_a_fully_unreachable_site_shows_the_edge_and_switch_down_not_connected(): void
    {
        // #165: both the SD-WAN appliance and the switch went unreachable (device-
        // unreachable), but the appliance's last poll said tunnels up and the ISP gateway
        // still pings, so the map drew the edge "up · 15 tunnels up" and an amber switch —
        // a dead site looking connected. The unreachable appliance must read DOWN with no
        // stale tunnel claim, and a standalone unreachable switch must read DOWN.
        $site = Site::factory()->create();
        Circuit::factory()->create(['site_id' => $site->id, 'status' => 'up', 'isp_name' => 'Comcast', 'circuit_id' => 'CKT-165']);
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sc165-sdw', 'status' => 'active']);
        $switch = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'sc165-sw', 'status' => 'active']);
        Tunnel::factory()->count(3)->create(['device_id' => $edge->id, 'status' => 'up']); // stale "all up"
        // Stale next-hop, frozen "reachable" from before the appliance went dark.
        $nh = \App\Models\DeviceNextHop::create(['device_id' => $edge->id, 'ip_address' => '173.8.45.6', 'interface' => 'wan0', 'status' => 'up']);
        foreach ([$edge, $switch] as $d) {
            DeviceAlarm::factory()->create(['device_id' => $d->id, 'alarm_id' => 'device-unreachable', 'severity' => 'critical', 'cleared_at' => null]);
        }

        $user = User::factory()->create();
        $body = $this->actingAs($user)->getJson("/api/sites/{$site->id}/topology")->assertOk()->json();

        $byId = collect($body['nodes'])->keyBy('id');
        $this->assertSame('down', $byId["ec-{$edge->id}"]['status'], 'unreachable appliance must read down, not up');
        $this->assertSame('appliance unreachable', $byId["ec-{$edge->id}"]['tunnels'], 'no stale tunnel count on an unreachable appliance');
        $this->assertSame('down', $byId["sw-{$switch->id}"]['status'], 'a standalone unreachable switch must read down, not warn');
        $this->assertSame('down', $byId["nh-{$nh->id}"]['status'], 'a stale next-hop on an unreachable appliance must read down, not up');

        // The overlay is drawn either as its own arc, or — where the underlay is
        // already on the map — carried by that underlay link. Either way it must not
        // read as up while the appliance is unreachable.
        $overlay = array_filter($body['edges'], fn ($e) => ($e['overlay'] ?? false) || ($e['carries_overlay'] ?? false));
        $this->assertNotEmpty($overlay);
        foreach ($overlay as $e) {
            $this->assertNotSame('up', $e['status'], 'SD-WAN overlay must not read up when the appliance is unreachable');
        }
    }
    ...
    public function test_ip_sla_down_on_a_wan_is_a_degraded_incident_even_though_the_next_hop_is_reachable(): void
    {
        ...
        $this->assertStringContainsString('backup WAN', $res->json('incident.summary'));
        // Remediation must be modem-reboot-then-ISP, NOT "check the SilverPeak→modem
        // cable" — the link is up and the gateway (often at the ISP head-end) ARPs.
        $action = $res->json('incident.action');
        $this->assertStringContainsString('Reboot the cable modem', $action);
        $this->assertStringContainsString('ISP ticket', $action);
        $this->assertStringNotContainsString('WAN port', $action);
    ...
    public function test_snmp_tunnel_alarm_surfaces_even_when_the_ssh_table_shows_all_up(): void
    {
        ...
        // The label stays short — it sits inside a node card and was being clipped.
        $this->assertSame('tunnels down', $edgeNode['tunnels']);
    }

    public function test_a_per_tunnel_snmp_alarm_names_the_hub_that_is_down(): void
    {
        // The per-tunnel alarm carries the peer, so the count and the hub row are both
        // knowable. Matching only the ':Tunnel' rollup missed these entirely: an
        // appliance with a live critical tunnel alarm drew an all-green hub grid and an
        // 'up' SD-WAN overlay line.
        $site = Site::factory()->create();
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        foreach (range(1, 5) as $i) {
            Tunnel::factory()->create(['device_id' => $edge->id, 'status' => 'up', 'hub' => 'AZURE', 'last_checked_at' => now()->subHours(3)]);
        }
        \App\Models\DeviceAlarm::create([
            'device_id' => $edge->id,
            'alarm_id' => 'ec:65537:to_AZURE-PRI_Broadband1-DIA1',
            'severity' => 'critical',
            'description' => 'Tunnel state is Down — to_AZURE-PRI_Broadband1-DIA1',
            'first_seen_at' => now(),
        ]);

        $res = $this->actingAs(User::factory()->create())->getJson("/api/sites/{$site->id}/topology");
        $res->assertOk();

        $edgeNode = collect($res->json('nodes'))->firstWhere('type', 'edge');
        // AZURE-PRI rolls up to the AZURE hub row the SSH sweep built.
        // Domain fields plus provenance — not the literal array, which now also carries
        // the source and the time of the reading.
        $this->assertCount(1, $edgeNode['tunnel_hubs']);
        $this->assertSame(
            ['hub' => 'AZURE', 'total' => 5, 'down' => 1, 'measured' => true],
            array_intersect_key($edgeNode['tunnel_hubs'][0], array_flip(['hub', 'total', 'down', 'measured'])),
        );
        $this->assertSame('1/5 tunnels down', $edgeNode['tunnels']);
        // Detail is known, so there is nothing to caveat as pending refresh.
        $this->assertFalse($edgeNode['tunnels_stale']);
    }

    public function test_a_latency_breach_does_not_paint_a_hub_down(): void
    {
        $site = Site::factory()->create();
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        Tunnel::factory()->create(['device_id' => $edge->id, 'status' => 'up', 'hub' => 'AZURE', 'last_checked_at' => now()]);
        \App\Models\DeviceAlarm::create([
            'device_id' => $edge->id,
            'alarm_id' => 'ec:327686:to_AZURE-PRI_Managment',
            'severity' => 'warning',
            'description' => 'The second-sampled average latency exceeds the threshold of 1000ms',
            'first_seen_at' => now(),
        ]);
        ...
        $edgeNode = collect($res->json('nodes'))->firstWhere('type', 'edge');
        // Degraded-but-up. These are open constantly; treating them as outages would
        // paint every hub red permanently.
        $this->assertCount(1, $edgeNode['tunnel_hubs']);
        $this->assertSame(
            ['hub' => 'AZURE', 'total' => 1, 'down' => 0, 'measured' => true],
            array_intersect_key($edgeNode['tunnel_hubs'][0], array_flip(['hub', 'total', 'down', 'measured'])),
        );
        $this->assertSame('1 tunnels up', $edgeNode['tunnels']);
    }
    ...
    public function test_analyst_can_save_and_reset_topology_positions_and_everyone_sees_them(): void
    {
        $site = Site::factory()->create();
        ...
        // Analyst saves an arrangement.
        $this->actingAs($analyst)->postJson("/api/sites/{$site->id}/topology/positions", [
            'positions' => ["sw-{$sw->id}" => ['x' => 420.5, 'y' => 88], 'bad key' => ['x' => 1, 'y' => 1]],
        ])->assertOk();

        // Everyone (even a viewer) gets the saved positions; the malformed key is dropped.
        $positions = $this->actingAs($viewer)->getJson("/api/sites/{$site->id}/topology")->json('positions');
        $this->assertSame(420.5, $positions["sw-{$sw->id}"]['x']);
        $this->assertArrayNotHasKey('bad key', $positions);

        // Viewer cannot save.
        $this->actingAs($viewer)->postJson("/api/sites/{$site->id}/topology/positions", [
            'positions' => ["sw-{$sw->id}" => ['x' => 1, 'y' => 2]],
        ])->assertStatus(403);

        // Reset clears them.
        $this->actingAs($analyst)->deleteJson("/api/sites/{$site->id}/topology/positions")->assertOk();
        $this->assertEmpty($this->actingAs($viewer)->getJson("/api/sites/{$site->id}/topology")->json('positions'));
    }

    public function test_switches_are_tiered_by_hop_distance_from_the_core(): void
    {
        // core (col 3) → floor switch (col 4) → access switch (col 5), so a big fabric
        // reads cores → floors → access instead of piling every switch in one column.
        $site = Site::factory()->create();
        Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        $core = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'CORE']);
        $floor = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'FLOOR']);
        $access = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'ACCESS']);
        // The core is the fabric root: it uplinks to the SD-WAN (router LLDP).
        \App\Models\LldpNeighbor::create(['device_id' => $core->id, 'local_port' => 'ge-0/0/47', 'remote_sysname' => 'sdw', 'remote_port' => 'tlan0', 'neighbor_type' => 'router', 'remote_device_id' => null, 'last_seen_at' => now()]);
        // core ← floor ← access, each resolved by LLDP.
        \App\Models\LldpNeighbor::create(['device_id' => $floor->id, 'local_port' => 'xe-0/1/0', 'remote_sysname' => 'CORE', 'remote_port' => 'xe-0/0/1', 'remote_device_id' => $core->id, 'last_seen_at' => now()]);
        \App\Models\LldpNeighbor::create(['device_id' => $access->id, 'local_port' => 'ge-0/0/48', 'remote_sysname' => 'FLOOR', 'remote_port' => 'ge-0/0/1', 'remote_device_id' => $floor->id, 'last_seen_at' => now()]);
        $user = User::factory()->create();

        $nodes = collect($this->actingAs($user)->getJson("/api/sites/{$site->id}/topology")->json('nodes'))->keyBy('device_id');

        $this->assertSame(3, $nodes[$core->id]['col']);
        $this->assertSame(4, $nodes[$floor->id]['col']);
        $this->assertSame(5, $nodes[$access->id]['col']);
    }

    public function test_a_switch_with_no_lldp_is_not_falsely_wired_to_the_sdwan_and_firewall(): void
    {
        // HQ #893 shape: cores are LLDP-connected, but an access switch has LLDP
        // disabled (empty neighbours). It used to be treated as its own cluster core
        // and wired to BOTH the SD-WAN and the firewall — false links. It must instead
        // get a single dashed "uplink unknown" link to the real fabric core.
        $site = Site::factory()->create();
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        $fw = Device::factory()->create(['site_id' => $site->id, 'role' => 'firewall', 'name' => 'fw']);
        $core = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'CORE']);
        $orphan = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'ORPHAN']);
        // The core sees the SD-WAN router over LLDP → it's the real fabric core.
        \App\Models\LldpNeighbor::create(['device_id' => $core->id, 'local_port' => 'ge-0/0/47', 'remote_sysname' => 'sdw', 'remote_port' => 'tlan0', 'neighbor_type' => 'router', 'remote_device_id' => null, 'last_seen_at' => now()]);
        // $orphan has NO LldpNeighbor rows at all.
        $user = User::factory()->create();

        $edges = collect($this->actingAs($user)->getJson("/api/sites/{$site->id}/topology")->json('edges'));

        // No false SD-WAN or firewall link to the orphan.
        $this->assertFalse($edges->contains(fn ($e) => $e['from'] === "ec-{$edge->id}" && $e['to'] === "sw-{$orphan->id}"), 'orphan must not link to the SD-WAN');
        $this->assertFalse($edges->contains(fn ($e) => $e['from'] === "sw-{$orphan->id}" && $e['to'] === "fw-{$fw->id}"), 'orphan must not trunk to the firewall');
        // One dashed, uncertain uplink to the core instead.
        $uplink = $edges->first(fn ($e) => $e['from'] === "sw-{$orphan->id}" && $e['to'] === "sw-{$core->id}");
        $this->assertNotNull($uplink, 'orphan must get a dashed uplink to the fabric core');
        $this->assertTrue($uplink['dashed'] ?? false);
    }

    public function test_a_lone_switch_with_no_resolved_core_is_not_left_floating(): void
    {
        // Regression: when NO switch has a resolved uplink there's no core to anchor an
        // orphan to. The lone switch must still get an inferred edge uplink, not render
        // with zero edges (a floating, disconnected node).
        $site = Site::factory()->create();
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        $sw = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'LONE']);
        // No LLDP at all on the switch → no resolved uplink → primaryCore stays null.
        $user = User::factory()->create();

        $edges = collect($this->actingAs($user)->getJson("/api/sites/{$site->id}/topology")->json('edges'));

        $touchesSwitch = $edges->contains(fn ($e) => $e['from'] === "sw-{$sw->id}" || $e['to'] === "sw-{$sw->id}");
        $this->assertTrue($touchesSwitch, 'a lone switch must not float — it falls back to an inferred uplink');
    }

    public function test_a_firewall_cabled_switch_is_not_treated_as_an_uplink_unknown_orphan(): void
    {
        // Regression: a switch whose only resolved managed LLDP neighbour is the firewall
        // has a real uplink. It must NOT also get a contradictory dashed "no LLDP" line.
        $site = Site::factory()->create();
        Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        $fw = Device::factory()->create(['site_id' => $site->id, 'role' => 'firewall', 'name' => 'fw']);
        $core = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'CORE']);
        $sw = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'FWSW']);
        // Core uplinks to the SD-WAN → it's the primary core.
        \App\Models\LldpNeighbor::create(['device_id' => $core->id, 'local_port' => 'ge-0/0/47', 'remote_sysname' => 'sdw', 'remote_port' => 'tlan0', 'neighbor_type' => 'router', 'remote_device_id' => null, 'last_seen_at' => now()]);
        // FWSW's only resolved managed neighbour is the firewall.
        \App\Models\LldpNeighbor::create(['device_id' => $sw->id, 'local_port' => 'ge-0/0/1', 'remote_sysname' => 'fw', 'remote_port' => 'port5', 'remote_device_id' => $fw->id, 'last_seen_at' => now()]);
        $user = User::factory()->create();

        $edges = collect($this->actingAs($user)->getJson("/api/sites/{$site->id}/topology")->json('edges'));

        $dashedToCore = $edges->contains(fn ($e) => $e['from'] === "sw-{$sw->id}" && $e['to'] === "sw-{$core->id}" && ($e['dashed'] ?? false));
        $this->assertFalse($dashedToCore, 'a firewall-cabled switch is not an uplink-unknown orphan');
    }

    public function test_a_leaf_switch_with_only_endpoint_lldp_is_still_not_a_core(): void
    {
        // The real "Ross" bug: the access switch advertises LLDP to a phone/AP but its
        // uplink to a managed switch hasn't resolved. It has LLDP, so it's not "silent",
        // yet it's still a leaf — it must NOT be wired to the SD-WAN or firewall.
        $site = Site::factory()->create();
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        $fw = Device::factory()->create(['site_id' => $site->id, 'role' => 'firewall', 'name' => 'fw']);
        $core = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'CORE']);
        $ross = Device::factory()->create(['site_id' => $site->id, 'role' => 'switch', 'name' => 'ROSS']);
        // Core is the real fabric core (SD-WAN router uplink).
        \App\Models\LldpNeighbor::create(['device_id' => $core->id, 'local_port' => 'ge-0/0/47', 'remote_sysname' => 'sdw', 'remote_port' => 'tlan0', 'neighbor_type' => 'router', 'remote_device_id' => null, 'last_seen_at' => now()]);
        // Ross only sees a phone — LLDP present, but no resolved switch/edge uplink.
        \App\Models\LldpNeighbor::create(['device_id' => $ross->id, 'local_port' => 'ge-0/0/5', 'remote_sysname' => 'Polycom-1234', 'remote_port' => 'port1', 'neighbor_type' => 'other', 'remote_device_id' => null, 'last_seen_at' => now()]);
        $user = User::factory()->create();

        $edges = collect($this->actingAs($user)->getJson("/api/sites/{$site->id}/topology")->json('edges'));

        $this->assertFalse($edges->contains(fn ($e) => $e['from'] === "ec-{$edge->id}" && $e['to'] === "sw-{$ross->id}"), 'a leaf must not link to the SD-WAN');
        $this->assertFalse($edges->contains(fn ($e) => $e['from'] === "sw-{$ross->id}" && $e['to'] === "fw-{$fw->id}"), 'a leaf must not trunk to the firewall');
        $this->assertTrue($edges->contains(fn ($e) => $e['from'] === "sw-{$ross->id}" && $e['to'] === "sw-{$core->id}" && ($e['dashed'] ?? false)), 'a leaf gets a dashed uplink to the core');
    }
    ...
    public function test_org_roll_up_infers_a_branch_hub_from_its_tunnels_when_none_is_set(): void
    {
        // A branch with no hub_site_id/hub relation used to fall into "Unassigned", even
        // though its EdgeConnect builds tunnels to the hub. Infer the hub from those
        // tunnels so the branch groups under it and is flagged as inferred.
        \Illuminate\Support\Facades\Cache::flush();
        $user = User::factory()->create();

        $hub = Site::factory()->create(['name' => '#893 HQ Orlando FL', 'site_type' => 'hub']);
        Device::factory()->create(['site_id' => $hub->id, 'role' => 'edgeconnect', 'name' => 'FL0001-HQ-PRI_SDW']);

        $branch = Site::factory()->create(['name' => '#052 North Tallahassee FL', 'site_type' => 'branch', 'hub_site_id' => null]);
        $edge = Device::factory()->create(['site_id' => $branch->id, 'role' => 'edgeconnect', 'name' => 'FL0036-SC052_SDW']);
        // The bonded tunnels to the hub carry its label (suffix already stripped).
        Tunnel::factory()->count(4)->create(['device_id' => $edge->id, 'status' => 'up', 'hub' => 'FL0001-HQ']);

        $sites = $this->actingAs($user)->getJson('/api/topology')->assertOk()->json('sites');
        $row = collect($sites)->firstWhere('id', $branch->id);

        $this->assertSame([$hub->id], $row['hub_site_ids'], 'branch should be homed to the inferred hub');
        $this->assertTrue($row['hub_inferred'], 'the branch is flagged as hub-inferred');
    }

    public function test_org_roll_up_does_not_infer_a_hub_for_a_mesh_only_branch(): void
    {
        // Tunnels to non-hub peers (mesh) must NOT resolve to a hub — the branch stays
        // genuinely unassigned rather than being homed to a random peer.
        \Illuminate\Support\Facades\Cache::flush();
        $user = User::factory()->create();

        Site::factory()->create(['name' => '#893 HQ Orlando FL', 'site_type' => 'hub'])
            ->devices()->create(['name' => 'FL0001-HQ-PRI_SDW', 'role' => 'edgeconnect', 'vendor' => 'silverpeak', 'model' => 'EC', 'ip_address' => '10.0.0.9', 'snmp_version' => 'v2c']);

        $branch = Site::factory()->create(['site_type' => 'branch', 'hub_site_id' => null]);
        $edge = Device::factory()->create(['site_id' => $branch->id, 'role' => 'edgeconnect', 'name' => 'branch-sdw']);
        Tunnel::factory()->create(['device_id' => $edge->id, 'status' => 'up', 'hub' => 'GA0001-SC34']); // a mesh peer, not the hub

        $sites = $this->actingAs($user)->getJson('/api/topology')->assertOk()->json('sites');
        $row = collect($sites)->firstWhere('id', $branch->id);

        $this->assertSame([], $row['hub_site_ids']);
        $this->assertFalse($row['hub_inferred']);
    }
}
