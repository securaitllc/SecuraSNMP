...
class TopologyControllerTest extends TestCase
{
    ...
    public function test_a_stale_tunnel_rollup_alarm_does_not_inflate_the_count_over_the_ssh_table(): void
    {
        // 3 SSH-polled tunnels, all up (failed over) — but a generic ec:65537:Tunnel
        // rollup alarm lingers. The per-tunnel table wins: 0 down, so the by-hub
        // panel and the count agree.
        $site = Site::factory()->create();
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect', 'name' => 'sdw']);
        foreach (range(1, 3) as $i) {
            Tunnel::factory()->create(['device_id' => $edge->id, 'status' => 'up', 'hub' => 'HQ']);
        }
        ...
        $this->assertStringContainsString('3 tunnels up', (string) $edgeNode['tunnels']);   // not "1/3 down"
        $this->assertSame(0, collect($edgeNode['tunnel_hubs'])->sum('down'));
    }
