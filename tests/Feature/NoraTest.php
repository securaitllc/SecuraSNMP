...
class NoraTest extends TestCase
{
    ...
    public function test_the_message_actually_builds_with_a_valid_message_id(): void
    {
        // Mail::fake() never constructs the Symfony message, so it cannot see a bad
        // header. Symfony wants the addr-spec BARE and adds the angle brackets itself;
        // handing it "<id@host>" throws RfcComplianceException — which would have 500'd
        // the first real send the moment NORA_ENABLED was turned on. Use the array
        // transport so the headers are genuinely built.
        config([
            'mail.default' => 'array',
            'nora.enabled' => true,
            'nora.address' => 'NORA@lumen.com',
            'nora.from_address' => 'noc@example.test',
        ]);
        $c = $this->lumenCircuit();
        $u = User::factory()->create(['role' => 'analyst']);

        $this->actingAs($u)
            ->postJson("/api/circuits/{$c->id}/nora/send", ['kind' => 'open_ticket'])
            ->assertCreated();
        ...
        // The header on the wire is exactly what we stored, brackets included — that
        // is what NORA's reply quotes back in In-Reply-To.
        $this->assertSame($row->message_id, $sent->getHeaders()->get('Message-ID')->getBodyAsString());
    }
    ...
    public function test_utilization_asks_for_the_windows_nora_reports(): void
    {
        // Nodus measures throughput on the APPLIANCE WAN PORT, not the circuit — the
        // caveat is printed in the UI. Lumen's own figure is the carrier-side truth,
        // and it is what settles whether a site is genuinely oversubscribed.
        $m = (new NoraComposer)->compose($this->lumenCircuit(), 'utilization');

        $this->assertStringContainsString('Traffic utilization', $m['subject']);
        $this->assertStringContainsString('3 hours, 24 hours, 7 days and 30 days', $m['body']);
        $this->assertStringContainsString('445453113', $m['body']);
    }

    public function test_maintenance_inquiry_covers_past_and_future(): void
    {
        // Feeds the maintenance suppression Nodus already has but nobody populates.
        // Alarming on work the carrier already told us about is noise we can delete.
        $m = (new NoraComposer)->compose($this->lumenCircuit(), 'maintenance');

        $this->assertStringContainsString('GCR', $m['body']);
        $this->assertStringContainsString('last 7 days and the next 14 days', $m['body']);
    }

    public function test_open_tickets_query_needs_no_ticket_number(): void
    {
        // Catches tickets someone opened outside Nodus, so isp_ticket can be reconciled.
        $m = (new NoraComposer)->compose($this->lumenCircuit(), 'open_tickets');

        $this->assertStringContainsString('list any open tickets', $m['body']);
    }

    public function test_a_note_carries_the_current_evidence_and_the_traced_hop(): void
    {
        $c = $this->lumenCircuit(['status' => 'up', 'loss_polls_pct' => 45, 'loss_peak_pct' => 80, 'isp_ticket' => '35327405']);
        CircuitPathProbe::create([
            'circuit_id' => $c->id, 'target' => $c->monitored_ip, 'trigger' => 'auto', 'tool' => 'mtr',
            'cycles' => 20, 'hops' => [], 'worst_hop' => 5, 'worst_hop_host' => 'ae31-527.bar3.orlando1.level3.net',
            'worst_hop_loss_pct' => 44, 'ran_at' => now()->subHour(),
        ]);

        $m = (new NoraComposer)->compose($c->fresh(), 'add_note');

        $this->assertStringContainsString('35327405', $m['subject']);
        $this->assertStringContainsString('lossy on 45% of polls', $m['body']);
        $this->assertStringContainsString('ae31-527.bar3.orlando1.level3.net', $m['body']);
        $this->assertStringContainsString('44% at that hop', $m['body']);
    }
}
