...
class AlertNotifierTest extends TestCase
{
    ...
    public function test_a_failed_send_does_not_write_the_webhook_secret_into_the_log(): void
    {
        // For Slack and Teams the webhook URL *is* the credential. The HTTP client
        // quotes the full request URL in its failure message, so one timeout used to
        // persist the secret into notification_logs.error — which the notifications
        // API hands back to anyone who can read the log.
        $secret = 'https://hooks.slack.com/services/T00000000/B00000000/SuperSecretToken1234';

        Http::fake(fn () => throw new RuntimeException("cURL error 28: Operation timed out for {$secret}"));
        ...
        $error = (string) NotificationLog::where('status', 'failed')->value('error');

        $this->assertNotSame('', $error, 'the failure should still be recorded');
        $this->assertStringNotContainsString('SuperSecretToken1234', $error);
        $this->assertStringNotContainsString('/services/', $error);
        $this->assertStringContainsString('timed out', $error, 'the reason must survive redaction');
    }
    ...
    public function test_health_reports_when_nothing_can_be_notified(): void
    {
        ...
        $this->actingAs($user)->getJson('/api/health/pollers')
            ...
            ->assertJsonPath('alerting.warning', 'No notification channel is enabled. Alarms are raised and recorded, but nobody is told — this dashboard is the only way to find out.');
    ...
    public function test_a_delivery_that_reached_nobody_is_retried(): void
    {
        // A 503 from Teams or a DNS blip must not silently lose a critical alert.
        // AlertNotifier swallows the send error by design, so the job itself has to
        // fail — otherwise $tries is decoration and the queue never retries.
        // A loopback target makes the send-time SSRF guard throw — the same failure
        // shape as a 503 or a DNS blip, without racing the suite's catch-all Http fake.
        NotificationChannel::factory()->create([
            'type' => 'webhook',
            'min_severity' => 'warning',
            'config' => ['url' => 'http://127.0.0.1/hook'],
        ...
        $job = new DeliverAlertNotification('open', 'critical', 'Circuit down', 'Site: Jacksonville');
        $job->setJob(Mockery::mock(QueueJob::class));   // a real queued job, not sync
        ...
        try {
            $job->handle();
            $this->fail('A delivery that reached nobody must throw so the queue retries it.');
        } catch (RuntimeException $e) {
            $this->assertStringContainsString('every channel', $e->getMessage());
        }

        $this->assertDatabaseHas('notification_logs', ['status' => 'failed']);
    }

    public function test_a_partly_delivered_alert_is_not_retried(): void
    {
        // Somebody WAS told. Retrying re-runs the whole fan-out, so the channel that
        // already worked would page the NOC three times for one alarm — during exactly
        // the outage that broke the other channel.
        NotificationChannel::factory()->create([
            'type' => 'webhook',
            'min_severity' => 'warning',
            'config' => ['url' => 'https://hooks.example.test/hook'],   // faked 200
        ]);
        NotificationChannel::factory()->create([
            'type' => 'webhook',
            'min_severity' => 'warning',
            'config' => ['url' => 'http://127.0.0.1/hook'],             // refused at send time
        ]);

        $job = new DeliverAlertNotification('open', 'critical', 'Circuit down', 'Site: Jacksonville');
        $job->setJob(Mockery::mock(QueueJob::class));

        $job->handle();   // must NOT throw

        $this->assertDatabaseHas('notification_logs', ['status' => 'sent']);
        $this->assertDatabaseHas('notification_logs', ['status' => 'failed']);
    }

    public function test_a_failing_channel_never_breaks_the_poller_sweep(): void
    {
        // The observers dispatch this from inside a poll loop, and on the sync
        // connection SyncQueue rethrows into the sweep. A dead webhook must not take
        // circuits:monitor down with it.
        NotificationChannel::factory()->create([
            'type' => 'webhook',
            'min_severity' => 'warning',
            'config' => ['url' => 'http://127.0.0.1/hook'],
        ...
        CircuitAlert::create(['circuit_id' => $circuit->id, 'started_at' => now()]);   // no throw

        $this->assertDatabaseHas('notification_logs', ['status' => 'failed']);
    }

    public function test_an_enabled_channel_with_no_destination_cannot_notify(): void
    {
        // "Enabled" is not "usable". A blank destination throws on every send while
        // still counting as configured — ready by the old measure, silent in fact.
        $user = User::factory()->create(['role' => 'admin']);
        NotificationChannel::create([
            'name' => 'Broken email', 'type' => 'email', 'enabled' => true,
            'min_severity' => 'warning', 'config' => ['email' => ''],
        ]);

        $this->actingAs($user)->getJson('/api/health/pollers')
            ->assertJsonPath('alerting.channels_enabled', 1)
            ->assertJsonPath('alerting.channels_usable', 0)
            ->assertJsonPath('alerting.can_notify', false)
            ->assertJsonPath('alerting.warning', 'Every enabled notification channel is missing its destination, so no alarm can be delivered.');
    }

    public function test_a_critical_only_channel_says_warnings_reach_nobody(): void
    {
        $user = User::factory()->create(['role' => 'admin']);
        NotificationChannel::create([
            'name' => 'Pager', 'type' => 'teams', 'enabled' => true,
            'min_severity' => 'critical', 'config' => ['webhook_url' => 'https://example.logic.azure.com/x'],
        ]);

        $this->actingAs($user)->getJson('/api/health/pollers')
            ->assertJsonPath('alerting.can_notify', true)
            ->assertJsonPath('alerting.warning', 'Only critical alarms are delivered. Warnings are recorded but nobody is told.');
    }
}
