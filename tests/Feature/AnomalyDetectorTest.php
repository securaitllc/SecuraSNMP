...
class AnomalyDetectorTest extends TestCase
{
    ...
    public function test_the_hour_matched_baseline_survives_midnight(): void
    {
        // This is why the suite failed the first time it ran after 00:00 UTC: the
        // hour filter measured 23:00 as twenty-three hours from 00:00, discarded the
        // whole evening baseline, and left the detector comparing the spike against
        // itself. Detection went dead for an hour every night and looked like calm.
        Carbon::setTestNow(Carbon::parse('2026-05-14 00:05:00'));
        ...
        $this->assertNotNull(
            Anomaly::open()->where('metric', 'throughput')->where('entity_id', $if->id)->first(),
            'a 10x spike five minutes after midnight is the same anomaly it is at noon'
        );
    ...
    public function test_a_port_that_always_discards_still_raises_an_anomaly(): void
    {
        // The HQ firewall LAG tail-dropped ~1.8% of every packet for five weeks. Because
        // it had ALWAYS done so, the port's own median was ~14,700 a poll, the z-score
        // sat at zero, and no row ever appeared. A flat-but-terrible port is exactly the
        // case the relative test is blind to.
        $if = DeviceInterface::factory()->create(['status' => 'up', 'speed_bps' => 0]);
        foreach (range(1, 23) as $i) {
            InterfaceMetricHistory::create(['device_interface_id' => $if->id, 'recorded_at' => now()->subMinutes(25 - $i), 'status' => 'up',
                'in_octets_delta' => 0, 'out_octets_delta' => 0, 'in_discards_delta' => 0,
                // Dead flat at a high rate: no deviation to detect, ever.
                'out_discards_delta' => 14_700 + ($i % 3) * 10]);
        }
        ...
        $this->assertDatabaseHas('anomalies', [
            ...
            'direction' => 'spike', 'resolved_at' => null,
            // Discards should be zero — the port's own history is not the expectation.
            'baseline' => 0.0,
        ]);
    }

    public function test_discards_a_multicast_count_explains_are_not_congestion(): void
    {
        // An EdgeConnect WAN port on a shared bridge declines multicast by design, and
        // the counters come out identical:  RX mcast 1759152 / RX discards 1759152.
        // Reading the discard counter alone made 150 of 172 open findings on this fleet
        // artifacts of normal multicast handling.
        $if = DeviceInterface::factory()->create(['status' => 'up', 'speed_bps' => 0]);
        foreach (range(1, 23) as $i) {
            InterfaceMetricHistory::create(['device_interface_id' => $if->id, 'recorded_at' => now()->subMinutes(25 - $i), 'status' => 'up',
                'in_octets_delta' => 0, 'out_octets_delta' => 0, 'out_discards_delta' => 0,
                'in_discards_delta' => 4_000, 'in_multicast_delta' => 4_000]);
        }

        (new AnomalyDetector)->scanInterfaces();

        $this->assertDatabaseMissing('anomalies', ['entity_type' => 'interface', 'entity_id' => $if->id, 'metric' => 'discards']);
    }

    public function test_discards_beyond_the_multicast_count_still_fire(): void
    {
        // Only what multicast accounts for is excused. A port dropping far more than it
        // received in multicast is dropping traffic, and must still surface.
        $if = DeviceInterface::factory()->create(['status' => 'up', 'speed_bps' => 0]);
        foreach (range(1, 23) as $i) {
            InterfaceMetricHistory::create(['device_interface_id' => $if->id, 'recorded_at' => now()->subMinutes(25 - $i), 'status' => 'up',
                'in_octets_delta' => 0, 'out_octets_delta' => 0, 'out_discards_delta' => 0,
                'in_discards_delta' => 20_000, 'in_multicast_delta' => 4_000]);
        }

        (new AnomalyDetector)->scanInterfaces();

        $this->assertDatabaseHas('anomalies', [
            'entity_type' => 'interface', 'entity_id' => $if->id, 'metric' => 'discards', 'resolved_at' => null,
        ]);
    }

    public function test_a_port_discarding_below_the_floor_stays_quiet(): void
    {
        // 178 of 24,268 fleet interfaces sit in the 50-499 band. They are ordinary and
        // must not each become a row, or the absolute floor just trades one blind spot
        // for a feed nobody reads.
        $if = DeviceInterface::factory()->create(['status' => 'up', 'speed_bps' => 0]);
        foreach (range(1, 23) as $i) {
            InterfaceMetricHistory::create(['device_interface_id' => $if->id, 'recorded_at' => now()->subMinutes(25 - $i), 'status' => 'up',
                'in_octets_delta' => 0, 'out_octets_delta' => 0, 'in_discards_delta' => 0, 'out_discards_delta' => 343]);
        }
    ...
    public function test_one_bad_poll_on_a_clean_port_is_not_sustained(): void
    {
        // A single burst is not a fault. All of the last SUSTAIN polls must clear the
        // floor, or a momentary spike on an otherwise clean port raises a row.
        $if = DeviceInterface::factory()->create(['status' => 'up', 'speed_bps' => 0]);
        foreach (range(1, 22) as $i) {
            InterfaceMetricHistory::create(['device_interface_id' => $if->id, 'recorded_at' => now()->subMinutes(25 - $i), 'status' => 'up',
                'in_octets_delta' => 0, 'out_octets_delta' => 0, 'in_discards_delta' => 0, 'out_discards_delta' => 0]);
        }
        InterfaceMetricHistory::create(['device_interface_id' => $if->id, 'recorded_at' => now(), 'status' => 'up',
            'in_octets_delta' => 0, 'out_octets_delta' => 0, 'in_discards_delta' => 0, 'out_discards_delta' => 90_000]);
    ...
    public function test_a_sustained_cpu_spike_opens_a_device_anomaly_above_the_floor(): void
    {
        ...
        foreach (range(1, 22) as $i) {
            DeviceHealthHistory::create(['device_id' => $device->id, 'recorded_at' => now()->subMinutes(60 - $i * 2),
                'cpu_pct' => 8 + ($i % 3), 'mem_pct' => 40, 'temperature_c' => 30]);
        }
        foreach ([88, 90, 86] as $j => $v) {
            DeviceHealthHistory::create(['device_id' => $device->id, 'recorded_at' => now()->subMinutes(4 - $j),
