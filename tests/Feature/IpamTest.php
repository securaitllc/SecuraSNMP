...
class IpamTest extends TestCase
{
    ...
    public function test_one_mac_answering_for_most_of_a_range_is_not_counted_as_occupancy(): void
    {
        // SC198. The appliance's own ARP table binds a single Dell workstation MAC to
        // ~150 of the 254 addresses in 10.200.77.0/24 — every one STALE, none of them
        // a device we can place. Believed, they read as 150 occupied addresses and put
        // the range at 80% "filling up" against 53 real endpoints.
        ...
        $this->assertSame(150, $range['unverified'], 'reported, not counted');
        $this->assertSame(54, $range['seen'], '53 real endpoints plus the appliance');
        $this->assertSame(21, $range['pct'], 'a fifth of the range, not four fifths');
        $this->assertSame('ok', $range['state']);
        $this->assertStringContainsString('unidentified MAC', $range['note']);
    }
    ...
    public function test_the_detail_marks_a_blanketed_address_unverified_rather_than_discovered(): void
    {
        ...
        $this->assertSame(2, $detail['summary']['live'], 'the host and the appliance, not 151');
    }
}
