...
class IpamTest extends TestCase
{
    ...
    public function test_addresses_sharing_one_mac_are_flagged_but_still_counted_as_used(): void
    {
        // SC198, and the correction that followed it. A single Dell MAC answers on ~150
        // of the 254 addresses in 10.200.77.0/24 — a duplicate MAC, since the same one
        // answers at other sites on other switch ports.
        //
        // Every one of those addresses answered ARP inside the last day, so they ARE
        // occupied; only the identity is unknown. An earlier version excluded them from
        // occupancy, which read this full range as a fifth full — the exact reading that
        // sends someone to deploy into a subnet with no room.
        ...
        $this->assertSame(150, $range['unverified'], 'flagged');
        $this->assertSame(204, $range['seen'], '53 endpoints, 150 shared-MAC, the appliance — all live');
        $this->assertSame(80, $range['pct'], 'genuinely four fifths full');
        $this->assertSame('warning', $range['state'], 'a full range must read full');
        $this->assertStringContainsString('share one MAC', $range['note']);
    }
