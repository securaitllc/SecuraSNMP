...
class IpamTest extends TestCase
{
    ...
    /** Put many addresses behind ONE MAC, which the arp() helper deliberately never does. */
    private function oneMac(Site $site, Device $device, array $ips, string $mac): void
    {
        foreach ($ips as $ip) {
            ArpEntry::create([
                'device_id' => $device->id,
    ...
    public function test_addresses_sharing_one_mac_are_reported_but_are_not_capacity(): void
    {
        // SC198, and the second correction. A single Dell MAC answers on ~150 of the
        // 254 addresses in 10.200.77.0/24.
        //
        // This used to COUNT as occupancy, on the reasoning that every one of those
        // addresses answered ARP so something must be there. The live fleet settled
        // it: all 151 of them are learned on ONE access port
        // (FL0082-SC198SWA001 ge-0/0/22), and no NIC holds 151 addresses on one port.
        // It is a duplicate-MAC artefact, and counting it painted this range and
        // 10.200.54.0/24 amber "Filling up" at 81% and 72% when their identifiable
        // occupancy is 54 of 254.
        //
        // Colour is severity here, and "we cannot identify 151 addresses" is a
        // data-quality problem, not a capacity one. So capacity is judged on what can
        // be identified — and the unverified figure is still reported in full beside
        // it, so a range thick with them is never read as empty either.
        $site = Site::factory()->create(['name' => '#198 Fort Lauderdale']);
        $gw = $this->device($site, '10.200.77.254');

        $this->arp($site, $gw, array_map(fn ($i) => "10.200.77.{$i}", range(50, 102)));
        $this->oneMac($site, $gw, array_map(fn ($i) => "10.200.77.{$i}", range(103, 252)), 'D4:A2:CD:4E:99:F4');

        $range = collect((new Ipam)->ranges()['sites'][0]['ranges'])->firstWhere('cidr', '10.200.77.0/24');

        $this->assertSame(150, $range['unverified'], 'still flagged, in full');
        $this->assertSame(204, $range['answered'], 'everything that replied is still reported');
        $this->assertSame(54, $range['seen'], '53 endpoints and the appliance — what can be identified');
        $this->assertSame(21, $range['pct']);
        $this->assertSame('ok', $range['state'], 'a duplicate MAC must not read as a full subnet');
        $this->assertStringContainsString('no host identified', $range['note']);
    }

    public function test_a_gateway_answering_for_addresses_it_holds_is_still_trusted(): void
    {
        // The HQ FortiGate proxy-ARPs for its NAT pools. That MAC resolves to a device
        // we can SEE holding one of the addresses, so the cap must not touch it however
        // many it covers — otherwise the fix erases real firewall consumption.
        $site = Site::factory()->create();
        $fw = $this->device($site, '10.200.9.254', 'firewall');
        \App\Models\InterfaceAddress::create([
            'device_id' => $fw->id, 'ip' => '10.200.9.10', 'prefix_len' => 24,
            'is_public' => false, 'first_seen_at' => now(), 'last_seen_at' => now(),
        ]);

        $this->oneMac($site, $fw, array_map(fn ($i) => "10.200.9.{$i}", range(10, 39)), '00:09:0F:09:01:06');

        $range = collect((new Ipam)->ranges()['sites'][0]['ranges'])->firstWhere('cidr', '10.200.9.0/24');

        $this->assertSame(0, $range['unverified'], 'a placeable gateway is exempt');
        $this->assertSame(31, $range['seen'], '30 pool addresses plus the firewall');
    }

    public function test_a_few_addresses_on_one_mac_are_left_alone(): void
    {
        // A handful on one MAC is ordinary — a host with secondaries, a small pool. The
        // rule fires on a MAC blanketing the range, not on any MAC seen twice.
        $site = Site::factory()->create();
        $gw = $this->device($site, '10.200.31.254');
        $this->oneMac($site, $gw, ['10.200.31.10', '10.200.31.11', '10.200.31.12'], '00:1A:2B:3C:4D:5E');

        $range = collect((new Ipam)->ranges()['sites'][0]['ranges'])->firstWhere('cidr', '10.200.31.0/24');

        $this->assertSame(0, $range['unverified']);
        $this->assertSame(4, $range['seen'], 'three addresses plus the appliance, all counted');
    }

    public function test_the_detail_marks_a_blanketed_address_unverified_rather_than_discovered(): void
    {
        // "Discovered" asserts a host was found there. Nothing here establishes that,
        // and the address is not free either — something answered. It gets its own
        // state so the page can show it and say why.
        $site = Site::factory()->create(['name' => '#198 Fort Lauderdale']);
        $gw = $this->device($site, '10.200.77.254');
        $this->arp($site, $gw, ['10.200.77.50']);
        $this->oneMac($site, $gw, array_map(fn ($i) => "10.200.77.{$i}", range(103, 252)), 'D4:A2:CD:4E:99:F4');

        $detail = (new Ipam)->detail('10.200.77.0/24');
        $rows = collect($detail['rows']);

        $blanketed = $rows->firstWhere('ip', '10.200.77.150');
        $this->assertSame('unverified', $blanketed['state']);
        $this->assertSame(150, $blanketed['unverified_count'], 'the page can say why');

        $this->assertSame('discovered', $rows->firstWhere('ip', '10.200.77.50')['state'], 'a real host is untouched');
        $this->assertSame(150, $detail['summary']['unverified']);
        $this->assertSame(152, $detail['summary']['live'], 'the shared-MAC addresses are live too');
    }
}
