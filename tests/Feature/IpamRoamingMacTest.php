...
class IpamRoamingMacTest extends TestCase
{
    ...
    public function test_a_mac_the_switch_no_longer_lists_says_so(): void
    {
        // `show ethernet-switching table | match 18:4a:53:ce:49:e4` returned nothing at
        // #024 while the page still printed ge-0/0/41. A walk that came back WITHOUT the
        // MAC is positive evidence it left — a different claim from "not confirmed
        // lately", and the two were rendering identically.
        $sw = Device::factory()->create(['site_id' => $this->here->id, 'role' => 'switch', 'name' => 'FL0018-SC024SWA001']);
        $if = DeviceInterface::factory()->create(['device_id' => $sw->id, 'if_name' => 'ge-0/0/41', 'status' => 'up']);
        MacAddress::create([
            'mac' => self::MAC, 'device_id' => $sw->id, 'device_interface_id' => $if->id,
            'vlan' => 'WORKSTATIONS_PRINTERS',
            'first_seen_at' => now()->subMonth(), 'last_seen_at' => now()->subDay(),
            'absent_since' => now()->subHours(20),
        ]);

        $row = $this->row();

        $this->assertNotNull($row['switch_absent_since'], 'the walk proved it is gone');
        $this->assertSame('ge-0/0/41', $row['switch_port'], 'the old port is kept as history, not as a location');
    }

    public function test_a_mac_still_in_the_table_carries_no_absence(): void
    {
        // The control: a present MAC must not be reported as departed.
        $this->learnedAt($this->here, 'FL0018-SC024SWA001', 'ge-0/0/20', now());

        $this->assertNull($this->row()['switch_absent_since']);
    }
