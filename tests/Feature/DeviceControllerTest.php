...
class DeviceControllerTest extends TestCase
{
    ...
    public function test_device_list_exposes_reachability_from_the_down_alarm(): void
    {
        $up = Device::factory()->create();
        $down = Device::factory()->create();
        \App\Models\DeviceAlarm::factory()->create(['device_id' => $down->id, 'alarm_id' => 'device-unreachable', 'cleared_at' => null]);

        $rows = collect($this->actingAs(User::factory()->create())->getJson('/api/devices')->assertOk()->json('data'));
        $this->assertTrue($rows->firstWhere('id', $down->id)['is_down']);
        $this->assertFalse($rows->firstWhere('id', $up->id)['is_down']);
    }
}
