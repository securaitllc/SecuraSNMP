...
class UpdateControllerTest extends TestCase
{
    ...
    public function test_reports_update_available_from_the_version_file_on_the_default_branch(): void
    {
        $current = $this->currentVersion();
        // The contents API returns the VERSION file base64-encoded.
        Http::fake([
            'api.github.com/*' => Http::response(['content' => base64_encode("99.0.0\n")]),
        ]);
    ...
    public function test_reports_no_update_when_current_is_latest(): void
    {
        $current = $this->currentVersion();
        Http::fake([
            'api.github.com/*' => Http::response(['content' => base64_encode($current."\n")]),
        ]);
