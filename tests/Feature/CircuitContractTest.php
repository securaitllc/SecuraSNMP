...
class CircuitContractTest extends TestCase
{
    ...
    public function test_contract_status_buckets_by_days_to_expiry(): void
    {
        ...
        $this->assertSame('expired', $mk(OpsTime::now()->subDay())->contractStatus());
        $this->assertSame('warning', $mk(OpsTime::now()->addDays(20))->contractStatus());
        $this->assertSame('notice', $mk(OpsTime::now()->addDays(50))->contractStatus());
        $this->assertSame('ok', $mk(OpsTime::now()->addDays(200))->contractStatus());
    }

    public function test_expiring_within_scope(): void
    {
        Circuit::factory()->create(['contract_end_date' => OpsTime::now()->addDays(45)]);
        Circuit::factory()->create(['contract_end_date' => OpsTime::now()->addDays(200)]);
    ...
    {
        $admin = User::factory()->create(['role' => 'admin']);
        $circuit = Circuit::factory()->create(['contract_end_date' => OpsTime::now()->subMonths(3)->startOfDay()]);
        ...
        $this->assertSame(OpsTime::now()->addMonths(12)->toDateString(), $circuit->refresh()->contract_end_date->toDateString());
    }
