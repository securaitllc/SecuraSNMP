...
class MacPollerTest extends TestCase
{
    ...
    public function test_a_mac_the_walk_no_longer_lists_is_marked_absent_not_left_looking_present(): void
    {
        // The whole bug behind "121 addresses on ge-0/0/22": the port went down, its
        // switching table emptied, and every row it had ever learned sat unchanged for
        // the 90-day retention. A successful poll that does not list a MAC is positive
        // evidence it left the table, and that has to be written down.
        $device = Device::factory()->create();
        $port = DeviceInterface::factory()->create(['device_id' => $device->id, 'if_index' => 561, 'if_name' => 'ge-0/0/22']);

        $gone = MacAddress::create([
            'device_id' => $device->id,
        ...
        // A healthy walk that returns a DIFFERENT, still-present MAC.
        $walker = function (Device $d, string $oid) {
            if (str_contains($oid, '17.7.1.2.2.1.2')) {
                return ".1.3.6.1.2.1.17.7.1.2.2.1.2.2.0.0.12.17.34.51 = INTEGER: 561\n";
            }
        ...
        $this->assertNotNull($gone->fresh()->absent_since, 'a MAC the switch stopped reporting must be marked gone');
        $this->assertNotNull($gone->fresh()->device_interface_id, 'but the port it was on stays readable');
        $this->assertNull(
            MacAddress::where('mac', '00:00:0C:11:22:33')->first()->absent_since,
            'the MAC the walk did return is present',
        );
    }

    public function test_a_mac_that_comes_back_clears_its_absence(): void
    {
        $device = Device::factory()->create();
        DeviceInterface::factory()->create(['device_id' => $device->id, 'if_index' => 561, 'if_name' => 'ge-0/0/5']);
        MacAddress::create([
            'device_id' => $device->id,
            ...
            'absent_since' => now()->subHours(5),
        ]);

        $walker = fn (Device $d, string $oid) => str_contains($oid, '17.7.1.2.2.1.2')
            ? ".1.3.6.1.2.1.17.7.1.2.2.1.2.2.0.0.12.17.34.51 = INTEGER: 561\n"
            : '';
        (new MacPoller($walker))->poll($device);

        $this->assertNull(MacAddress::where('mac', '00:00:0C:11:22:33')->first()->absent_since);
    }

    public function test_a_rejected_corrupt_read_never_marks_anything_absent(): void
    {
        // The batch guard returns before any write. A corrupt walk must not be read as
        // "everything left the switch" — that would blank the whole fleet's placement.
        $device = Device::factory()->create();
        $row = MacAddress::create([
            'device_id' => $device->id,
            'mac' => 'D4:A2:CD:4E:99:F4',
            'vlan' => '30',
            'first_seen_at' => now()->subDays(20),
        ...
        $this->assertNull($row->fresh()->absent_since);
    }
