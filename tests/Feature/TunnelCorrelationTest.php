...
class TunnelCorrelationTest extends TestCase
{
    ...
    public function test_it_matches_across_zero_padding_between_tunnel_label_and_device_name(): void
    {
        // Real #063: the edge is named "…SC063" but the hub labels the tunnel "…SC63".
        // Before canonical-token matching this one-char gap left the hub tunnel alarm on
        // the hub instead of rolling it under the branch that was actually down.
        $hubSite = Site::factory()->create(['name' => '#000 Azure', 'site_type' => 'hub']);
        $hub = Device::factory()->create(['site_id' => $hubSite->id, 'role' => 'edgeconnect', 'name' => 'AZR-PRI_SDW']);
        ...
        $this->down($spoke, 'ec:196625:gw:70.164.53.177', 'Next-hop unreachable — gw:70.164.53.177');
        $this->down($hub, 'ec:65537:to_LA0001-SC63_DIA2-Broadband1', 'Tunnel state is Down — to_LA0001-SC63_DIA2-Broadband1');

        $out = (new TunnelCorrelation)->analyze();

        $this->assertCount(1, $out['suppressed_alarm_ids'], 'SC63 tunnel label must match the SC063 edge');
        $this->assertSame($spokeSite->id, $out['incidents'][0]['site_id']);
    }
}
