...
class DeviceLldpControllerTest extends TestCase
{
    ...
    public function test_transport_interfaces_like_tlan0_are_accepted_but_injection_is_rejected(): void
    {
        \Illuminate\Support\Facades\Queue::fake();
        $admin = User::factory()->admin()->create();
        $device = Device::factory()->create([
            'vendor' => 'silverpeak', 'ssh_username' => 'admin', 'ssh_credential' => 'secret', 'ssh_credential_id' => null,
        ]);

        // tlan0 / twan1 are valid EdgeConnect transport interfaces.
        $this->actingAs($admin)->postJson("/api/devices/{$device->id}/lldp/enable", ['interfaces' => ['tlan0', 'twan1']])
            ->assertStatus(202);
        // A payload that could reach the CLI is still rejected.
        $this->actingAs($admin)->postJson("/api/devices/{$device->id}/lldp/enable", ['interfaces' => ['lan0; reboot']])
            ->assertStatus(422);
    }
