...
class ImportDiaCutoverTest extends TestCase
{
    ...
    public function test_the_renumbered_knoxville_site_carries_its_old_addresss_cutover(): void
    {
        // SC 189 cut over as SC 730 at 2609 Phyllis St, Jacksonville, was renumbered,
        // and has since moved to Knoxville — while SC 106 later moved into that same
        // Jacksonville building. Confirmed by Reynaldo. The service-centre number is
        // the key precisely because two relocations cross at one address here.
        ...
        $c = $dia->fresh();
        $this->assertSame('2024-03-26', $c->install_date?->toDateString());
        $this->assertSame('2027-03-26', $c->contract_end_date?->toDateString());
    }

    public function test_the_two_sites_that_swapped_buildings_keep_their_own_cutovers(): void
    {
        // The pair that made this look self-contradictory: 189 and 106 must NOT end up
        // sharing a date just because they shared an address at different times.
        $s189 = Site::factory()->create(['site_number' => '189']);
        $s106 = Site::factory()->create(['site_number' => '106']);
        $c189 = Circuit::factory()->create(['site_id' => $s189->id, 'circuit_type' => 'fiber', 'install_date' => null]);
        $c106 = Circuit::factory()->create(['site_id' => $s106->id, 'circuit_type' => 'fiber', 'install_date' => null]);

        $this->artisan('circuits:import-dia-cutover', ['--apply' => true])->assertSuccessful();

        $this->assertSame('2024-03-26', $c189->fresh()->install_date?->toDateString());
        $this->assertSame('2024-07-17', $c106->fresh()->install_date?->toDateString());
    }
