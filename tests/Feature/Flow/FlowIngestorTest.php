...
class FlowIngestorTest extends TestCase
{
    ...
    public function test_it_reads_a_string_protocol_from_goflow2_v2(): void
    {
        // goflow2 v2.2.1 emits proto as a NAME ("TCP"), not the IANA number — the old
        // (int) cast turned every flow into "proto-0". Both forms must work.
        $device = Device::factory()->create(['site_id' => Site::factory()->create()->id, 'ip_address' => '10.11.9.230']);

        (new FlowIngestor(new FlowClassifier))->ingest([
            json_encode(['sampler_address' => '10.11.9.230', 'src_addr' => '10.11.55.23', 'dst_addr' => '54.183.140.32', 'proto' => 'TCP', 'dst_port' => 443, 'bytes' => 766, 'packets' => 1]),
            json_encode(['sampler_address' => '10.11.9.230', 'src_addr' => '10.11.55.24', 'dst_addr' => '8.8.8.8', 'proto' => 17, 'dst_port' => 53, 'bytes' => 100, 'packets' => 1]),
        ]);

        $protos = FlowRecord::orderBy('id')->pluck('protocol')->all();
        $this->assertSame(['tcp', 'udp'], $protos, 'string "TCP" and numeric 17 both normalize');
    }

    public function test_it_attributes_a_flow_by_the_flow_exporter_ip(): void
    {
        // The switch exports sFlow from me0 (10.200.67.2) but Nodus polls it on irb.x
        // (10.200.67.254). The agent-id must still tie the flow to the device.
        $device = Device::factory()->create([
            'site_id' => Site::factory()->create()->id,
            'ip_address' => '10.200.67.254', 'flow_exporter_ip' => '10.200.67.2',
        ]);

        $line = json_encode([
            'sampler_address' => '10.200.67.2', 'src_addr' => '10.86.10.1', 'dst_addr' => '8.8.8.8',
            'proto' => 17, 'bytes' => 100, 'packets' => 1,
        ]);

        (new FlowIngestor(new FlowClassifier))->ingest([$line]);

        $this->assertSame($device->id, FlowRecord::first()->device_id, 'matched by flow_exporter_ip (me0), not the monitored IP');
    }
