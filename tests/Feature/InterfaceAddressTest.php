...
class InterfaceAddressTest extends TestCase
{
    ...
    public function test_a_vendors_internal_control_plane_address_is_not_recorded(): void
    {
        // Juniper carries 128.0.0.1/2 on bme0 — the same address on every switch, with
        // a /2 prefix no real allocation would have. It is platform plumbing, and left
        // in it was 805 rows of noise across the address map.
        $site = Site::factory()->create();
        $device = Device::factory()->create([
            'site_id' => $site->id, 'role' => 'switch',
            'ip_address' => '10.11.9.252', 'snmp_version' => 'v2c',
        ]);
        DeviceInterface::factory()->create(['device_id' => $device->id, 'if_index' => 3, 'if_name' => 'bme0.0']);
        DeviceInterface::factory()->create(['device_id' => $device->id, 'if_index' => 4, 'if_name' => 'vlan.10']);

        $ifIndex = implode("\n", [
            '.1.3.6.1.2.1.4.20.1.2.128.0.0.1 = INTEGER: 3',
            '.1.3.6.1.2.1.4.20.1.2.10.11.9.252 = INTEGER: 4',
        ]);
        $mask = implode("\n", [
            '.1.3.6.1.2.1.4.20.1.3.128.0.0.1 = IpAddress: 192.0.0.0',
            '.1.3.6.1.2.1.4.20.1.3.10.11.9.252 = IpAddress: 255.255.255.0',
        ]);

        $n = (new InterfaceAddressCollector(
            fn (Device $d, string $oid) => str_contains($oid, '.20.1.2') ? $ifIndex : $mask
        ))->collect($device);

        $this->assertSame(1, $n, 'only the real management address is kept');
        $this->assertNull(InterfaceAddress::where('ip', '128.0.0.1')->first());
        $this->assertNotNull(InterfaceAddress::where('ip', '10.11.9.252')->first());
    }
}
