...
class MaintenanceStalenessTest extends TestCase
{
    ...
    public function test_opening_a_window_takes_effect_at_once(): void
    {
        // The TTL is a ceiling on staleness, not a delay somebody has to sit through.
        // A window written through the model flushes the memo, so the operator who just
        // clicked Start reloads the page and sees it — no 30-second wait, no travel().
        $site = Site::factory()->create();
        $device = Device::factory()->create(['site_id' => $site->id]);
    ...
    {
        // The other half of the contract: a dashboard build walks thousands of rows and
        // must not re-ask per row. Counted rather than asserted on a stale answer, so
        // the test still means something now that a write flushes the memo.
        $site = Site::factory()->create();
        $devices = Device::factory()->count(3)->create(['site_id' => $site->id]);
        MaintenanceWindow::create([
            'name' => 'Site cutover', 'site_id' => $site->id,
            'starts_at' => now()->subMinute(), 'ends_at' => now()->addDays(2),
        ]);

        Maintenance::covers($devices[0]->id);   // builds the memo
        ...
        $this->assertSame(0, $queries, 'a built memo must answer without touching the database');
    }
}
