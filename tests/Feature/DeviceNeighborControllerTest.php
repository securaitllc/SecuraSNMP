...
class DeviceNeighborControllerTest extends TestCase
{
    ...
    public function test_a_disconnected_endpoint_is_still_listed_with_when_it_left(): void
    {
        // The port went down; the operator needs to know what had been on it.
        $device = Device::factory()->create();
        LldpNeighbor::create([
            'device_id' => $device->id,
        ...
        $response->assertOk();
        $response->assertJsonPath('0.local_port', 'ge-0/0/10');
        $response->assertJsonPath('0.extension', '500206');
        $response->assertJsonPath('0.remote_mac', '02:00:5E:05:15:32');
        $this->assertNotNull($response->json('0.absent_since'));
    }

    public function test_live_endpoints_are_listed_before_disconnected_ones(): void
    {
        $device = Device::factory()->create();
        LldpNeighbor::create([
            'device_id' => $device->id, 'local_port' => 'ge-0/0/2', 'remote_sysname' => 'gone',
            'last_seen_at' => now()->subDay(), 'absent_since' => now()->subDay(),
        ]);
        LldpNeighbor::create([
            'device_id' => $device->id, 'local_port' => 'ge-0/0/47', 'remote_sysname' => 'live',
            'last_seen_at' => now(),
        ]);
        ...
        // Port order would have put the disconnected one first.
        $response->assertJsonPath('0.remote_sysname', 'live');
        $response->assertJsonPath('1.remote_sysname', 'gone');
    }
