...
class OsintTest extends TestCase
{
    ...
    public function test_enum_sidecar_token_can_come_from_settings_without_a_yaml_edit(): void
    {
        // URL now defaults to http://enum:8099, and the shared token can be set in Settings
        // (the 'osint_enum' provider) instead of the app-service YAML.
        \App\Models\OsintIntegration::create(['provider' => 'osint_enum', 'api_key' => 'stack-token', 'enabled' => true]);
        Http::fake([
            'crt.sh/*' => Http::response([], 200),
            'api.certspotter.com/*' => Http::response([], 200),
            'enum:8099/enum' => Http::response(['subdomains' => ['sso.nowsso.com'], 'sources' => ['dnsx-brute' => 'ok (1)']], 200),
        ]);
        $this->app->instance(OsintDomainService::class, new OsintDomainService(
            fn (array $cmd) => $cmd[0] === 'whois' ? 'Creation Date: '.now()->subDays(4)->format('Y-m-d')."\n" : ''
        ));
        ...
        $this->assertContains('sso.nowsso.com', collect($res->json('result.subdomains'))->pluck('name')->all());
    }
