...
class DeviceHostnameTest extends TestCase
{
    ...
    public function test_the_operators_naming_convention_is_not_a_rename(): void
    {
        // Massey's first live read flagged 143 of 304 devices. These are the shapes
        // that made up almost all of them; none is a change on the box.
        foreach ([
            ['FL0004-SC177_SDW', 'FL0004-SC177'],
            ['FL0005-SC005_SDW', 'FL0005-SC05'],
            ['FL0001-HQSWA-FL2', 'HQSWA-FL2.masseyservices.com'],
            ['FL0063-SC887SWA01#', 'FL0063-SC887SWA01.masseyservices.com'],
            ['FL0063-SC887_SDW1', 'FL0063-SC887'],
        ] as [$record, $box]) {
            $d = $this->reachable(['name' => $record]);
            (new HostnamePoller($this->walker($box)))->poll($d);
            $this->assertFalse(HostnamePoller::drifted($d->refresh()), "{$record} vs {$box} is convention, not drift");
            $this->assertTrue(HostnamePoller::conventionOnly($d));
        }
    }

    public function test_a_real_rename_survives_the_convention_filter(): void
    {
        foreach ([
            ['FL0068-SC068_SDW', 'FL0081-SC68'],      // the site code itself changed
            ['FL0063-SC887_SDW1', 'FL0063-CC-PRI'],   // a different name entirely
            ['AZR-FW01', 'fw-fortigate-01'],          // renamed on the console
        ] as [$record, $box]) {
            $d = $this->reachable(['name' => $record]);
            (new HostnamePoller($this->walker($box)))->poll($d);
            $this->assertTrue(HostnamePoller::drifted($d->refresh()), "{$record} vs {$box} is a real difference");
        }
    }
