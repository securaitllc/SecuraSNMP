...
class NvdFeedSourceTest extends TestCase
{
    ...
    public function test_it_pages_in_bounded_chunks_and_never_asks_for_nvd_max(): void
    {
        // Asking NVD for its 2000-record maximum is what took the collector down on
        // 15 September: one decoded page was large enough to starve the circuit poller,
        // whose pings then blew their timeout and reported 232 of 255 circuits down.
        // A background catalog refresh must stay small enough to be harmless.
        $urls = [];
        $source = new NvdFeedSource(function (string $url) use (&$urls): ?array {
            $urls[] = $url;
        ...
        foreach ($urls as $url) {
            ...
            $this->assertLessThanOrEqual(500, (int) $q['resultsPerPage'],
                'a page large enough to threaten the host is never requested');
        }
    }
}
