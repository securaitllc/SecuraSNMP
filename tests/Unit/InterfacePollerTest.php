...
class InterfacePollerTest extends TestCase
{
    ...
    public function test_a_polling_gap_rebaselines_instead_of_flapping(): void
    {
        // The poller was starved (sequential-sweep gap) and stopped polling a device.
        // When it resumes, a state change that happened DURING the gap must not fire a
        // false up->down "flap" — the stored state is stale, so re-baseline.
        $device = Device::factory()->create(['snmp_version' => 'v2c']);
        (new InterfacePoller($this->fakeWalker([
            '.1.3.6.1.2.1.2.2.1.2' => 'ifDescr.9 = STRING: ge-0/0/9',
        ...
        $interface = DeviceInterface::where('device_id', $device->id)->where('if_index', 9)->first();
        $interface->update(['last_polled_at' => now()->subMinutes(30)]);   // a polling gap
        ...
        $this->assertSame('down', $interface->fresh()->status);
        $this->assertSame(0, InterfaceAlert::where('device_interface_id', $interface->id)->count(), 'a change during a polling gap must not flap-alarm');
    }
    ...
    public function test_a_port_that_recovers_during_a_polling_gap_still_closes_its_open_alert(): void
    {
        // H2 stuck-alert fix: a port flapped down (alert opens), then the poller was
        // starved/restarted (a >600s gap), and the port came back UP before the first
        // post-restart poll. The old edge-triggered close (! $wasUp && up) never fired
        // because the gap re-baselines $wasUp to the current up state, so the alert hung
        // open on the Alarms page forever. Recovery must be LEVEL-triggered on up.
        $device = Device::factory()->create(['snmp_version' => 'v2c']);
        (new InterfacePoller($this->fakeWalker([
            '.1.3.6.1.2.1.2.2.1.2' => 'ifDescr.9 = STRING: ge-0/0/9',
            '.1.3.6.1.2.1.2.2.1.7' => 'ifAdminStatus.9 = INTEGER: up(1)',
        ...
        $interface = DeviceInterface::where('device_id', $device->id)->where('if_index', 9)->first();
        $this->assertSame(1, InterfaceAlert::where('device_interface_id', $interface->id)->whereNull('ended_at')->count(), 'alert opened on the down flap');

        // The poller was down for a while; the alert's port recovered during the gap.
        $interface->update(['last_polled_at' => now()->subMinutes(30)]);
        (new InterfacePoller($this->fakeWalker([
            '.1.3.6.1.2.1.2.2.1.2' => 'ifDescr.9 = STRING: ge-0/0/9',
            '.1.3.6.1.2.1.2.2.1.7' => 'ifAdminStatus.9 = INTEGER: up(1)',
            '.1.3.6.1.2.1.2.2.1.8' => 'ifOperStatus.9 = INTEGER: up(1)',
        ...
        $this->assertSame('up', $interface->fresh()->status);
        $this->assertSame(0, InterfaceAlert::where('device_interface_id', $interface->id)->whereNull('ended_at')->count(), 'a recovered port must close its open alert even across a polling gap');
    }
    ...
    private const JNX_TAG = '.1.3.6.1.4.1.2636.3.40.1.5.1.5.1.5';
    private const JNX_NAME = '.1.3.6.1.4.1.2636.3.40.1.5.1.5.1.2';

    public function test_juniper_vlan_mib_provides_the_real_tag(): void
    {
        $device = \App\Models\Device::factory()->create(['snmp_version' => 'v2c', 'vendor' => 'juniper']);

        // jnxExVlanTag (real 802.1Q id) + jnxExVlanName, keyed by jnxExVlanIndex
        // (2, 4, 8) which is NOT the tag (4, 999, 0). Wins over Q-BRIDGE; the
        // reserved tag 0 (untagged default) is not stored as a VLAN.
        (new InterfacePoller($this->fakeWalker([
            self::JNX_TAG => "jnx.2 = Gauge32: 4\njnx.4 = Gauge32: 999\njnx.8 = Gauge32: 0",
            self::JNX_NAME => "jnx.2 = STRING: ENDPOINTS\njnx.4 = STRING: MASSEY-WIFI\njnx.8 = STRING: default",
            '.1.3.6.1.2.1.17.7.1.4.3.1.1' => "q.2 = STRING: ENDPOINTS\nq.4 = STRING: MASSEY-WIFI",
        ])))->poll($device);

        $vlans = \App\Models\DeviceVlan::where('device_id', $device->id)->pluck('name', 'vlan_id');
        $this->assertSame('ENDPOINTS', $vlans[4]);
        $this->assertSame('MASSEY-WIFI', $vlans[999]);
        $this->assertNull($vlans[2] ?? null);   // internal index not stored
        $this->assertNull($vlans[0] ?? null);   // reserved tag 0 skipped
        $this->assertSame('jnx', $device->fresh()->vlan_source);
    }

    public function test_a_dropped_jnx_walk_does_not_flap_to_qbridge(): void
    {
        $device = \App\Models\Device::factory()->create(['snmp_version' => 'v2c', 'vendor' => 'juniper', 'vlan_source' => 'jnx']);
        \App\Models\DeviceVlan::create(['device_id' => $device->id, 'vlan_id' => 999, 'name' => 'MASSEY-WIFI', 'status' => 'active', 'last_seen_at' => now()]);

        // jnxExVlanTag drops (empty) this poll, but Q-BRIDGE still answers with the
        // internal index. The switch is known-jnx, so it must NOT remap to Q-BRIDGE.
        (new InterfacePoller($this->fakeWalker([
            '.1.3.6.1.2.1.17.7.1.4.3.1.1' => 'q.4 = STRING: MASSEY-WIFI',
        ])))->poll($device);

        $vlans = \App\Models\DeviceVlan::where('device_id', $device->id)->where('status', 'active')->pluck('name', 'vlan_id');
        $this->assertSame(['999' => 'MASSEY-WIFI'], $vlans->toArray());
        $this->assertNull(\App\Models\DeviceVlan::where('device_id', $device->id)->where('vlan_id', 4)->first());
    }

    public function test_a_dropped_name_walk_does_not_blank_a_known_name(): void
    {
        $device = \App\Models\Device::factory()->create(['snmp_version' => 'v2c', 'vendor' => 'juniper']);

        (new InterfacePoller($this->fakeWalker([
            self::JNX_TAG => 'jnx.2 = Gauge32: 999',
            self::JNX_NAME => 'jnx.2 = STRING: MASSEY-WIFI',
        ])))->poll($device);
        $this->assertSame('MASSEY-WIFI', \App\Models\DeviceVlan::where('device_id', $device->id)->where('vlan_id', 999)->first()->name);

        // Name walk drops (empty), tag walk still answers — the good name survives.
        (new InterfacePoller($this->fakeWalker([
            self::JNX_TAG => 'jnx.2 = Gauge32: 999',
        ])))->poll($device);
        $this->assertSame('MASSEY-WIFI', \App\Models\DeviceVlan::where('device_id', $device->id)->where('vlan_id', 999)->first()->name);
    ...
    public function test_vlan_tag_comes_from_the_name_suffix_not_the_oid_index(): void
    {
        $device = \App\Models\Device::factory()->create(['snmp_version' => 'v2c']);

        // Juniper appends the REAL 802.1Q tag to the name; the OID index (5, 7) is
        // an internal index that here differs from the tag (999, 99).
        (new InterfacePoller($this->fakeWalker([
            '.1.3.6.1.2.1.17.7.1.4.3.1.1' => "vlan.5 = STRING: \"MASSEY-WIFI+999\"\n"
                ."vlan.7 = STRING: \"SC_VOIP-99+99\"\n"
                .'vlan.20 = STRING: "VOICE"',
        ])))->poll($device);

        $vlans = \App\Models\DeviceVlan::where('device_id', $device->id)->pluck('name', 'vlan_id');
        // Real tag stored, name cleaned of the "+tag" suffix.
        $this->assertSame('MASSEY-WIFI', $vlans[999]);
        // A legitimate "-99" in the name is preserved (only the "+99" is stripped).
        $this->assertSame('SC_VOIP-99', $vlans[99]);
        // No "+tag" suffix → OID index is the tag, name untouched.
        $this->assertSame('VOICE', $vlans[20]);
    }

    public function test_a_duplex_change_is_recorded(): void
    {
        $device = Device::factory()->create(['snmp_version' => 'v2c']);
        $base = [
        ...
        // First poll: full duplex.
        (new InterfacePoller($this->fakeWalker($base + ['.1.3.6.1.2.1.10.7.2.1.19' => 'dot3.1 = INTEGER: 3'])))->poll($device);
        $this->assertSame('full', DeviceInterface::first()->duplex);

        // Second poll: it flips to half — recorded as a change with the old value.
        (new InterfacePoller($this->fakeWalker($base + ['.1.3.6.1.2.1.10.7.2.1.19' => 'dot3.1 = INTEGER: 2'])))->poll($device);
        $iface = DeviceInterface::first();
        $this->assertSame('half', $iface->duplex);
        $this->assertSame('full', $iface->prev_duplex);
        $this->assertNotNull($iface->duplex_changed_at);
    }

    public function test_fortigate_ports_with_empty_ifdescr_are_named_from_ifname(): void
    {
        $device = Device::factory()->create(['snmp_version' => 'v2c', 'vendor' => 'fortigate']);

        // A FortiGate: idx 1 has a configured ifDescr, idx 4 has an EMPTY ifDescr
        // (net-snmp prints `= ""` with no type token, which the parser drops), and
        // idx 6 has neither. All three report an ifOperStatus, and ifName carries
        // the real port names. Every interface must be stored — keyed off oper
        // status, not ifDescr — so a firewall's ports aren't silently lost.
        (new InterfacePoller($this->fakeWalker([
            '.1.3.6.1.2.1.2.2.1.2' => 'ifDescr.1 = STRING: "Spectrum BB"',
            '.1.3.6.1.2.1.31.1.1.1.1' => "ifName.1 = STRING: wan1\nifName.4 = STRING: internal1",
            '.1.3.6.1.2.1.2.2.1.8' => "ifOperStatus.1 = INTEGER: 1\nifOperStatus.4 = INTEGER: 1\nifOperStatus.6 = INTEGER: 2",
            '.1.3.6.1.2.1.2.2.1.7' => "ifAdminStatus.1 = INTEGER: 1\nifAdminStatus.4 = INTEGER: 1\nifAdminStatus.6 = INTEGER: 2",
        ])))->poll($device);

        $stored = DeviceInterface::where('device_id', $device->id)->orderBy('if_index')->get();
        $this->assertSame([1, 4, 6], $stored->pluck('if_index')->all());
        // idx 1: ifDescr wins. idx 4: falls back to ifName. idx 6: synthetic label.
        $this->assertSame('Spectrum BB', $stored->firstWhere('if_index', 1)->if_name);
        $this->assertSame('internal1', $stored->firstWhere('if_index', 4)->if_name);
        $this->assertSame('if6', $stored->firstWhere('if_index', 6)->if_name);
    }
}
