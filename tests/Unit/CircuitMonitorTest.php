...
class CircuitMonitorTest extends TestCase
{
    ...
    public function test_a_live_appliance_wan_alarm_overrides_a_stale_reachable_nexthop(): void
    {
        ...
        DeviceAlarm::create(['device_id' => $edge->id, 'alarm_id' => 'ec:196625:gw:97.68.131.145', 'severity' => 'critical', 'description' => 'Next-hop unreachable', 'first_seen_at' => now(), 'cleared_at' => null]);
    ...
    public function test_an_unreachable_appliance_cannot_hold_a_circuit_up_with_stale_nexthops(): void
    {
        ...
        DeviceAlarm::create(['device_id' => $edge->id, 'alarm_id' => 'device-unreachable', 'severity' => 'critical', 'description' => 'Device is DOWN', 'first_seen_at' => now(), 'cleared_at' => null]);
    ...
    public function test_ipsla_down_alarm_also_suppresses_the_nexthop_override(): void
    {
        ...
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect']);
        DeviceNextHop::create(['device_id' => $edge->id, 'ip_address' => '97.68.131.145', 'interface' => 'wan0', 'status' => 'up']);
        DeviceAlarm::create(['device_id' => $edge->id, 'alarm_id' => 'ec:262189:Ping for sp-ipsla.silverpeak.cloud,8.8.8.8,8.8.4.4 on Port wan0 tunnel N/A label Broadband1', 'severity' => 'warning', 'description' => 'IP SLA monitor Down', 'first_seen_at' => now(), 'cleared_at' => null]);
        $circuit = Circuit::factory()->create(['site_id' => $site->id, 'wan_interface' => 'wan0', 'gateway_ip' => '97.68.131.145', 'monitored_ip' => '97.68.131.145', 'status' => 'up', 'monitor_via' => 'icmp']);
    ...
    public function test_one_reachable_appliance_still_speaks_for_the_site(): void
    {
        ...
        DeviceAlarm::create(['device_id' => $dead->id, 'alarm_id' => 'device-unreachable', 'severity' => 'critical', 'description' => 'Device is DOWN', 'first_seen_at' => now(), 'cleared_at' => null]);
    ...
    public function test_a_cleared_wan_alarm_does_not_suppress_the_override(): void
    {
        ...
        $edge = Device::factory()->create(['site_id' => $site->id, 'role' => 'edgeconnect']);
        DeviceNextHop::create(['device_id' => $edge->id, 'ip_address' => '108.188.128.1', 'interface' => 'wan0', 'status' => 'up']);
        DeviceAlarm::create(['device_id' => $edge->id, 'alarm_id' => 'ec:196625:gw:108.188.128.1', 'severity' => 'critical', 'description' => 'Next-hop unreachable', 'first_seen_at' => now()->subMinutes(10), 'cleared_at' => now()->subMinutes(5)]);
        $circuit = Circuit::factory()->create(['site_id' => $site->id, 'wan_interface' => 'wan0', 'gateway_ip' => '108.188.128.1', 'monitored_ip' => '108.188.128.1', 'status' => 'up', 'monitor_via' => 'icmp']);
    ...
    public function test_a_flap_within_five_minutes_reopens_the_same_alert_not_a_new_ticket(): void
    {
        ...
        $down = fn () => (new CircuitMonitor(fn (string $ip) => ['loss' => 100, 'rtt' => null]))->check($circuit->fresh());
    ...
    {
        $circuit = Circuit::factory()->create(['status' => 'up']);
        // A real outage: ping completed and reported total loss.
        $monitor = new CircuitMonitor(fn (string $ip) => ['loss' => 100, 'rtt' => null]);
    ...
    {
        // A circuit that is genuinely unreachable: ping RAN and reported total loss.
        $circuit = Circuit::factory()->create(['status' => 'up']);
        $monitor = new CircuitMonitor(fn (string $ip) => ['loss' => 100, 'rtt' => null]);
    ...
    public function test_a_probe_that_never_ran_grades_nothing(): void
    {
        // null from the pinger means the probe did not COMPLETE — the process would not
        // launch, or it hung and was killed. That is unmeasured, not down.
        //
        // Treating it as 100% loss took the fleet down on 15 September: queued path
        // probes loaded the collector, pings blew their timeout, and 232 of 255 circuits
        // reported down at once while every device stayed reachable.
        $circuit = Circuit::factory()->create(['status' => 'up', 'sustained_loss_pct' => 0]);
        $monitor = new CircuitMonitor(fn (string $ip) => null);

        $monitor->check($circuit);

        $circuit->refresh();
        $this->assertSame('up', $circuit->status, 'an unmeasured circuit keeps its last verdict');
        $this->assertFalse($circuit->last_measured_ok, 'but it is flagged as unmeasured');
        $this->assertNotNull($circuit->last_checked_at, 'and the attempt is still stamped');
        // No invented history row, and no alert.
        $this->assertDatabaseCount('circuit_metric_history', 0);
        $this->assertDatabaseMissing('circuit_alerts', ['circuit_id' => $circuit->id, 'ended_at' => null]);
    }

    public function test_a_measured_circuit_is_flagged_measured(): void
    {
        $circuit = Circuit::factory()->create(['status' => 'up', 'last_measured_ok' => false]);
        $monitor = new CircuitMonitor(fn (string $ip) => ['loss' => 0, 'rtt' => 9.1]);

        $monitor->check($circuit);

        $this->assertTrue($circuit->fresh()->last_measured_ok);
    }
    ...
    public function test_sdwan_circuit_uses_the_wan_sourced_ping_not_direct_icmp(): void
    {
        ...
        $sdwanArgs = null;
        $monitor = new CircuitMonitor(
            function () use (&$icmpCalled) {
                $icmpCalled = true;

                return null;
            },
            function ($dev, $wan, $target) use (&$sdwanArgs) {
                $sdwanArgs = [$dev->id, $wan, $target];
    ...
    public function test_the_sweep_does_not_grade_a_circuit_the_batch_never_measured(): void
    {
        // checkAll() goes through measure(), NOT check(). Guarding only check() is why
        // the 15 September fix changed nothing in production: every batch entry was
        // null (no ICMP left the host at all — tcpdump showed zero echo requests in a
        // 70-second window) and measure() turned each one into 100% loss, reporting
        // 232 of 255 circuits down while manual pings to the same addresses returned 0%.
        $circuit = Circuit::factory()->create([
            'status' => 'up', 'monitor_via' => 'icmp', 'monitoring_enabled' => true,
            'sustained_loss_pct' => 0,
        ]);

        $monitor = new CircuitMonitor(
            fn (string $ip) => ['loss' => 0, 'rtt' => 5.0],
            null,
            // The batch ran but could not measure this address.
            fn (array $ips) => array_fill_keys($ips, null),
        );
        ...
        $circuit->refresh();
        $this->assertSame('up', $circuit->status, 'an unmeasured circuit keeps its last verdict');
        $this->assertFalse($circuit->last_measured_ok);
        // No invented reading.
        $this->assertDatabaseCount('circuit_metric_history', 0);
        $this->assertDatabaseMissing('circuit_alerts', ['circuit_id' => $circuit->id, 'ended_at' => null]);
    }

    public function test_the_sweep_still_opens_an_outage_on_a_real_total_loss(): void
    {
        // The batch measured it and it is genuinely unreachable — that must still alarm.
        $circuit = Circuit::factory()->create([
            'status' => 'up', 'monitor_via' => 'icmp', 'monitoring_enabled' => true,
        ]);

        $monitor = new CircuitMonitor(
            fn (string $ip) => ['loss' => 100, 'rtt' => null],
            null,
            fn (array $ips) => array_fill_keys($ips, ['loss' => 100, 'rtt' => null]),
        );
        ...
        $circuit->refresh();
        $this->assertSame('down', $circuit->status);
        $this->assertTrue($circuit->last_measured_ok);
        $this->assertDatabaseHas('circuit_alerts', ['circuit_id' => $circuit->id, 'ended_at' => null]);
    }
}
