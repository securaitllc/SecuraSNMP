...
class ArpCollectorTest extends TestCase
{
    ...
    public function test_resolveall_walks_a_core_switch_that_routes_a_vlan(): void
    {
        // HQ's cores gateway the 10.11.x VLANs on irb interfaces and hold the only ARP
        // for them. This app has one `switch` role, so a core is indistinguishable from
        // an access switch by role — 10.11.55.0/24 read as an empty range while
        // FL0001-HQSWC03 held ~50 live hosts on irb.55. Select on routing, not role.
        $core = Device::factory()->create(['role' => 'switch', 'snmp_version' => 'v2c', 'name' => 'FL0001-HQSWC03']);
        \App\Models\DeviceInterface::factory()->create([
            'device_id' => $core->id, 'if_index' => 555, 'if_name' => 'irb.55',
        ]);

        $this->collector()->resolveAll();

        $this->assertDatabaseHas('arp_entries', ['device_id' => $core->id, 'ip' => '10.0.1.232']);
    }

    public function test_resolveall_leaves_an_access_switch_alone(): void
    {
        // No SVI, no routing, no ARP worth an SNMP walk across ~150 switches.
        $access = Device::factory()->create(['role' => 'switch', 'snmp_version' => 'v2c']);
        \App\Models\DeviceInterface::factory()->create([
            'device_id' => $access->id, 'if_index' => 12, 'if_name' => 'ge-0/0/12',
        ]);

        $this->collector()->resolveAll();

        $this->assertDatabaseMissing('arp_entries', ['device_id' => $access->id]);
    }
