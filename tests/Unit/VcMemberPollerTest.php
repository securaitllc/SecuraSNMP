...
class VcMemberPollerTest extends TestCase
{
    ...
    public function test_a_member_missing_past_the_grace_window_is_reaped_but_a_recent_one_is_kept(): void
    {
        // A switch replaced weeks ago leaves a stale "missing" member that a new member
        // replaces in a different slot — it must be reaped so the VC count is right,
        // while a member offline only briefly stays visible so a dead member still alarms.
        $device = Device::factory()->create(['vendor' => 'juniper', 'role' => 'switch']);
        (new VcMemberPoller($this->walker($this->threeMemberVc())))->poll($device);

        // Member 2 was pulled long ago; member 1 just dropped this cycle.
        DeviceMember::where('device_id', $device->id)->where('member_id', 2)
            ->update(['status' => 'missing', 'absent_since' => now()->subDays(30)]);

        $reduced = $this->threeMemberVc();
        foreach (['2', '3', '5', '6', '8'] as $c) {
            unset($reduced[$c][1], $reduced[$c][2]); // only member 0 answers now
        }
        (new VcMemberPoller($this->walker($reduced)))->poll($device);

        $this->assertNull($device->members()->where('member_id', 2)->first(), 'the long-gone member is reaped');
        $recent = $device->members()->where('member_id', 1)->first();
        $this->assertNotNull($recent, 'a just-offline member is kept');
        $this->assertSame('missing', $recent->status);
        $this->assertSame(2, $device->members()->count()); // member 0 present + member 1 missing
    }
