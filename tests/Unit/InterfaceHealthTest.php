...
class InterfaceHealthTest extends TestCase
{
    ...
    public function test_a_silverpeak_port_reads_unmeasured_not_clean(): void
    {
        // ECOS answers ifInErrors/ifOutErrors with zero on every interface of every
        // appliance — 623 sampled, never once non-zero, while Juniper switches on the
        // same poll report real figures. So a green "Clean" on an SD-WAN WAN port was
        // never a reading; it was the absence of one. The carrier handoff carrying the
        // duplex mismatch rendered exactly that way.
        $if = $this->iface();
        $if->setRelation('device', new Device(['vendor' => 'silverpeak']));

        $this->assertSame('unmeasured', InterfaceHealth::classify($if, 0, 0, Carbon::now())['status']);
    }

    public function test_a_platform_that_does_report_still_reads_clean(): void
    {
        $if = $this->iface();
        $if->setRelation('device', new Device(['vendor' => 'juniper']));

        $this->assertSame('clean', InterfaceHealth::classify($if, 0, 0, Carbon::now())['status']);
    }
}
