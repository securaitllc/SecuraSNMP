...
class ReportServiceTest extends TestCase
{
    ...
    public function test_circuit_report_adds_sla_target_budget_and_breach(): void
    {
        ...
        // A 5-hour (300 min) outage — over the 216-min budget a 99.5% target allows in 30 days.
        CircuitAlert::create(['circuit_id' => $circuit->id, 'started_at' => $start->addDay()->toMutable(), 'ended_at' => $start->addDay()->addHours(5)->toMutable()]);
        ...
        $this->assertSame(99.5, $row['sla_target']);           // fleet default is now 99.5 for all types
        $this->assertSame(216.0, $row['downtime_budget_min']); // 30d × (1 − 0.995)
        $this->assertSame('Breach', $row['sla_status']);       // 300 min > 216 budget
