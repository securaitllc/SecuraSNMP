...
class DeviceMonitorTest extends TestCase
{
    ...
    public function test_checkall_uses_the_parallel_batch_and_records_each_result(): void
    {
        $up = Device::factory()->create(['status' => 'active', 'ip_address' => '10.0.0.1']);
        $down = Device::factory()->create(['status' => 'active', 'ip_address' => '10.0.0.2']);

        // Batch pinger returns pre-measured results; the single-host pinger must NOT
        // be consulted for devices covered by the batch.
        $monitor = new DeviceMonitor(
            fn (string $ip) => throw new \RuntimeException("single pinger used for {$ip} — should have used the batch"),
            fn (array $ips) => ['10.0.0.1' => 12.5, '10.0.0.2' => null],
        );

        $monitor->checkAll();

        $this->assertSame(12.5, (float) DeviceMetricHistory::where('device_id', $up->id)->value('response_time_ms'));
        $this->assertNull(DeviceMetricHistory::where('device_id', $down->id)->value('response_time_ms'));
    }
    ...
    public function test_a_device_down_raises_a_critical_alarm_only_after_the_debounce(): void
    {
        ...
        $this->assertMatchesRegularExpression('/^MSTK\d{8}$/', $alarm->ticket_number);   // investigation ticket
    }
