...
class LldpCollectorTest extends TestCase
{
    ...
    private function switchSeesEdge(string $advertisedName = 'jax-ec01'): array
    {
        return [
            '.1.0.8802.1.1.2.1.3.7.1.3' => '.1.0.8802.1.1.2.1.3.7.1.3.5 = STRING: "ge-0/0/5"',
            '.1.0.8802.1.1.2.1.4.1.1.9' => '.1.0.8802.1.1.2.1.4.1.1.9.0.5.1 = STRING: "'.$advertisedName.'"',
    ...
    public function test_a_neighbour_advertising_the_bare_site_name_still_links_to_the_device(): void
    {
        // Real fleets record the appliance with a role suffix (AL0001-SC208_SDW) while
        // the appliance advertises the bare site name. Exact matching left
        // remote_device_id null, so the endpoint row had no link to click through.
        $switch = Device::factory()->create(['role' => 'switch']);
        $edge = Device::factory()->create(['role' => 'edgeconnect', 'name' => 'AL0001-SC208_SDW']);

        (new LldpCollector($this->walker($this->switchSeesEdge('AL0001-SC208'))))->discover($switch);

        $this->assertSame($edge->id, LldpNeighbor::where('device_id', $switch->id)->first()->remote_device_id);
    }

    public function test_an_ambiguous_name_prefix_is_left_unlinked(): void
    {
        $switch = Device::factory()->create(['role' => 'switch']);
        Device::factory()->create(['role' => 'edgeconnect', 'name' => 'AL0001-SC208_SDW']);
        Device::factory()->create(['role' => 'edgeconnect', 'name' => 'AL0001-SC208_SDW2']);

        (new LldpCollector($this->walker($this->switchSeesEdge('AL0001-SC208'))))->discover($switch);

        // Guessing between two devices sends the operator to the wrong one; no link
        // is the honest outcome.
        $this->assertNull(LldpNeighbor::where('device_id', $switch->id)->first()->remote_device_id);
    }
