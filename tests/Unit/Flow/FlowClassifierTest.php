...
class FlowClassifierTest extends TestCase
{
    ...
    public function test_enterprise_ports_are_named(): void
    {
        // The big "Unclassified" chunk was Windows/AD/file services — now named.
        $cases = [
            [445, 'File sharing (SMB)', 'File sharing'],
            [389, 'LDAP (Directory)', 'Directory'],
            [88, 'Kerberos', 'Directory'],
            [1433, 'SQL Server', 'Database'],
            [554, 'Video (RTSP)', 'Video surveillance'],
            [1812, 'RADIUS', 'Directory'],
            [5246, 'CAPWAP (AP mgmt)', 'Infrastructure'],
        ];
        foreach ($cases as [$port, $app, $cat]) {
            [$a, $c] = $this->c->classify('203.0.113.9', '10.86.10.5', $port, 40000, 'tcp');
            $this->assertSame($app, $a, "port $port app");
            $this->assertSame($cat, $c, "port $port category");
        }
    }

    public function test_cdn_ranges_are_named_over_generic_web(): void
    {
        [$app] = $this->c->classify('23.11.32.159', '10.11.10.16', 443, 51000, 'tcp');
        $this->assertSame('Akamai CDN', $app, 'Akamai IP named instead of generic Web/TLS');
    }
