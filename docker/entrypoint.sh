...
# once. The pollers container must NOT also migrate — compose gates it behind the
# web container's healthcheck so the schema is ready before the loops start.
if [ "$APP_ROLE" != "pollers" ]; then
...
if [ "$APP_ROLE" = "pollers" ]; then
  # Supervisors + snmptrapd run in the background; hold PID 1 open.
  echo "[entrypoint] role=pollers — monitoring loops running"
