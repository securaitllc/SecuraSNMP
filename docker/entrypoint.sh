...
# Web tier: php-fpm worker pool (concurrent) + nginx front. Replaces the old
# single-threaded `php artisan serve`, so multiple NOC screens / the wallboard TV
# / a user firing several API calls at once no longer queue behind each other.
# --allow-to-run-as-root: the app already ran as root under `artisan serve`, and
# the pollers write storage/ as root — keeping fpm root too avoids a permission
# split without changing the security posture.
