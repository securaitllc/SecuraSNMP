...
# Migrations + first-boot admin run ONCE, on the primary only. With multiple web
# replicas, only the one with MIGRATE=1 migrates — else replicas race the schema. The
# all-in-one always migrates. pollers/flows/worker never migrate (they wait on the
# primary's healthcheck via compose depends_on).
if [ "$APP_ROLE" = "all" ] || { [ "$APP_ROLE" = "web" ] && [ "${MIGRATE:-0}" = "1" ]; }; then
  php artisan migrate --force
  php artisan app:ensure-admin
fi

# Cache config is baked at boot; clear stale caches so a redis switch takes effect.
...
if [ "$APP_ROLE" != "web" ]; then
  ...
  # Dedicated queue workers — scale alert/notification/discovery jobs off the pollers.
  # 'worker' role runs several; 'all' already runs one in the SNMP block below.
  if [ "$APP_ROLE" = "worker" ]; then
    n=$(( ${QUEUE_WORKERS:-4} ))
    echo "[entrypoint] role=worker — starting ${n} queue workers"
    i=1
    while [ "$i" -le "$n" ]; do
      supervise "queue-$i" 0 queue:work --sleep=1 --tries=1 --timeout=3600
      i=$((i + 1))
    done
  fi

  # SNMP/SSH monitoring loops — pollers role (or all-in-one) ONLY. flows + worker skip.
  if [ "$APP_ROLE" = "pollers" ] || [ "$APP_ROLE" = "all" ]; then
  #         label        heartbeat?  artisan command
  supervise circuits     1  circuits:monitor --once
  ...
  supervise fg-alarms    1  fortigate:alarms
  supervise lldp         1  lldp:discover
  supervise arp          1  arp:poll
  supervise addresses    1  addresses:poll
...
if [ "$APP_ROLE" = "pollers" ] || [ "$APP_ROLE" = "flows" ] || [ "$APP_ROLE" = "worker" ]; then
