...
FROM php:8.3-fpm
RUN apt-get update && apt-get install -y --no-install-recommends iputils-ping traceroute mtr-tiny snmp snmptrapd \
    nginx curl libzip-dev libpng-dev \
    netbase whois dnsutils openssl unzip \
    && rm -rf /var/lib/apt/lists/*
# netbase ships /etc/services (whois 43/tcp nicname). php:8.3-fpm (debian-slim) omits
# it, so whois fails with "getaddrinfo(...): Servname not supported for ai_socktype"
# — the OSINT whois lookup returned nothing on prod until this was added.
# OSINT tool: whois + dig (dnsutils) + openssl power domain enrichment; subfinder
# (ProjectDiscovery) adds subdomain enumeration on top of crt.sh's HTTP lookup.
RUN curl -sSL https://github.com/projectdiscovery/subfinder/releases/download/v2.6.6/subfinder_2.6.6_linux_amd64.zip -o /tmp/sf.zip \
    && unzip -o /tmp/sf.zip -d /usr/local/bin subfinder && rm /tmp/sf.zip \
    && chmod +x /usr/local/bin/subfinder
# pdo_mysql for the DB; zip + gd are required by phpspreadsheet (xlsx export);
# pcntl lets queue:work handle signals cleanly; redis (pecl) backs cache/session/queue.
