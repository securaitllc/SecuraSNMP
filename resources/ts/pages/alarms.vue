...
interface AlarmRow {
   ...
   * in this list — this only says somebody is already on it, so the row reads as
   * expected work rather than as an unexplained critical.
   */
...
// The flat log holds DEVICE alarms. Circuit outages, port, tunnel and gateway
// alerts live in their own tables and only appear in the By-ISP view — so this
// list can read "Active 0" while the bell rings. State the gap instead of
// letting an empty table stand in for a quiet night.
const bell = useAlertsStore()
const unlistedActive = computed(() => Math.max(0, bell.activeCount - (counts.value.active ?? 0)))
...
// Rows on a device under maintenance are muted, not removed and not recoloured —
// severity still owns the chip colour (house rule), the row just stops competing for
// attention with the alarms nobody is working on yet.
function rowProps({ item }: { item: AlarmRow }) {
  return item.in_maintenance ? { class: 'alarm-row--maintenance' } : {}
}

const maintenanceCount = computed(() => visibleAlarms.value.filter(a => a.in_maintenance).length)
...
<template>
  <div>
    ...
    <div v-if="maintenanceCount > 0" class="alarm-maint-note mono">
      <VIcon icon="ri-tools-line" size="14" />
      <span>{{ maintenanceCount }} of these are on devices under an active maintenance window — expected, and not counted as needing attention.</span>
    </div>

    <div v-if="scope === 'active' && unlistedActive > 0" class="alarm-gap mono">
      <VIcon icon="ri-error-warning-line" size="14" />
      {{ unlistedActive }} active {{ unlistedActive === 1 ? 'incident is' : 'incidents are' }} a circuit, port, tunnel or gateway alert — not a device alarm, so not in this log.
      <a href="#" @click.prevent="view = 'isp'">Open By ISP</a>
    </div>
      ...
      >
        <div class="inc__head">
          ...
          <div class="inc__title">
            <div v-if="inc.kind === 'tunnel'" class="inc__root">
              <strong>{{ inc.root_device_name }}</strong> — {{ inc.reason }}
              <span class="inc__site">· SD-WAN transport</span>
            </div>
            <div v-else class="inc__root">
              Root cause: <strong>{{ inc.root_device_name }}</strong> unreachable
              <span class="inc__site">· {{ inc.site_name }}</span>
            </div>
            <div v-if="inc.kind === 'tunnel'" class="inc__sub">
              <strong>{{ inc.affected_total }}</strong> spoke tunnel alarm{{ inc.affected_total === 1 ? '' : 's' }} rolled up from the hubs/peers — this end is the cause
            </div>
            <div v-else class="inc__sub">
    ...
    <VCard class="list-surface">
      ...
      <VDataTable
        ...
        :items-per-page="25"
        density="compact"
        class="alarms-table mt-2 stack-sm"
      ...
      >
        ...
        <template #item.device_name="{ item }">
          {{ item.device_name ?? '—' }}
          <span v-if="item.in_maintenance" class="alarm-maint">IN MAINTENANCE</span>
        </template>
...
/* Dimmed and rail-marked. Still legible, still sortable, still clearable — it just
   stops reading as the emergency it is not. */
.alarms-table :deep(tr.alarm-row--maintenance td) { opacity: .72; }
.alarms-table :deep(tr.alarm-row--maintenance td:first-child) {
  box-shadow: inset 3px 0 0 rgba(var(--v-theme-on-surface), .35);
}
.alarm-maint {
  display: inline-block; margin-inline-start: 8px; padding: 1px 6px; border-radius: 4px;
  font-size: 9.5px; font-weight: 700; letter-spacing: .06em; white-space: nowrap;
  color: rgba(var(--v-theme-on-surface), .75); background: rgba(var(--v-theme-on-surface), .1);
}
.alarm-maint-note {
  display: flex; align-items: center; gap: 8px;
  margin-block-start: 12px; padding: 9px 12px; font-size: 11px;
  color: rgba(var(--v-theme-on-surface), .7); background: rgba(var(--v-theme-on-surface), .05);
  border-inline-start: 3px solid rgba(var(--v-theme-on-surface), .35);
}
...
.alarm-gap {
  display: flex; align-items: center; gap: 8px;
  margin-block-start: 12px; padding: 9px 12px; font-size: 11px;
  color: rgb(var(--v-theme-warning)); background: rgba(var(--v-theme-warning), .08);
  border-inline-start: 3px solid rgb(var(--v-theme-warning));
}
.alarm-gap a { color: rgb(var(--v-theme-warning)); text-decoration: underline; }
</style>
