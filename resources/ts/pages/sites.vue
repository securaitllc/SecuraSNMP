...
const siteMaintenanceMap = computed(() => alertsStore.siteMaintenanceMap)

// Three states, not two. A site with no alerts at all is UP; one whose only alerts
// are on devices inside a maintenance window is neither UP nor IMPACTED, and saying
// UP over a switch somebody is replacing is the failure this app keeps repeating.
function statusOf(id: number): 'impacted' | 'maintenance' | 'up' {
  if ((siteAlertMap.value[id] ?? 0) > 0)
    return 'impacted'

  return (siteMaintenanceMap.value[id] ?? 0) > 0 ? 'maintenance' : 'up'
}
...
<template>
  <div>
    ...
    >
      <strong>{{ impactedSites }}</strong> site{{ impactedSites > 1 ? 's' : '' }} impacted by active alerts — highlighted below:
    ...
    <div class="site-grid pa-4 pt-2">
      ...
      <template v-else>
        <div class="site-row site-row--head cap">
          <span /><span>#</span><span>Site</span><span>Region</span>
          <span class="site-row__n">Devices</span><span class="site-row__n">Circuits</span><span class="site-row__n">Alarms</span>
          <span>Status</span><span>Address</span><span />
        </div>
        <!-- One ledger row per site; the chevron lazy-loads the per-site NOC
             overview (unchanged data path) beneath it. -->
        <div
        ...
        >
          <!-- One ledger row per site. The expanded overview opens beneath it. -->
          <div class="site-row" role="button" tabindex="0" @click="expanded = expanded.includes(item.id) ? expanded.filter(i => i !== item.id) : [...expanded, item.id]" @keydown.enter="expanded = expanded.includes(item.id) ? expanded.filter(i => i !== item.id) : [...expanded, item.id]">
            <span class="site-card__dot" :class="`is-${statusOf(item.id) === 'impacted' ? 'alert' : statusOf(item.id) === 'maintenance' ? 'maint' : 'ok'}`" />
            <span class="site-card__code mono">{{ item.site_number ?? '—' }}</span>
            <span class="site-card__name mono">{{ item.name }}</span>
            <span class="site-row__region">{{ item.region ?? '—' }}</span>
            <span class="site-row__n mono">{{ item.devices_count ?? 0 }}</span>
            <span class="site-row__n mono">{{ item.circuits_count ?? 0 }}</span>
            <span class="site-row__n mono" :class="{ 'kpi__n--alert': (siteAlertMap[item.id] ?? 0) > 0 }">{{ siteAlertMap[item.id] ?? 0 }}</span>
            <span class="site-row__status mono" :class="statusOf(item.id) === 'impacted' ? 'kpi__n--alert' : statusOf(item.id) === 'maintenance' ? 'site-row__maint' : 'site-row__ok'">{{ statusOf(item.id) === 'impacted' ? 'IMPACTED' : statusOf(item.id) === 'maintenance' ? 'MAINTENANCE' : 'UP' }}</span>
            <span class="site-row__addr">{{ item.address ?? '—' }}</span>
            <span class="site-card__actions" @click.stop>
              <VBtn v-if="auth.isAdmin" icon="ri-edit-line" variant="text" size="small" @click.stop="openEditDialog(item)" />
              <VBtn v-if="auth.isAdmin" icon="ri-delete-bin-line" variant="text" size="small" @click.stop="deleteSite(item)" />
              <VBtn
          ...
          >
            ...
            >
              ...
              <div class="d-flex flex-wrap ga-2 mb-4">
                ...
                <!-- Red only for ports that actually RAISED something. A port can be
                     down, unsuppressed and silent — newly discovered, or its alert
                     cleared — and colouring that red beside the alarm chips is how
                     #196 read as four problems when one port was at fault. -->
                <VChip
                  size="small"
                  :color="(overviewFor(item.id)!.summary.interfaces_alerting ?? 0) > 0 ? 'error' : 'success'"
                  variant="tonal"
                >
                  {{ overviewFor(item.id)!.summary.interfaces_alerting ?? 0 }} interfaces alerting
                </VChip>
                <VChip
                  v-if="overviewFor(item.id)!.summary.interfaces_down > (overviewFor(item.id)!.summary.interfaces_alerting ?? 0)"
                  size="small"
                  color="secondary"
                  variant="tonal"
                  title="Physical ports that are down and not suppressed, but have raised no alert."
                >
                  {{ overviewFor(item.id)!.summary.interfaces_down }} down, no alert
                </VChip>
                <VChip
                  size="small"
...
.site-grid { display: flex; flex-direction: column; gap: 0; }
.site-row {
  display: grid;
  grid-template-columns: 26px 60px 220px 110px 70px 70px 70px 120px 1fr 110px;
  align-items: center;
  gap: 0 12px;
  padding: 0 12px;
  min-block-size: 36px;
...
.site-row:hover { background: #0E1219; }
.site-row--head { cursor: default; min-block-size: 30px; border-block-end: 1px solid var(--nodus-rule); }
.site-row--head:hover { background: transparent; }
.site-row__n { text-align: end; color: var(--nodus-ink-dim); }
.site-row--head .site-row__n { color: inherit; }
.site-row__region, .site-row__addr { font-size: 12px; color: var(--nodus-ink-dim); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.site-row__status { font-size: 11px; letter-spacing: .06em; }
.site-row__ok { color: rgb(var(--v-theme-success)); }
.site-row__maint { color: rgba(var(--v-theme-on-surface), .6); }
.site-grid__state {
  grid-column: 1 / -1;
}
/* Terminal: a ruled region, a 3px severity bar on its left, nothing lifted. */
.site-card {
...
.site-card--alert { border-inline-start-color: rgb(var(--v-theme-error)); }
.site-card--open { border-inline-start-color: rgb(var(--v-theme-primary)); }
...
/* Hollow: something is down, but it is expected and already being handled. */
.site-card__dot.is-maint {
  background: transparent;
  box-shadow: inset 0 0 0 1.5px rgba(var(--v-theme-on-surface), .55);
}
.site-card__name {
  min-inline-size: 0;
  font-weight: 600;
...
.site-card__detail {
  margin: 0 0 0 12px;
  border-block-start: 1px solid var(--nodus-rule);
}
