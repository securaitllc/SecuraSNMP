...
function buildSitePayload() {
  return {
    ...
    // An owned site carries no lease — send nulls so switching to Owned can never
    // leave a stale end date behind (the API clears them too, belt and braces).
    lease_end_date: form.value.occupancy === 'owned' || form.value.lease_end_date === '' ? null : form.value.lease_end_date,
    lease_notes: form.value.occupancy === 'owned' ? null : (form.value.lease_notes || null),
...
<template>
  ...
  <VDialog v-model="isDialogOpen" max-width="720" scrollable>
    <VCard class="ce">
      <div class="ce-head">
        ...
        <div class="ce-body">
          ...
          <div class="ce-content">
            ...
            <section id="sec-identity" class="ce-sect">
              ...
              <VRow dense>
                <VCol cols="12"><VTextField v-model="form.name" label="Name" variant="outlined" density="comfortable" /></VCol>
              ...
              <VRow dense>
                <VCol cols="12">
                  <div class="d-flex align-start ga-2">
                    <VTextField v-model="form.address" label="Address" variant="outlined" density="comfortable" hide-details class="flex-grow-1" />
                ...
                <VCol cols="6"><VTextField v-model="form.latitude" label="Latitude" type="number" variant="outlined" density="comfortable" class="ce-mono" placeholder="28.5384" /></VCol>
                <VCol cols="6"><VTextField v-model="form.longitude" label="Longitude" type="number" variant="outlined" density="comfortable" class="ce-mono" placeholder="-81.3789" /></VCol>
              </VRow>
            </section>

            <!-- Lease: the end date drives ISP contract decisions. Massey owns some
                 locations outright — ticking that disables the fields (and the payload
                 sends nulls, so a stale date can never survive the switch). -->
            <section id="sec-lease" class="ce-sect">
              ...
              >
                <template #label>
                  ...
                  <span class="text-medium-emphasis text-caption ms-2">— no lease to track</span>
                </template>
              </VCheckbox>
              <VRow dense>
                <VCol cols="12" sm="5">
                ...
                <VCol cols="12" sm="7">
                  <VTextField
                    ...
                    density="comfortable"
                    :disabled="form.occupancy === 'owned'"
                    :placeholder="form.occupancy === 'owned' ? 'Not applicable' : 'Renewal option, landlord, move plans…'"
                  />
                </VCol>
              </VRow>
            </section>
