 ...
 * no button anywhere.
 */
const clearableAlerts = computed(() => filteredAlerts.value.filter(a => a.alarm_ref || a.tunnel_alert_ref))

/** The row id to select by, whichever kind of alert this is. */
function alertRef(a: { alarm_ref?: number | null, tunnel_alert_ref?: number | null }) {
  return (a.alarm_ref ?? a.tunnel_alert_ref) as number
}
...
function toggleAll() {
  selectedAlarms.value = allSelected.value ? [] : clearableAlerts.value.map(a => alertRef(a))
}
...
async function submitBulkClear() {
  ...
  try {
    ...
    const deviceIds = clearableAlerts.value.filter(a => a.alarm_ref && picked.has(a.alarm_ref)).map(a => a.alarm_ref as number)
    const tunnelIds = clearableAlerts.value.filter(a => !a.alarm_ref && a.tunnel_alert_ref && picked.has(a.tunnel_alert_ref)).map(a => a.tunnel_alert_ref as number)
      ...
      <VCard :title="alertsFilterLabel[alertsFilter]">
        ...
        <VCardText>
          ...
          <div
            ...
            class="aa-row"
            :class="[`sev-${alert.severity}`, {
              'aa-row--selected': (alert.alarm_ref || alert.tunnel_alert_ref) && selectedAlarms.includes(alertRef(alert)),
          ...
          >
            ...
            <VCheckboxBtn
              v-if="canBulkClear"
              class="aa-check"
              :model-value="(alert.alarm_ref || alert.tunnel_alert_ref) ? selectedAlarms.includes(alertRef(alert)) : false"
              :disabled="!alert.alarm_ref && !alert.tunnel_alert_ref"
              density="compact"
              hide-details
              :style="(alert.alarm_ref || alert.tunnel_alert_ref) ? {} : { visibility: 'hidden' }"
              @update:model-value="(alert.alarm_ref || alert.tunnel_alert_ref) ? toggleAlarm(alertRef(alert)) : undefined"
              @click.stop
            />
