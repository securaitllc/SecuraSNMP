      ...
      <VCol cols="12" lg="8">
        <VCard>
          ...
          <div v-for="(r, i) in (data?.resolved_today?.rows ?? [])" :key="i" class="rt-row mono">
            ...
            <span class="rt-dur text-disabled">{{ r.duration_min != null ? (r.duration_min >= 60 ? `${Math.floor(r.duration_min / 60)}h ${r.duration_min % 60}m` : `${r.duration_min}m`) : '—' }}</span>
...
.rt-row {
  display: grid; grid-template-columns: 26px 150px 1fr 90px 70px; align-items: center;
