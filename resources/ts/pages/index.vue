...
import { easternInputToIso, formatDateTime } from '@/utils/datetime'
...
const data = computed<DashboardSummary | null>(() => store.loaded
  ? { sites: store.sites, availability: store.availability, traffic: store.traffic!, alerts: store.alerts, contracts_expiring: store.contracts, counts: store.counts!, resolved_today: store.resolved ?? undefined }
  : null)
...
const kpis = computed(() => {
  ...
  // Five headline numbers, each with the denominator it is meaningless without and
  // the 24h shape that says whether the night got better or worse. Severity tone is
  // the app's usual contract: red = service affecting, amber = degraded, green = good.
  return [
    {
      label: 'Impacted sites',
      value: c.sites_impacted ?? 0,
      of: c.sites,
      detail: `${c.circuits_down} down · ${c.tunnels_degraded ?? 0} degraded`,
      tone: (c.sites_impacted ?? 0) > 0 ? 'error' : '',
      series: t?.sites_impacted ?? [],
      action: open('all', c.active_alerts),
    },
    {
      label: 'Devices reachable',
      value: c.devices_reachable ?? c.devices,
      of: c.devices,
      // Unmeasured is called out, never folded into "unreachable". A stopped poller
      // and a dead device are different problems and need different people.
      detail: [
        `${c.devices - (c.devices_reachable ?? c.devices) - (c.devices_unmeasured ?? 0)} unreachable`,
        (c.devices_unmeasured ?? 0) > 0 ? `${c.devices_unmeasured} not measured` : null,
      ].filter(Boolean).join(' · '),
      tone: (c.devices_unmeasured ?? 0) > 0 ? 'warning' : '',
      series: (t?.devices_down ?? []).map(v => -v),
    },
    {
      label: 'Circuits up',
      value: c.circuits_up ?? 0,
      of: c.circuits_total ?? 0,
      detail: [
        `${c.circuits_down} down`,
        (c.circuits_unmeasured ?? 0) > 0 ? `${c.circuits_unmeasured} not measured` : null,
        `${c.circuits_maintenance ?? 0} in maintenance`,
      ].filter(Boolean).join(' · '),
      // A green tile requires the fleet to be measured AND up. Unmeasured is not a
      // pass — that is the whole lesson of 15 September.
      tone: (c.circuits_unmeasured ?? 0) > 0 ? 'warning' : (c.circuits_down > 0 ? '' : 'success'),
      series: (t?.circuits_down ?? []).map(v => -v),
      action: open('circuit', c.circuits_down),
    },
    {
      label: 'Tunnels down',
      value: c.tunnels_down,
      of: c.tunnels_total ?? 0,
      detail: `${c.tunnels_degraded ?? 0} link-quality breaches`,
      tone: c.tunnels_down > 0 ? 'warning' : '',
      series: t?.tunnels_down ?? [],
      action: open('tunnel', c.tunnels_down),
    },
    {
      label: 'Interfaces down',
      value: c.interfaces_down,
      of: c.interfaces_total ?? 0,
      // Muted ports are named, never just omitted: a quiet number has to read as
      // "8 are muted", not as "nothing is wrong here".
      detail: `${c.interfaces_suppressed ?? 0} muted at onboarding`,
      tone: c.interfaces_down > 0 ? 'warning' : '',
      series: t?.interfaces_down ?? [],
      action: open('interface', c.interfaces_down),
    },
  ]
})
...
// How long a thing was open, at the precision that reads: minutes under an hour,
// hours under a day, days after that. "149h 9m" hides that a gateway alert sat
// open for six days.
...
async function saveDispatch(alert: DashboardAlert) {
  ...
  try {
    await api(alert.dispatch_url, { method: 'POST', body: {
      // datetime-local is a naive wall-time. Read it as EASTERN — the zone every
      // timestamp in this app is displayed in — so what was typed is what is stored,
      // whatever zone the operator's laptop is set to.
      dispatch_at: easternInputToIso(dispatchAtDraft.value),
...
/**
 * Anything with a row behind it can be cleared — a DeviceAlarm, or an SSH-derived
 * tunnel alert. Tunnel alerts were excluded here, and since nothing else in the UI
 * called their clear endpoint either, a tunnel whose peer had migrated away could
 * never be silenced: permanently down, never coming back up under that name, and
 * no button anywhere.
 */
const clearableAlerts = computed(() => filteredAlerts.value.filter(a => clearableRefs(a).length > 0))

/**
 * Every row id this entry can clear — its own, or its members'.
 *
 * An incident has no row of its own: it is a wrapper around the signals that make
 * it up. With no ref it had no checkbox, and on the unfiltered list that is the ONLY
 * row an operator sees — the five SC63 tunnels were inside it, each perfectly
 * clearable, reachable only by first filtering to Tunnels. Clearing the incident
 * clears what it is made of.
 */
function clearableRefs(a: DashboardAlert): number[] {
  if (a.type === 'incident')
    return (a.members ?? []).flatMap(m => clearableRefs(m as DashboardAlert))

  const ref = a.alarm_ref ?? a.tunnel_alert_ref
...
/** Ticking an incident ticks every signal inside it, which is what clearing it means. */
function toggleAlert(a: DashboardAlert) {
  const refs = clearableRefs(a)
  if (refs.length === 0)
    return

  const on = refs.every(r => selectedAlarms.value.includes(r))

  selectedAlarms.value = on
    ? selectedAlarms.value.filter(r => !refs.includes(r))
    : [...new Set([...selectedAlarms.value, ...refs])]
}

function toggleAll() {
  selectedAlarms.value = allSelected.value ? [] : clearableAlerts.value.flatMap(a => clearableRefs(a))
}
...
async function submitBulkClear() {
  ...
  bulkBusy.value = true
  try {
    // The two kinds live in different tables and clear through different endpoints.
    const picked = new Set(selectedAlarms.value)

    // Flatten incidents so a selected wrapper contributes the signals inside it.
    const flat: DashboardAlert[] = []
    for (const a of filteredAlerts.value) {
      if (a.type === 'incident' && a.members)
        flat.push(...(a.members as DashboardAlert[]))
      else
        flat.push(a)
    }

    const deviceIds = flat.filter(a => a.alarm_ref && picked.has(a.alarm_ref)).map(a => a.alarm_ref as number)
    const tunnelIds = flat.filter(a => !a.alarm_ref && a.tunnel_alert_ref && picked.has(a.tunnel_alert_ref)).map(a => a.tunnel_alert_ref as number)

    if (deviceIds.length) {
      await api('/api/alarms/bulk-clear', {
        method: 'POST',
        body: { ids: deviceIds, note: bulkNote.value || null },
      })
    }

    // No bulk route for these — one call each, which is fine: a migrated site is a
    // handful of tunnels, not a fleet.
    for (const id of tunnelIds) {
      await api(`/api/tunnel-alerts/${id}/clear`, {
        method: 'POST',
        body: { note: bulkNote.value || null },
      })
    }
...
interface Insights { trend_24h: number[]; raised_24h: number; resolved_24h: number; mttr_minutes: number | null; top_offenders: { site_id: number; site_name: string; count: number }[]; anomalies_open: number
  /**
   * Whether an alarm can actually reach a person. Nodus polled this fleet for weeks
   * with no channel enabled — every alarm raised, queued, delivered to nobody — and
   * no screen said so. This is that screen.
   */
  alerting?: { channels_enabled: number; channels_usable: number; can_notify: boolean; warning: string | null } }
...
// ---- basemap settings (admin) --------------------------------------------
// CARTO stamps "API KEY REQUIRED" across tiles fetched without a key, so the key
// is configured right here, on the card where the watermark shows.
interface MapSettings { provider: string, has_key: boolean, masked_key: string | null }
const mapSettingsOpen = ref(false)
const mapSettings = ref<MapSettings | null>(null)
const mapKeyInput = ref('')
const mapSaving = ref(false)
const mapTestMsg = ref('')
const mapTestOk = ref(false)

async function openMapSettings() {
  mapKeyInput.value = ''
  mapTestMsg.value = ''
  mapSettingsOpen.value = true
  mapSettings.value = await api<MapSettings>('/api/map-settings').catch(() => null)
}
...
async function saveMapKey() {
  ...
  mapTestMsg.value = ''
  try {
    mapSettings.value = await api<MapSettings>('/api/map-settings', {
      method: 'PUT',
      // Blank means "keep the existing key" — the API treats it that way too.
      body: { api_key: mapKeyInput.value },
    })
    mapKeyInput.value = ''
    // Tiles are cached per key fingerprint, so a new key is picked up immediately —
    // just force Leaflet to re-request by remounting the map.
    mapReloadKey.value++
  }
  catch (e) {
    mapTestOk.value = false
    mapTestMsg.value = apiErrorMessage(e, 'Could not save the key.')
  }
  finally {
...
async function clearMapKey() {
  mapSaving.value = true
  try {
    mapSettings.value = await api<MapSettings>('/api/map-settings', { method: 'PUT', body: { clear_key: true } })
    mapReloadKey.value++
  }
  finally {
    mapSaving.value = false
...
async function testMapKey() {
  mapSaving.value = true
  try {
    const r = await api<{ ok: boolean, message: string }>('/api/map-settings/test', { method: 'POST' })
    mapTestOk.value = r.ok
    mapTestMsg.value = r.message
  }
  catch (e) {
    mapTestOk.value = false
    mapTestMsg.value = apiErrorMessage(e, 'Could not test the key.')
  }
  finally {
    mapSaving.value = false
  }
...
// Bumped after a key change so the map remounts and re-fetches its tiles.
const mapReloadKey = ref(0)
</script>

<template>
  <div>
    <!-- A monitoring platform that cannot reach anyone is a dashboard. Say so here,
         on the screen people actually watch, not only on a health endpoint nothing
         in this app ever calls. -->
    <VAlert
      v-if="insights?.alerting && insights.alerting.warning"
      :type="insights.alerting.can_notify ? 'warning' : 'error'"
      variant="tonal"
      density="comfortable"
      class="mb-4"
    ...
    >
      <div class="d-flex align-center flex-wrap ga-3">
        <span>{{ insights.alerting.warning }}</span>
        <VSpacer />
        <VBtn v-if="auth.canAct" size="small" variant="tonal" to="/notifications" prepend-icon="ri-settings-3-line">
          Set up alerting
        </VBtn>
      </div>
    </VAlert>
    ...
    <!-- The five headline numbers. Each carries its denominator and its 24h shape:
         a bare count cannot say whether the night got better or worse, and "4
         impacted" reads differently across 12 sites than across 131. -->
    <div class="kpi-strip mb-4">
      <VCard
        v-for="k in kpis"
        :key="k.label"
        class="kpi-card"
        :class="{ 'is-danger': k.tone === 'error', 'cursor-pointer': !!k.action }"
        @click="k.action && k.action()"
      >
        <div class="kpi-body">
          <div class="kpi-label text-medium-emphasis">{{ k.label }}</div>
          <div class="d-flex align-baseline ga-2">
            <div class="kpi-value mono" :class="k.tone ? `text-${k.tone}` : ''">{{ k.value }}</div>
            <div v-if="k.of" class="kpi-of mono text-disabled">/ {{ k.of }}</div>
          </div>
          <div class="kpi-detail text-medium-emphasis">{{ k.detail }}</div>
          <Sparkline
            v-if="k.series.length"
            :points="k.series"
            :color="k.tone ? `rgb(var(--v-theme-${k.tone}))` : 'rgba(var(--v-theme-on-surface), .45)'"
            :width="140"
            :height="28"
            class="kpi-spark"
          />
    ...
    <!-- The fleet map answers "one site or the whole region" at a glance, which
         decides truck roll vs ISP escalation. Top Talkers rides alongside it: the
         map does not need the full width, and on its own it left a dead column. -->
    <VRow class="mb-2">
      <VCol cols="12" lg="8">
        <VCard>
          <VCardItem>
            ...
            <template #append>
              <div class="d-flex align-center ga-2">
                <!-- Basemap key lives here, where the watermark is actually visible. -->
                <VBtn
                  v-if="auth.isAdmin"
                  icon="ri-settings-3-line"
                  size="x-small"
                  variant="text"
                  title="Basemap settings — add the CARTO API key that removes the map watermark"
    ...
    <VRow class="mb-2">
      ...
      <!-- What closed since the operator's day began. Real information for a
           handover, and it is what fills this column when the active list is
           short — an empty panel reads as nothing happened, which is never true. -->
      <VCol cols="12" lg="8">
        <VCard>
          <VCardItem>
            <VCardTitle>Resolved today</VCardTitle>
            <template #append>
              <span class="mono text-caption text-disabled">{{ data?.resolved_today?.count ?? 0 }} closed</span>
            </template>
          </VCardItem>
          <div v-if="!(data?.resolved_today?.rows?.length)" class="rt-empty">Nothing has cleared yet today.</div>
          <div v-for="(r, i) in (data?.resolved_today?.rows ?? [])" :key="i" class="rt-row mono">
            <span class="rt-dot" />
            <span class="rt-site">{{ r.site_name ?? '—' }}</span>
            <span class="rt-title">{{ r.title }}<span v-if="r.by" class="text-disabled"> · cleared by {{ r.by }}</span><span v-else class="text-disabled"> · recovered</span></span>
            <span class="rt-dur text-disabled">{{ durationLabel(r.duration_min) }}</span>
            <span class="rt-at text-disabled">{{ formatDateTime(r.ended_at).split(', ').pop() }}</span>
          </div>
        </VCard>
      </VCol>
      ...
      <VCard :title="alertsFilterLabel[alertsFilter]">
        ...
        <VCardText>
          ...
          <div
            ...
            class="aa-row"
            :class="[`sev-${alert.severity}`, {
              'aa-row--selected': clearableRefs(alert).length > 0 && clearableRefs(alert).every(r => selectedAlarms.includes(r)),
              'aa-row--maintenance': (alert as any).in_maintenance,
            }]"
          >
            ...
            <VCheckboxBtn
              v-if="canBulkClear"
              class="aa-check"
              :model-value="clearableRefs(alert).length > 0 && clearableRefs(alert).every(r => selectedAlarms.includes(r))"
              :disabled="clearableRefs(alert).length === 0"
              density="compact"
              hide-details
              :style="clearableRefs(alert).length > 0 ? {} : { visibility: 'hidden' }"
            ...
            <div class="aa-body">
              <div class="aa-titleline">
                ...
                <!-- Expected, not ignored: the device is being worked on. Says so on
                     the row rather than leaving a red critical with no explanation. -->
                <span v-if="(alert as any).in_maintenance" class="aa-maint">IN MAINTENANCE</span>
                <span
                  class="aa-title"
                  role="button"
                  tabindex="0"
    ...
    <!-- Basemap settings drawer (admin) -->
    <VDialog
      v-model="mapSettingsOpen"
      transition="slide-x-reverse-transition"
      content-class="nodus-drawer nodus-drawer--sm"
      scrollable
    >
      <VCard>
        <VCardItem>
          <VCardTitle class="d-flex align-center ga-2">
            <VIcon icon="ri-map-2-line" size="20" />Basemap
          </VCardTitle>
        </VCardItem>
        <VCardText>
          <p class="text-body-2 text-medium-emphasis mb-4">
            CARTO stamps <strong>API KEY REQUIRED</strong> across basemap tiles fetched without a key.
            Add a free key from
            <a href="https://carto.com/basemaps/apikey" target="_blank" rel="noopener">carto.com/basemaps/apikey</a>
            to get clean tiles. Without one the map still works — just watermarked.
          </p>

          <VAlert
            v-if="mapSettings && !mapSettings.has_key"
            type="warning" variant="tonal" density="compact" class="mb-4"
          >
            No key saved — tiles are watermarked.
          </VAlert>
          <VAlert
            v-else-if="mapSettings?.masked_key"
            type="success" variant="tonal" density="compact" class="mb-4"
          >
            Key saved: <span class="mono">{{ mapSettings.masked_key }}</span>
          </VAlert>

          <VTextField
            v-model="mapKeyInput"
            label="CARTO API key"
            :placeholder="mapSettings?.has_key ? 'Leave blank to keep the current key' : 'Paste the key from carto.com'"
            variant="outlined"
            density="comfortable"
            autocomplete="off"
          />
          ...
          <VAlert
            v-if="mapTestMsg"
            :type="mapTestOk ? 'success' : 'warning'"
        ...
        <VCardActions class="px-4 pb-4 ga-2">
          <VBtn variant="text" :disabled="mapSaving || !mapSettings?.has_key" @click="testMapKey">Test</VBtn>
          <VBtn
            v-if="mapSettings?.has_key"
            variant="text" color="error" :disabled="mapSaving" @click="clearMapKey"
          >
            Remove
          </VBtn>
          <VSpacer />
          <VBtn variant="text" :disabled="mapSaving" @click="mapSettingsOpen = false">Close</VBtn>
          <VBtn color="primary" variant="flat" :loading="mapSaving" :disabled="!mapKeyInput" @click="saveMapKey">Save</VBtn>
        </VCardActions>
      </VCard>
    </VDialog>
</template>
...
.kpi-strip {
  ...
  gap: 0;
  border: 1px solid var(--nodus-rule);
}
.kpi-strip > .kpi-card { border: 0; border-inline-end: 1px solid var(--nodus-rule); }
.kpi-strip > .kpi-card:last-child { border-inline-end: 0; }
...
.kpi-body { padding: 14px 20px; }
.kpi-label {
  font-size: 10px; line-height: 1.2; letter-spacing: .12em; text-transform: uppercase; font-weight: 600;
  /* Cards get narrow at seven across; keep the label on one line. */
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
.kpi-value { font-size: 28px; font-weight: 500; line-height: 1.2; margin-block-start: 6px; letter-spacing: 0; }
.kpi-of { font-size: .8rem; }
.kpi-detail { font-size: .74rem; line-height: 1.3; margin-block-start: 2px; }
.kpi-spark { display: block; inline-size: 100%; margin-block-start: 6px; }
.traffic-stat { font-size: 1.1rem; font-weight: 700; letter-spacing: -.02em; }
@media (min-width: 960px) and (max-width: 1279px) {
  .kpi-body { padding: 10px 12px; }
  .kpi-value { font-size: 1.3rem; }
}

.kpi-card { box-shadow: none !important; transition: background .12s ease; }
.kpi-card.cursor-pointer:hover { background: #0E1219; }

/* Resolved-today ledger */
.rt-row {
  display: grid; grid-template-columns: 26px 200px 1fr 80px 110px; align-items: center;
  padding: 9px 20px; font-size: 11px;
  border-block-end: 1px solid #161B23;
}
.rt-row > span { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.rt-row:last-child { border-block-end: 0; }
.rt-dot { inline-size: 7px; block-size: 7px; background: rgb(var(--v-theme-success)); }
.rt-site { color: rgb(var(--v-theme-on-surface)); }
.rt-title { font-family: 'IBM Plex Sans', sans-serif; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.rt-dur, .rt-at { text-align: end; }
.rt-empty { padding: 16px 20px; font-size: 12px; color: var(--nodus-ink-dim); }
...
/* A device under maintenance is expected to be down. Keep the row — it is not
   healthy and must not vanish — but drop the severity wash and the red rail so it
   stops competing for attention with the outages nobody planned. */
.aa-row--maintenance { opacity: .72; }
.aa-row--maintenance .aa-stripe { background: rgba(var(--v-theme-on-surface), .28) !important; }
.aa-row--maintenance .aa-ic { color: rgba(var(--v-theme-on-surface), .5) !important; }
.aa-maint {
  font-size: 9px; letter-spacing: .12em; font-weight: 600;
  padding: 1px 6px; border: 1px solid rgba(var(--v-theme-on-surface), .25);
  color: rgba(var(--v-theme-on-surface), .6); white-space: nowrap;
}
.aa-stripe {
  flex: 0 0 3px;
  align-self: stretch;
