...
async function loadAnomalies() {
  try { anomalies.value = await api<AnomalyRow[]>('/api/anomalies') }
  catch { /* non-fatal */ }
}
  ...
  <div>
    ...
    <VRow class="mb-2">
      ...
      <VCol cols="12" lg="4">
        ...
        <!-- Anomalies — robust-z metrics off baseline, the real list (not just a count) -->
        <VCard class="mb-4">
          <VCardItem>
          ...
          <VCardText class="pt-0">
            ...
            <RouterLink
              v-for="a in anomalies.slice(0, 5)"
              :key="a.id"
        ...
        <VCard>
          ...
          <VCardText class="pt-0">
            ...
            <RouterLink
              v-for="(t, i) in topTalkers.slice(0, 5)"
              :key="i"
              to="/flows"
              class="mini-row"
            >
              <span class="mini-main">
                <span class="mini-title mono">{{ t.src_ip }} <VIcon icon="ri-arrow-right-line" size="13" class="text-disabled" /> {{ t.dst_ip }}</span>
                <span class="mini-sub"><VChip size="x-small" variant="tonal" color="info">{{ t.app ?? 'Unclassified' }}</VChip> · {{ t.flows }} flows</span>
              </span>
              <span class="mini-bytes mono">{{ fmtBytes(t.bytes) }}</span>
            </RouterLink>
          </VCardText>
        </VCard>
      </VCol>
