...
onMounted(() => {
  store.startPolling()
  loadInsights()
  insightsTimer = setInterval(loadInsights, 30000)
})
...
const alarmView = ref<'grouped' | 'flat'>('flat')
  ...
  <div>
    ...
    <div class="hero-strip mb-4">
        ...
        <div class="hero-ring">
          <svg viewBox="0 0 36 36" class="ring-svg">
            ...
            <circle
              ...
              :stroke-dasharray="`${data ? ringHealthy : 0} 100`" stroke-dashoffset="25" stroke-linecap="round" transform="rotate(-90 18 18)"
            />
          </svg>
          <div>
            <div class="hero-big">{{ data ? fleetHealth.healthy : '—' }}<small v-if="data"> / {{ fleetHealth.total }}</small></div>
            <div class="hero-sub">sites healthy</div>
            <div v-if="data" class="hero-legend">
      ...
      <VCard class="hero-card">
        ...
        <div class="hero-big mb-2">{{ data ? (data.counts.active_alerts ?? 0) : '—' }}<small> open</small></div>
        <div class="sev-split">
          ...
          <!-- only claim "All clear" once the dashboard has actually loaded — not while
               it is still fetching (that showed a false empty state next to live alarms) -->
          <div v-if="data && !data.counts.active_alerts" class="ss-clear">All clear</div>
          <div v-else-if="!data" class="ss-clear" style="opacity:.5">Loading…</div>
        </div>
        <div v-if="data" class="hero-legend mt-2">
    ...
    <VRow class="mb-2">
            ...
            >
              <div class="alert-main">
                ...
                <div class="alert-meta">
                  ...
                  <span class="alert-sub text-truncate">{{ alert.subtitle }}</span>
                </div>
              </div>
              <div class="alert-aside">
                <span class="alert-time">{{ since(alert.started_at) }}</span>
                <span
      ...
      <VCol cols="12" lg="4">
        ...
        <RouterLink v-if="(insights?.anomalies_open ?? 0) > 0" to="/anomalies" class="anomaly-tile mb-4">
          <VIcon icon="ri-pulse-line" size="20" class="text-warning" />
          <span><b>{{ insights?.anomalies_open }}</b> anomal{{ insights?.anomalies_open === 1 ? 'y' : 'ies' }}</span>
          <span class="anomaly-tile__sub">off baseline · review</span>
          <VIcon icon="ri-arrow-right-s-line" size="18" class="ms-auto text-medium-emphasis" />
        </RouterLink>
        <VCard>
          <VCardItem>
            <VCardTitle class="text-body-1">Top offenders · 7 days</VCardTitle>
            <VCardSubtitle>Sites with the most alarms this week</VCardSubtitle>
          </VCardItem>
          <VCardText class="pt-0">
            ...
            <RouterLink
              ...
              class="offender-row"
              :title="`${o.count} alarms in the last 7 days — open ${o.site_name}`"
            >
              <span class="offender-rank">{{ i + 1 }}</span>
              <span class="offender-name">{{ o.site_name }}</span>
              <!-- min 8% so even a low-count site keeps a visible bar next to a dominant one -->
              <span class="offender-bar"><i :style="{ width: `${Math.max(8, Math.round((o.count / offenderMax) * 100))}%` }" /></span>
...
/* Severity left-stripe — instant scan of the row's seriousness (flat, no glow). */
.alert-row::before {
  content: "";
  grid-column: 1;
  align-self: stretch;
...
.alert-main { grid-column: 2; min-width: 0; display: flex; flex-direction: column; gap: 4px; }
.alert-top { display: flex; align-items: center; gap: 8px; min-width: 0; }
.alert-title {
  font-weight: 600;
  font-size: 13.5px;
  white-space: nowrap;
...
.alert-meta { display: flex; align-items: center; gap: 8px; min-width: 0; }
.alert-sub { font-size: 12px; color: rgba(var(--v-theme-on-surface), 0.55); }
.alert-aside {
  grid-column: 3;
  display: flex;
...
.alert-time { font-size: 12px; color: rgba(var(--v-theme-on-surface), 0.6); }
.alert-tk { font-family: ui-monospace, Menlo, monospace; font-size: 11px; color: rgba(var(--v-theme-on-surface), 0.4); }
...
.anomaly-tile { display: flex; align-items: center; gap: 10px; text-decoration: none; color: rgb(var(--v-theme-on-surface));
  border: 1px solid rgba(var(--v-theme-warning), .4); background: rgba(var(--v-theme-warning), .07);
  border-radius: 11px; padding: 12px 14px; font-size: 13.5px;
  b { font-variant-numeric: tabular-nums; }
  &__sub { font-size: 11.5px; color: rgba(var(--v-theme-on-surface), .55); }
  &:hover { border-color: rgb(var(--v-theme-warning)); } }
