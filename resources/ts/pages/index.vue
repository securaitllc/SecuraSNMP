...
import { easternInputToIso } from '@/utils/datetime'
...
/** Fleet WAN throughput, formatted for the traffic panel. */
...
const trafficSeries = computed(() => (data.value?.traffic?.series ?? []).map(p => p.in + p.out))

const discardPct = computed(() => {
  const t = data.value?.traffic
  if (!t)
    return '—'
  const total = t.in_total + t.out_total
  return total > 0 ? `${((t.discards_total / total) * 100).toFixed(3)}%` : '0%'
})
...
<template>
  <div>
    ...
    <!-- The fleet map, full width: it is the only element that answers "one site or
         the whole region" at a glance, which decides truck roll vs ISP escalation. -->
    <VRow class="mb-2">
      <VCol cols="12">
        <VCard>
          <VCardItem>
    ...
    <VRow class="mb-2">
      <!-- Fleet WAN throughput. WAN-only on purpose: summing every polled interface
           counts LAN ports and uplinks too, so one packet gets counted twice and the
           number means nothing in bytes. These are the fleet's edge ports. -->
      <VCol cols="12" lg="8">
        <VCard>
          <VCardItem>
            <VCardTitle class="text-body-1 d-flex align-center ga-2">
              <VIcon icon="ri-exchange-line" size="18" class="text-primary" />Fleet WAN Throughput
            </VCardTitle>
            <VCardSubtitle>
              Last 24 hours · {{ data?.traffic?.wan_ports ?? 0 }} WAN ports
            </VCardSubtitle>
          </VCardItem>
          <VCardText class="pt-0">
            <div
              v-if="!(data?.traffic?.wan_ports ?? 0)"
              class="text-medium-emphasis text-body-2 py-2"
            >
              No circuit resolves to a WAN port yet — set each circuit's WAN interface,
              or let the appliance's next-hop table map it.
            </div>
            <template v-else>
              <div class="d-flex ga-6 mb-2">
                <div>
                  <div class="text-caption text-medium-emphasis">In</div>
                  <div class="traffic-stat mono">{{ bytes(data?.traffic?.in_total) }}</div>
                </div>
                <div>
                  <div class="text-caption text-medium-emphasis">Out</div>
                  <div class="traffic-stat mono">{{ bytes(data?.traffic?.out_total) }}</div>
                </div>
                <div>
                  <div class="text-caption text-medium-emphasis">Discards</div>
                  <div class="traffic-stat mono">{{ discardPct }}</div>
                </div>
              </div>
              <Sparkline
                v-if="trafficSeries.length"
...
.kpi-value { font-size: 1.5rem; font-weight: 700; line-height: 1.25; margin-block-start: 2px; letter-spacing: -.02em; }
