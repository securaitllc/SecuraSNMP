...
const displayedDevices = computed(() => {
  const scoped = statusFilter.value
    ? siteScoped.value.filter(d => deviceStatusKey(d) === statusFilter.value)
    : siteScoped.value
...
const deviceStatusCounts = computed(() => {
  ...
  for (const d of siteScoped.value) acc[deviceStatusKey(d)]++
  return acc
})
...
function exportInventory() {
  ...
  a.href = '/api/reports/device-inventory/export'
  a.download = 'nodus-device-inventory.xlsx'
