...
// (looking glass + config backup live there).
...
const headers = [
  ...
  { title: 'ST', key: 'status', align: 'center', width: 56 },
  { title: 'Resp Time', key: 'health', sortable: false, width: 138 },
...
const deviceSort = ref<{ key: string, order: 'asc' | 'desc' }[]>([{ key: 'created_at', order: 'desc' }])
...
function exportInventory() {
  // Same-origin GET carries the Sanctum session cookie; the Excel workbook
  // comes back as a download (Content-Disposition attachment).
  const a = document.createElement('a')
  a.href = '/api/inventory/export'
  a.download = 'securasnmp-inventory.xlsx'
...
const suppressSnackbar = ref(false)
const suppressMsg = ref('')
async function clearFalseInterfaceAlarms() {
  ...
  try {
    ...
    suppressMsg.value = res.suppressed > 0
      ? `Muted ${res.suppressed} false interface-down alarm${res.suppressed === 1 ? '' : 's'}. A port that comes back up re-arms automatically.`
      : 'No down admin-up interfaces to clear.'
    suppressSnackbar.value = true
...
async function loadInterfaceGraph() {
  ...
  try {
    ...
    interfaceOctetsSeries.value = [
      { name: 'In', data: metrics.map(m => [new Date(m.recorded_at).getTime(), m.in_octets_delta]) },
      { name: 'Out', data: metrics.map(m => [new Date(m.recorded_at).getTime(), m.out_octets_delta]) },
    ]
    interfaceDiscardsSeries.value = [
      { name: 'In', data: metrics.map(m => [new Date(m.recorded_at).getTime(), m.in_discards_delta]) },
      { name: 'Out', data: metrics.map(m => [new Date(m.recorded_at).getTime(), m.out_discards_delta]) },
    ]
  }
  catch {
    if (requestId === interfaceGraphRequestId)
    ...
    >
      ...
      <template #item.actions="{ item }">
        <VBtn
          ...
          variant="text"
          size="small"
          @click="openDevicePage(item)"
        />
        <VBtn
          icon="ri-list-check-2"
          variant="text"
          size="small"
          @click="openInterfaces(item)"
        />
        <VBtn
