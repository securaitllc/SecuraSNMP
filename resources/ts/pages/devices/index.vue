...
<template>
  <div>
    ...
    <!-- Hostname reconciliation. Every drifted device is pre-selected, so the
         whole migration is one button; unticking is how an exception is kept. -->
    <VDialog v-model="isDriftOpen" transition="slide-x-reverse-transition" content-class="nodus-drawer" scrollable>
      <VCard title="Rename devices to their own hostname">
        <VCardText class="pt-2">
          <div class="text-body-2 text-medium-emphasis mb-4">
            These devices report a hostname over SNMP that differs from the name on record —
            normally a replaced appliance that came up at the same address. Renaming updates
            the name shown everywhere; the previous name is kept on the device with the date.
          </div>

          <div v-for="d in drift" :key="d.id" class="hd-row">
            <VCheckboxBtn v-model="driftPicked" :value="d.id" density="compact" />
            <div class="hd-body">
              <div class="hd-names">
                <span class="hd-was">{{ d.name }}</span>
                <VIcon icon="ri-arrow-right-line" size="14" class="mx-2 text-medium-emphasis" />
                <span class="hd-now">{{ d.snmp_hostname }}</span>
              </div>
              <div class="hd-meta text-caption text-medium-emphasis">
                {{ d.site_name ?? 'No site' }} · {{ d.role }} · {{ d.ip_address }}
              </div>
            </div>
          </div>
        </VCardText>
        <VCardActions>
          <span class="text-caption text-medium-emphasis ms-3">{{ driftPicked.length }} selected</span>
          <VSpacer />
...
.hd-row {
  ...
  gap: 4px;
  padding: 10px 0;
  border-block-end: 1px solid rgb(var(--v-theme-on-surface), .08);
}
.hd-row:last-child { border-block-end: 0; }
.hd-body { min-inline-size: 0; }
.hd-names { display: flex; align-items: center; flex-wrap: wrap; font-size: .875rem; }
.hd-was { color: rgb(var(--v-theme-on-surface), .55); text-decoration: line-through; }
.hd-now { font-weight: 600; }
.hd-meta { margin-block-start: 2px; }
</style>
