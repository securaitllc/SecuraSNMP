...
import { easternChartMs, easternInputToIso, formatDateTime } from '@/utils/datetime'
...
// Baseline-deviation anomalies scoped to this device + its interfaces — surfaced here
// so an operator who clicks an anomaly on the dashboard lands ON the problem instead of
// the top of the page. Shape mirrors AnomalyController's payload.
interface DeviceAnomaly {
  id: number; entity_type: 'interface' | 'device' | 'circuit'; entity: string
  sub: string | null; metric: string; metric_key: string; direction: string
  baseline: number | null; observed: number | null; z_score: number
  series?: (number | null)[]; detected_at: string | null; last_seen_at: string | null
}
const deviceAnomalies = ref<DeviceAnomaly[]>([])
const focusedAnomalyId = ref<number | null>(null)

async function loadAnomalies() {
  deviceAnomalies.value = (await api<{ data: DeviceAnomaly[] }>(`/api/anomalies?device=${deviceId.value}`).catch(() => ({ data: [] as DeviceAnomaly[] }))).data
}

// Severity colour by |z|, matching the dashboard rail: ≥5σ red, ≥3σ amber, else info.
function zColor(z: number): string {
  return Math.abs(z) >= 5 ? 'rgb(var(--v-theme-error))' : Math.abs(z) >= 3 ? 'rgb(var(--v-theme-warning))' : 'rgb(var(--v-theme-info))'
}

// Open the interface an anomaly points at (interface anomalies only), so the row is a
// one-click path from "throughput off baseline" to that port's detail/history.
function openAnomaly(a: DeviceAnomaly) {
  if (a.entity_type !== 'interface')
    return
  const match = interfaces.value.find(i => i.if_name === a.entity)
  if (match)
    void openInterface(match)
}
...
async function loadAll() {
  ...
  try {
    ...
    // Deep-link from a dashboard interface alarm (?interface=ge-0/0/5): open that exact
    // port for troubleshooting (TDR, notes, history) instead of hunting the list.
    const wantIf = route.query.interface
    if (typeof wantIf === 'string' && wantIf) {
      const match = ifaces.find(i => i.if_name === wantIf)
      if (match)
        void openInterface(match)
    }
    void loadSparklines() // non-blocking — sparklines fill in a moment later
    // Deep-link from a dashboard anomaly click (?focusAnomaly=42): scroll to the anomalies
    // section and highlight the exact row so the operator lands on the problem.
    const wantAnom = Number(route.query.focusAnomaly)
    if (wantAnom && deviceAnomalies.value.some(a => a.id === wantAnom)) {
      focusedAnomalyId.value = wantAnom
      requestAnimationFrame(() => setTimeout(() => scrollToSection('anomalies'), 150))
    }
...
// ---- planned maintenance -----------------------------------------------------
// A window suppresses notifications AND takes the planned downtime out of the
// availability figures. It is never a claim that the device is healthy.
const maintOpen = ref(false)
const maintSaving = ref(false)
const maintHours = ref<number | 'custom'>(4)
const maintUntil = ref('')
const maintReason = ref('')
const maintError = ref('')

/** Long jobs are normal — a switch swap or a site build runs for weeks. */
const maintDurations = [
  ...
  { title: '2 weeks', value: 336 },
  { title: '30 days', value: 720 },
  { title: 'Until a date I pick…', value: 'custom' },
]

const maintenanceLeft = computed(() => {
  const until = device.value?.maintenance_until
  if (!until)
    return ''
  const mins = Math.round((new Date(until).getTime() - Date.now()) / 60000)
  if (mins <= 0)
    return ''
  if (mins < 60)
    return `${mins}m left`
  ...
  // Past a day a countdown in hours stops being readable — give the end date.
  return `${Math.round(mins / 1440)}d left · until ${new Date(until).toLocaleDateString()}`
})

/** Re-read just the device, so the badge reflects the new state immediately. */
async function refreshDevice() {
  const r = await api<{ data: Device }>(`/api/devices/${deviceId.value}`)
...
async function startMaintenance() {
  ...
  maintError.value = ''
  try {
    const custom = maintHours.value === 'custom'

    await api(`/api/devices/${deviceId.value}/maintenance`, {
      method: 'POST',
      body: custom
        ? { ends_at: easternInputToIso(maintUntil.value), reason: maintReason.value || null }
        : { hours: maintHours.value, reason: maintReason.value || null },
    })
    ...
    maintError.value = e?.data?.errors?.ends_at?.[0]
      ?? e?.data?.errors?.hours?.[0]
      ?? e?.data?.message
...
async function endMaintenance() {
  maintSaving.value = true
  try {
    await api(`/api/devices/${deviceId.value}/maintenance`, { method: 'DELETE' })
    await refreshDevice()
  }
  finally { maintSaving.value = false }
}
...
function unreachableBands(ms: { recorded_at: string, response_time_ms: number | null }[]) {
  ...
  return capped.map(s => ({ x: s.start - step / 2, x2: s.end + step / 2, fillColor: SEV_CRITICAL, opacity: 0.16, borderWidth: 0 }))
}
...
// Device-level MAC table: every endpoint this device has learned, with its IP (from the
// site's ARP), searchable — so an operator can hand a sysadmin the MAC ↔ IP ↔ port here.
interface DeviceMac {
  id: number
  mac: string
  ip: string | null
...
const macQuery = ref('')
const filteredDeviceMacs = computed(() => {
  const term = macQuery.value.trim().toLowerCase()
  if (!term)
    return deviceMacs.value
  return deviceMacs.value.filter(m => [m.mac, m.ip, m.oui_vendor, m.interface_name].some(v => (v ?? '').toLowerCase().includes(term)))
})
async function loadDeviceMacs() {
  try { deviceMacs.value = (await api<{ data: DeviceMac[] }>(`/api/mac-addresses?device_id=${deviceId.value}`)).data }
  catch { deviceMacs.value = [] }
}
...
const ifTrafficOptions = computed(() => {
  ...
  const colors = metric === 'errors' ? [SEV_CRITICAL, SEV_WARNING] : metric === 'discards' ? [SEV_WARNING, '#A855F7'] : [SEV_OK, '#3B82F6']
    ...
    annotations: isTraffic
      ? {
          yaxis: avgIn > 0
            ...
            : [],
          xaxis: ifErrorEvents.value.map(x => ({ x, borderColor: SEV_CRITICAL, strokeDashArray: 0, opacity: 0.5, label: { text: 'errors', style: { color: '#fff', background: SEV_CRITICAL, fontSize: '9px' } } })),
          points: t.peakMbps > 0
            ? [{ x: t.peakAt, y: +t.peakMbps.toFixed(3), marker: { size: 5, fillColor: SEV_OK, strokeColor: '#fff' }, label: { text: `peak ${t.peakMbps >= 1000 ? `${(t.peakMbps / 1000).toFixed(1)}G` : `${t.peakMbps.toFixed(0)}M`}bps`, style: { color: '#fff', background: SEV_OK, fontSize: '9px' } } }]
            : [],
        }
...
const healthMeta: Record<string, { label: string, color: string, icon: string }> = {
  ...
  // Not a verdict — the absence of one. Silver Peak ECOS returns zero for
  // ifInErrors/ifOutErrors on every interface, so "Clean" on an SD-WAN port was
  // reporting a counter that was never counting. Grey, not green: the operator
  // needs to know the port is unread, not that it is well.
  unmeasured: { label: 'Not measured', color: 'secondary', icon: 'ri-indeterminate-circle-line' },
}
...
const pageSections = computed(() => [
  ...
  { id: 'anomalies', label: 'Anomalies', icon: 'ri-line-chart-line', show: deviceAnomalies.value.length > 0, chip: deviceAnomalies.value.length || null, attn: deviceAnomalies.value.some(a => Math.abs(a.z_score) >= 5) },
  { id: 'mac-table', label: 'MAC Table', icon: 'ri-fingerprint-line', show: deviceMacs.value.length > 0, chip: deviceMacs.value.length || null, attn: false },
  ...
  <div>
      ...
      <div class="d-flex align-center ga-3 pa-4">
        ...
        <div class="flex-grow-1">
          <div class="d-flex align-center ga-2 flex-wrap">
            ...
            <VChip
              v-if="device?.in_maintenance"
              size="small"
              color="info"
              variant="flat"
              prepend-icon="ri-tools-line"
            >
              In maintenance{{ maintenanceLeft ? ` · ${maintenanceLeft}` : '' }}
            </VChip>
            <VBtn
              v-if="device?.in_maintenance"
              size="x-small" variant="tonal" :loading="maintSaving" @click="endMaintenance"
            >
              End maintenance
            </VBtn>
            <VBtn
              v-else-if="device"
              size="x-small" variant="tonal" prepend-icon="ri-tools-line" @click="maintOpen = true"
            >
              Maintenance
            </VBtn>
            <VChip
          ...
          <section>
            ...
            <dl class="dev-detail__dl">
              ...
              <dd class="mono">
                ...
                <!-- The unit behind this IP changed after a recovery. Kept visible
                     because an inventory value that changes silently is a story
                     nobody can reconstruct months later. -->
                <span
                  v-if="device?.previous_serial_number"
                  class="text-caption text-medium-emphasis d-block"
                >
                  Hardware replaced{{ device.hardware_changed_at ? ' ' + formatDateTime(device.hardware_changed_at) : '' }} — was {{ device.previous_serial_number }}
                </span>
              </dd>
    ...
    <VRow id="interfaces" class="mb-1 scroll-anchor">
          ...
          >
            ...
            <template #item.spark="{ item }">
              <svg v-if="sparkPoints(sparklines[item.id]?.in)" class="if-spark" width="88" height="22" viewBox="0 0 88 22">
                <polyline fill="none" :stroke="SEV_OK" stroke-width="1.4" :points="sparkPoints(sparklines[item.id]?.in)" />
      ...
      <VCol cols="12">
        ...
        <!-- MAC Table: every endpoint this device has learned, with its IP (from the site's
             ARP) and port — searchable, so a MAC ↔ IP ↔ port trace is one place. -->
        <!-- Anomalies: baseline-deviation on this device + its interfaces. Deep-link target
             for a dashboard anomaly click — the ?focusAnomaly row is highlighted here. -->
        <VCard v-if="deviceAnomalies.length" id="anomalies" class="mb-5 scroll-anchor">
          <VCardItem>
            <VCardTitle class="d-flex align-center flex-wrap ga-2">
              <VIcon icon="ri-line-chart-line" size="20" />
              Anomalies
              <VChip size="small" label class="ms-1">{{ deviceAnomalies.length }}</VChip>
              <span class="text-medium-emphasis text-body-2 ms-1">off baseline</span>
            </VCardTitle>
          </VCardItem>
          <VDataTable
            :headers="[
              { title: 'Metric', key: 'metric', minWidth: 120 },
              { title: 'Where', key: 'entity', minWidth: 180 },
              { title: 'Trend', key: 'series', width: 90, sortable: false },
              { title: 'Baseline → Observed', key: 'observed', minWidth: 170, sortable: false },
              { title: 'Deviation', key: 'z_score', width: 110 },
              { title: 'Since', key: 'detected_at', minWidth: 150 },
            ]"
            :items="deviceAnomalies"
            :items-per-page="10"
            density="compact"
            :row-props="(d) => ({ class: d.item.id === focusedAnomalyId ? 'anom-focus-row' : '' })"
          >
            <template #item.metric="{ item }">
              <span class="mini-stripe d-inline-block me-2" :style="{ background: zColor(item.z_score), width: '3px', height: '14px', verticalAlign: 'middle' }" />
              {{ item.metric }}
            </template>
            <template #item.entity="{ item }">
              <a v-if="item.entity_type === 'interface'" class="font-mono anom-link" @click="openAnomaly(item)">{{ item.entity }}</a>
              <span v-else class="font-mono">{{ item.entity }}</span>
              <VChip size="x-small" variant="tonal" class="ms-1">{{ item.entity_type }}</VChip>
            </template>
            <template #item.series="{ item }">
              <Sparkline :points="item.series ?? []" :color="zColor(item.z_score)" :width="72" :height="20" />
            </template>
            <template #item.observed="{ item }">
              <span class="text-medium-emphasis">{{ item.baseline?.toLocaleString() ?? '—' }}</span>
              <VIcon icon="ri-arrow-right-line" size="14" class="mx-1 text-medium-emphasis" />
              <span :style="{ color: zColor(item.z_score) }">{{ item.observed?.toLocaleString() ?? '—' }}</span>
              <VIcon :icon="item.direction === 'low' ? 'ri-arrow-down-line' : 'ri-arrow-up-line'" size="14" class="ms-1" :style="{ color: zColor(item.z_score) }" />
            </template>
            <template #item.z_score="{ item }">
              <span class="font-weight-medium" :style="{ color: zColor(item.z_score) }">{{ item.z_score >= 0 ? '+' : '' }}{{ item.z_score.toFixed(1) }}σ</span>
            </template>
            <template #item.detected_at="{ item }">{{ formatDateTime(item.detected_at) }}</template>
          </VDataTable>
        </VCard>

        <VCard v-if="deviceMacs.length" id="mac-table" class="mb-5 scroll-anchor">
          <VCardItem>
            <VCardTitle class="d-flex align-center flex-wrap ga-2">
              <VIcon icon="ri-fingerprint-line" size="20" />
              MAC Table
              <VChip size="small" label class="ms-1">{{ deviceMacs.length }}</VChip>
              <VSpacer />
              <VTextField
                v-model="macQuery"
                placeholder="MAC, IP, vendor, port…"
                prepend-inner-icon="ri-search-line"
                density="compact"
                hide-details
                clearable
          ...
          <VDataTable
            :headers="[
              ...
              { title: 'Vendor (OUI)', key: 'oui_vendor', minWidth: 180 },
              { title: 'Port', key: 'interface_name', minWidth: 130 },
              { title: 'VLAN', key: 'vlan', width: 80 },
              { title: 'Last seen', key: 'last_seen_at', minWidth: 160 },
            ]"
            :items="filteredDeviceMacs"
            :items-per-page="15"
            density="compact"
          >
            <template #item.mac="{ item }"><CopyBtn :text="item.mac" class="font-mono" /></template>
            ...
            <template #no-data><div class="pa-4 text-medium-emphasis">No MACs match.</div></template>
          </VDataTable>
        </VCard>
    ...
    <!-- Planned maintenance: mutes notifications and takes the downtime out of the
         availability figures. It never claims the device is healthy. -->
    <VDialog v-model="maintOpen" max-width="440">
      <VCard>
        <VCardTitle>Put this device under maintenance</VCardTitle>
        <VCardText>
          <p class="text-body-2 text-medium-emphasis mb-4">
            Alerts stay quiet and the downtime is excluded from availability reporting.
            The device still shows as <b>in maintenance</b> rather than reachable.
          </p>
          <VAlert v-if="maintError" type="error" variant="tonal" density="compact" class="mb-4">
            {{ maintError }}
          </VAlert>
          <VSelect
            v-model="maintHours" :items="maintDurations"
            item-title="title" item-value="value" label="For how long"
            variant="outlined" density="comfortable" class="mb-4"
          />
          <VTextField
            v-if="maintHours === 'custom'"
            v-model="maintUntil" type="datetime-local" label="Ends at"
            variant="outlined" density="comfortable" class="mb-4"
          />
          <VTextField
            v-model="maintReason" label="Reason" placeholder="Switch firmware upgrade"
            variant="outlined" density="comfortable"
          />
        </VCardText>
        <VCardActions class="justify-end pa-4 pt-0">
          <VBtn variant="tonal" @click="maintOpen = false">
            Cancel
          </VBtn>
          <VBtn
            color="primary" :loading="maintSaving"
            :disabled="maintHours === 'custom' && !maintUntil"
...
.anom-link { color: rgb(var(--v-theme-primary)); cursor: pointer; text-decoration: none; }
.anom-link:hover { text-decoration: underline; }
/* Deep-linked anomaly row: a brief pulse so the eye lands on it after the scroll. */
.anom-focus-row > td { background: rgba(var(--v-theme-warning), 0.14) !important; }
