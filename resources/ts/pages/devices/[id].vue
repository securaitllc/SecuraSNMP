...
async function runTool(tool: 'ping' | 'traceroute' | 'snmpwalk' | 'snmptest') {
...
const interfaceHeaders = [
  ...
  { title: 'Status', key: 'status', width: 108 },
  { title: 'In', key: 'in_octets', align: 'end' as const },
  { title: 'Out', key: 'out_octets', align: 'end' as const },
  { title: 'Utilization', key: 'utilization', width: 200, sortable: false },
  { title: 'In disc Δ', key: 'in_discards_delta', align: 'end' as const },
  { title: 'Out disc Δ', key: 'out_discards_delta', align: 'end' as const },
]
        ...
        <VCard>
          ...
          >
            ...
            <template #item.status="{ item }">
              <VChip
                size="x-small" label
                :color="item.admin_status === 'down' ? 'secondary' : item.status === 'up' ? 'success' : 'error'"
                variant="tonal"
                class="if-status"
              >
                {{ item.admin_status === 'down' ? 'Disabled' : item.status === 'up' ? 'Up' : 'Down' }}
              </VChip>
            </template>
            ...
            <template #item.in_discards_delta="{ item }">
              <span class="if-num" :class="item.in_discards_delta > 0 ? 'text-error font-weight-medium' : 'text-medium-emphasis'">{{ item.in_discards_delta }}</span>
            </template>
            <template #item.out_discards_delta="{ item }">
              <span class="if-num" :class="item.out_discards_delta > 0 ? 'text-error font-weight-medium' : 'text-medium-emphasis'">{{ item.out_discards_delta }}</span>
            </template>
          </VDataTable>
        </VCard>
      </VCol>
