...
// A VLAN name worth showing next to the green id — with any echoed id stripped
// so it isn't shown twice. Handles a "+id"/"-id" suffix ("MASSEY-WIFI+999"),
// a name that IS the id ("30"), and "VLANnnnn".
function vlanName(v: DeviceVlan): string {
  const id = String(v.vlan_id)
  let n = (v.name || '').trim()
  // Drop a trailing separator + the id (with optional leading zeros): MASSEY-WIFI+999.
  n = n.replace(new RegExp(`\\s*[+\\-_/]\\s*0*${id}$`), '').trim()
  if (!n || n === id || /^vlan0*\d+$/i.test(n))
    return ''
  return n
}
...
async function runTdr() {
  ...
  tdrOpen.value = true
  try {
    tdrResult.value = await api<ToolResult>(`/api/devices/${deviceId.value}/tools/tdr`, { method: 'POST', body: { interface: selectedInterface.value.if_name } })
  }
  catch (e: any) {
...
const pageSections = computed(() => [
  ...
  { id: 'endpoints', label: 'Connected Endpoints', icon: 'ri-plug-line', show: sortedNeighbors.value.length > 0, chip: liveNeighborCount.value || null, attn: false },
          ...
          <VCardText>
            ...
            <div class="vlan-grid">
              <div
                ...
                class="vlan-tag"
              >
                <span class="vlan-tag__id">{{ vlan.vlan_id }}</span>
        ...
        <VCardItem>
          ...
          <template #append>
            <div class="d-flex align-center ga-3">
              ...
              <VBtnToggle v-model="interfaceRange" density="comfortable" variant="outlined" mandatory divided class="if-seg if-seg--narrow">
        ...
        <VCardText>
          ...
          <VRow class="mb-2">
            <VCol cols="6" sm="3">
              ...
              <div v-if="linkChangeNote" class="text-caption text-warning mt-1">⚠ {{ linkChangeNote }}</div>
            </VCol>
            <VCol cols="6" sm="3">
          ...
          <div class="if-chart-head">
            <VBtnToggle v-model="chartMetric" density="comfortable" variant="outlined" mandatory divided class="if-seg">
              <VBtn v-for="o in chartMetricOptions" :key="o.value" :value="o.value" size="small" :prepend-icon="o.icon">{{ o.label }}</VBtn>
            </VBtnToggle>
...
/* Segmented metric selector — matches the app's compact toggle style. */
.if-seg {
  block-size: 34px;
  flex: none;                 /* never let a neighbour (totals text) squish it */
  border-radius: 8px;
...
.if-seg :deep(.v-btn) {
  block-size: 34px;
  min-inline-size: 92px;      /* room for an icon + label (Discards / Errors) */
  padding-inline: 14px;
  white-space: nowrap;
  font-size: 0.8rem;
  font-weight: 600;
...
/* The range toggle has short labels (1h/6h) — narrower segments. */
.if-seg--narrow :deep(.v-btn) { min-inline-size: 52px; }
.if-seg :deep(.v-btn .v-icon) { font-size: 15px; margin-inline-end: 4px; }
.if-seg :deep(.v-btn--active) {
  background: rgba(var(--v-theme-primary), 0.16);
  color: rgb(var(--v-theme-primary));
  opacity: 1;
}
.if-seg :deep(.v-btn:not(:last-child)) { border-inline-end: 1px solid rgba(var(--v-border-color), var(--v-border-opacity)); }
