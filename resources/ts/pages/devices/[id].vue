...
const interfaceHeaders = [
  ...
  { title: 'Health', key: 'health', width: 132 },
  { title: '24h', key: 'spark', width: 104, sortable: false },
  { title: 'Utilization', key: 'utilization', width: 210, sortable: false },
  { title: 'Peak', key: 'peak_util_pct', align: 'end' as const, width: 74 },
  { title: 'In', key: 'in_octets', align: 'end' as const },
  { title: 'Out', key: 'out_octets', align: 'end' as const },
]
  ...
  return {
    ...
    annotations: {
      yaxis: avgIn > 0
        ...
        : [],
      // CRC-error bursts as red markers along the time axis.
      ...
      points: t.peak > 0
        ? [{ x: t.peakAt, y: +(t.peak / 1e6).toFixed(3), marker: { size: 5, fillColor: '#22c55e', strokeColor: '#fff' }, label: { text: `peak ${(t.peak / 1e6).toFixed(0)}M`, style: { color: '#fff', background: '#16a34a', fontSize: '9px' } } }]
        : [],
    },
  }
})
...
const ifTrafficTotals = computed(() => {
  ...
  for (const m of interfaceMetrics.value) {
    const bps = Math.max(bitsPerSec(m.in_octets_delta), bitsPerSec(m.out_octets_delta))
    if (bps > peak) { peak = bps; peakAt = easternChartMs(m.recorded_at) }
  }
  return { inB, outB, peak, peakAt }
})
      ...
      <VCol cols="12" md="8">
        <VCard>
          ...
          >
            ...
            <template #item.in_octets="{ item }">
              <span class="if-num">{{ formatBytes(item.in_octets) }}</span>
            </template>
            <template #item.out_octets="{ item }">
              <span class="if-num">{{ formatBytes(item.out_octets) }}</span>
            </template>
          </VDataTable>
        </VCard>
      </VCol>
          ...
          <VCardText>
            ...
            <div class="vlan-grid">
              ...
              >
                ...
                <span class="vlan-tag__name">{{ vlan.name || 'unnamed' }}</span>
              </div>
            </div>
          </VCardText>
        </VCard>
      ...
      <VCard v-if="selectedInterface">
        ...
        <VCardText>
          ...
          <div class="if-chart-head">
            <span class="text-subtitle-2">Traffic ({{ interfaceRange }})</span>
            <span v-if="interfaceMetrics.length" class="text-caption text-medium-emphasis">
              {{ formatBytes(ifTrafficTotals.inB) }} in / {{ formatBytes(ifTrafficTotals.outB) }} out · peak {{ formatBps(ifTrafficTotals.peak) }}bps
            </span>
          </div>
          <VueApexCharts
...
.if-chart-head { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; margin-block-end: 6px; flex-wrap: wrap; }
