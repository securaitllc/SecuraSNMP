...
// The poller already stores a clean name + the real tag, so just hide a name
// that is empty, literally the id, or a bare "VLANnnnn" placeholder.
function vlanName(v: DeviceVlan): string {
  const n = (v.name || '').trim()
  if (!n || n === String(v.vlan_id) || /^vlan0*\d+$/i.test(n))
    return ''
  return n
}
