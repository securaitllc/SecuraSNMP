...
interface LearnedMac { mac: string, oui_vendor: string | null, vlan: number, first_seen_at: string, last_seen_at: string }
...
async function loadInterfaceData() {
  ...
  const [metrics, alerts, macs] = await Promise.all([
    ...
    api<LearnedMac[]>(`/api/mac-addresses?interface_id=${id}`),
  ])
  interfaceMetrics.value = metrics
  interfaceAlerts.value = alerts
  interfaceMacs.value = macs
