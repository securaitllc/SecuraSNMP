...
<template>
  <div>
    <div class="page-head">
      <div class="page-head__id">
        <div class="isp-header-mark">
    ...
    >
      <template #item.name="{ item }">
        <span class="d-flex align-center ga-3 py-1">
          <span class="isp-logo mono">{{ (item.name || '?').slice(0, 2).toUpperCase() }}</span>
          <span class="font-weight-medium">{{ item.name }}</span>
        </span>
      </template>
      <template #item.support_phone="{ item }">
        <span
          v-if="item.support_phone"
          class="mono text-high-emphasis"
        >{{ item.support_phone }}</span>
        <span
          v-else
          class="text-disabled"
        >—</span>
      ...
      <template #item.circuits_down_count="{ item }">
        <VChip
          v-if="(item.circuits_down_count ?? 0) > 0"
          size="x-small"
          color="error"
          variant="tonal"
          class="font-weight-medium"
        >
          {{ item.circuits_down_count }} down
        </VChip>
        <VChip
          v-else
          size="x-small"
                  ...
                  <VCard variant="tonal" class="pa-3">
                    <div class="d-flex align-center ga-2 mb-2">
                      ...
                      <a
                        ...
                        class="mono font-weight-medium text-high-emphasis"
                    ...
                    >
                      ...
                      <a class="mono" :href="`tel:${overviewFor(item.id)!.contact.account_rep_mobile}`">{{ overviewFor(item.id)!.contact.account_rep_mobile }}</a>
                    </div>
                    <div
                    ...
                    >
                      ...
                      <a class="mono" :href="`tel:${overviewFor(item.id)!.contact.account_rep_phone}`">{{ overviewFor(item.id)!.contact.account_rep_phone }}</a>
                    </div>
                    <div
