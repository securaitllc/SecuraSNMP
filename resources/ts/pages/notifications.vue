...
function emptyWindow() {
  ...
  const local = (d: Date) => easternInputValue(d.toISOString())
  ...
  try {
    await api('/api/maintenance-windows', {
      ...
      body: {
        ...
        starts_at: easternInputToIso(windowForm.value.starts_at),
        ends_at: easternInputToIso(windowForm.value.ends_at),
...
<template>
  ...
  <div v-else class="d-flex flex-column gap-6">
    <div class="mb-n2">
      <h4 class="text-h4 mb-1">Notifications</h4>
      <p class="text-body-2 text-medium-emphasis mb-0">
        How Nodus reaches the NOC — mail server, alert channels and maintenance windows.
      </p>
    </div>
    <!-- SMTP server -->
    ...
    <!-- Channel dialog -->
    <VDialog v-model="isChannelDialogOpen" transition="slide-x-reverse-transition" content-class="nodus-drawer">
    ...
    <!-- Window dialog -->
    <VDialog v-model="isWindowDialogOpen" transition="slide-x-reverse-transition" content-class="nodus-drawer">
