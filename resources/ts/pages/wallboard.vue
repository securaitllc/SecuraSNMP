...
// Sites whose only alarms are on devices inside a maintenance window. They are not
// healthy and not impacted, so the healthy tile must say where they went — a
// "128/131 healthy" with three sites unaccounted for reads as a rounding error.
const maintenanceSites = computed(() => (data.value?.sites ?? []).filter(s => s.health === 'maintenance').length)
  ...
  <div class="wallboard">
    ...
    <div class="wb-tiles">
      ...
      <div class="wb-tile ok">
        ...
        <div class="wb-label">Sites Healthy<span v-if="maintenanceSites" class="wb-sub"> · {{ maintenanceSites }} in maintenance</span></div>
      </div>
    </div>
...
.wb-tile.bad .wb-num { color: rgb(var(--v-theme-error)); }
...
.wb-sub { text-transform: none; letter-spacing: 0; color: #64748b; }
...
.wb-pinhdr { font-size: 0.74rem; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; color: rgb(var(--v-theme-error)); margin: 4px 0 10px; }
...
.wb-row.down, .wb-row.critical { border-inline-start-color: rgb(var(--v-theme-error)); background: #1a1113; }
.wb-row.degraded, .wb-row.warning { border-inline-start-color: rgb(var(--v-theme-warning)); }
...
.wb-tkt.pending { color: rgb(var(--v-theme-error)); font-weight: 700; }
