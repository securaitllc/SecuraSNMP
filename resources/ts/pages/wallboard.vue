...
<template>
  <div class="wallboard">
    ...
    <div class="wb-alerts">
      <div v-if="!hasAny" class="wb-clear">✓ All clear</div>
      ...
      <template v-else>
        ...
        >
          ...
          <span v-else-if="r.isp_ticket" class="wb-tkt">🎫 {{ r.isp_ticket }}<span v-if="r.dispatch_at"> · 🚚 {{ dispatchLabel(r.dispatch_at) }}</span></span>
          ...
          <div class="wb-track" :class="{ 'wb-scrolling': scrolling }" :style="scrolling ? { animationDuration: scrollDuration } : undefined">
            <div ref="copy" class="wb-copy">
              <div v-for="r in scrollRows" :key="r.key" class="wb-row" :class="r.state">
                ...
                <span v-if="r.path" class="wb-path">{{ r.path }}</span>
                <span class="wb-count">{{ r.count }} alarm{{ r.count > 1 ? 's' : '' }}</span>
                <span v-if="r.isp_ticket" class="wb-tkt">🎫 {{ r.isp_ticket }}<span v-if="r.dispatch_at"> · 🚚 {{ dispatchLabel(r.dispatch_at) }}</span></span>
                <span class="wb-age">{{ ageOf(r.oldest_at) }}</span>
              </div>
            </div>
