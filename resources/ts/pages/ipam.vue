...
type Row = {
  ip: string
  state: 'assigned' | 'discovered' | 'conflict'
  mac: string | null
  vendor: string | null
...
const search = ref('')
const kindFilter = ref<'all' | 'wan' | 'lan' | 'attn'>('all')
...
const counts = computed(() => {
  ...
  return {
    all: new Set(all.map(r => r.cidr)).size,
    wan: new Set(all.filter(r => r.kind === 'wan').map(r => r.cidr)).size,
    lan: new Set(all.filter(r => r.kind === 'lan').map(r => r.cidr)).size,
    attn: new Set(all.filter(r => r.state !== 'ok').map(r => r.cidr)).size,
  }
})
...
const detailFilter = ref<'all' | 'assigned' | 'discovered' | 'conflict'>('all')
...
function stateColor(s: string) {
  return s === 'conflict' ? 'error' : s === 'assigned' ? 'primary' : 'info'
}
...
<template>
  <div>
    ...
    <VNavigationDrawer v-model="detailOpen" temporary location="end" width="720" class="nodus-drawer">
      <VCard flat class="d-flex flex-column" style="block-size: 100%">
        ...
        <div class="pa-5 flex-grow-1 overflow-auto">
          ...
          <div class="list-pills mb-3">
            <button
              v-for="f in ['all', 'assigned', 'discovered', 'conflict']" :key="f"
          ...
          <VTable density="compact">
            <thead>
              <tr>
                <th>Address</th><th>State</th><th>On interface</th><th>MAC</th><th>Host</th><th>Seen</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="r in detailVisible" :key="r.ip">
                ...
                <td>
                  <div>{{ r.device_name || (r.vendor ? r.vendor : 'Unassigned host') }}</div>
                  <div v-if="r.device_name && r.vendor" class="text-caption text-disabled">
                    {{ r.vendor }}
                  </div>
                </td>
