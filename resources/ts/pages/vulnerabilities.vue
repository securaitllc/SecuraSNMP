...
interface DevFinding {
  id: number; cve_id: string; severity: string; cvss_score: number | null
  summary: string | null; reference_url: string | null; matched_constraint: string | null; state: string
}
const expandedDevice = ref<number | null>(null)
const deviceFindings = ref<Record<number, DevFinding[]>>({})
const deviceLoading = ref<number | null>(null)
async function toggleDevice(id: number) {
  ...
  if (!deviceFindings.value[id]) {
    deviceLoading.value = id
    try {
      const res = await api<{ data: DevFinding[] }>(`/api/devices/${id}/vulnerabilities`)
      deviceFindings.value = { ...deviceFindings.value, [id]: res.data }
    }
    finally { deviceLoading.value = null }
  }
}
...
<template>
  <div class="d-flex flex-column ga-6">
        ...
        <VCard class="h-100">
          ...
          <VList class="py-0">
            <template v-for="(d, i) in summary?.top_devices ?? []" :key="d.device_id">
              <VListItem class="cursor-pointer" @click="toggleDevice(d.device_id)">
                <template #prepend>
                ...
                <template #append>
                  <div class="d-flex align-center ga-1">
                    ...
                    <VIcon :icon="expandedDevice === d.device_id ? 'ri-arrow-up-s-line' : 'ri-arrow-down-s-line'" size="18" class="text-medium-emphasis" />
                  </div>
                </template>
              </VListItem>

              <!-- Inline CVEs for the expanded device — the actual vulnerabilities,
                   not just a count, each clickable through to NVD. -->
              <div v-if="expandedDevice === d.device_id" class="exposed-cves px-4 pb-3">
                <div v-if="deviceLoading === d.device_id" class="text-caption text-medium-emphasis py-2">Loading CVEs…</div>
                <div v-else-if="!(deviceFindings[d.device_id]?.length)" class="text-caption text-medium-emphasis py-2">No open CVEs.</div>
                <div
                  v-for="f in (deviceFindings[d.device_id] ?? [])"
                  :key="f.id"
                  class="d-flex align-center ga-2 py-1"
                >
                  <VChip :color="sevColor(f.severity)" size="x-small" label class="text-capitalize" style="min-inline-size: 66px; justify-content: center">{{ f.severity }}</VChip>
                  <span class="font-weight-bold text-caption" :class="`text-${sevColor(f.severity)}`" style="inline-size: 30px">{{ f.cvss_score?.toFixed(1) ?? '—' }}</span>
                  <a :href="f.reference_url ?? '#'" target="_blank" rel="noopener" class="cve-link d-inline-flex align-center ga-1 text-no-wrap">
                    <code class="text-caption">{{ f.cve_id }}</code><VIcon icon="ri-external-link-line" size="12" />
                  </a>
                  <span class="text-caption text-medium-emphasis flex-grow-1">{{ f.summary }}</span>
                  <VChip size="x-small" variant="tonal" label class="text-no-wrap">{{ f.matched_constraint }}</VChip>
                </div>
                <RouterLink :to="`/devices/${d.device_id}`" class="text-caption d-inline-block mt-1">Open device page →</RouterLink>
              </div>
    ...
    <VCard>
      <VCardItem>
        <VCardTitle>Findings</VCardTitle>
        <template #append>
          <div class="d-flex flex-wrap align-center ga-2">
            <VSelect
              v-model="vendorFilter" :items="vendors" label="Vendor" clearable density="compact"
              variant="outlined" hide-details style="min-width: 140px" class="text-capitalize"
            />
            <VBtnToggle v-model="stateFilter" density="compact" variant="outlined">
              <VBtn value="open" size="small">Open</VBtn>
              <VBtn value="acknowledged" size="small">Ack’d</VBtn>
            </VBtnToggle>
          </div>
        </template>
      </VCardItem>
      ...
      >
        ...
        <template #item.cvss_score="{ item }">
          <span class="font-weight-bold" :class="`text-${sevColor(item.severity)}`">{{ item.cvss_score?.toFixed(1) ?? '—' }}</span>
        </template>
