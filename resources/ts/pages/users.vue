...
// Rank order, matching App\Support\Roles. You can only hand out your own rank and
// below — the API enforces this, and showing a role the server will reject just
// hands the operator a 422 for a choice we knew was impossible.
const ROLE_RANK: Record<string, number> = { display: 0, viewer: 1, analyst: 2, admin: 3, super_admin: 4 }

const allRoleOptions = [
  { title: 'Super-Admin (OSINT)', value: 'super_admin' },
...
const roleOptions = computed(() => {
  const ceiling = ROLE_RANK[auth.user?.role ?? ''] ?? 0

  return allRoleOptions.filter(o => (ROLE_RANK[o.value] ?? 0) <= ceiling)
})

// Accounts above your rank are read-only to you; the API refuses the write either way.
function canManage(user: ManagedUser) {
  return (ROLE_RANK[user.role] ?? 0) <= (ROLE_RANK[auth.user?.role ?? ''] ?? 0)
}
...
async function saveUser() {
  ...
  try {
    ...
    // Show the server's reason. A 422 carries per-field messages; swallowing them
    // left an operator staring at "check the fields" with no idea which one — the
    // password rule (12+ chars, mixed case, a number) was the usual culprit.
    const errs = e?.data?.errors
    errorMessage.value = errs
      ? Object.values(errs as Record<string, string[]>).flat().join(' ')
      : (e?.data?.message || 'Could not save the user. Check the fields and try again.')
  }
  finally {
...
// Unenroll: wipe the user's 2FA so they enrol a fresh authenticator on next login
// (their MFA requirement stays as-is). The fix for a lost/replaced phone.
async function resetTwoFactor(user: ManagedUser) {
  if (!confirm(`Reset two-factor for "${user.name}"? They'll set up a new authenticator on their next login.`))
    return

  try {
    await api(`/api/users/${user.id}/reset-two-factor`, { method: 'POST' })
    await loadUsers()
  }
  catch {
    errorMessage.value = 'Could not reset two-factor for this user.'
  }
}
  ...
  <div>
    ...
    >
      ...
      <template #item.actions="{ item }">
        ...
        />
        <VBtn
          v-if="item.two_factor_enabled && canManage(item)"
          icon="ri-shield-keyhole-line"
          variant="text"
          size="small"
          color="warning"
          title="Reset two-factor (lost authenticator)"
          @click="resetTwoFactor(item)"
        />
        <VBtn
