...
function sparkOptionsFor(circuit: Circuit) {
  const problem = circuitLatest.value[circuit.id] === null || (circuit.last_loss_pct ?? 0) > 0

  return { ...sparklineOptions, colors: [problem ? '#ef4444' : '#22c55e'] }
}
  ...
  <VCard title="Circuits">
    ...
    >
      ...
      <template #item.status="{ item }">
        ...
        <VChip
          v-else-if="(item.last_loss_pct ?? 0) >= 20"
          size="small"
          color="warning"
          variant="flat"
          prepend-icon="ri-signal-wifi-error-line"
          :title="`Packet loss ${item.last_loss_pct}% — still passing traffic but degraded`"
        >
          Degraded · {{ item.last_loss_pct }}% loss
        </VChip>
        <span
          v-else
        ...
        >
          ...
          />
          up<span
            v-if="(item.last_loss_pct ?? 0) > 0"
            class="text-warning text-caption"
          > · {{ item.last_loss_pct }}% loss</span>
        </span>
      </template>
