...
async function openRtGraph(circuit: Circuit) {
  ...
  bwHistory.value = null
  await Promise.all([loadRtGraph(), loadBwHistory()])
}
  ...
  >
    <VCard :title="`${rtGraphCircuit?.isp_name ?? ''} ${rtGraphCircuit?.circuit_id ?? ''}`">
      ...
      <VCardText>
        ...
        <!-- Bandwidth, measured on the EdgeConnect WAN port this circuit lands on. -->
        <VDivider class="my-5" />
        <div class="d-flex align-center flex-wrap ga-3 mb-2">
          ...
          <VSpacer />
          <!-- Peak and the Live control travel together, so a wrap of the long sentence
               above never strands the button on a line of its own. -->
        ...
        <!-- Live: reads the WAN counters straight off the appliance every few
             seconds, so it shows now rather than the last 90s poll. -->
        <!-- The reason must outlive the toggle: a failed probe stops live mode, so an
             error nested inside v-if="bwLiveOn" would unmount before it could be read. -->
        ...
        <VAlert
          v-else-if="!bwHistory.points.length"
          type="info" variant="tonal" density="compact"
        >
