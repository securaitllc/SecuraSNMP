...
const headers = [
  ...
  { title: 'ISP', key: 'isp_name', width: 116 },
  { title: 'Type', key: 'circuit_type', width: 70 },
  { title: 'Circuit ID', key: 'circuit_id', width: 152 },
  { title: 'Status', key: 'status', width: 96 },
  { title: 'Response Time', key: 'health', sortable: false, width: 120 },
  ...
  <div>
    ...
    >
      <template #item.site_name="{ item }">
        <span
          class="cx-stripe"
          :style="{ background: `rgb(var(--v-theme-${circuitStatusOf(item) === 'down' ? 'error' : circuitStatusOf(item) === 'degraded' ? 'warning' : circuitStatusOf(item) === 'maintenance' ? 'secondary' : 'success'}))` }"
        />
      ...
      <template #item.circuit_id="{ item }">
        <div class="cx-ckt">
          ...
          <div class="cx-ckt-bw mono">
            <span v-if="item.contract_down_mbps || item.contract_up_mbps">{{ item.contract_down_mbps ?? '—' }}/{{ item.contract_up_mbps ?? '—' }}</span>
...
/* Left status stripe, flush to the row's leading edge (colour = severity). */
.cx-stripe {
  position: absolute;
  inset-block: 6px;
...
.cx-table :deep(th:nth-child(1)), .cx-table :deep(td:nth-child(1)) { width: 172px; }
.cx-table :deep(th:nth-child(2)), .cx-table :deep(td:nth-child(2)) { width: 118px; }
.cx-table :deep(th:nth-child(3)), .cx-table :deep(td:nth-child(3)) { width: 74px; }
.cx-table :deep(th:nth-child(4)), .cx-table :deep(td:nth-child(4)) { width: 156px; }
.cx-table :deep(th:nth-child(5)), .cx-table :deep(td:nth-child(5)) { width: 96px; }
.cx-table :deep(th:nth-child(6)), .cx-table :deep(td:nth-child(6)) { width: 116px; }
