...
const SLA_DEFAULT_BY_TYPE: Record<string, number> = { fiber: 99.9, cable: 99.5, lte: 99.0 }
...
function emptyForm() {
  return {
    ...
    isp_name: '',
    circuit_type: 'fiber',
    sla_target_pct: null as number | null,
      ...
      <VCardText>
        ...
        <VForm @submit.prevent="saveCircuit">
          <VRow>
            ...
            <VCol cols="12">
              <div class="text-caption text-medium-emphasis mt-1">
                Contract — install &amp; expiration dates drive renewal tracking and expiry alerts.
                Contract length is captured when you Renew.
              </div>
            </VCol>
            <VCol cols="6">
              <VTextField
            ...
            <VCol cols="6">
              <VTextField
                v-model.number="form.sla_target_pct"
                label="SLA target %"
                type="number"
                step="0.1"
                min="0"
                max="100"
                clearable
                :placeholder="`default ${slaDefaultForType(form.circuit_type)}`"
                hint="Blank = by-type default (fiber 99.9 / cable 99.5 / lte 99.0)"
                persistent-hint
              />
            </VCol>
            <VCol cols="6">
              <VTextField
                v-model.number="form.contract_down_mbps"
                label="Contract download (Mbps)"
                type="number"
                min="0"
                clearable
                placeholder="e.g. 300"
            ...
            <VCol cols="6">
              <VTextField
                v-model.number="form.contract_up_mbps"
                label="Contract upload (Mbps)"
                type="number"
                min="0"
                clearable
                placeholder="e.g. 20"
