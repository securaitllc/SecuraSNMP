...
async function loadCircuitResponse(circuit: Circuit) {
  try {
    const metrics = await api<CircuitMetric[]>(`/api/circuits/metrics?circuit_id=${circuit.id}&range=6h`)
    const points = metrics.map(m => m.response_time_ms)
    circuitSparklines.value[circuit.id] = points.length > 0 ? points : [null]
    circuitLatest.value[circuit.id] = metrics.length > 0 ? metrics[metrics.length - 1].response_time_ms : undefined
  }
  catch {
...
const rtSeries = ref<{ name: string, data: [number, number | null][] }[]>([])
...
async function loadRtGraph() {
  ...
  try {
    ...
    rtSeries.value = [
      { name: 'Response time', data: metrics.map(m => [Date.parse(m.recorded_at), m.response_time_ms]) },
    ]
  }
  finally {
    if (requestId === rtRequestId)
...
const headers = [
  { title: 'Site', key: 'site_id' },
  { title: 'ISP', key: 'isp_name' },
  { title: 'Circuit ID', key: 'circuit_id' },
  { title: 'Status', key: 'status' },
  { title: 'Response Time', key: 'health', sortable: false },
...
async function loadData() {
  ...
  await Promise.all(circuits.value.map(loadCircuitResponse))
}
...
async function saveCircuit() {
  ...
  try {
    ...
    errorMessage.value = 'Could not save the circuit. Check the fields and try again.'
  }
  finally {
...
async function toggleMonitoring(circuit: Circuit) {
  ...
  if (!enabling && !confirm(`Pause monitoring for "${circuit.circuit_id}"? It won't ping or alarm until you resume it — use this for a planned disconnect.`))
    return
  try {
    ...
    >
        ...
        <div class="d-flex align-center ga-3">
          <div style="width: 72px;">
            <VueApexCharts
              ...
              :series="[{ data: circuitSparklines[item.id] ?? [null] }]"
            />
          </div>
          <span
            class="text-caption"
      ...
      <template #item.actions="{ item }">
        ...
        <VBtn
          ...
          variant="text"
          size="small"
          :title="item.monitoring_enabled === false ? 'Resume monitoring' : 'Pause monitoring (planned disconnect)'"
    ...
    <VCard :title="`Response Time — ${rtGraphCircuit?.isp_name ?? ''} ${rtGraphCircuit?.circuit_id ?? ''}`">
      ...
      <VCardText>
        <div class="text-caption text-medium-emphasis mb-2">
          Round-trip latency in milliseconds. Gaps are timeouts (no response).
        </div>
        <div
