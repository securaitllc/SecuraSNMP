...
interface Correlation {
  ...
  suspect_hop: {
    ...
    end_loss_pct: number
    healthy_crossings: number
    healthy_crossings_lossy: number
...
// Entities that deviated TOGETHER. Fifty latency anomalies from one egress change
// is one event; fifty rows is how the one that matters gets buried.
...
const sharedEvents = ref<SharedEvent[]>([])

const METRIC_LABEL: Record<string, string> = {
  latency: 'latency', loss: 'packet loss', throughput: 'throughput',
  discards: 'discards', errors: 'errors', cpu: 'CPU', memory: 'memory', temperature: 'temperature',
}
const METRIC_UNIT: Record<string, string> = { latency: ' ms', loss: '%', cpu: '%', memory: '%', temperature: '°C' }

function shiftText(e: SharedEvent) {
  const u = METRIC_UNIT[e.metric] ?? ''
  const sign = e.shift > 0 ? '+' : ''

  return `${sign}${e.shift}${u}`
}
...
async function load() {
  loading.value = true
  try {
    const res = await api<{ data: Anomaly[], summary: Summary, correlations: Correlation[], shared_events: SharedEvent[] }>('/api/anomalies')
    rows.value = res.data
    summary.value = res.summary
    correlations.value = res.correlations ?? []
    sharedEvents.value = res.shared_events ?? []
  }
...
function ispReport(c: Correlation) {
  ...
  // Lead with loss that reached the DESTINATION. Loss at a hop is what a carrier's
  // ICMP policers produce and the first thing they will (correctly) dismiss.
  if (c.suspect_hop?.end_loss_pct)
    lines.push(`Loss to the far end of the path: up to ${c.suspect_hop.end_loss_pct}% (measured to the destination, not to an intermediate hop).`)

  // A hop only goes in a carrier ticket when the healthy circuits crossing it are
  // clean. Every probe leaves the same building, so the first carrier hop is on
  // every path and looks guilty if you only count the sick ones.
  if (c.suspect_hop?.discriminating) {
    lines.push(`Suspect hop: ${c.suspect_hop.host} — named by ${c.suspect_hop.named_by} of `
      ...
      + (c.suspect_hop.healthy_crossings
        ? `, and lossy on ${c.suspect_hop.healthy_crossings_lossy} of ${c.suspect_hop.healthy_crossings} unaffected circuits that also cross it.`
        : '; no unaffected circuit traced across it in the comparison window.'))
  }
...
<template>
  <div class="anom">
    ...
    <!-- One event, not N. Sits above the per-entity list because deciding "that was
         my change" in one read is the difference between triage and noise. -->
    <VCard v-for="(e, i) in sharedEvents" :key="`shared-${i}`" class="corr corr--shared mb-4">
      <div class="corr__head">
        <VIcon icon="ri-git-merge-line" size="18" class="corr__ic" />
        <div class="corr__title">
          <div class="corr__h">
            {{ e.members }} entities changed {{ METRIC_LABEL[e.metric] ?? e.metric }} together — {{ shiftText(e) }}
          </div>
          <div class="corr__sub">
            {{ e.baseline_median }} → {{ e.observed_median }}{{ METRIC_UNIT[e.metric] ?? '' }} median, within minutes of each other
            <template v-if="e.detected_at"> · {{ new Date(e.detected_at).toLocaleString() }}</template>
          </div>
        </div>
      </div>
      <div class="corr__body">
        <p class="corr__reason mb-0">{{ e.reason }}</p>
        <div v-if="e.carriers.length > 1" class="corr__carriers">
          <span class="cap">spans</span>
          <VChip v-for="c in e.carriers" :key="c" size="x-small" variant="tonal">{{ c }}</VChip>
        </div>
      </div>
    </VCard>
    ...
    <VCard v-for="c in correlations" :key="c.isp_name" class="corr mb-4">
      ...
      <div v-if="c.suspect_hop?.end_loss_pct" class="corr__hop">
        <VIcon icon="ri-focus-3-line" size="15" />
        <span>Loss reaching the far end: <strong>{{ c.suspect_hop.end_loss_pct }}%</strong></span>
        <span class="corr__dim">measured to the destination, not to an intermediate hop</span>
      </div>

      <div v-if="c.suspect_hop?.discriminating" class="corr__hop">
      ...
      <!-- The hop every path crosses is not evidence. Saying so on the banner is the
           only thing that stops it being quoted at a carrier. -->
      <div v-else-if="c.suspect_hop" class="corr__hop corr__hop--shared">
        <VIcon icon="ri-information-line" size="15" />
        <span><code>{{ c.suspect_hop.host }}</code> is shared transit, not a suspect</span>
        <span class="corr__dim">
          named by {{ c.suspect_hop.named_by }} of {{ c.suspect_hop.of_probed }} affected circuits, but clean on
          {{ c.suspect_hop.healthy_crossings }} unaffected {{ c.suspect_hop.healthy_crossings === 1 ? 'circuit' : 'circuits' }} that also cross it — don't quote it
        </span>
      </div>
...
/* A shared event is context, not an accusation — it explains why N rows below moved
   at once. Neutral, so it never reads as a fault of its own. */
.corr--shared { border-inline-start: 3px solid rgb(var(--v-theme-on-surface), .25); }
.corr--shared .corr__ic { color: rgb(var(--v-theme-on-surface), .6); }
.corr__reason { font-size: 12.5px; color: rgb(var(--v-theme-on-surface), .8); }
.corr__carriers { display: flex; align-items: center; gap: 6px; margin-block-start: 10px; flex-wrap: wrap; }
/* Shared transit is the opposite of a finding — it must not wear the warning
   colour that says "quote this at the carrier". */
.corr__hop--shared { background: rgba(var(--v-theme-on-surface), .02);
  code { color: rgb(var(--v-theme-on-surface), .7); font-weight: 500; } }
