  ...
  <div class="anom">
    ...
    <VCard v-for="c in correlations" :key="c.isp_name" class="corr mb-4">
      ...
      <div v-if="openCorrelation === c.isp_name" class="corr__list">
        <RouterLink v-for="x in c.circuits" :key="x.id" class="crow" :to="`/circuits?circuit=${x.id}`">
          <span class="crow__site">{{ x.site_number ? `#${x.site_number} ` : '' }}{{ x.site_name ?? '—' }}</span>
          <span class="crow__cid">{{ x.circuit_id ?? '—' }}</span>
          <code class="crow__ip">{{ x.monitored_ip }}</code>
...
.crow { display: grid; grid-template-columns: 1.4fr .9fr .8fr 1.1fr .9fr; gap: 12px; align-items: baseline;
  padding: 7px 0; font-size: 12.5px; text-decoration: none; color: inherit;
  border-block-end: 1px solid rgba(var(--v-theme-on-surface), .05);
  &:hover { color: rgb(var(--v-theme-primary)); }
  &__site { font-weight: 600; }
  &__cid, &__ip { font-family: var(--mono); color: rgb(var(--v-theme-on-surface), .6); }
