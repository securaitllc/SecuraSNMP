...
interface Summary {
  open: number
  by_metric: { throughput: number, discards: number, latency: number }
...
function fmtVal(a: Anomaly, v: number) {
  return a.metric_key === 'throughput' ? fmtBytes(v) : a.metric_key === 'latency' ? `${v.toFixed(0)} ms` : v.toFixed(0)
}
  ...
  <div class="anom">
    ...
    <div class="strip mb-4">
      ...
      <VCard class="scard">
        ...
        <div class="mrows">
          <button v-for="m in (['throughput','discards','latency'] as const)" :key="m" class="mrow" :class="{ on: metricFilter === m }" @click="metricFilter = metricFilter === m ? null : m">
    ...
    <div class="list-pills mb-3">
      ...
      <button v-for="m in (['throughput','discards','latency'] as const)" :key="m" type="button" class="list-pill text-capitalize" :class="{ 'list-pill--on': metricFilter === m }" @click="metricFilter = metricFilter === m ? null : m">
