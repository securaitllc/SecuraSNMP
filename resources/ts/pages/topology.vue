...
function nodeTypeIcon(type: string): string {
  return ({
    ...
    edge: 'ri-router-line', switch: 'ri-git-merge-line', fw: 'ri-shield-check-line', device: 'ri-server-line',
...
const layout = computed(() => {
  ...
  return { nodes, edges, w: Math.max(totalW, 900), h }
})
...
// Mitel MiNet phones advertise "regDN 400206,MINET_6920" — regDN is the extension
...
function onPointerUp() { panning.value = false }
  ...
  <div>
    <div class="mb-2">
      ...
      <div class="text-body-2 text-medium-emphasis">
        Logical view · hover a node to trace its path · live, updates every 30s
      </div>
    </div>
      ...
      <div class="canvas-holder">
        ...
        >
          ...
          <g
            ...
            :key="n.id"
            class="node"
            :class="{ 'is-dim': nodeDim(n.id), sel: n.id === selectedId }"
            tabindex="0"
            role="button"
            :aria-label="n.label"
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Appliance reports SD-WAN tunnels <strong>DOWN</strong> (SNMP). The per-hub
              detail below is from the last SSH poll and may be stale — treat it as
              pending refresh, not confirmed all-up.
            </VAlert>
            ...
            <template v-if="tunnelHubView.aggregate">
              <div class="sn-hub-row">
                <VIcon
                  :icon="tunnelHubView.downCount > 0 ? 'ri-error-warning-line' : 'ri-checkbox-circle-line'"
                  size="15"
                  :color="tunnelHubView.downCount > 0 ? 'error' : 'success'"
                />
                ...
                <VChip
                  ...
                  class="ms-auto"
                >
                  {{ tunnelHubView.downCount > 0 ? `${tunnelHubView.downCount} with tunnels down` : 'all up' }}
                </VChip>
              </div>
              <div
            ...
            <template v-else>
              ...
              >
                <VIcon
                  :icon="h.down > 0 ? 'ri-error-warning-line' : 'ri-cloud-line'"
                  size="15"
                  :color="h.down > 0 ? 'error' : 'success'"
                />
                <span class="sn-hub-name">{{ h.hub }}</span>
                <VChip
                  size="x-small"
                  ...
                  class="ms-auto"
                >
                  {{ h.down > 0 ? `${h.down}/${h.total} down` : `${h.total} up` }}
                </VChip>
              </div>
            </template>
          </div>
...
.canvas-holder { position: relative; padding: 8px; }
...
.if-scroll { max-height: 260px; overflow-y: auto; overflow-x: auto; }
