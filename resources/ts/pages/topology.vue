...
const layout = computed(() => {
  ...
  for (const c of cols)
    rowW[c] = byCol[c].reduce((sum, n) => sum + widthOf(n), 0) + (byCol[c].length - 1) * GAP_X
  const totalW = Math.max(900, ...cols.map(c => rowW[c])) + PAD * 2
  ...
  const nodes = t.nodes.map((n) => {
    ...
    const nw = widthOf(n)
    let x = PAD + (totalW - PAD * 2 - rowW[n.col]) / 2
    for (let k = 0; k < i; k++)
      x += widthOf(col[k]) + GAP_X
    const y = PAD + n.col * (NH + GAP_Y)
  ...
  let w = totalW
  let hh = PAD + (maxCol + 1) * (NH + GAP_Y) - GAP_Y + PAD + 30
