...
const layout = computed(() => {
  ...
  const edges = t.edges.filter(e => pos[e.from] && pos[e.to]).map((e, i) => {
    ...
    if (e.overlay) {
      ...
      // Upper tier → lower tier, a vertical S-curve; the label sits beside it.
      const up = a.y < b.y ? a : b; const lo = a.y < b.y ? b : a
      const x1 = up.x + up.w / 2; const y1 = up.y + NH
      const x2 = lo.x + lo.w / 2; const y2 = lo.y
      const my = (y1 + y2) / 2
      d = `M ${x1} ${y1} C ${x1} ${my}, ${x2} ${my}, ${x2} ${y2}`
      // Sit the label ON the link, not floating out to one side of it. For this
      // curve the t=0.5 point is exactly the midpoint of the two endpoints, and the
      // label already carries its own backing plate, so it reads as part of the
      // line instead of a caption stranded in empty canvas.
      lx = (x1 + x2) / 2; ly = my
    }
