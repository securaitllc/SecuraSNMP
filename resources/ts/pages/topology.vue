...
const layout = computed(() => {
  ...
  const edges = t.edges.filter(e => pos[e.from] && pos[e.to]).map((e, i) => {
    ...
    if (e.overlay) {
      ...
      if (fanIntoEdge) {
        d = `M ${x1} ${y1} L ${x2} ${y2}`
        // Label beside the line, never on it: offset perpendicular from the
        // midpoint so it hugs the wire without covering it.
        const dx = x2 - x1; const dy = y2 - y1
        const len = Math.hypot(dx, dy) || 1
        const off = tw / 2 + 8
        lx = (x1 + x2) / 2 + (dy / len) * off
        ly = (y1 + y2) / 2 - (dx / len) * off
      }
      else {
