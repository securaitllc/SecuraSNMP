...
const layout = computed(() => {
  ...
  const edges = t.edges.filter(e => pos[e.from] && pos[e.to]).map((e, i) => {
    ...
    if (e.overlay) {
      ...
      if (fanIntoEdge) {
        // Orthogonal fan-in, not a pair of diagonals: each next-hop drops straight
        // down, runs along a shared bus just above the appliance, then turns into
        // it. Two diagonals converging on one point draw a triangle, which is a
        // shape the network does not have — it reads as a relationship between the
        // next-hops themselves. Right angles read as cabling, and because every
        // link shares the same bus line the horizontals sit collinear and merge
        // instead of crossing.
        const busY = y2 - 26
        d = `M ${x1} ${y1} V ${busY} H ${x2} V ${y2}`
        // Label against the vertical drop, on the outward side so it never lands
        // inside the fan.
