  ...
  if (sel) {
    ...
    if (colOf[sel] !== undefined) {
      ...
      for (let guard = 0; (colOf[cur] ?? 0) > 0 && guard < 20; guard++) {
        const up = layout.value.edges
          .filter((e) => {
            const other = e.from === cur ? e.to : (e.to === cur ? e.from : null)
            return other != null && (colOf[other] ?? 99) < (colOf[cur] ?? 0)
          })
          // nearest lower column first = the physical uplink, not a skip-level link
          .sort((a, b) => (colOf[b.from === cur ? b.to : b.from] ?? 0) - (colOf[a.from === cur ? a.to : a.from] ?? 0))[0]
        if (!up) break
        const other = up.from === cur ? up.to : up.from
        order.push(other); nodeSet.add(other); edgeSet.add(`${up.from}-${up.to}`)
        cur = other
      }
    }
  }
...
// The traced path as node objects, ISP-first (reads left→right like the graph).
    ...
    >
      ...
      >
        ...
        >
          ...
          <div v-if="tracePathNodes.length > 1" class="sn-path">
            <div class="sn-path__lbl">Path to internet</div>
            <div class="sn-path__hops">
              <template v-for="(hn, i) in tracePathNodes" :key="hn.id">
                <VIcon v-if="i > 0" icon="ri-arrow-right-s-line" size="14" class="sn-path__arr" />
                <button type="button" class="sn-path__hop" :class="{ on: hn.id === selectedId }" @click="selectedId = hn.id">
                  <span class="sn-path__dot" :style="{ background: statusColor(hn.status) }" />{{ hn.label }}
                </button>
              </template>
            </div>
          </div>
