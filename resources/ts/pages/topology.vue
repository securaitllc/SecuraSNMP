...
function edgeKind(e: TopologyEdge): string {
  ...
  if (e.ha)
    return 'HA sync'
  if (e.stp_blocked)
    return `${e.label} · STP blocked`

  // The overlay rides this underlay — worth saying on hover, but it must not
  // change how the link is drawn.
  return e.carries_overlay ? `${e.label} · carries SD-WAN overlay` : e.label
}
...
function statusColor(s: string): string {
  // 'unknown' is neither good nor bad — it is the absence of a measurement, and
  // must never borrow the colour of either.
  if (s === 'unknown')
    return 'rgba(var(--v-theme-on-surface), .45)'
...
// Link load, written the way an operator reads a circuit: 24.5 Mbps, not 24500000.
...
const layout = computed(() => {
  ...
  // Tiers run TOP-DOWN: providers, then next-hops, the edge, the LAN gateway,
  // switches, endpoints — the way an engineer draws a site on a whiteboard, and
  // the way the approved design reads it. The API's `col` is the tier index.
  ...
  // Every node is exactly as wide as its longest line needs — the full name
  // always shows. 46px left (glyph + pad), text, 28px right (status mark).
  const widthOf = (n: TopologyNode): number => {
    const serialLen = n.serial ? (`S/N ${n.serial}`).length : 0
    const epLen = Math.max(endpointSummary(n).length, tunnelDownSummary(n).length + 3)
    return Math.round(Math.max(148, 46 + Math.max((n.label?.length ?? 0) * NAME_PX, (n.sub?.length ?? 0) * SUB_PX, serialLen * SUB_PX, epLen * SUB_PX) + 28))
  }

  const GAP_X = 44   // between nodes in a tier
  const GAP_Y = 84   // between tiers — room for the link label
  const PAD = 24

  // A tier wider than the frame WRAPS into rows, the way a dense site (#893 HQ,
  // a dozen switches on one core) has to — otherwise the drawing shrinks to fit
  // its widest tier and nothing is readable. Rows inside a tier sit close; the
  // gap between tiers stays wide for the link labels.
  const MAX_ROW_W = 1280
  const GAP_ROW = 22
  const rowsOf: Record<number, TopologyNode[][]> = {}
  const rowW: Record<number, number[]> = {}
  for (const c of cols) {
    ...
    for (const n of byCol[c]) {
      ...
      const next = widths[cur] + (rows[cur].length ? GAP_X : 0) + w
      if (rows[cur].length && next > MAX_ROW_W) {
        rows.push([n]); widths.push(w)
      }
      else {
        rows[cur].push(n); widths[cur] = next
      }
    }
    rowsOf[c] = rows; rowW[c] = widths
  }
  const totalW = Math.max(900, ...cols.flatMap(c => rowW[c])) + PAD * 2

  // Tier tops are cumulative: a wrapped tier is taller than one row.
  const tierY: Record<number, number> = {}
  let yCursor = PAD
  for (let c = 0; c <= maxCol; c++) {
    tierY[c] = yCursor
    const rows = rowsOf[c]?.length ?? 1
    yCursor += rows * NH + (rows - 1) * GAP_ROW + GAP_Y
  }

  const pos: Record<string, { x: number, y: number, w: number }> = {}
  const nodes = t.nodes.map((n) => {
    const rows = rowsOf[n.col]
    const r = rows.findIndex(row => row.includes(n))
    const row = rows[r]
    const i = row.indexOf(n)
    const nw = widthOf(n)
    let x = PAD + (totalW - PAD * 2 - rowW[n.col][r]) / 2
    for (let k = 0; k < i; k++)
      x += widthOf(row[k]) + GAP_X
    const y = tierY[n.col] + r * (NH + GAP_ROW)
    // An operator-saved position overrides the auto layout.
    const saved = savedPositions.value[n.id]
    const nx = saved ? saved.x : Math.round(x)
    const ny = saved ? saved.y : y
    pos[n.id] = { x: nx, y: ny, w: nw }

    // A passive HA standby is idle, not "up" — muted so it reads distinctly.
    // A device inside an active maintenance window keeps its real status (the down
    // marker still draws — it IS down) but loses the alarm red: somebody is already
    // on it, and a wall of red for planned work is how real outages get missed.
    const color = n.ha_role === 'standby'
      ? 'rgb(var(--v-theme-secondary))'
      : (n.in_maintenance ? 'rgba(var(--v-theme-on-surface), .5)' : statusColor(n.status))
  ...
  const edges = t.edges.filter(e => pos[e.from] && pos[e.to]).map((e, i) => {
    ...
    if (e.overlay) {
      // The SD-WAN overlay rides beside the physical path: an arc out to the right.
      const ax = a.x + a.w; const ay = a.y + NH / 2
      const bx = b.x + b.w; const by = b.y + NH / 2
      const bulge = Math.max(ax, bx) + 110
      d = `M ${ax} ${ay} C ${bulge} ${ay}, ${bulge} ${by}, ${bx} ${by}`
      // Label outside the arc, clear of whatever sits in the tier it passes.
      lx = bulge + tw / 2 + 6; ly = (ay + by) / 2
    }
    else if (e.ha) {
      // HA pair sits side by side in one tier: a short horizontal link.
      const left = a.x < b.x ? a : b; const right = a.x < b.x ? b : a
      const y = left.y + NH / 2
      d = `M ${left.x + left.w} ${y} L ${right.x} ${y}`
      lx = (left.x + left.w + right.x) / 2; ly = y - 10
    }
    else if (Math.round(a.y) === Math.round(b.y)) {
      // Two peers in the same tier (an access-fabric mesh): elbow the link
      // BELOW the tier so it never crosses the cards.
      const x1 = a.x + a.w / 2; const x2 = b.x + b.w / 2
      const yb = a.y + NH; const busY = yb + 30
      d = `M ${x1} ${yb} V ${busY} H ${x2} V ${yb}`
      lx = (x1 + x2) / 2; ly = busY + 14
    }
    else {
      // Upper tier → lower tier.
      const up = a.y < b.y ? a : b; const lo = a.y < b.y ? b : a
      const x1 = up.x + up.w / 2; const y1 = up.y + NH
      const x2 = lo.x + lo.w / 2; const y2 = lo.y
      const my = (y1 + y2) / 2

      // Several next-hops converge on ONE appliance, so those links fan in from
      // different x to the same point. Drawn as S-curves they bow away from each
      // other and from the cards, which is what made that tier look like spaghetti.
      // A straight run states the same fact and reads as a fan-in. Everything else
      // — the circuit links especially — keeps its curve.
      const fanIntoEdge = /^(nh|gw)-/.test(e.from) && /^ec-/.test(e.to)

      if (fanIntoEdge) {
        // Several next-hops land on one appliance, and the shape used to say more
        // than the network does. Two straight diagonals drew a triangle between the
        // next-hops; routing them orthogonally down onto a shared bus closed a
        // rectangle instead. Neither relationship exists — each next-hop simply
        // ends on the same box.
        //
        // So each link LEAVES its next-hop vertically and ENTERS the appliance
        // vertically, bending only near the end. Both control points sit on their
        // own endpoint's x, which is what keeps the departure and arrival straight;
        // the asymmetry (late first control, early second) holds the line vertical
        // for most of the drop and turns it in at the bottom. Nothing encloses
        // anything, and it stays readable as more next-hops are added.
        const span = y2 - y1
        d = `M ${x1} ${y1} C ${x1} ${y1 + span * 0.72}, ${x2} ${y2 - span * 0.33}, ${x2} ${y2}`
        // Label against the still-vertical part of the run, on the outward side so
        // it never lands inside the fan.
        const outward = x1 <= x2 ? -1 : 1
        lx = x1 + outward * (tw / 2 + 8)
        ly = y1 + span * 0.38
      }
      else {
        d = `M ${x1} ${y1} C ${x1} ${my}, ${x2} ${my}, ${x2} ${y2}`
        lx = (x1 + x2) / 2 + tw / 2 + 10; ly = my
      }
    }
  ...
  // Two links whose midpoints land on the same spot stack their labels into an
  // unreadable smear. #045 has two parallel circuits, so it drew "circuit" twice
  // 5px apart — identical text at almost the same coordinates, which reads as a
  // drop shadow rather than as two labels. Nudge any label that would overlap one
  // already placed; each link keeps its own label, they just stop sitting on top
  // of each other.
  const placedLabels: { lx: number, ly: number, tw: number }[] = []
  for (const e of edges) {
    let guard = 0
    while (
      guard++ < 6
      && placedLabels.some(p => Math.abs(p.lx - e.lx) < (p.tw + e.tw) / 2 && Math.abs(p.ly - e.ly) < 14)
    )
      e.ly += 16

    placedLabels.push({ lx: e.lx, ly: e.ly, tw: e.tw })
  }

  // Grow the viewBox to include any operator-dragged nodes so nothing clips.
  let w = totalW
  let hh = yCursor - GAP_Y + PAD + 30
...
const statusLabel: Record<string, string> = { up: 'Reachable', warn: 'Degraded', down: 'Down', unknown: 'Not measured — one end unreachable' }
...
// Every load takes a ticket. A response is applied only if it is still the
// newest request AND still for the site on screen — otherwise a slow refresh
// for the site you just left lands after the one you asked for and puts the old
// map back.
let loadSeq = 0
...
async function loadTopology(preserveSelection = false) {
  ...
  try {
    const fetched = await api<Topology>(`/api/sites/${siteId}/topology`)
    if (seq !== loadSeq || siteId !== selectedSiteId.value)
      return
    topo.value = fetched
    const t = topo.value
...
// One watcher for both refs: opening a site from the org view changes mode AND
// selectedSiteId in the same tick, and two watchers meant two concurrent loads
// of the same map racing each other.
watch([selectedSiteId, mode], ([, m]) => {
  if (m === 'org') {
    loadOrg()
...
const orgSearch = ref('')
const orgFilter = ref<'impacted' | 'all' | 'healthy' | 'maintenance'>('impacted')
const orgStats = computed(() => {
  // "Impacted" means somebody has to do something. A site under an active maintenance
  // window does not qualify, and it is not healthy either — it is counted on its own.
  const maintenance = org.value.filter(s => s.state === 'maint').length
  const impacted = org.value.filter(s => s.state !== 'up' && s.state !== 'maint').length
  return { total: org.value.length, impacted, maintenance, healthy: org.value.length - impacted - maintenance }
})
// Default to showing everything when nothing is broken, else lead with exceptions.
watch(org, (list) => {
  orgFilter.value = list.some(s => s.state !== 'up' && s.state !== 'maint') ? 'impacted' : 'all'
}, { once: true })
const orgFiltered = computed(() => {
  ...
  return org.value.filter((s) => {
    const byState = orgFilter.value === 'all'
      || (orgFilter.value === 'impacted' && s.state !== 'up' && s.state !== 'maint')
      || (orgFilter.value === 'healthy' && s.state === 'up')
      || (orgFilter.value === 'maintenance' && s.state === 'maint')
...
const orgGroups = computed(() => {
  ...
  const groups = hubs.map((hub) => {
    ...
    return {
      ...
      impacted: [hub, ...branches].filter(m => m.state !== 'up' && m.state !== 'maint').length,
      maintenance: [hub, ...branches].filter(m => m.state === 'maint').length,
...
<template>
  <div>
    ...
    <div class="topo-controls mb-4">
      ...
      <!-- Bound to the site list itself, not a mapped copy. `:items="sites.map(...)"`
           handed VAutocomplete a new array on every re-render — and this page
           re-renders every 30s and on every node hover — so the menu could drop the
           click that picks a site, leaving the previous site's map on screen. -->
      <VAutocomplete
        v-if="mode === 'site'"
    ...
    <!-- SITE: the schematic on the left, the facts on the right -->
    <div v-if="mode === 'site'" class="topo-site">
    <VCard class="topo-canvas">
          ...
          >
            ...
            <template v-if="n.type === 'nexthop'">
              <rect
                ...
                rx="10"
              />
              <g :transform="`translate(${n.x + 12}, ${n.y + NH / 2 - 10}) scale(${20 / NODE_GLYPH_BOX})`">
                <path
                  class="glyph"
                  :d="nodeGlyphs.gw"
                />
            ...
            <rect
              ...
              :fill="n.color"
            />
            <g :transform="`translate(${n.x + 16}, ${n.y + NH / 2 - 11}) scale(${22 / NODE_GLYPH_BOX})`">
              <path
                class="glyph"
                :d="nodeGlyphs[n.type]"
              />
            ...
            <use
              ...
              height="8"
            />
            <title>{{ n.label }} · {{ n.sub }}{{ n.serial ? ` · S/N ${n.serial}` : '' }}{{ n.in_maintenance ? ' · under maintenance' : '' }}</title>
            <!-- Says it in words on the node itself. The grey alone would be read as
                 "no data", which is a different and worse claim. -->
            <text
              v-if="n.in_maintenance"
              class="nmaint"
              :x="n.x + n.nw - 16"
              :y="n.y + 15"
              text-anchor="end"
      ...
      <div v-if="hoverCard" class="topo-hc" :style="hoverCardStyle">
        <template v-if="hoverCard.node">
          ...
          <!-- A next-hop is one link. It reports THAT port's load and counters; the
               device-wide roll-up below belongs to switches and appliances. -->
          <template v-if="hoverCard.node.link">
            <template v-if="hoverCard.node.link.measured">
              <div class="topo-hc__row">
                <span>Link load in</span><b>{{ formatBps(hoverCard.node.link.in_bps) }}</b>
              </div>
              <div class="topo-hc__row">
                <span>Link load out</span><b>{{ formatBps(hoverCard.node.link.out_bps) }}</b>
              </div>
              <div class="topo-hc__row">
                <span>Utilisation</span><b>{{ hoverCard.node.link.in_util_pct }}% in · {{ hoverCard.node.link.out_util_pct }}% out</b>
              </div>
              <div class="topo-hc__bar">
                <i :style="{ width: `${Math.min(100, Math.max(hoverCard.node.link.in_util_pct ?? 0, hoverCard.node.link.out_util_pct ?? 0))}%`, background: metricColor(Math.max(hoverCard.node.link.in_util_pct ?? 0, hoverCard.node.link.out_util_pct ?? 0)) }" />
              </div>
              <div class="topo-hc__row">
                <span>Errors Δ</span><b :class="{ bad: (hoverCard.node.link.errors ?? 0) > 0 }">{{ hoverCard.node.link.errors }}</b>
              </div>
              <div class="topo-hc__row">
                <span>Discards Δ</span><b :class="{ warnv: (hoverCard.node.link.discards ?? 0) > 0 }">{{ hoverCard.node.link.discards }}</b>
              </div>
              <div class="topo-hc__foot">
                {{ hoverCard.node.link.if_name }}<template v-if="hoverCard.node.link.as_of"> · {{ formatDateTime(hoverCard.node.link.as_of) }}</template>
              </div>
            </template>
            <!-- Zeros would read as a quiet link. This is the absence of a reading. -->
            <div v-else class="topo-hc__unknown">
              <VIcon icon="ri-question-line" size="12" />
              <span>No load measured — {{ hoverCard.node.link.why }}</span>
            </div>
          </template>
          <template v-else-if="hoverCard.node.stats">
    ...
    <aside class="topo-rail">
    <!-- Root-cause banner -->
    <VAlert
      v-if="topo?.incident.active"
      type="error"
      variant="tonal"
      class="topo-rail__incident"
      border="start"
    >
        ...
        <div>
          ...
          <div v-if="topo.incident.layers" class="topo-layers mt-2">
            ...
            <span class="topo-layer" :class="`ls-${topo.incident.layers.next_hop.state}`">
              <VIcon :icon="topo.incident.layers.next_hop.state === 'unknown' ? 'ri-question-fill' : (topo.incident.layers.next_hop.state === 'up' ? 'ri-checkbox-circle-fill' : 'ri-close-circle-fill')" size="13" />
              Next-hop — {{ topo.incident.layers.next_hop.label }}
            </span>
            <span class="topo-layer" :class="`ls-${topo.incident.layers.tunnels.state}`">
              <VIcon :icon="topo.incident.layers.tunnels.state === 'unknown' ? 'ri-question-fill' : (topo.incident.layers.tunnels.state === 'up' ? 'ri-checkbox-circle-fill' : (topo.incident.layers.tunnels.state === 'degraded' ? 'ri-error-warning-fill' : 'ri-close-circle-fill'))" size="13" />
    ...
    <div v-if="selectedNode" class="topo-rail__node">
      <div>
        <VCard
          v-if="selectedNode"
          class="pa-4 h-100"
        >
          ...
          <div class="d-flex align-center ga-3 mb-3">
            ...
            <div class="d-flex flex-column align-end ga-1">
              ...
              <!-- The status above stays true. This says why nobody needs to be called. -->
              <VChip
                v-if="selectedNode.in_maintenance"
                size="x-small"
                color="secondary"
                variant="tonal"
                label
          ...
          <div v-if="selectedNode.type !== 'cluster'" class="sn-meta">
            ...
            <div
              v-if="selectedNode.cleared_note"
              class="sn-meta-row"
            >
              <VIcon
              ...
              <span class="val">{{ selectedNode.cleared_note }}</span>
            </div>
            <div
          ...
          >
            <!-- SNMP health freezes at the last reading when a device goes quiet. A
                 9% CPU bar beside "unreachable" reads as a healthy appliance. -->
            <div
              v-if="selectedNode.health.measured === false"
              class="sn-unmeasured mb-2"
            >
              <VIcon
                icon="ri-question-line"
                size="13"
              />
              {{ selectedNode.health.why ?? 'not measured' }}{{ selectedNode.health.as_of ? ` · ${formatDateTime(selectedNode.health.as_of)}` : '' }}
            </div>
            <div
            ...
            >
              ...
              <div class="sn-bar">
                <div
                  class="sn-bar-fill"
                  :style="{ width: `${Math.min(100, selectedNode.health.cpu_pct)}%`, background: selectedNode.health.measured === false ? 'rgba(var(--v-theme-on-surface), .25)' : metricColor(selectedNode.health.cpu_pct) }"
                />
              </div>
            </div>
            <div
            ...
            >
              ...
              <div class="sn-bar">
                ...
                <div
                  class="sn-bar-fill"
                  :style="{ width: `${Math.min(100, selectedNode.health.mem_pct)}%`, background: selectedNode.health.measured === false ? 'rgba(var(--v-theme-on-surface), .25)' : (selectedNode.role === 'edgeconnect' ? 'rgb(var(--v-theme-success))' : metricColor(selectedNode.health.mem_pct)) }"
                />
              </div>
            </div>
          ...
          >
            ...
            <template v-else>
              ...
              >
                <VIcon
                  :icon="h.down > 0 ? 'ri-error-warning-line' : (h.measured === false ? 'ri-question-line' : (selectedNode.tunnels_stale ? 'ri-time-line' : 'ri-cloud-line'))"
                  size="15"
                  :color="h.down > 0 ? 'error' : ((h.measured === false || selectedNode.tunnels_stale) ? 'grey' : 'success')"
                />
                <span class="sn-hub-name">{{ h.hub }}</span>
                <!-- An appliance that will not answer cannot refresh this table. Its
                     rows are last-known, so they are never shown as "N up". -->
                <VChip
                  size="x-small"
                  :color="h.down > 0 ? 'error' : ((h.measured === false || selectedNode.tunnels_stale) ? 'grey' : 'success')"
                  variant="tonal"
        ...
        <div class="d-flex align-center flex-wrap ga-3 justify-space-between">
          ...
          <div class="d-flex align-center ga-2 flex-wrap">
            ...
            <div class="topo-modetoggle">
              ...
              <button
                v-if="orgStats.maintenance"
                type="button"
                class="seg-btn"
                :class="{ active: orgFilter === 'maintenance' }"
                @click="orgFilter = 'maintenance'"
              >
                Maintenance ({{ orgStats.maintenance }})
              </button>
              <button
                type="button"
                class="seg-btn"
      ...
      <template v-else>
        ...
        >
          ...
          >
            ...
            <VChip
              size="x-small"
              :color="g.hub.state === 'crit' ? 'error' : g.hub.state === 'warn' ? 'warning' : g.hub.state === 'maint' ? 'secondary' : 'success'"
              variant="tonal"
            >{{ g.hub.summary }}</VChip>
            <VSpacer />
            <span class="text-caption text-medium-emphasis">
              {{ g.total }} branches ·
              <span :class="g.impacted > 0 ? 'text-error font-weight-medium' : ''">{{ g.impacted }} impacted</span><span v-if="g.maintenance"> · {{ g.maintenance }} in maintenance</span>
            </span>
          </div>
          <div
...
.seg-btn:hover { background: rgba(var(--v-theme-on-surface), 0.05); color: rgba(var(--v-theme-on-surface), 0.85); }
.seg-btn.active {
...
.topo-legend {
  ...
  background: rgba(var(--v-theme-surface), 0.82); border: 1px solid rgba(var(--v-theme-on-surface), 0.1);
  border-radius: 10px; padding: 8px 14px; font-size: 11.5px; color: rgba(var(--v-theme-on-surface), 0.72);
...
.node:hover .card { stroke: rgba(var(--v-theme-on-surface), 0.28); }
.node.sel .card { stroke: rgb(var(--v-theme-primary)); stroke-width: 2; filter: drop-shadow(0 4px 16px rgba(var(--v-theme-primary), .22)); }
...
.nmaint { fill: rgba(var(--v-theme-on-surface), 0.55); font-size: 8.5px; font-weight: 700; letter-spacing: .08em; font-family: ui-monospace, Menlo, monospace; }
...
/* Remix Icon paths are filled artwork, not strokes — see utils/nodeGlyphs.ts. */
.glyph { fill: rgb(var(--v-theme-on-surface)); stroke: none; opacity: .82; }
...
/* Nothing probes the cable itself. With one end unreachable the link is not known
   to be up, so it is drawn as unknown — grey and dashed, never a green wire with a
   flowing dot. */
.edge.unknown .wire { stroke: rgba(var(--v-theme-on-surface), 0.3); stroke-dasharray: 3 6; }
.edge.unknown .elabel { fill: rgba(var(--v-theme-on-surface), 0.5); }
...
.sn-icon { display: grid; place-items: center; width: 40px; height: 40px; border-radius: 10px; flex-shrink: 0;
  ...
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--sn-c) 10%, transparent); }
...
.ha-box { padding: 10px 12px; border-radius: 10px; background: rgba(var(--v-theme-on-surface), 0.035); border: 1px solid rgba(var(--v-theme-on-surface), 0.09); font-size: 12.5px; }
...
.lldp-box { padding: 10px 12px; border-radius: 10px; background: rgba(var(--v-theme-on-surface), 0.035); border: 1px solid rgba(var(--v-theme-on-surface), 0.09); font-size: 12.5px; }
...
.topo-hc__unknown {
  display: flex; align-items: flex-start; gap: 6px; margin-block-start: 7px;
  font-size: 11px; line-height: 1.45; color: rgba(var(--v-theme-on-surface), .6);
}
.topo-hc__foot {
  margin-block-start: 7px; font-size: 10.5px; font-family: ui-monospace, Menlo, monospace;
...
.sn-unmeasured { display: flex; align-items: center; gap: 6px; font-size: 11px; color: rgba(var(--v-theme-on-surface), .55); }
.topo-layer.ls-unknown { color: rgba(var(--v-theme-on-surface), .55); background: rgba(var(--v-theme-on-surface), .06); border-color: rgba(var(--v-theme-on-surface), .2); }
...
/* ── Terminal overrides ───────────────────────────────────────────────────
   Square nodes on a hairline grid, mono labels, colour only for state. Placed
   last so they win over the rules above at equal specificity. */
.topo-modetoggle { border: 0; border-radius: 0; background: transparent; gap: 4px; }
.seg-btn {
  border: 1px solid var(--nodus-rule) !important; border-radius: 0; min-width: 0; padding: 4px 10px;
  font-family: 'IBM Plex Mono', ui-monospace, monospace; font-size: 11px; font-weight: 500; letter-spacing: .06em; text-transform: uppercase;
  color: var(--nodus-ink-dim);
}
.seg-btn.active { background: #1A2130; border-color: #1A2130 !important; color: rgb(var(--v-theme-on-surface)); }
.topo-canvas {
  background-image: linear-gradient(#11151C 1px, transparent 1px), linear-gradient(90deg, #11151C 1px, transparent 1px);
  background-size: 20px 20px;
}
.arrange-hint { border-radius: 0; box-shadow: none; font-family: 'IBM Plex Mono', monospace; font-size: 10px; letter-spacing: .08em; text-transform: uppercase; }
.node .card { fill: #0E1219; stroke: var(--nodus-rule); rx: 0; ry: 0; }
.node:hover .card { stroke: #2A3140; }
.node.sel .card { stroke: rgb(var(--v-theme-primary)); stroke-width: 1.5; filter: none; }
.node text { font-family: 'IBM Plex Mono', ui-monospace, monospace; }
.node .nh-pill { rx: 0; ry: 0; }
.edge .wire { stroke-width: 1.5; }
.edge.down .wire { stroke-width: 1.5; stroke-dasharray: 6 5; }
.edge.down.root .wire { stroke-width: 2; }
.edge .elabel { font-family: 'IBM Plex Mono', ui-monospace, monospace; font-size: 10px; }
.flow-dot { filter: none; }
.topo-legend { font-family: 'IBM Plex Mono', ui-monospace, monospace; font-size: 10px; letter-spacing: .04em; color: var(--nodus-ink-dim); }
.topo-legend .ln { border-radius: 0; height: 2px; }
.topo-legend .dt { border-radius: 0; width: 7px; height: 7px; }
.topo-hc { border-radius: 0 !important; box-shadow: none !important; border: 1px solid var(--nodus-rule); font-family: 'IBM Plex Mono', ui-monospace, monospace; }
.topo-layers .topo-layer { border-radius: 0; font-family: 'IBM Plex Mono', ui-monospace, monospace; font-size: 10px; letter-spacing: .06em; text-transform: uppercase; }
.sn-icon { border-radius: 0 !important; }
.sn-route__hop, .sn-route__btn { border-radius: 0 !important; }

/* Site view: schematic left, rail right. One ruled region. */
.topo-site { display: grid; grid-template-columns: minmax(0, 1fr) 400px; border: 1px solid var(--nodus-rule); }
.topo-site > .topo-canvas { border: 0; }
.topo-rail { border-inline-start: 1px solid var(--nodus-rule); max-block-size: min(80vh, 860px); overflow-y: auto; display: flex; flex-direction: column; }
/* The rail is a flex column with a max height, and Vuetify gives .v-alert
   flex: 1 1 0% plus overflow: hidden. overflow:hidden cancels the automatic
   minimum size that normally stops a flex item shrinking below its content, so
   the incident alert collapsed to 25px and hid 325px of content — the summary,
   the root cause and every layer chip. It sizes to its content; only the node
   panel below it scrolls. */
.topo-rail__incident { margin: 0 !important; border-radius: 0 !important; border-block-end: 1px solid var(--nodus-rule); font-size: 12px; flex: 0 0 auto; overflow: visible; }
.topo-rail > .v-alert { flex: 0 0 auto; }
.topo-rail__incident .call-isp-num { font-size: 16px; }
.topo-rail__node > div > .v-card { border: 0; border-block-end: 1px solid var(--nodus-rule); }
.topo-rail .sn-meta-row { font-family: 'IBM Plex Mono', ui-monospace, monospace; font-size: 11px; }
@media (max-width: 1100px) { .topo-site { grid-template-columns: 1fr; } .topo-rail { border-inline-start: 0; border-block-start: 1px solid var(--nodus-rule); max-block-size: none; } }
</style>
