...
<template>
  <div class="flows-page">
    ...
    <template v-else-if="overview">
      ...
      <div class="grid2">
        <div class="panel">
          <div class="panel-head">Top talkers <VBtn size="x-small" variant="tonal" color="primary" class="ml-auto" prepend-icon="ri-list-check" @click="loadRaw">View raw flows</VBtn></div>
          <table class="ftable">
        ...
        <table class="ftable mono">
          ...
          <tbody>
            <tr v-for="(r, i) in rawRows" :key="i">
              <td class="muted">{{ fmtTime(r.flow_start ?? r.recorded_at) }}</td>
              <td>{{ r.device?.name ?? '—' }}</td>
