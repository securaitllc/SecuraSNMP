...
import { withStackedCellProps } from '@/utils/stackedTable'
import { api } from '@/composables/useApi'

interface ReportField { key: string, label: string, align?: string, default?: boolean }
interface ReportMeta { type: string, label: string, time_scoped: boolean, supports_role: boolean, supports_radius?: boolean, fields: ReportField[] }
...
// Mileage band for the nearby-sites report. Straight-line, so it is a shortlist to
// route rather than a schedule — see ReportService::sitesNearby.
const minMiles = ref(10)
const maxMiles = ref(20)
// Sites are grouped into drive-able runs, nearest-neighbour from the origin.
const stopsPerRun = ref(5)
...
const typeColor: Record<string, string> = {
  'circuit-availability': SEV_OK,
  'device-availability': '#4C8DFF',
  'tunnel-availability': '#8B7CF6',
  'alarm-summary': SEV_WARNING,
...
function buildParams(): URLSearchParams {
  ...
  if (current.value?.supports_radius) {
    p.set('min_miles', String(minMiles.value))
    p.set('max_miles', String(maxMiles.value))
    p.set('stops_per_run', String(stopsPerRun.value))
  }
...
/**
 * Severity painting for ANY report.
 *
 * A row may carry a `_tone` map ({ column: 'critical'|'warning'|'notice'|'muted' }).
 * It is not a declared column, so it never renders as one and never reaches the
 * export — it just tells the table which cells to colour. That keeps a report's
 * urgency in the data rather than in a per-column template.
 */
function toneClass(item: Record<string, any>, key: string): string {
  const tone = item?._tone?.[key]

  return tone ? `rt-tone rt-${tone}` : ''
}
...
function uptimeColor(v: number): string {
  return v >= 99.9 ? SEV_OK : v >= 99 ? SEV_WARNING : SEV_CRITICAL
}
</script>
    ...
    <VCard class="list-surface">
      <VCardText>
        ...
        <div class="report-filters">
          ...
          <div class="rf">
            <span class="rf__l">{{ current?.supports_radius ? 'Measure from' : 'Site' }}</span>
            <VSelect
              v-model="siteId"
              :items="siteItems"
              item-title="name"
          ...
          <div v-if="current?.supports_radius" class="rf">
            <span class="rf__l">Miles</span>
            <VTextField
              v-model.number="minMiles"
              type="number"
              min="0"
              density="compact"
              hide-details
              label="From"
              style="min-inline-size: 92px;"
            />
            <VTextField
              v-model.number="maxMiles"
              type="number"
              min="0"
              density="compact"
              hide-details
        ...
        <VDataTable
          ...
          :items-per-page="25"
          density="compact"
          class="report-table mt-2 stack-sm"
          :cell-props="withStackedCellProps(({ item, column }) => ({ class: toneClass(item as any, String(column.key)) }))"
        >
...
/* Severity cells — same treatment as the circuits status column: a faint tinted
   wash, a 3px leading tick and the value in the severity colour. Driven by each
   row's `_tone` map, so every report gets it for free. */
.report-table :deep(td.rt-tone) {
  position: relative;
  padding-inline-start: 16px !important;
  font-weight: 600;
}
.report-table :deep(td.rt-tone::before) {
  content: '';
  position: absolute;
  inset-block: 8px;
...
.report-table :deep(td.rt-critical) { color: rgb(var(--v-theme-error)); background: rgba(var(--v-theme-error), 0.1); }
.report-table :deep(td.rt-warning) { color: rgb(var(--v-theme-warning)); background: rgba(var(--v-theme-warning), 0.1); }
.report-table :deep(td.rt-notice) { color: rgb(var(--v-theme-info)); background: rgba(var(--v-theme-info), 0.09); }
.report-table :deep(td.rt-muted) {
