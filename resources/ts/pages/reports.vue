...
const customValid = computed(() => range.value !== 'custom' || (!!customFrom.value && !!customTo.value))
  ...
  <div>
    ...
    <div class="report-tabs mt-4">
      <button
        ...
        :key="r.type"
        class="report-tab"
        :class="{ 'report-tab--on': r.type === selectedType }"
        @click="selectType(r.type)"
      >
        <span class="report-tab__dot" :style="{ background: typeColor[r.type] ?? '#98A2B2' }" />
        {{ r.label }}
      </button>
    </div>

    <VCard class="report-surface">
      <VCardText>
        <!-- Filter bar -->
...
.report-tab__dot { inline-size: 7px; block-size: 7px; border-radius: 50%; flex: none; }
.report-tab--on {
  color: rgb(var(--v-theme-on-surface));
  background: rgb(var(--v-theme-surface));
  border-color: rgba(var(--v-border-color), var(--v-border-opacity));
}
.report-surface { border-start-start-radius: 0; }
