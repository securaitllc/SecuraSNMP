...
export const useAlertsStore = defineStore('alerts', {
  state: () => ({
    ...
    resolved: null as DashboardSummary['resolved_today'] | null,
    loaded: false,
    error: false,
    _timer: null as ReturnType<typeof setInterval> | null,
  }),
  getters: {
    // The bell badge = grouped incidents (matches the dashboard list + drops when an
    // incident is cleared), not the raw pre-grouping signal count.
    activeCount: state => state.counts?.active_incidents ?? state.counts?.active_alerts ?? 0,
    ...
    // site id -> alerts on devices inside a maintenance window. Kept separate from the
    // map above so a site being worked on is not counted as impacted — and, just as
    // importantly, is never allowed to read as UP with a dead switch in it.
    siteMaintenanceMap: (state): Record<number, number> =>
      Object.fromEntries(state.sites.map(s => [s.id, s.maintenance_alert_count ?? 0])),
  ...
  actions: {
    async refresh() {
      try {
        ...
        this.resolved = d.resolved_today ?? null
        this.loaded = true
        this.error = false
      }
      catch {
