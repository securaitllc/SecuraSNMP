...
export const themes: Record<string, ThemeDefinition> = {
  ...
  dark: {
    ...
    colors: {
      ...
      'secondary-darken-1': '#5A6478',
      'on-secondary': '#fff',
      // Severity is the Terminal palette — the same three values as
      // `utils/severity.ts`, which is what the chart paths use where a CSS
      // variable cannot reach. These were #16A34A / #D97706, inherited from the
      // pre-Terminal palette and never updated, so the theme disagreed with the
      // documented severity colours and with every page that hardcoded them.
      // Amber also gains real contrast on the near-black ground: #D97706 is
      // 6.06:1 against #0B0E14 where #F5A623 is 9.53:1 (WCAG, measured). That
      // matters because amber is the "degraded" signal on a wall display and has
      // to read from across the room. Green is a wash on contrast — #16A34A is
      // 5.86:1 and #2BA24E is 5.87:1 — so that swap is about agreeing with the
      // documented palette, nothing else.
      'success': '#2BA24E',
      'success-darken-1': '#25883F',
      'on-success': '#fff',
      'info': '#0EA5E9',
      'info-darken-1': '#0C93D1',
      'on-info': '#fff',
      'warning': '#F5A623',
      'warning-darken-1': '#D18D1D',
      'on-warning': '#0B0E14',
      'error': '#E5484D',
      'error-darken-1': '#CE3F44',
      'on-error': '#fff',
      // Terminal ground. Near-black, cool, one shade of surface above it and a
      // hairline between — the whole visual system is rules, not cards.
      'background': '#0B0E14',
      'on-background': '#D7DCE4',
      'surface': '#0B0E14',
      'on-surface': '#D7DCE4',
      'grey-50': '#1B2029',
      'grey-100': '#232833',
    ...
    variables: {
      ...
      'disabled-opacity': 0.4,
      'dragged-opacity': 0.1,
      // Exact hairline, not a translucent guess — every rule in the Terminal
      // system is this one colour.
      'border-color': '#1E242E',
      'border-opacity': 1,
      'table-header-color': '#0E1219',
      'high-emphasis-opacity': 0.9,
      'medium-emphasis-opacity': 0.7,
