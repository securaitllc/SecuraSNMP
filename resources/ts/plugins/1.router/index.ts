...
router.beforeEach(async to => {
  ...
  // Wall-display kiosk accounts can only ever see the wallboard — send every
  // other destination there (the backend fences the API to match).
  if (auth.isDisplay && to.name !== 'wallboard')
    return { name: 'wallboard' }

  if (to.name === 'login' && auth.isAuthenticated)
    return { name: 'root' }

  // MFA is enforced and this account hasn't enrolled — funnel it into setup and
  // keep it there until enrolled (the setup page itself is exempt to avoid a loop).
  if (auth.needsMfaSetup && to.path !== '/security/two-factor-setup')
    return { path: '/security/two-factor-setup' }

  if (!auth.needsMfaSetup && to.path === '/security/two-factor-setup' && auth.isAuthenticated)
    return { name: 'root' }
