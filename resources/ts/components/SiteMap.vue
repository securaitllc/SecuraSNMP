...
// A circular severity badge (green healthy / red alerting), OpenNMS-style. Shows
// the active-alert count when a site is alerting, otherwise a small solid dot.
function siteIcon(site: DashboardSiteHealth): L.DivIcon {
  const crit = site.health === 'critical'
  const cls = crit ? 'is-critical' : 'is-good'
  const label = crit && site.active_alert_count > 0 ? String(site.active_alert_count) : ''
  return L.divIcon({
    className: '',
    html: `<span class="sm-pin ${cls} ${crit ? 'sm-pin--ping' : ''}">${label}</span>`,
    iconSize: [24, 24],
    iconAnchor: [12, 12],
  })
}

// Cluster badge: sized by how many sites it holds.
//
// A red badge reading "10" was read as ten sites down when one site in the cluster
// had a single alarm. So when a cluster is impacted the big number is the count of
// IMPACTED sites, with the cluster total kept beside it as context.
function clusterIcon(c: L.MarkerCluster): L.DivIcon {
  ...
  const body = impacted > 0
    ? `<b>${impacted}</b><i class="sm-cluster-total">/${count}</i>`
    : String(count)
  const title = impacted > 0
    ? `${impacted} of ${count} sites impacted`
    : `${count} sites · all healthy`
  return L.divIcon({
    className: '',
    html: `<span class="sm-cluster ${impacted > 0 ? 'is-critical' : 'is-good'}" style="width:${size}px;height:${size}px" title="${title}">${body}</span>`,
    iconSize: [size, size],
...
<template>
  <div class="site-map">
    ...
    <div class="site-map__legend">
      <span class="site-map__legend-item"><span class="site-map__legend-dot is-good" />Healthy</span>
      <span class="site-map__legend-item"><span class="site-map__legend-dot is-critical" />Alerting</span>
      <span class="site-map__legend-hint">scroll zoom · drag pan · click a site</span>
    </div>
  </div>
</template>
...
:deep(.sm-pin.is-good) { background: rgb(var(--v-theme-success)); }
:deep(.sm-pin.is-critical) { background: rgb(var(--v-theme-error)); }
:deep(.sm-cluster) {
  /* flex, not grid: the impacted count and the total sit side by side on one line. */
  display: flex; align-items: baseline; justify-content: center; gap: 1px; border-radius: 50%;
  color: #fff; font-weight: 700; font-size: 13px;
  border: 2px solid rgb(var(--v-theme-surface));
  box-shadow: 0 2px 6px rgba(0, 0, 0, .4);
}
:deep(.sm-cluster.is-good) { background: rgba(var(--v-theme-success), 0.92); }
:deep(.sm-cluster.is-critical) { background: rgba(var(--v-theme-error), 0.92); }
/* impacted / total — the impacted count leads, the total trails smaller so the
   badge reads "1 of 10", never "10 down". */
:deep(.sm-cluster) b { font-size: 14px; }
:deep(.sm-cluster-total) { font-style: normal; font-size: 10px; font-weight: 600; opacity: .8; }

@media (prefers-reduced-motion: no-preference) {
  :deep(.sm-pin--ping)::after {
    content: ''; position: absolute; inset: -2px; border-radius: 50%;
    border: 2px solid rgb(var(--v-theme-error));
    animation: smPing 1.8s ease-out infinite;
  }
  @keyframes smPing {
...
.site-map__legend-dot.is-good { background: rgb(var(--v-theme-success)); }
.site-map__legend-dot.is-critical { background: rgb(var(--v-theme-error)); }
.site-map__legend-hint { color: rgba(var(--v-theme-on-surface), 0.45); }
@media (max-width: 640px) { .site-map__legend-hint { display: none; } }
</style>
