...
// Version token = fingerprint of the active basemap key. Tiles are served
// `immutable` for 30 days and the path alone never changes, so without this a
// browser keeps its stored WATERMARKED tiles for a month after a key is added.
const tileVersion = ref('')

function tileUrl(dark: boolean): string {
  const base = `/map-tiles/${dark ? 'dark' : 'light'}/{z}/{x}/{y}`

  return tileVersion.value ? `${base}?v=${tileVersion.value}` : base
}

async function loadTileVersion() {
  try {
    const r = await fetch('/map-tiles/version', { credentials: 'same-origin' })
    if (r.ok)
      tileVersion.value = (await r.json()).v ?? ''
  }
  catch {
    // No token just means the URL stays unversioned — the map still draws.
  }
}
...
// Health from the site's alarms: DOWN (a service-affecting critical), DEGRADED
// (only warnings), MAINTENANCE (alarms, but every one of them on a device inside an
// active window), or HEALTHY. Derived from the alert list so a warning-only site
// reads amber, not red — the flat dashboard uses the same severity split.
//
// Maintenance is its own state rather than either of the two easy answers. Red is
// wrong: nobody needs to be called. Green is worse: the site is genuinely not
// carrying traffic, and a green pin over a dead switch is the failure this app keeps
// being dug out of.
function siteSeverity(site: DashboardSiteHealth): 'down' | 'degraded' | 'maintenance' | 'healthy' {
  const at = (props.alerts ?? []).filter(a => a.site_id === site.id)
  const live = at.filter(a => !a.in_maintenance)
  if (live.some(a => a.severity === 'critical'))
    return 'down'
  if (live.length > 0 || site.active_alert_count > 0)
    return 'degraded'
  if (at.length > 0 || (site.maintenance_alert_count ?? 0) > 0)
    return 'maintenance'
  return 'healthy'
}

const healthCounts = computed(() => {
  let healthy = 0; let degraded = 0; let down = 0; let maintenance = 0
  for (const s of plotted.value) {
    const sev = siteSeverity(s)
    if (sev === 'down') down++
    else if (sev === 'degraded') degraded++
    else if (sev === 'maintenance') maintenance++
    else healthy++
  }
  return { healthy, degraded, down, maintenance }
})
...
// Which sites are on the map, and where. Changes only when a site is added,
// removed or moved — NOT when its alarm counts tick over.
const boundsSignature = computed(() =>
  plotted.value.map(s => `${s.id}@${s.latitude},${s.longitude}`).sort().join('|'))

// Everything the pins actually render from. The dashboard re-polls every 30s, so
// this is compared instead of deep-watching 130+ site objects: a string compare
// per poll rather than a full recursive traversal, and no rebuild at all when
// nothing a marker draws has changed.
const markerSignature = computed(() =>
  plotted.value
    .map(s => `${s.id}@${s.latitude},${s.longitude}:${siteSeverity(s)}:${s.device_count}/${s.circuit_count}/${s.active_alert_count}`)
    .join('|'))

let lastBoundsSignature = ''

/** One site's rendered state — what a pin actually draws from. */
function siteState(s: DashboardSiteHealth): string {
  return `${siteSeverity(s)}:${s.device_count}/${s.circuit_count}/${s.active_alert_count}/${s.maintenance_alert_count ?? 0}`
}

function tooltipHtml(s: DashboardSiteHealth): string {
  const maint = (s.maintenance_alert_count ?? 0) > 0 ? ` · ${s.maintenance_alert_count} in maintenance` : ''

  return `<strong>${esc(s.name)}</strong><br>${s.device_count} devices · ${s.circuit_count} circuits · ${s.active_alert_count} active${maint}`
}

function makeMarker(site: DashboardSiteHealth): L.Marker {
  ...
  marker.bindTooltip(tooltipHtml(site), { direction: 'top', offset: [0, -12] })
  marker.on('click', () => {
...
// Live markers by site id, and the state each was last drawn with.
const markers = new Map<number, L.Marker>()
const drawnState = new Map<number, string>()

/**
 * Refresh the ALARM STATE of each pin without rebuilding the map.
 *
 * The locations do not change between polls — the alarms do. This used to clear
 * every layer and recreate all ~130 markers on each 30s dashboard refresh, which
 * was the slow part and also re-framed the map. Now a site whose alarms changed
 * gets its own icon/tooltip swapped in place, and markers are only added or
 * removed when a site genuinely appears or disappears.
 */
function syncMarkers() {
  if (!map || !cluster)
    return

  const want = new Map(plotted.value.map(s => [s.id, s]))
  ...
  for (const site of plotted.value) {
    ...
    if (!existing) {
      ...
      drawnState.set(site.id, state)
      added.push(marker)
      severityChanged = true
      continue
    }

    if (drawnState.get(site.id) === state)
      continue // nothing this pin draws has changed — leave it alone

    const before = (existing.options as any).health
    const after = siteSeverity(site) === 'down' ? 'critical' : 'good'
    existing.setIcon(siteIcon(site))
    existing.setTooltipContent(tooltipHtml(site))
    ;(existing.options as any).health = after
    drawnState.set(site.id, state)
    if (before !== after)
      severityChanged = true
  }
  ...
  if (added.length)
    cluster.addLayers(added)

  // The cluster badge counts impacted children, so it only needs recomputing when
  // a site crossed the down/not-down line (or the set changed) — not on every tick.
  if (severityChanged)
    cluster.refreshClusters()

  // Only frame the map when the SET of sites changes. Re-fitting on every 30s poll
  // yanked the view back and threw away whatever the operator had panned or zoomed to.
  if (plotted.value.length && boundsSignature.value !== lastBoundsSignature) {
    lastBoundsSignature = boundsSignature.value
...
onMounted(() => {
  if (!mapEl.value)
    return
  // Non-blocking: the map draws immediately on the unversioned URL, then re-points
  // once the token arrives (which is what evicts a browser-cached watermarked tile).
  void loadTileVersion()
...
onBeforeUnmount(() => {
  ...
  cluster = null
  tiles = null
  // Drop the marker bookkeeping too, or a remount would think every pin already exists.
  markers.clear()
  drawnState.clear()
  lastBoundsSignature = ''
})

// Signature compare, not `deep: true`: a deep watch over 130+ sites ran on every
// 30s dashboard poll and rebuilt every marker even when nothing had changed.
watch(markerSignature, syncMarkers)
// Flip the basemap when the app theme toggles (light map ↔ dark map).
watch(isDark, dark => tiles?.setUrl(tileUrl(dark)))
// Re-point the layer once the version token lands (or changes after a key save).
watch(tileVersion, () => tiles?.setUrl(tileUrl(isDark.value)))
</script>
...
<template>
  <div class="site-map">
    ...
    <div class="site-map__legend">
      ...
      <span v-if="healthCounts.maintenance" class="site-map__legend-item"><span class="site-map__legend-dot is-maintenance" /><b>{{ healthCounts.maintenance }}</b> maintenance</span>
    </div>
  </div>
</template>
...
/* Grey, and hollow — reads as "known and accounted for", never as a clear signal. */
:deep(.sm-pin.is-maintenance) { background: transparent; border: 2px dashed rgba(var(--v-theme-on-surface), .55); }
...
.site-map__legend-dot.is-maintenance { background: transparent; border: 1px dashed rgba(var(--v-theme-on-surface), .55); }
</style>
