...
// Cluster badge: sized by count, coloured red if any child site is alerting.
function clusterIcon(c: L.MarkerCluster): L.DivIcon {
  const count = c.getChildCount()
  const crit = c.getAllChildMarkers().some(m => (m.options as any).health === 'critical')
  const size = count < 10 ? 34 : count < 50 ? 42 : 50
  return L.divIcon({
    className: '',
    html: `<span class="sm-cluster ${crit ? 'is-critical' : 'is-good'}" style="width:${size}px;height:${size}px">${count}</span>`,
    iconSize: [size, size],
...
onMounted(() => {
  if (!mapEl.value)
    return
  map = L.map(mapEl.value, { scrollWheelZoom: true, worldCopyJump: true })
