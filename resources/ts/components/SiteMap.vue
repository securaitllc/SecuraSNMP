...
function siteState(s: DashboardSiteHealth): string {
  return `${siteSeverity(s)}:${s.device_count}/${s.circuit_count}/${s.active_alert_count}`
}

function tooltipHtml(s: DashboardSiteHealth): string {
  return `<strong>${esc(s.name)}</strong><br>${s.device_count} devices · ${s.circuit_count} circuits · ${s.active_alert_count} active`
}
