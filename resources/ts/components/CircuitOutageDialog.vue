...
// The input shows, and returns, EASTERN wall-clock — the zone every timestamp in
// this app is displayed in — so what an operator types is what gets stored.
const toDatetimeLocal = easternInputValue
...
async function post(path: 'ticket' | 'acknowledge' | 'clear' | 'dispatch') {
  ...
  try {
    ...
    const body = path === 'ticket'
      ...
      : path === 'dispatch'
        ? {
            dispatch_at: easternInputToIso(dispatchAt.value),
            dispatch_end_at: easternInputToIso(dispatchEndAt.value),
