...
// datetime-local wants LOCAL "YYYY-MM-DDTHH:mm". toISOString() would shift the value
// by the UTC offset, so a 08:00 window opened for editing came back four hours out.
...
async function save() {
  ...
  try {
    ...
    await api(`/api/circuits/${id}/dispatch`, {
      method: 'POST',
      body: {
        dispatch_at: form.value.dispatch_at || null,
        dispatch_end_at: form.value.dispatch_end_at || null,
