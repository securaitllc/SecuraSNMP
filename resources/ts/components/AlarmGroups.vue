...
interface GTicket { isp_ticket: string | null, dispatch_at: string | null, dispatch_end_at: string | null, dispatch_note: string | null, dispatch_by_name: string | null }
...
// The bell reads /api/dashboard; this panel reads /api/alarms/grouped. They are two
// queries over two sets of tables, so they CAN disagree — and when they did, this
// panel printed a green "No active alarms" while four alarms were ringing. An
// unexplained gap is now stated rather than painted over: silence here has to mean
// silence everywhere, or it means nothing at all.
const bell = useAlertsStore()
const unaccounted = computed(() => {
  const shown = sites.value.reduce((n, s) => n + s.groups.reduce((m, g) => m + g.alarms.length, 0), 0)

  return Math.max(0, bell.activeCount - shown)
})
...
const form = ref({ isp_ticket: '', dispatch_at: '', dispatch_end_at: '', dispatch_note: '' })

// The input shows, and returns, EASTERN wall-clock, matching every timestamp the app
// displays. Posting the raw datetime-local value made the API (app timezone UTC) read
// a 09:00 dispatch as 09:00 UTC and hand back 05:00 EDT.
const toLocalInput = easternInputValue
...
function openEdit(siteName: string | null, g: Group) {
  ...
  form.value = {
    ...
    dispatch_at: g.ticket?.dispatch_at ? toLocalInput(g.ticket.dispatch_at) : '',
    dispatch_end_at: g.ticket?.dispatch_end_at ? toLocalInput(g.ticket.dispatch_end_at) : '',
...
async function save() {
  ...
  try {
    ...
    await api(`/api/circuits/${id}/dispatch`, {
      method: 'POST',
      body: {
        dispatch_at: easternInputToIso(form.value.dispatch_at),
        dispatch_end_at: easternInputToIso(form.value.dispatch_end_at),
        note: form.value.dispatch_note || null,
      },
    })
    dialog.value = false
    await load()
...
<template>
  <div class="agroups">
    ...
    <div v-else-if="!sites.length && unaccounted > 0" class="ag-empty ag-empty--gap">
      <VIcon icon="ri-error-warning-line" size="16" color="warning" class="me-1" />
      {{ unaccounted }} active {{ unaccounted === 1 ? 'alarm is' : 'alarms are' }} not grouped here — switch to Flat to see {{ unaccounted === 1 ? 'it' : 'them' }}.
    </div>
    ...
    <div v-else-if="unaccounted > 0" class="ag-empty ag-empty--gap">
      <VIcon icon="ri-error-warning-line" size="16" color="warning" class="me-1" />
      {{ unaccounted }} more active {{ unaccounted === 1 ? 'alarm is' : 'alarms are' }} not shown in this view — switch to Flat.
    </div>
      ...
      >
        ...
        <div v-if="g.kind === 'circuit' && (g.ticket?.isp_ticket || g.ticket?.dispatch_at)" class="ag-ticket">
          ...
          <span v-if="g.ticket?.dispatch_at" class="ag-disp">
            <VIcon icon="ri-truck-line" size="16" class="me-1" />{{ formatDateTime(g.ticket.dispatch_at) }}<span v-if="g.ticket.dispatch_end_at"> – {{ formatDateTime(g.ticket.dispatch_end_at) }}</span>
        ...
        <!-- nested alarms -->
        <div class="ag-alarms">
          <div v-for="a in g.alarms" :key="a.key" class="ag-alarm" :class="[a.severity, { 'ag-alarm--maint': a.in_maintenance }]">
            ...
            <div class="ag-alarm__row">
              ...
              <span v-if="a.in_maintenance" class="ag-maint">IN MAINTENANCE</span>
              <RouterLink
                v-if="a.device_id"
    ...
    <VDialog v-model="dialog" transition="slide-x-reverse-transition" content-class="nodus-drawer nodus-drawer--sm">
      <VCard>
        ...
        <VCardText class="d-flex flex-column ga-4 pt-4">
          ...
          <VTextField v-model="form.dispatch_at" label="Dispatch window start" type="datetime-local" hide-details />
          <VTextField v-model="form.dispatch_end_at" label="Window end (optional)" type="datetime-local" hide-details />
...
/* A gap between this panel and the bell is a fault, so it reads as one. */
.ag-empty--gap {
  color: rgb(var(--v-theme-warning));
  background: rgb(var(--v-theme-warning), .08);
  border-inline-start: 3px solid rgb(var(--v-theme-warning));
  padding: 12px;
}
/* Terminal grouping: a caps site label, then one ruled block per provider with a
   3px severity bar on its left. The head is one shade above the ground. */
.ag-site { margin-bottom: 14px; }
.ag-site-name { font-size: 10px; font-weight: 600; letter-spacing: .12em; text-transform: uppercase; margin: 4px 0 6px; color: rgb(var(--v-theme-on-surface)); }

.ag-card {
  border: 1px solid var(--nodus-rule);
  border-inline-start: 3px solid var(--nodus-ink-dim);
  margin-bottom: 8px; overflow: hidden;
  background: rgb(var(--v-theme-background));
}
.ag-card.is-down { border-color: rgba(var(--v-theme-error), .27); border-inline-start-color: rgb(var(--v-theme-error)); }
.ag-card.is-degraded { border-color: rgba(var(--v-theme-warning), .27); border-inline-start-color: rgb(var(--v-theme-warning)); }
.ag-card.is-bucket { opacity: 1; }

.ag-head { display: flex; align-items: center; gap: 14px; padding: 9px 12px; flex-wrap: wrap; background: #0E1219; font-family: 'IBM Plex Mono', ui-monospace, monospace; }
.ag-isp { font-weight: 600; font-size: 12px; }
.ag-path { font-family: inherit; font-size: 11px; color: var(--nodus-ink-dim); background: transparent; padding: 0; }
.ag-state { font-weight: 600; font-size: 10px; letter-spacing: .1em; text-transform: uppercase; }
...
.ag-alarms { padding: 0; }
.ag-alarm { display: flex; flex-direction: column; gap: 6px; padding: 9px 12px;
  border-block-start: 1px solid #161B23; background: transparent; margin: 0; }
.ag-alarm.critical .ag-sev, .ag-alarm.warning .ag-sev { font-size: 10px; }
...
/* The device is being worked on: dim the row, keep the severity word its own colour
   (severity owns that, always) and say the reason in plain text. */
.ag-alarm--maint { opacity: .72; }
.ag-maint { font-size: 9.5px; font-weight: 700; letter-spacing: .06em; padding: 1px 6px; border-radius: 4px;
  white-space: nowrap; color: rgba(var(--v-theme-on-surface), .75); background: rgba(var(--v-theme-on-surface), .1); }
.ag-atitle { font-size: .86rem; flex: 0 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.ag-dev { display: inline-flex; align-items: center; gap: 3px; font-family: 'IBM Plex Mono', ui-monospace, monospace; font-size: 11px; flex: none;
  color: var(--nodus-ink-dim); background: transparent; padding: 0;
  text-decoration: none; }
.ag-neighbor { font-size: .66rem; color: rgb(var(--v-theme-on-surface), .6);
  border: 1px dashed rgba(var(--v-border-color), .4); border-radius: 0; padding: 1px 7px; }
.ag-mac { display: inline-flex; align-items: center; gap: 5px; font-family: ui-monospace, monospace; font-size: .64rem;
  color: rgb(var(--v-theme-on-surface), .7); background: rgba(var(--v-border-color), .1); border-radius: 0; padding: 1px 6px; }
...
.ag-inherit { font-family: ui-monospace, monospace; font-size: .64rem; color: rgb(var(--v-theme-warning));
  border: 1px solid rgba(var(--v-theme-warning), .35); border-radius: 0; padding: 1px 6px; }
