    ...
    <VCard title="Check for Updates">
      <VCardText>
        ...
        <template v-else-if="updateResult">
          ...
          <VAlert
            v-if="updateResult.configured === false"
            type="warning"
            variant="tonal"
          >
            Update check isn't configured (no GitHub token on the server), so this can't tell whether a newer version exists.
          </VAlert>

          <VAlert
            v-else-if="!updateResult.update_available"
            type="success"
            variant="tonal"
          >
