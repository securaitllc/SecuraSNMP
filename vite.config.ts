...
// The version this bundle was built from — baked in so the running SPA can detect
// when a newer version has been deployed and prompt the operator to reload (kills the
// "I deployed but don't see the change" stale-bundle problem).
const appVersion = (() => {
  try {
