 ...
 *
 * That history is only safe to keep because every row also records when the MAC
 * STOPPED appearing. A successful poll that does not list a MAC is positive evidence
 * it has left the forwarding table, so those rows are stamped absent_since. Without
 * it, "what was connected months ago" and "what is connected now" are the same row,
 * and readers downstream (IPAM) present one as the other.
 *
...
class MacPoller
{
    ...
    public function poll(Device $device, ?Carbon $now = null): int
    {
        ...
        foreach ($fdb as $e) {
            ...
            // Seen in this walk, so it is present again — clears an earlier absence.
            $row->absent_since = null;
            $row->save();
            $count++;
        }

        // Everything this device knew that the walk did NOT return has left the
        // forwarding table. Recorded, not deleted: an operator still needs to see what
        // used to be on a port, and a later poll that finds it again clears the mark.
        // A short walk is possible, so this is a soft mark that self-heals — never a
        // delete. Only reached once plausibleBatch() has accepted the read.
        MacAddress::where('device_id', $device->id)
