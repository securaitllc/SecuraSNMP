...
class MacPoller
{
    ...
    // jnxExVlanTag: Juniper VLAN index → real 802.1Q tag. On Juniper the dot1q
    // fdbId is that internal index, not the tag — remap it through this.
    ...
    public function poll(Device $device, ?Carbon $now = null): int
    {
        ...
        // Juniper: remap the dot1q fdbId (a VLAN index) to the real tag. Empty on
        // non-Juniper gear, where the fdbId already IS the 802.1Q VLAN.
        ...
        foreach ($fdb as $e) {
            ...
            $vlan = $vlanTags[$e['vlan']] ?? $e['vlan'];

            $row = MacAddress::firstOrNew(['device_id' => $device->id, 'mac' => $mac, 'vlan' => $vlan]);
            if (! $row->exists) {
