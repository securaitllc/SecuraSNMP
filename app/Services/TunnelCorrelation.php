...
class TunnelCorrelation
{
    ...
    public function analyze(): array
    {
        ...
        // Tokens are compared in a canonical form (leading zeros stripped per digit-run),
        // because a hub's tunnel label and the peer's device name disagree on zero-padding
        // — the hub names the tunnel "to_LA0001-SC63" while the edge is "LA0001-SC063_SDW".
        // Left unnormalized, that one-character mismatch left every tunnel symptom on the
        // hub instead of rolling it under the branch that was actually down (#063).
        //
        // Index EVERY name the appliance has answered to, not just the record's. A
        // rename on the console is invisible to `devices.name` until an operator
        // adopts it, but the peers pick it up within minutes — so on 18 September
        // the hubs began raising "to_SC125-ECB01_*" while the record still read
        // "GA0008-SC125_SDW" (tokens sc125-ecb1 vs ga8-sc125). Nothing matched, so
        // every tunnel symptom for that branch stayed on the HQ hubs and read as an
        // HQ fault. `snmp_hostname` is what the box says now and `previous_name` is
        // what it said before an adopt, which covers the window on either side of
        // the rename. `name` is written last so it still wins any collision.
        $edges = Device::where('role', 'edgeconnect')->get(['id', 'site_id', 'name', 'snmp_hostname', 'previous_name']);
        $tokenToSite = [];
        foreach ($edges as $e) {
            if ($e->site_id === null) {
                continue;
            }
            foreach ([$e->previous_name, $e->snmp_hostname, $e->name] as $alias) {
                $t = self::canonicalToken(self::labelToken((string) $alias));
                if ($t !== '') {
        ...
        $siteNames = Site::whereIn('id', array_keys($unhealthy))->pluck('name', 'id');
        ...
        foreach ($tunnelAlarms as $a) {
            ...
            $token = $token !== null ? self::canonicalToken($token) : null;
    ...
    /**
     * Canonical form for comparing appliance tokens: strip leading zeros inside each
     * digit-run so "la0001-sc63" and "la0001-sc063" (and "la1-sc63") all compare equal.
     * The peer number keeps its value ("sc63" ≠ "sc163"), only zero-padding is ignored.
     */
    public static function canonicalToken(string $token): string
    {
        return (string) preg_replace_callback('/\d+/', fn ($m) => ltrim($m[0], '0') ?: '0', strtolower(trim($token)));
    }
