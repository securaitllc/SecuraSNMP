...
class InterfaceAddressCollector
{
    ...
    public function collect(Device $device): int
    {
        ...
        $interfaces = DeviceInterface::where('device_id', $device->id)
            ->pluck('id', 'if_index');
        // The port NAME for each ifIndex, so an address can be judged by the interface
        // it sits on and not only by the block it falls in.
        $ifNames = DeviceInterface::where('device_id', $device->id)
        ...
        foreach ($ifIndexes as $ip => $ifIndex) {
            ...
            // By address AND by interface. Filtering on the address alone only catches
            // the blocks we happen to know: em2.32768 carries 192.168.1.2 on every
            // Juniper chassis, which is ordinary private space and was recorded as real
            // addressing. See App\Support\NetworkInterface.
            if (str_starts_with($ip, '127.') || self::isInternal($ip)
                || NetworkInterface::isControlPlane($ifNames[$ifIndex] ?? null)) {
                continue;
            }
