...
class ReportService
{
    /** Addresses one MAC must not hold: past this it is distorting a subnet's occupancy. */
    private const MAC_ADDRESS_FLOOD = 20;
    ...
    public const TYPES = [
        ...
        'isp-tickets' => 'ISP Tickets',
        'sites-nearby' => 'Sites Near a Location',
    ...
    private const COLUMNS = [
        ...
        'isp-tickets' => [
            ['key' => 'ticket_number', 'label' => 'ISP Ticket', 'default' => true],
            ['key' => 'name', 'label' => 'Circuit', 'default' => true],
            ['key' => 'site_name', 'label' => 'Site', 'default' => true],
            ['key' => 'isp_name', 'label' => 'ISP', 'default' => true],
            ['key' => 'reason', 'label' => 'Raised For', 'default' => true],
            ['key' => 'status', 'label' => 'Status', 'default' => true],
            ['key' => 'opened_at', 'label' => 'Opened', 'default' => true],
            ['key' => 'opened_by', 'label' => 'Opened By', 'default' => true],
            ['key' => 'open_days', 'label' => 'Days Open', 'align' => 'end', 'default' => true],
            ['key' => 'closed_at', 'label' => 'Closed', 'default' => false],
            ['key' => 'closed_by', 'label' => 'Closed By', 'default' => false],
            ['key' => 'note', 'label' => 'Note', 'default' => false],
        ],
        'duplicate-macs' => [
            ['key' => 'mac', 'label' => 'MAC', 'default' => true],
            ['key' => 'vendor', 'label' => 'Vendor', 'default' => true],
            ['key' => 'sites', 'label' => 'Sites', 'align' => 'end', 'default' => true],
            ['key' => 'site_name', 'label' => 'Site', 'default' => true],
            ['key' => 'switch', 'label' => 'Switch', 'default' => true],
            ['key' => 'port', 'label' => 'Port', 'default' => true],
            ['key' => 'vlan', 'label' => 'VLAN', 'default' => true],
            ['key' => 'ip', 'label' => 'Address', 'default' => true],
            ['key' => 'addresses', 'label' => 'IPs here', 'align' => 'end', 'default' => true],
            ['key' => 'last_seen', 'label' => 'Last seen', 'default' => true],
            ['key' => 'ports_total', 'label' => 'Ports fleet-wide', 'align' => 'end', 'default' => false],
        ],
        'sites-nearby' => [
            ['key' => 'run', 'label' => 'Run', 'default' => true],
            ['key' => 'stop', 'label' => 'Stop', 'align' => 'end', 'default' => true],
            ['key' => 'site_name', 'label' => 'Site', 'default' => true],
            ['key' => 'leg_miles', 'label' => 'Leg', 'align' => 'end', 'default' => true],
            ['key' => 'miles', 'label' => 'From origin', 'align' => 'end', 'default' => true],
            ['key' => 'bearing', 'label' => 'Direction', 'default' => true],
            ['key' => 'devices', 'label' => 'Devices', 'align' => 'end', 'default' => true],
            ['key' => 'circuits', 'label' => 'Circuits', 'align' => 'end', 'default' => true],
            ['key' => 'address', 'label' => 'Address', 'default' => true],
            ['key' => 'status', 'label' => 'Status', 'default' => true],
            ['key' => 'coordinates', 'label' => 'Coordinates', 'default' => false],
    ...
    public function generate(string $type, CarbonInterface $start, CarbonInterface $end, array $filters = []): array
    {
        $built = match ($type) {
            ...
            'isp-tickets' => $this->ispTickets($filters),
            'site-leases' => $this->siteLeases($filters),
            'sites-nearby' => $this->sitesNearby($filters),
            'duplicate-macs' => $this->duplicateMacs($filters),
    ...
    private function deviceAvailability(CarbonInterface $start, CarbonInterface $end, array $filters): array
    {
        ...
        // Planned work is not an outage — subtract every window covering each device.
        $maintenance = MaintenanceWindow::spansForDevices($devices, $start, $end);

        $periodMin = $this->periodMinutes($start, $end);
        $rows = $devices->map(function (Device $d) use ($spans, $start, $end, $periodMin, $maintenance) {
            $avail = $this->fmtAvailability($spans[$d->id] ?? [], $start, $end, $maintenance[$d->id] ?? []);
    ...
    /**
     * Every ISP ticket raised on a circuit, newest first.
     *
     * The gap this closes: circuits.isp_ticket holds one number and forgets the last
     * one. An operator who logged a ticket against a packet-loss finding could see it
     * on that circuit's row and nowhere else — not which tickets are open across the
     * fleet, not who raised them, not how long the carrier has had them.
     *
     * @param  array<string, mixed>  $filters
     * @return array{rows: array, summary: array}
     */
    private function ispTickets(array $filters): array
    {
        $tickets = CircuitIspTicket::with('circuit.site')
            ->when($filters['site_id'] ?? null, fn ($q, $id) => $q->whereHas('circuit', fn ($c) => $c->where('site_id', $id)))
            ->latest('opened_at')
            ->get()
            ->filter(fn ($t) => $t->circuit !== null);

        $now = now();
        $rows = $tickets->map(function (CircuitIspTicket $t) use ($now) {
            $open = $t->closed_at === null;
            $until = $t->closed_at ?? $now;
            $days = $t->opened_at ? (int) floor(abs($until->diffInDays($t->opened_at))) : null;

            // Colour carries how long the carrier has had it, and only while it is
            // still open — a closed ticket that ran long is history, not a problem.
            $tone = ! $open ? null : ($days >= 14 ? 'error' : ($days >= 5 ? 'warning' : null));

            return [
                'ticket_number' => $t->ticket_number,
                'name' => $t->circuit->circuit_id,
                'site_name' => $t->circuit->site?->name ?? '—',
                'isp_name' => $t->circuit->isp_name ?? '—',
                'reason' => $t->reason ?: '—',
                'status' => $open ? 'Open' : 'Closed',
                'opened_at' => $t->opened_at?->toDateString() ?? '—',
                'opened_by' => $t->opened_by ?: '—',
                'open_days' => $days ?? '—',
                'closed_at' => $t->closed_at?->toDateString() ?? '—',
                'closed_by' => $t->closed_by ?: '—',
                'note' => $t->note ?: '—',
                '_tone' => array_filter([
                    'status' => $open ? ($tone ?? 'info') : null,
                    'open_days' => $tone,
                ]),
            ];
        })->values()->all();

        $open = $tickets->whereNull('closed_at');

        return [
            'rows' => $rows,
            'summary' => [
                ['label' => 'Tickets', 'value' => (string) $tickets->count()],
                ['label' => 'Open now', 'value' => (string) $open->count()],
                ['label' => 'Circuits affected', 'value' => (string) $tickets->pluck('circuit_id')->unique()->count()],
                // The number worth acting on: a ticket the carrier has sat on.
                ['label' => 'Open over 14 days', 'value' => (string) $open->filter(
                    fn ($t) => $t->opened_at && abs($t->opened_at->diffInDays(now())) >= 14,
                )->count()],
            ],
        ];
    }
        ...
        $rows = $circuits->map(function (Circuit $c) use ($spans, $start, $end, $periodMin) {
            ...
            return [
                ...
                '_tone' => array_filter([
                    ...
                    // No tone on a missing LEC. Most DIA circuits are bought straight
                    // from the carrier and have no last-mile reseller to record, so
                    // marking every one as a gap would put a rail on ~120 fiber rows
                    // and bury the handful of cable circuits that genuinely lack one.
                    // The "No LEC on file" figure in the summary carries that instead.
                ]),
            ];
        })->all();
     ...
     */
    /**
     * One MAC seen at more than one SITE — and every port it answers on.
     *
     * A MAC is globally unique by design, so the same one at two locations means
     * something is cloning it. Massey's Dell fleet does exactly that: one address
     * answers on 151 of a /24 at #198 and 130 of another at #013, which made both
     * subnets read as filling up when their real occupancy is a fifth of that. It
     * also poisons MAC search — an operator tracing an endpoint lands on whichever
     * site the poller happened to see last.
     *
     * Virtual MACs are excluded because they are SUPPOSED to repeat: 00:00:5E:00:01:xx
     * and :02:xx are VRRP/VRRPv3 routers and every gateway pair at every site carries
     * one; multicast and broadcast are not endpoints at all. Including them would bury
     * the real finding under 131 sites of expected noise.
     *
     * One row per (MAC, site, port), so the report says where to go and what to
     * unplug. Sorted by how widely a MAC has spread.
     *
     * @return array{rows: array, summary: array}
     */
    private function duplicateMacs(array $filters): array
    {
        $minSites = max(2, (int) ($filters['min_sites'] ?? 2));

        $rows = DB::table('mac_addresses')
            ->join('devices', 'devices.id', '=', 'mac_addresses.device_id')
            ->leftJoin('sites', 'sites.id', '=', 'devices.site_id')
            ->leftJoin('device_interfaces', 'device_interfaces.id', '=', 'mac_addresses.device_interface_id')
            ->whereNotNull('devices.site_id')
            ->where('mac_addresses.mac', 'not like', '00:00:5E:00:01:%')
            ->where('mac_addresses.mac', 'not like', '00:00:5E:00:02:%')
            ->where('mac_addresses.mac', 'not like', '01:00:5E:%')
            ->where('mac_addresses.mac', 'not like', '33:33:%')
            ->where('mac_addresses.mac', '!=', 'FF:FF:FF:FF:FF:FF')
            ->where('mac_addresses.mac', '!=', '00:00:00:00:00:00')
            ->when($filters['site_id'] ?? null, fn ($q, $id) => $q->where('devices.site_id', $id))
            ->get([
                'mac_addresses.mac', 'mac_addresses.oui_vendor', 'mac_addresses.vlan',
                'mac_addresses.last_seen_at', 'devices.site_id', 'devices.name as device_name',
                'sites.name as site_name', 'device_interfaces.if_name',
            ]);

        // Only MACs that genuinely span sites. One site's stack learning an endpoint
        // on two ports is normal, not a clone.
        $siteCount = [];
        foreach ($rows as $r) {
            $siteCount[$r->mac][$r->site_id] = true;
        }
        $spread = array_filter($siteCount, fn ($sites) => count($sites) >= $minSites);
        if ($spread === []) {
            return ['rows' => [], 'summary' => ['macs' => 0, 'sites' => 0, 'ports' => 0, 'min_sites' => $minSites]];
        }

        $portsPerMac = [];
        foreach ($rows as $r) {
            if (isset($spread[$r->mac])) {
                $portsPerMac[$r->mac][$r->site_id.'|'.$r->device_name.'|'.$r->if_name] = true;
            }
        }

        // The addresses each clone answers for, per site — the figure that explains a
        // subnet reading full. Keyed by site because a private range repeats fleet-wide.
        $ipsBySiteMac = [];
        $firstIp = [];
        DB::table('arp_entries')->whereIn('mac', array_keys($spread))->whereNotNull('site_id')
            ->orderByDesc('last_seen_at')
            ->get(['mac', 'site_id', 'ip'])
            ->each(function ($a) use (&$ipsBySiteMac, &$firstIp) {
                $key = $a->mac.'|'.$a->site_id;
                $ipsBySiteMac[$key][$a->ip] = true;
                $firstIp[$key] ??= $a->ip;
            });
        ...
        foreach ($rows as $r) {
            ...
            $key = $r->mac.'|'.$r->site_id;
            $addresses = count($ipsBySiteMac[$key] ?? []);
            $sites = count($spread[$r->mac]);
            ...
            $out[] = [
                ...
                'switch' => $r->device_name,
                'port' => $r->if_name ?: '—',
                'vlan' => $r->vlan ?: '—',
                'ip' => $firstIp[$key] ?? '—',
                'addresses' => $addresses,
                'last_seen' => $r->last_seen_at
                    ? CarbonImmutable::parse($r->last_seen_at)->setTimezone(OpsTime::zone())->format('Y-m-d H:i')
                    : '—',
                'ports_total' => count($portsPerMac[$r->mac] ?? []),
                // A clone answering for many addresses is the one distorting IPAM.
                '_tone' => array_filter([
                    'sites' => $sites >= 5 ? 'error' : 'warning',
                    'addresses' => $addresses >= self::MAC_ADDRESS_FLOOD ? 'error' : null,
                ]),
            ];
        }

        usort($out, fn ($a, $b) => [$b['sites'], $a['mac'], $a['site_name']] <=> [$a['sites'], $b['mac'], $b['site_name']]);

        return [
            'rows' => $out,
            'summary' => [
                'macs' => count($spread),
                'sites' => count(array_unique(array_column($out, 'site_name'))),
                'ports' => count($out),
                'min_sites' => $minSites,
            ],
        ];
    ...
    /**
     * Sites within a mileage band of one origin site — the planning list for a
     * migration run, where the question is "what else can I do the same day".
     *
     * Distance is great-circle, not drive time. On a fleet spread across a metro that
     * understates real travel by a good margin, so the band is a shortlist to route,
     * never a schedule. The column says Miles for that reason.
     *
     * A site with no coordinates is INCLUDED, flagged, and sorted last. Dropping it
     * would make an unmeasured site read exactly like a site that isn't nearby, which
     * is the same absence-means-good trap this app keeps having to fix. The operator
     * has to see that the answer is incomplete.
     */
    private function sitesNearby(array $filters): array
    {
        $originId = (int) ($filters['site_id'] ?? 0);
        $min = (float) ($filters['min_miles'] ?? 0);
        $max = (float) ($filters['max_miles'] ?? 20);
        if ($max < $min) {
            [$min, $max] = [$max, $min];
        }
        $perRun = max(1, (int) ($filters['stops_per_run'] ?? 5));

        $origin = $originId ? Site::find($originId) : null;
        if (! $origin) {
            return ['rows' => [], 'summary' => [
                ['label' => 'Origin', 'value' => 'Pick a site to measure from'],
            ]];
        }
        if ($origin->latitude === null || $origin->longitude === null) {
            return ['rows' => [], 'summary' => [
                ['label' => 'Origin', 'value' => $origin->name],
                ['label' => 'Problem', 'value' => 'This site has no coordinates on file — nothing can be measured from it'],
            ]];
        }

        $sites = Site::withCount(['devices', 'circuits'])->where('id', '!=', $origin->id)->orderBy('name')->get();
        ...
        foreach ($sites as $site) {
            ...
            $miles = self::greatCircleMiles(
                (float) $origin->latitude, (float) $origin->longitude,
                (float) $site->latitude, (float) $site->longitude,
            );
            if ($miles >= $min && $miles <= $max) {
                $inBand[] = ['site' => $site, 'miles' => $miles];
            }
        }

        $runs = self::planRuns($origin, $inBand, $perRun);

        $rows = [];
        $runMiles = [];
        foreach ($runs as $i => $run) {
            $runLabel = 'Run '.($i + 1);
            $runMiles[$runLabel] = 0.0;
            foreach ($run as $n => $stop) {
                $site = $stop['site'];
                $runMiles[$runLabel] += $stop['leg'];
                $rows[] = [
                    'run' => $runLabel,
                    'stop' => $n + 1,
                    'site_name' => $site->name,
                    'leg_miles' => round($stop['leg'], 1),
                    'miles' => round($stop['miles'], 1),
                    'bearing' => self::compassPoint(
                        (float) $origin->latitude, (float) $origin->longitude,
                        (float) $site->latitude, (float) $site->longitude,
                    ),
                    'devices' => $site->devices_count,
                    'circuits' => $site->circuits_count,
                    'address' => $site->address ?: '—',
                    'coordinates' => round((float) $site->latitude, 5).', '.round((float) $site->longitude, 5),
                    'region' => $site->region ?? '—',
                    'status' => 'In band',
                ];
            }
        ...
        // Unplotted sites are appended, never dropped. A site nobody can measure must
        // not read the same as a site that is simply not nearby — that is the
        // absence-means-good trap, and here it would mean quietly skipping a location
        // on a migration run.
        foreach ($unplotted as $site) {
            $rows[] = [
                'run' => 'Unplaced',
                'stop' => '—',
                'site_name' => $site->name,
                ...
                'coordinates' => '—',
                'region' => $site->region ?? '—',
                'status' => 'No coordinates — not measured',
                '_tone' => ['status' => 'warning', 'run' => 'warning', 'miles' => 'warning'],
            ];
        }

        $summary = [
            ['label' => 'Origin', 'value' => $origin->name],
            ['label' => 'Band', 'value' => self::trimNum($min).' to '.self::trimNum($max).' miles'],
            ['label' => 'Runs', 'value' => (string) count($runs)],
            ['label' => 'Sites in band', 'value' => (string) count($inBand)],
            ['label' => 'Devices in band', 'value' => (string) collect($inBand)->sum(fn ($e) => $e['site']->devices_count)],
        ];
        foreach ($runMiles as $label => $m) {
            $summary[] = ['label' => $label.' driving', 'value' => self::trimNum(round($m, 1)).' miles (straight line)'];
        }
        if ($unplotted !== []) {
            $summary[] = ['label' => 'Not measured', 'value' => count($unplotted).' sites have no coordinates'];
        }
    ...
    /**
     * Split the in-band sites into drive-able runs, each an ordered list of stops.
     *
     * Nearest-neighbour from the origin: take the closest unvisited site, then the
     * closest to THAT one, and so on. It is the classic greedy tour — not optimal, but
     * it produces the order a person would drive anyway, and it keeps a run inside one
     * pocket of the map instead of criss-crossing the metro.
     *
     * A run ends when it is full (stops per run) or when the next hop is a long one:
     * a leg more than twice the run's average so far, and over 5 miles, means the
     * cluster has been exhausted and the next site belongs to a different trip.
     *
     * Straight-line throughout — see the note on sitesNearby. Real roads, water and
     * one-ways will change the order; this is the shortlist to route, not the route.
     *
     * @param  list<array{site: Site, miles: float}>  $inBand
     * @return list<list<array{site: Site, miles: float, leg: float}>>
     */
    private static function planRuns(Site $origin, array $inBand, int $perRun): array
    {
        $remaining = $inBand;
        $runs = [];
        ...
        while ($remaining !== []) {
            $run = [];
            $fromLat = (float) $origin->latitude;
            $fromLon = (float) $origin->longitude;
            $legs = [];

            while ($remaining !== [] && count($run) < $perRun) {
                $bestKey = null;
                $bestLeg = INF;
                foreach ($remaining as $key => $entry) {
                    $leg = self::greatCircleMiles(
                        $fromLat, $fromLon,
                        (float) $entry['site']->latitude, (float) $entry['site']->longitude,
                    );
                    if ($leg < $bestLeg) {
                        $bestLeg = $leg;
                        $bestKey = $key;
                ...
                // A hop far longer than this run's own legs means the pocket is done.
                $avg = $legs === [] ? null : array_sum($legs) / count($legs);
                if ($run !== [] && $avg !== null && $bestLeg > 5 && $bestLeg > $avg * 2) {
                    break;
                }

                $entry = $remaining[$bestKey];
                unset($remaining[$bestKey]);
                $legs[] = $bestLeg;
                $run[] = ['site' => $entry['site'], 'miles' => $entry['miles'], 'leg' => $bestLeg];
                $fromLat = (float) $entry['site']->latitude;
                $fromLon = (float) $entry['site']->longitude;
            }
    ...
    /** 12.0 -> "12", 12.5 -> "12.5". */
    private static function trimNum(float $n): string
    {
        return rtrim(rtrim(number_format($n, 1, '.', ''), '0'), '.');
    }

    /** Great-circle distance in statute miles. */
    private static function greatCircleMiles(float $lat1, float $lon1, float $lat2, float $lon2): float
    {
        $r = 3958.8;
    ...
    /** Compass point from origin to target — "which way do I drive". */
    private static function compassPoint(float $lat1, float $lon1, float $lat2, float $lon2): string
    {
        $dLon = deg2rad($lon2 - $lon1);
        $y = sin($dLon) * cos(deg2rad($lat2));
        $x = cos(deg2rad($lat1)) * sin(deg2rad($lat2)) - sin(deg2rad($lat1)) * cos(deg2rad($lat2)) * cos($dLon);
        $deg = fmod(rad2deg(atan2($y, $x)) + 360, 360);
    ...
    private function fmtAvailability(array $spans, CarbonInterface $start, CarbonInterface $end, array $excluded = []): array
    {
        $a = self::availability($spans, $start, $end, $excluded);
    ...
    /**
     * Seconds of the given spans that fall inside [start, end], overlaps merged so an
     * overlapping pair is not subtracted twice.
     *
     * @param  array<int, array{starts_at: CarbonInterface, ends_at: CarbonInterface}>  $spans
     */
    private static function mergedSeconds(array $spans, CarbonInterface $start, CarbonInterface $end): int
    {
        $ranges = [];
        foreach ($spans as $sp) {
            $a = max($sp['starts_at']->getTimestamp(), $start->getTimestamp());
            $b = min($sp['ends_at']->getTimestamp(), $end->getTimestamp());
            if ($b > $a) {
                $ranges[] = [$a, $b];
            }
        }
        ...
        $total = 0;
        [$cs, $ce] = $ranges[0];
        foreach (array_slice($ranges, 1) as [$a, $b]) {
            if ($a > $ce) {
                $total += $ce - $cs;
                [$cs, $ce] = [$a, $b];
            } elseif ($b > $ce) {
    ...
    /** How much of one outage fell inside a maintenance window. */
    private static function overlapSeconds(CarbonInterface $from, CarbonInterface $to, array $spans): int
    {
        return self::mergedSeconds($spans, $from, $to);
    }
     ...
     */
    public static function availability(array $spans, CarbonInterface $start, CarbonInterface $end, array $excluded = []): array
    {
        ...
        // Planned maintenance is excluded from BOTH sides of the calculation: the time
        // spent in a window is not downtime, and the window itself is not part of the
        // measured period. Subtracting it from downtime alone would still let a long
        // planned job drag the percentage down.
        $excludedSeconds = self::mergedSeconds($excluded, $start, $end);
        $measured = max(1, $windowSeconds - $excludedSeconds);
        ...
        foreach ($spans as $span) {
            ...
            if ($e->greaterThan($s)) {
                ...
                // Whatever part of this outage fell inside a maintenance window was
                // planned, and does not count against the device.
                $seconds -= self::overlapSeconds($s, $e, $excluded);
                if ($seconds > 0) {
                    $downtime += $seconds;
                    $incidents++;
                }
        ...
        $uptime = max(0, min(100, 100 - ($downtime / $measured * 100)));
