...
class ReportService
{
    ...
    private function selectColumns(string $type, ?array $fields): array
    {
        ...
        return array_values(array_filter($all, fn ($c) => in_array($c['key'], $fields, true)));
    }
    ...
    private function circuitAvailability(CarbonInterface $start, CarbonInterface $end, array $filters): array
    {
        ...
        $rows = $circuits->map(fn (Circuit $c) => ['name' => $c->circuit_id, 'scope' => $c->site?->name ?? '—']
            + $this->fmtAvailability($spans[$c->id] ?? [], $start, $end))->all();
    ...
    private function deviceAvailability(CarbonInterface $start, CarbonInterface $end, array $filters): array
    {
        ...
        $rows = $devices->map(fn (Device $d) => ['name' => $d->name, 'scope' => $d->site?->name ?? '—']
            + $this->fmtAvailability($spans[$d->id] ?? [], $start, $end))->all();
    ...
    private function tunnelAvailability(CarbonInterface $start, CarbonInterface $end, array $filters): array
    {
        ...
        $rows = $tunnels->map(fn (Tunnel $t) => ['name' => $t->tunnel_name, 'scope' => $t->device?->name ?? '—']
            + $this->fmtAvailability($spans[$t->id] ?? [], $start, $end))->all();
    ...
    private function deviceInventory(array $filters): array
    {
        ...
        return [
            'rows' => $rows,
            'summary' => [
                ['label' => 'Devices', 'value' => (string) count($rows)],
                ['label' => 'Vendors', 'value' => (string) $devices->pluck('vendor')->filter()->unique()->count()],
