...
class EdgeConnectAlarmPoller
{
    ...
    public function poll(Device $device): void
    {
        ...
        // A tunnel to a paused REMOTE circuit is named on the HUB, not the paused
        // appliance (e.g. "to_FL0092-HCF_Broadband1-LTE" reported by the hub SDW).
        // Match those cross-site so pausing HCF's LTE also silences the hub's
        // "tunnel down to HCF-LTE" alarm.
        ...
        foreach ($names as $index => $name) {
            ...
            if ($this->belongsToMutedWan($name, $source, $mutedWans, $mutedGwIps)
                || $this->belongsToMutedTunnel($source, $tunnelMatchers)) {
                continue;
            }
    ...
    /**
     * Is this a tunnel alarm to a paused remote circuit? The source must be a
     * tunnel label ("to_…") that carries BOTH a paused site's token AND a token
     * for that site's paused WAN.
     *
     * @param  list<array{site: string, wans: list<string>}>  $matchers
     */
    private function belongsToMutedTunnel(string $source, array $matchers): bool
    {
        if ($matchers === []) {
            return false;
        }
