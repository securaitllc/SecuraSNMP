...
class FlowIngestor
{
    ...
    public function ingest(array $lines): int
    {
        ...
        foreach ($lines as $line) {
            ...
            // goflow2 v2 emits proto as a NAME string ("TCP"/"UDP"/"ESP"), older/other
            // exporters send the IANA number. Handle both.
            $protocol = $this->normalizeProtocol($f['proto'] ?? null);
    ...
    /** goflow2 proto may be a name ("TCP") or an IANA number (6) — normalize to a name. */
    private function normalizeProtocol(mixed $proto): string
    {
        if (is_string($proto) && ! ctype_digit($proto)) {
            return strtolower($proto);              // "TCP" -> "tcp"
        }

        return $this->classifier->protocol((int) $proto);
    }
