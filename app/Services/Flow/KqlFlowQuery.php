...
class KqlFlowQuery
{
    ...
    private const FIELDS = [
        ...
        'device' => ['__device', 'str'],   // by device name (partial match)
        'bytes' => ['bytes', 'int'],
        'packets' => ['packets', 'int'],
    ];
    ...
    private function applyClause(Builder $query, array $c): void
    {
        ...
        // Device by name → the flow's device_id must be one of the matching devices.
        if ($col === '__device') {
            $ids = Device::where('name', 'like', '%'.$this->escapeLike((string) $val).'%')->pluck('id');
            $query->whereIn('device_id', $ids->all() ?: [-1]);
