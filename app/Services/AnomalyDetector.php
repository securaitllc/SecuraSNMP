...
class AnomalyDetector
{
    ...
    /**
     * Active maintenance windows, resolved once per sweep.
     *
     ...
     *
     * Memoised on the instance: the sweep walks thousands of interfaces and must not
     * re-query per entity.
     *
     * @var array{global: bool, devices: array<int, int>, sites: array<int, int>}|null
     */
    private ?array $maintenance = null;

    /** @var array<int, int|null>|null device id => site id */
    private ?array $siteOfDevice = null;

    private function underMaintenance(?int $deviceId, ?int $siteId): bool
    {
        $this->maintenance ??= (function (): array {
            $windows = MaintenanceWindow::active()->get(['site_id', 'device_id']);

            return [
                // A window with neither a site nor a device covers the whole fleet.
                'global' => $windows->contains(fn ($w) => ! $w->site_id && ! $w->device_id),
                'devices' => $windows->pluck('device_id')->filter()->flip()->all(),
                'sites' => $windows->pluck('site_id')->filter()->flip()->all(),
            ];
        })();

        if ($this->maintenance['global']) {
            return true;
        }

        return ($deviceId !== null && isset($this->maintenance['devices'][$deviceId]))
            || ($siteId !== null && isset($this->maintenance['sites'][$siteId]));
    }

    /** Site a device belongs to, from one query for the whole fleet. */
    private function siteOf(?int $deviceId): ?int
    {
        if ($deviceId === null) {
            return null;
        }
        $this->siteOfDevice ??= Device::query()->pluck('site_id', 'id')->all();

        return $this->siteOfDevice[$deviceId] ?? null;
    }
    ...
    public function scanInterface(int $interfaceId): void
    {
        ...
        if ($this->underMaintenance($iface?->device_id, $this->siteOf($iface?->device_id))) {
            return;
        }
