...
class AnomalyDetector
{
    ...
    /**
     * Discards per poll that are abnormal no matter what the port's own history says.
     *
     * Measured off the fleet, not guessed: of 24,268 polled interfaces, 24,015 discard
     * nothing at all and only nine sit above this line. A port over it is not having a
     * bad hour — it is dropping traffic as a matter of course.
     */
    private const DISCARD_SUSTAINED_FLOOR = 500;

    /** Reported for an absolute discard hit. Not a computed z — there is no deviation. */
    private const DISCARD_ABSOLUTE_Z = 6.0;
    ...
    /** Polls the loss-frequency floor looks back over — matches CircuitMonitor's window. */
    private const LOSS_WINDOW = 20;

    /**
     * The z reported for a frequency-floor finding. There is no real z — the whole
     * point is that the baseline could not see it — so it is a fixed value above the
     * page's "high" threshold, and the observed/baseline pair says what it actually is:
     * lossy share of polls against an expected 0.
     */
    private const LOSS_FREQUENCY_Z = 6.0;
     ...
     */
    /**
     * Anomalies additionally skip the management and loopback ports. They are real
     * interfaces carrying real addresses — IPAM is right to keep them — but nothing
     * forwards traffic over them, so a throughput or discard baseline on one is noise.
     *
     * @var array<int, string>
     */
    private const NON_FORWARDING = ['/^lo\\d/i', '/^vme\\d/i', '/^me\\d/i', '/^fxp\\d/i'];
    ...
    public static function isInternalInterface(?string $ifName): bool
    {
        ...
        if (NetworkInterface::isControlPlane($name)) {
            return true;
        }

        foreach (self::NON_FORWARDING as $re) {
    ...
    public function scanInterface(int $interfaceId): void
    {
        ...
        $hist = InterfaceMetricHistory::where('device_interface_id', $interfaceId)
            ...
            ->get(['recorded_at', 'in_octets_delta', 'out_octets_delta', 'in_discards_delta', 'out_discards_delta', 'in_multicast_delta', 'in_errors_delta', 'out_errors_delta']);
        ...
        // Multicast a port declined to accept is not a drop — see InterfaceHealth. The
        // fleet's discard anomalies were overwhelmingly this, so it is subtracted before
        // anything here judges the series.
        $disc = $hist->map(fn ($h) => (float) max(0, $h->in_discards_delta + $h->out_discards_delta - (int) ($h->in_multicast_delta ?? 0)));
        ...
        // A port that has ALWAYS discarded teaches its own baseline that discarding is
        // normal, so the relative test above can never fire on it. Four LAG members to
        // the HQ firewall tail-dropped ~1.8% of every packet for five weeks and produced
        // no row, because 14,726 a poll WAS their median. Absence of deviation is not
        // absence of fault — the same floor circuit loss already carries.
        if ($discHit === null) {
            $recent = $disc->slice(-self::SUSTAIN)->all();
            if (count($recent) >= self::SUSTAIN && min($recent) >= self::DISCARD_SUSTAINED_FLOOR) {
                $discHit = [
                    'direction' => 'spike',
                    'z' => self::DISCARD_ABSOLUTE_Z,
                    // Discards SHOULD be zero. The expectation is the baseline here, not
                    // the port's own history — quoting that history back would restate
                    // the very thing that hid the fault.
                    'baseline' => 0.0,
                    'observed' => round((float) end($recent), 2),
                ];
            }
        }
    ...
    public function scanCircuit(int $circuitId): void
    {
        ...
        if (! $circuit || $this->underMaintenance(null, $circuit->site_id)) {
            return;
        }
        ...
        // ABSOLUTE FLOOR, beside the relative one. The baseline is the circuit's own
        // seven days, so a circuit that has been lossy for a day has taught the baseline
        // that loss is normal and nothing deviates from it. #037's DIA lost packets on
        // 47% of polls for a day — 96 of 206, peaks of 90% — and raised nothing, because
        // by then 47% WAS its baseline. A circuit losing packets on a quarter of its
        // recent probes is anomalous whatever it was doing last week.
        if ($lossHit === null) {
            $window = $loss->slice(-self::LOSS_WINDOW)->values();
            if ($window->count() >= self::LOSS_WINDOW) {
                $lossyShare = $window->filter(fn (float $l) => $l > 0)->count() / $window->count() * 100;
                if ($lossyShare >= CircuitMonitor::LOSSY_POLLS_DEGRADED_PCT) {
                    $lossHit = [
                        'observed' => round($lossyShare, 1),
                        'baseline' => 0.0,
                        'z' => self::LOSS_FREQUENCY_Z,
                        'direction' => 'spike',
                    ];
                }
            }
        }
        // CORROBORATION. Every figure above comes from pinging the carrier's own
        // gateway, and a router polices ICMP addressed to itself. On 20 September
        // fourteen circuits read 20-35% of polls lossy while their appliances carried
        // 6,000-9,000 packets apiece across the same circuits and lost NONE. The
        // appliance counts what crosses the wire; the ping counts what a rate limiter
        // felt like answering. Where they disagree, the packets win.
        if ($lossHit !== null && $this->transportSaysClean($circuit)) {
            $lossHit = null;
        }
    ...
    /**
     * Did the appliance measure real traffic across this circuit and find it clean?
     *
     * Only a reading with a usable sample counts. Null transport loss means too few
     * packets to judge, and that is NOT permission to dismiss the ICMP signal — an
     * unmeasured circuit falls back to the evidence we do have.
     */
    private function transportSaysClean(Circuit $circuit): bool
    {
        return $circuit->transport_loss_pct !== null
            && (float) $circuit->transport_loss_pct <= 0.0
            && $circuit->transport_measured_at !== null
            && $circuit->transport_measured_at->gt(now()->subHour());
    }
