...
class NextHopPoller
{
    ...
    /**
     * Close alerts the per-device pass can no longer close for itself.
     *
     * poll() returns early when `show system nexthops` gives nothing back — an
     * appliance refusing SSH, rotated credentials, a replacement unit whose output
     * does not parse — and it returns BEFORE the recovery close. The alert then
     * outlives the outage by however long SSH stays broken. Same for an alert whose
     * next-hop row was pruned: its foreign key is nulled, so no device pass can ever
     * match it again.
     *
     * This runs after the sweep, on every cycle, independent of any one device's SSH.
     * It closes what the evidence no longer supports — never opens anything.
     */
    public static function resolveStale(): int
    {
        $now = now();

        $closed = NextHopAlert::whereNull('ended_at')
            ->whereDoesntHave('nextHop', fn ($q) => $q
                ->where('status', 'down')
                ->where('last_checked_at', '>=', $now->copy()->subMinutes(NextHopAlert::EVIDENCE_MINUTES)))
            ->get();

        foreach ($closed as $alert) {
            // One model update at a time so the observer resolves each notification,
            // exactly as a recovery close does.
            $alert->update(['ended_at' => $now]);
        }

        if ($closed->isNotEmpty()) {
            Log::info('Closed next-hop alerts no longer supported by evidence', ['count' => $closed->count()]);
        }
