...
class FortiGateAlarmPoller
{
    ...
    // fgHaSystemMode: standalone(1), activeActive(2), activePassive(3).
    private const OID_HA_MODE = '.1.3.6.1.4.1.12356.101.13.1.1.0';

    /** The modes that mean this box is actually in a cluster. */
    private const HA_CLUSTERED_MODES = [2, 3];
    ...
    /**
     * The alarm families this poller raises and is therefore allowed to clear.
     * Log-derived alarms (fgl:) are owned by FortiGateLogAlarms and swept on a
     * quiet period instead — they are events, and never appear in a table to be
     * found missing from.
     *
     * @var list<string>
     */
    private const OWNED_PREFIXES = ['fg:vpn:', 'fg:ha:', 'fg:sys:'];
     ...
     *
     * Massey runs no IPsec: the WAN is EdgeConnect, and fgVpnTunEntStatus is empty
     * on all three firewalls because the table genuinely has no rows — confirmed by
     * probe, not assumed. So this check watches nothing there today, and that is the
     * correct behaviour rather than a fault to chase.
     *
     * It stays enabled anyway. The status walk is a single OID and it returns
     * immediately when the table is empty, so the cost is one walk per firewall per
     * poll — the difference between noticing the day someone stands a tunnel up and
     * not noticing. Set FORTIGATE_CHECK_VPN=false to stop even that.
     *
    ...
    {
        // A standalone FortiGate still answers fgHaStatsSyncStatus — with ONE row,
        // for itself, reading 0. Read as "out of sync" that is a critical alarm on
        // a box with no cluster to be out of sync with, which is exactly what
        // FGT60E-HQFW00 and AZR-FW01 raised within minutes of this shipping. An
        // empty table was guarded against; a populated one describing a cluster of
        // one was not.
        //
        // So the mode decides, not the table. Only an explicit clustered mode is
        // taken as clustered: standalone, an unparsable answer and no answer at all
        // all mean no HA alarms, because none of them is evidence of a cluster.
        if (! self::enabled('ha')) {
            return [];
        }

        $mode = $this->scalar($device, self::OID_HA_MODE);
        if ($mode === null || ! in_array($mode, self::HA_CLUSTERED_MODES, true)) {
            return [];
        }
        ...
        // One member is not a cluster either, whatever the mode claims — there is
        // nothing for it to be out of sync with.
        if (count($sync) < 2) {
            return [];
        }
        ...
        foreach ($sync as $index => $value) {
            ...
            // Without a serial we cannot say WHICH unit is out of sync, and
            // "member 1" tells a NOC nothing it can act on. An unidentifiable row
            // is not a reportable fault.
            $serial = trim($serials[$index] ?? '');
            if ($serial === '') {
                continue;
            }
    ...
    private function resourceAlarms(Device $device): array
    {
        ...
        $memory = self::enabled('memory') ? $this->scalar($device, self::OID_MEMORY) : null;
        ...
        $cpu = self::enabled('cpu') ? $this->scalar($device, self::OID_CPU) : null;
        ...
        $used = self::enabled('disk') ? $this->scalar($device, self::OID_DISK_USED) : null;
        $capacity = self::enabled('disk') ? $this->scalar($device, self::OID_DISK_CAP) : null;
    ...
    /**
     * Is this check switched on? Every state-derived check can be silenced from the
     * environment, because a false critical on a security device must not need a
     * release to stop. See config/fortigate.php.
     */
    private static function enabled(string $check): bool
    {
        return (bool) config("fortigate.checks.{$check}", true);
    }
    ...
    /**
     * Clear the state-derived alarms this device is no longer reporting.
     *
     * Scoped to the three families this poller actually owns. Matching the whole
     * `fg:` prefix would also sweep anything else that happened to start with it —
     * and log-derived alarms, which are never present in an SNMP table, would have
     * been cleared on every single pass.
     *
     * @param  list<string>  $seen
     */
    private function clearMissing(Device $device, array $seen): void
    {
        $owned = fn ($q) => $q->where(function ($q) {
            foreach (self::OWNED_PREFIXES as $prefix) {
                $q->orWhere('alarm_id', 'like', $prefix.'%');
            }
        });
