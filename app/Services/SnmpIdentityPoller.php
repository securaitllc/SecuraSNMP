...
{
    /**
     * How often identity is re-read, for every device — including one whose model,
     * serial and OS are all already known. Hardware gets swapped behind an IP, and
     * a record that can only ever be written once cannot notice.
     */
    public const REFRESH_HOURS = 1;
    ...
    // FORTINET-FORTIGATE-MIB fgSysVersion — "v7.2.10,build1706,240918 (GA.M)".
    // Confirmed answering on all three Massey firewalls by `fortigate:probe`.
    private const OID_FGT_VERSION = '.1.3.6.1.4.1.12356.101.4.1.1.0';
    ...
    public function poll(Device $device): void
    {
        ...
        // SD-WAN migration, a switch swap, an RMA). DeviceMonitor stamps this when an
        // unreachable alarm actually clears. See recheckIdentity().
        $recheck = $device->identity_recheck_at !== null;

        // ONE gate, for complete and incomplete identity alike.
        //
        // A complete identity used to return here for good, and the only thing that
        // ever reopened it was a recovery — so a unit swapped at the same IP that
        // never dropped ICMP long enough to raise and clear an alarm kept the dead
        // box's serial on the record indefinitely. There was no ceiling on how wrong
        // the inventory could get.
        //
        // The original storm this guard was written against came from re-walking
        // every 5-minute health cycle; hourly is twelve times less and is bounded by
        // the same ICMP gate and 30s walk cap below. A recheck is a one-shot and
        // bypasses the throttle — it would otherwise be swallowed for the whole
        // window, which is exactly when an engineer swaps hardware.
        if (! $recheck && $device->identity_attempted_at
            && $device->identity_attempted_at->gt(now()->subHours(self::REFRESH_HOURS))) {
            return;
        }
        ...
        if ($device->vendor === 'silverpeak') {
        ...
        } elseif ($device->vendor === 'fortigate') {
            // FortiOS keeps its version in the Fortinet enterprise tree. Without this
            // branch a FortiGate fell through to the Juniper-shaped sysDescr parse,
            // which never matches, so os_version was never read and — worse — never
            // CORRECTED. AZR-FW01 carried HQ-FW's "v7.4.12,build2902,260505" while
            // actually running v7.2.10, and no poll could ever notice: the field only
            // moves when the agent returns something, and nothing ever did.
            ...
            $osVersion = $this->firstNonEmpty(($this->walker)($device, self::OID_FGT_VERSION));
        } else {
            $sysDescr = trim(implode(' ', HealthPoller::parseWalk(($this->walker)($device, self::OID_SYS_DESCR))));
            $model = $this->firstNonEmpty(($this->walker)($device, self::OID_ENT_MODEL)) ?? $this->modelFromSysDescr($sysDescr);
            $serial = $this->firstNonEmpty(($this->walker)($device, self::OID_ENT_SERIAL));
        ...
        // Compare on EVERY pass, not only after a recovery. Reading the serial hourly
        // and then not looking at it would leave the inventory exactly as wrong as
        // before, just more expensively.
        $update += $this->recheckIdentity($device, $model, $serial, $osVersion, $recheck);

        $wasVersion = (string) $device->os_version;

        if ($update !== []) {
            $device->forceFill($update)->save();
        }

        // AFTER the write, never before: the job re-judges this device against the
        // firmware on its record, so dispatching mid-method would have it read the
        // version we were in the middle of replacing.
        //
        // The CVE catalog scan runs daily, so an upgrade used to leave findings
        // standing against the release the box had already left — 48 of them on one
        // firewall, each stamped with a version it no longer ran. The scanner
        // resolves those on its own; it simply had to be asked, and a version change
        // is the moment worth asking.
        if ($wasVersion !== '' && $wasVersion !== (string) $device->os_version) {
            ReassessDeviceVulnerabilities::dispatch($device->id);
        }
    }
     ...
     */
    private function recheckIdentity(Device $device, ?string $model, ?string $serial, ?string $osVersion, bool $force): array
    {
        ...
        $update = $device->identity_recheck_at !== null ? ['identity_recheck_at' => null] : [];

        $swapped = $this->isSet($serial) && $this->isSet($device->serial_number)
        ...
        if ($swapped) {
            ...
            DeviceHardwareChange::record($device, 'serial_number', $device->serial_number, $serial);
        ...
        // A model is the one identity field an operator types by hand, so it is only
        // overwritten when the evidence says the box itself changed: a different
        // serial, or an explicit post-recovery recheck. Taking it on every hourly
        // pass would quietly undo a correction someone made in the UI.
        if ($this->isSet($model) && ($swapped || $force)) {
            DeviceHardwareChange::record($device, 'model', $device->model, $model);
            $update['model'] = $model;
        }

        // OS version is expected to move — a firmware upgrade is a real event and
        // worth a trail entry — so it is taken whenever the agent returns it.
        if ($this->isSet($osVersion)) {
            DeviceHardwareChange::record($device, 'os_version', $device->os_version, $osVersion);
            $update['os_version'] = $osVersion;
        }
