...
class ArpCollector
{
    ...
    /**
     * ipNetToPhysicalState (IP-MIB) — the state `show arp` prints on every line.
     *
     * The old table says an entry EXISTS. This says whether the gateway still stands
     * behind it. On #024 every LAN neighbour reads STALE and only the two WAN gateways
     * read REACHABLE, which is ordinary for a Linux neighbour table — and without it a
     * cached mapping nobody has revalidated was reported as a confirmed host.
     *
     * Indexed ifIndex.addrType.length.a.b.c.d, so 1.4 for IPv4 sits between the ifIndex
     * and the address.
     */
    private const OID_STATE = '.1.3.6.1.2.1.4.35.1.7';

    /** @var array<int, string> IP-MIB ipNetToPhysicalState values. */
    private const STATES = [
        1 => 'reachable', 2 => 'stale', 3 => 'delay', 4 => 'probe',
        5 => 'invalid', 6 => 'unknown', 7 => 'incomplete',
    ];
    ...
    public function resolve(Device $device): int
    {
        ...
        // Optional: an appliance that does not answer IP-MIB leaves every state null,
        // which reads as "not known" rather than as a claim either way.
        try {
            $states = $this->parseStates(($this->walker)($device, self::OID_STATE));
        } catch (Throwable) {
            $states = [];
        }
        foreach ($entries as $ip => $e) {
            $entries[$ip]['state'] = $states[$ip] ?? null;
        }

        // An entry the gateway itself calls invalid or incomplete is a failed
        // resolution, not a host. It is dropped here rather than stored and filtered
        // later, so nothing downstream has to remember to.
        $entries = array_filter($entries, fn ($e) => ! in_array($e['state'], ['invalid', 'incomplete'], true));
        if ($entries === []) {
            return 0;
        }
    ...
    private function persist(Device $device, array $entries): void
    {
        ...
        ArpEntry::upsert(
            $rows,
            ['device_id', 'ip'],
            ['mac', 'if_index', 'interface', 'state', 'site_id', 'last_seen_at', 'updated_at'],
        );
    }

    /**
     * Parse ipNetToPhysicalState into address => state word.
     *
    ...
    private function parseStates(string $output): array
    {
        $out = [];
        foreach (explode("\n", $output) as $line) {
            if (! preg_match('/4\.35\.1\.7\.\d+\.1\.4\.((?:\d+\.){3}\d+)\s*=\s*(?:INTEGER:\s*)?(\d+)/', trim($line), $m)) {
                continue;
            }
            if (filter_var($m[1], FILTER_VALIDATE_IP, FILTER_FLAG_IPV4)) {
                $out[$m[1]] = self::STATES[(int) $m[2]] ?? null;
            }
        }
