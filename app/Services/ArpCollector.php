...
class ArpCollector
{
     ...
     *
     * Scoped to the SD-WAN edges AND the firewalls: both are layer-3 gateways holding
     * ARP for the subnets they route. A FortiGate that serves DHCP + gateways a VLAN
     * (e.g. VLAN 730 for Verkada at HQ) holds the only ARP for those hosts — the SD-WAN
     * edge never sees them, so without the firewall walk they have no IP in the MAC tool.
     * Access switches are still skipped: their ARP tables are mostly empty.
     */
    public function resolveAll(): void
    {
        Device::whereIn('role', ['edgeconnect', 'firewall'])
     ...
     *
     * @param  array<string, string>  $arp  ip => MAC
     */
    private function persist(Device $device, array $arp): void
    {
        $now = now();
        $rows = [];
        foreach ($arp as $ip => $mac) {
            $rows[] = [
                'device_id' => $device->id,
        ...
        \App\Models\ArpEntry::upsert($rows, ['device_id', 'ip'], ['mac', 'site_id', 'last_seen_at', 'updated_at']);
    }
