...
class NoraComposer
{
    public const KINDS = ['open_ticket', 'check_service', 'status', 'escalate', 'close'];
    ...
    public function compose(Circuit $circuit, string $kind, ?string $impact = null, ?string $note = null): array
    {
        ...
        $lines[] = 'Site: '.trim(($circuit->site?->site_number ? "#{$circuit->site->site_number} " : '').($circuit->site?->name ?? ''));
        if ($circuit->site?->address) {
            $lines[] = "Site address: {$circuit->site->address}";
        }
        $lines[] = '';

        if (in_array($kind, ['open_ticket', 'check_service'], true)) {
