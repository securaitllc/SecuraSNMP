...
class AnomalyCorrelation
{
    ...
    /**
     * Detections this far apart are not the same event. Ninety minutes, not ten:
     * the 19 September egress change opened its anomalies from 19:48 to 21:45 as
     * each circuit's sustain window filled at its own pace, and a ten-minute window
     * split one event into fragments too small to recognise.
     */
    ...
    /**
     * What share of a cluster must have moved alike. Deliberately a minority bar:
     * the crowd is evidence, the stragglers are not disqualifying.
     */
    public const MIN_ALIKE_SHARE = 0.55;
    ...
    private function describe(Collection $window): ?array
    {
        ...
        // Live data on 20 September grouped 23 of 34 — 68%, and a 70% bar rejected
        // the whole event over two percentage points. Real deviations arrive ragged:
        // they trickle in over hours, and a handful always move further than the
        // crowd. The floor that matters is MIN_MEMBERS moving alike, not the share
        // that did.
        if ($alike < self::MIN_MEMBERS || $alike / $shifts->count() < self::MIN_ALIKE_SHARE) {
            return null;
        }
