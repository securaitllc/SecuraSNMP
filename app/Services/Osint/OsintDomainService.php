...
class OsintDomainService
{
    ...
    private function subdomains(string $base): array
    {
        ...
        if (empty($enum['url']) || empty($enumToken)) {
            ...
            try {
                // Bounded so the heavy sidecar can never stall an interactive lookup past
                // the UI's 60s abort: the sidecar caps each tool at 15s, we cap the call at
                // 20s. On timeout the lookup still returns fast with the other sources.
