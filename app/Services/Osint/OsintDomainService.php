...
class OsintDomainService
{
    ...
    /** True only with the default (production) runner — the concurrent subfinder process
     *  runs live tools, which tests (custom fake runner) must not spawn. */
    private bool $liveTools;

    public function __construct(?callable $runner = null)
    {
        $this->liveTools = $runner === null;
    ...
    private function subdomains(string $base, ?string $vtKey): array
    {
        ...
        // subfinder (free CT/passive aggregation — NO VirusTotal quota) runs CONCURRENTLY
        // with the HTTP pool: start it non-blocking here, read it after the pool, so its
        // ~15s overlaps the pool and costs ~no extra wall-time. Skipped under a fake runner
        // (tests) so no real tool spawns.
        $subfinder = null;
        if ($this->liveTools) {
            try {
                $subfinder = new Process(['subfinder', '-silent', '-d', $base, '-timeout', '8']);
                $subfinder->setTimeout(15);
                $subfinder->start();
            } catch (\Throwable $e) {
                $subfinder = null;
            }
        ...
        $responses = Http::pool(function ($pool) use ($base, $vtKey, $enum, $enumToken, $enumOn) {
            ...
            if ($enumOn) {
                // No VT key forwarded — the app already queries VirusTotal directly, so
                // passing it here would DOUBLE the VT quota per lookup. enum's subfinder
                // runs on free sources.
                $reqs[] = $pool->as('enum')->timeout(20)->withHeaders(['x-enum-token' => (string) $enumToken])
                    ->post(rtrim($enum['url'], '/').'/enum', ['domain' => $base]);
            }
        ...
        // Collect the concurrent subfinder — it ran alongside the pool, so it's usually done.
        if ($subfinder) {
            try {
                $subfinder->wait();
                foreach (array_filter(array_map('trim', explode("\n", $subfinder->getOutput()))) as $name) {
                    $add($name, 'subfinder');
                }
                $this->subSources['subfinder'] = 'ok';
            } catch (\Throwable $e) {
                $this->subSources['subfinder'] = 'unavailable';
            }
        }
