...
class AlertNotifier
{
    ...
    /**
     * Queue an alert notification so delivery happens OFF the polling loop (a slow
     * SMTP/webhook must never block a sweep — see DeliverAlertNotification). In tests
     * the queue is sync, so this still runs inline.
     *
    ...
    public static function dispatch(
        ...
        DeliverAlertNotification::dispatch(
            $event, $severity, $subject, $body, $context, $deviceId, $siteId,
        );
    }

    /**
     * The actual fan-out (maintenance suppression + per-channel delivery). Runs in the
     * queue worker via DeliverAlertNotification; kept public so the job can call it.
     *
     * Every attempt is recorded and its error swallowed — delivery must never break the
     * caller. The per-channel log rows are RETURNED so the caller (the queue job) can
     * still see that nobody was reached and decide to retry; without that the job's
     * $tries would be decoration, because handle() would never fail.
     *
     * @param  'open'|'resolved'  $event
     * @param  'info'|'warning'|'critical'  $severity
     * @param  array<string, mixed>  $context
     * @return list<NotificationLog> one row per channel attempted; empty when the event
     *                               was suppressed, no channel matched, or the fan-out
     *                               itself blew up (nothing to retry in any of those).
     */
    public static function deliverNow(
        string $event,
        string $severity,
        string $subject,
        string $body,
        ...
        try {
            ...
            foreach ($channels as $channel) {
                $logs[] = self::deliver($channel, $event, $severity, $subject, $body, $context);
            }
    ...
    private static function deliver(
        ...
        try {
            ...
            $status = 'failed';
            $error = self::redact($e->getMessage(), $channel);
        }
    ...
    /**
     * Strip the credential out of a delivery error before it is written down.
     *
     * For Slack and Teams the webhook URL *is* the secret — anyone holding it can
     * post into the channel. An HTTP client failure quotes the full request URL in
     * its message ("cURL error 28: ... for https://hooks.slack.com/services/T…/B…/xxx"),
     * so a single timeout persisted the secret into notification_logs.error, which
     * the notifications API hands back to anyone who can read the log. A failed send
     * should not be a disclosure.
     *
     * Config values are replaced first (exact match, the common case), then any
     * surviving URL is cut back to scheme://host — a redirect or nested URL can name
     * an endpoint we never stored.
     */
    private static function redact(string $message, NotificationChannel $channel): string
    {
        foreach ((array) $channel->config as $value) {
            if (is_string($value) && strlen($value) >= 8 && str_contains($message, $value)) {
        ...
        return (string) preg_replace_callback(
            '#https?://[^\s\'"<>]+#i',
            fn (array $m) => rtrim((string) preg_replace('#^(https?://[^/?\#]+).*$#i', '$1', $m[0]), '/').'/[redacted]',
            $message,
        );
    }
}
