...
class InterfacePoller
{
    ...
    public function poll(Device $device): void
    {
        ...
        foreach ($names as $ifIndex => $name) {
            if (! array_key_exists($ifIndex, $statuses)) {
                // Incomplete SNMP response for this interface (e.g. a truncated
                // walk) — skip it this cycle rather than guessing it's down.
                continue;
            }

            $status = $this->isOperStatusUp($statuses[$ifIndex]) ? 'up' : 'down';
            ...
            $wasUp = $interface->exists ? $interface->status === 'up' : ($status === 'up');
