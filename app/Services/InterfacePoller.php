...
class InterfacePoller
{
    ...
    // ifInMulticastPkts (ifXTable). Collected so a discard can be EXPLAINED rather than
    // assumed: a routed WAN port on a shared bridge discards multicast it has no reason
    // to accept, and on an EdgeConnect the two counters come out identical. Without this
    // beside it, ifInDiscards alone reads as congestion — see InterfaceHealth.
    private const OID_IF_IN_MULTICAST = '.1.3.6.1.2.1.31.1.1.1.2';
    ...
    public function poll(Device $device): void
    {
        ...
        $inMulticast = $this->parseWalk(($this->walker)($device, self::OID_IF_IN_MULTICAST));
        ...
        foreach ($statuses as $ifIndex => $operRaw) {
            ...
            $newInMulticast = array_key_exists($ifIndex, $inMulticast) ? (int) $inMulticast[$ifIndex] : (int) ($interface->in_multicast ?? 0);
            ...
            $inMulticastDelta = $interface->exists ? max(0, $newInMulticast - $interface->in_multicast) : 0;
            ...
            $interface->fill([
                ...
                'in_multicast' => $newInMulticast,
                'in_multicast_delta' => $inMulticastDelta,
