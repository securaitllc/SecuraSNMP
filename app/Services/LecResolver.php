...
class LecResolver
{
    ...
    public function resolve(Circuit $circuit): array
    {
        ...
        // The ISP the circuit is BILLED under is a fourth, human-sourced claim about
        // the carrier. It only ever CORROBORATES — never answers on its own, because
        // on its own it would just echo the record we are trying to verify. And when
        // the biller is the aggregator it is dropped like any other masking carrier:
        // "billed by Lumen" is precisely the thing that hides the last mile.
        $billed = $this->brand($circuit->ispProvider?->name);
        if (in_array($billed, self::MASKING, true)) {
            $billed = null;
        }
        ...
        // Two technical signals naming DIFFERENT carriers is a genuine conflict, not a
        // majority to be broken — it must be looked at before anyone places a call.
        if (count($counts) > 1) {
            $confidence = 'verify';
        } elseif ($counts[$top] >= 2) {
            $confidence = 'high';
        } elseif ($billed === null) {
            $confidence = 'medium';               // one technical signal, nothing to check it against
        } elseif ($billed === $top) {
            $confidence = 'high';                 // one signal, independently corroborated by the biller
        } else {
            $confidence = 'verify';               // the wire and the invoice disagree — a human decides
        }

        return ['ip' => $ip, 'lec' => $top, 'confidence' => $confidence,
            'signals' => $signals + ($billed ? ['billed' => $billed] : []), 'evidence' => $evidence];
    }
