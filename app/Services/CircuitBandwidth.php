...
class CircuitBandwidth
{
    ...
    public function forMany(Collection $circuits): array
    {
        ...
        $labelsByDevice = self::labelIndex($interfaces);
        ...
        foreach ($circuits as $circuit) {
            $out[$circuit->id] = $this->build($circuit, $edgeIdsBySite, $ifaceByDeviceName, $samples, $portByDeviceGateway, $labelsByDevice);
        }
    ...
    public function wanInterfaceIds(): array
    {
        $circuits = Circuit::query()
            ...
            ->get(['id', 'site_id', 'circuit_id', 'wan_interface', 'gateway_ip']);
        if ($circuits->isEmpty()) {
            return [];
        }
        ...
        $ifaces = DeviceInterface::whereIn('device_id', $edges->pluck('id'))->get(['id', 'device_id', 'if_name', 'if_canonical_name']);
        $ifaceByDeviceName = self::nameIndex($ifaces);
        $labelsByDevice = self::labelIndex($ifaces);
        ...
        $ids = [];
        foreach ($circuits as $circuit) {
            [$ifaceId] = $this->resolvePort($circuit, $edgeIdsBySite[$circuit->site_id] ?? [], $ifaceByDeviceName, $portByDeviceGateway, $labelsByDevice);
            if ($ifaceId !== null) {
                $ids[$ifaceId] = true;
            }
        }
    ...
    /**
     * (device_id, lowercased ifDescr label) — the free text an engineer typed on the
     * port. On EdgeConnect and FortiGate that label frequently NAMES the circuit
     * ("Spectrum CID 40.L1XX.009358..CHTR"), which is the only evidence at a site
     * with several appliances that says which box a circuit lands on.
     *
     * @param  Collection<int,DeviceInterface>  $interfaces
     * @return array<int,array<int,string>> device_id => [interface id => label]
     */
    private static function labelIndex(Collection $interfaces): array
    {
        $index = [];
        foreach ($interfaces as $if) {
            $label = strtolower(trim((string) $if->if_name));
            if ($label !== '') {
                $index[$if->device_id][$if->id] = $label;
            }
        }
    ...
    /**
     * Which polled port a circuit actually lands on.
     *
     * A port NAME is not an identifier. Massey's HQ runs five appliances that each
     * have a "wan1", so two different circuits recorded as wan1 both resolved to
     * whichever appliance happened to be first and reported the same throughput —
     * one of those numbers was fiction. Evidence is used strongest-first:
     *
     *   1. the port's own label names the circuit  — exact, and survives a wrong
     *      wan_interface entry (that HQ circuit is really on FGT60E-HQFW00 wan2)
     *   2. the appliance's next-hop table maps the circuit's gateway to a port —
     *      names one appliance and one port, collected rather than typed
     *   3. the recorded port name, but ONLY where a single appliance at the site
     *      has a port of that name
     *
     * Ambiguity is reported, never guessed: a wrong number here sends an engineer
     * to the wrong provider.
     *
     * @return array{0:?int, 1:string, 2:bool, 3:?string} [interface id, port, inferred, why not]
     */
    private function resolvePort(Circuit $circuit, array $edgeIds, array $ifaceByDeviceName, array $portByDeviceGateway, array $labelsByDevice): array
    {
        $wan = strtolower(trim((string) $circuit->wan_interface));
        $gw = strtolower(trim((string) ($circuit->gateway_ip ?: '')));
        $code = strtolower(trim((string) $circuit->circuit_id));
        $portOf = function (int $ifaceId, int $deviceId) use ($ifaceByDeviceName): string {
            foreach ($ifaceByDeviceName[$deviceId] ?? [] as $name => $id) {
                if ($id === $ifaceId && str_starts_with($name, 'wan')) {
                    return $name;
                }
            }
        ...
        // 1. The label names the circuit. Short ids are skipped — a two-character
        //    code would match half the fleet's labels by accident.
        if (strlen($code) >= 6) {
            foreach ($edgeIds as $deviceId) {
                foreach ($labelsByDevice[$deviceId] ?? [] as $ifaceId => $label) {
                    if (str_contains($label, $code)) {
                        return [(int) $ifaceId, $portOf((int) $ifaceId, $deviceId) ?: $wan, true, null];
                    }
                }
            }
        }

        // 2. The recorded port, where it can only mean one thing.
        $named = [];
        if ($wan !== '') {
            foreach ($edgeIds as $deviceId) {
                if (isset($ifaceByDeviceName[$deviceId][$wan])) {
                    $named[$deviceId] = $ifaceByDeviceName[$deviceId][$wan];
                }
            }
        }
        if (count($named) === 1) {
            return [(int) reset($named), $wan, false, null];
        }

        // 3. The gateway names one appliance and one port — the tie-breaker.
        if ($gw !== '') {
            foreach ($edgeIds as $deviceId) {
                $port = $portByDeviceGateway[$deviceId][$gw] ?? null;
                if ($port !== null && isset($ifaceByDeviceName[$deviceId][$port])) {
                    return [(int) $ifaceByDeviceName[$deviceId][$port], $port, $port !== $wan, null];
                }
            }
        }

        if (count($named) > 1) {
            return [null, $wan, false, "port {$wan} exists on ".count($named).' appliances at this site — record the gateway IP so it can tell them apart'];
        }
        if ($wan === '') {
            return [null, '', false, 'no WAN port mapped'];
        }
        ...
        return [null, $wan, false, "port {$wan} not found on the appliance — {$hint}"];
    }
    ...
    private function build(Circuit $circuit, array $edgeIdsBySite, array $ifaceByDeviceName, array $samples, array $portByDeviceGateway = [], array $labelsByDevice = []): array
    {
        $blank = [
            'mapped' => false,
            'contract_known' => (bool) ($circuit->contract_down_mbps || $circuit->contract_up_mbps),
        ...
        [$ifaceId, $wan, $inferred, $why] = $this->resolvePort($circuit, $edgeIds, $ifaceByDeviceName, $portByDeviceGateway, $labelsByDevice);

        if ($ifaceId === null) {
            return array_merge($blank, ['reason' => $why]);
        }
        $edgeIds = $edgeIdsBySite[$circuit->site_id] ?? [];
        [$ifaceId, $wan, $inferred, $why] = $this->resolvePort($circuit, $edgeIds, $ifaceByDeviceName, $portByDeviceGateway, $labelsByDevice);
        ...
        // A contract is the DENOMINATOR, not the measurement. Throughput comes from
        // the port's own octet counters and is just as real without one — bailing
        // out here hid a number already in hand on seven Massey circuits. Only the
        // percentage needs a contract, and pct() already returns null without one.
        $contractKnown = (bool) ($circuit->contract_down_mbps || $circuit->contract_up_mbps);

        return [
            'mapped' => true,
            'contract_known' => $contractKnown,
            'wan_interface' => $wan,
