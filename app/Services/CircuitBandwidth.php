...
class CircuitBandwidth
{
    ...
    public function forMany(Collection $circuits): array
    {
        ...
        foreach ($circuits as $circuit) {
            $out[$circuit->id] = $this->build($circuit, $edgeIdsBySite, $ifaceByDeviceName, $samples);
        }
    ...
    private function build(Circuit $circuit, array $edgeIdsBySite, array $ifaceByDeviceName, array $samples): array
    {
        $blank = [
            'mapped' => false,
        ...
        $ifaceId = null;
        foreach ($edgeIdsBySite[$circuit->site_id] ?? [] as $deviceId) {
