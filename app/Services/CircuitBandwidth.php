...
class CircuitBandwidth
{
    ...
    public function wanInterfaceIds(): array
    {
        ...
        $ifaceByDeviceName = [];
        foreach (DeviceInterface::whereIn('device_id', $edges->pluck('id'))->get(['id', 'device_id', 'if_name']) as $if) {
            $ifaceByDeviceName[$if->device_id][strtolower(trim((string) $if->if_name))] = $if->id;
        }
    ...
    public function history(Circuit $circuit, int $hours = 24): array
    {
        ...
        $iface = DeviceInterface::whereIn('device_id', $edgeIds)
            ->whereRaw('LOWER(if_name) = ?', [$meta['wan_interface']])
            ->first();
