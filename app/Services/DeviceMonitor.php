...
class DeviceMonitor
{
    ...
    private function clearUnreachable(Device $device): void
    {
        ...
        $recovered = DeviceAlarm::where('device_id', $device->id)
        ...
        // A device that was away and is now back may not be the same hardware — an
        // SD-WAN migration or a switch swap looks exactly like this from here. Ask the
        // identity poller to re-read model/serial/OS once, so a replaced unit stops
        // wearing the dead one's serial. Stamped only on a REAL recovery (an alarm was
        // actually open); this method runs on every reachable poll, and stamping it
        // each time would put the whole fleet into a permanent identity re-walk.
        if ($recovered->isNotEmpty()) {
            $device->forceFill(['identity_recheck_at' => now()])->save();
        }
