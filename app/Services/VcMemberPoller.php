...
class VcMemberPoller
{
    ...
    /**
     * A member missing this many days is treated as physically removed (RMA/replaced),
     * not a transient outage, and is reaped so a swapped switch stops inflating the VC
     * count forever. Below this, a missing member stays visible so a genuinely dead
     * member still shows the chassis as degraded.
     */
    private const MISSING_PRUNE_DAYS = 7;
    ...
    public function poll(Device $device): void
    {
        ...
        // Reap members that have been missing past the grace window: a switch pulled
        // and replaced (a new member takes a different slot) would otherwise linger as a
        // stale row and inflate the VC member count forever. Guarded by the empty-walk
        // check above, so we only prune when the chassis actually answered this cycle.
        DeviceMember::where('device_id', $device->id)
            ->whereNotIn('member_id', $seen)
            ->where('status', 'missing')
            ->whereNotNull('absent_since')
            ->where('absent_since', '<', now()->subDays(self::MISSING_PRUNE_DAYS))
            ->delete();
