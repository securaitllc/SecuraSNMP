...
class LldpCollector
{
    ...
    private const OID_REM_CHASSIS_SUBTYPE = '.1.0.8802.1.1.2.1.4.1.1.4'; // lldpRemChassisIdSubtype — says how to read the id above
    ...
    /** How long a disconnected endpoint stays on record before being pruned. */
    private const RETAIN_ABSENT_DAYS = 90;
    ...
    public function discover(Device $device): void
    {
        ...
        // The chassis id is only interpretable alongside its subtype: subtype 4 is a
        // MAC, subtype 5 is an address family byte followed by an address. Guessing
        // from the value's length instead silently mis-reads one class or the other.
        $chassisSubtypes = $this->parse($walk(self::OID_REM_CHASSIS_SUBTYPE), self::OID_REM_CHASSIS_SUBTYPE);
        ...
        foreach ($sysNames as $index => $remoteSys) {
            ...
            $chassisRaw = $chassisIds[$index] ?? null;
            $chassis = $this->decodeChassisId($chassisRaw, $chassisSubtypes[$index] ?? null);
            $identity = $this->parseEndpointIdentity($remoteSys);
            // A neighbour that advertises its address as the chassis id (handsets do)
            // still gives us a usable management address even when it publishes no
            // lldpRemManAddr entry.
            $mgmtAddr = $mgmtAddr ?: $chassis['ip'];

            // Resolved after the address is known, because the address is the most
            // reliable way to tie a neighbour to a device we already monitor.
            $remoteDevice = $this->resolveRemote($remoteSys, $mgmtAddr);

            $attributes = [
                'remote_sysdesc' => $sysDesc,
                'remote_chassis_id' => $chassisRaw,
                'extension' => $identity['extension'],
                'endpoint_model' => $identity['model'],
                ...
                'last_seen_at' => now(),
                // Reported again, so it is no longer absent.
                'absent_since' => null,
            ];

            // Only write the MAC when LLDP actually advertised one. A handset
            // advertises its address instead, and its MAC is filled in from the
            // appliance's ARP table — writing null here would wipe that every sweep.
            if ($chassis['mac'] !== null) {
                $attributes['remote_mac'] = $chassis['mac'];
            }
        ...
        // Neighbors that vanished from this device's table are KEPT, stamped with
        // when they stopped being reported. Deleting them destroyed the answer to
        // "the port is down — what was on it?" at the exact moment it was asked.
        LldpNeighbor::where('device_id', $device->id)
            ->whereNotIn('id', $seen ?: [0])
            ->whereNull('absent_since')
            ->update(['absent_since' => now()]);

        // Bound the growth: a port that has cycled through endpoints for months does
        // not need every one of them. Long-gone rows are past any troubleshooting use.
        LldpNeighbor::where('device_id', $device->id)
            ->whereNotNull('absent_since')
            ->where('absent_since', '<', now()->subDays(self::RETAIN_ABSENT_DAYS))
            ->delete();
    }
    ...
    private function classify(string $sysName, ?string $sysDesc, ?string $capHex, bool $isManaged): string
    {
        ...
        // Mitel MiNet IP phones advertise as "regDN 500100,MINET_6930"; add minet/regdn
     ...
     */
    /** lldpRemChassisIdSubtype values we can actually interpret. */
    private const CHASSIS_SUBTYPE_MAC = 4;

    private const CHASSIS_SUBTYPE_NETWORK_ADDRESS = 5;

    /** IANA address family for IPv4, the first octet of a networkAddress chassis id. */
    private const IANA_FAMILY_IPV4 = 1;

    /**
     * Decode a chassis id into whichever of MAC / IP it actually carries.
     *
     * The subtype is what makes this unambiguous, and observed hardware genuinely
     * differs. Both of these are real:
     *
     *   subtype 4 (macAddress)     "02 00 5E 10 BE 13"  -> MAC 02:00:5E:10:BE:13
     *   subtype 5 (networkAddress) "01 0A 00 02 3B"     -> family 1 (IPv4), 10.0.2.59
     *
     * Deciding by octet count instead would read that second value as a malformed
     * MAC and lose the address entirely — and it is the class of endpoint whose MAC
     * we most want, so the failure would be silent and total.
     *
     * @return array{mac: ?string, ip: ?string}
     */
    private function decodeChassisId(?string $raw, ?string $subtype): array
    {
        $none = ['mac' => null, 'ip' => null];

        if ($raw === null || trim($raw) === '') {
            return $none;
        }

        // Octets arrive space- or colon-separated depending on the agent.
        $octets = preg_split('/[\s:-]+/', trim($raw), -1, PREG_SPLIT_NO_EMPTY) ?: [];
        foreach ($octets as $octet) {
            if (! preg_match('/^[0-9A-Fa-f]{1,2}$/', $octet)) {
                return $none;   // not a hex octet string; nothing to decode
            }
        }

        $subtypeId = $this->intFromSnmpValue($subtype);

        // No subtype (agent omitted it) — fall back to shape, which is safe only
        // because the two lengths do not overlap.
        if ($subtypeId === null) {
            $subtypeId = match (count($octets)) {
                6 => self::CHASSIS_SUBTYPE_MAC,
                5 => self::CHASSIS_SUBTYPE_NETWORK_ADDRESS,
                default => null,
            };
        }

        if ($subtypeId === self::CHASSIS_SUBTYPE_MAC && count($octets) === 6) {
            return ['mac' => $this->normaliseMac($octets), 'ip' => null];
        }

        if ($subtypeId === self::CHASSIS_SUBTYPE_NETWORK_ADDRESS && count($octets) === 5) {
            if (hexdec($octets[0]) !== self::IANA_FAMILY_IPV4) {
                return $none;   // IPv6 or something else; not decoded here
            }

            $ip = implode('.', array_map(fn ($o) => (string) hexdec($o), array_slice($octets, 1)));

            return ['mac' => null, 'ip' => filter_var($ip, FILTER_VALIDATE_IP) ? $ip : null];
        }

        return $none;
    }

    /** @param  array<int, string>  $octets */
    private function normaliseMac(array $octets): string
    {
        return strtoupper(implode(':', array_map(fn ($o) => str_pad($o, 2, '0', STR_PAD_LEFT), $octets)));
    }

    /**
     * SNMP integers arrive either bare ("4") or MIB-resolved ("macAddress(4)"),
     * depending on whether MIBs are loaded on the polling host.
     */
    private function intFromSnmpValue(?string $value): ?int
    {
        if ($value === null) {
            return null;
        }
        ...
        return preg_match('/^\d+$/', trim($value)) ? (int) trim($value) : null;
    }

    /**
     * Pull the telephony extension and model out of an advertised system name.
     *
     * Mitel handsets identify themselves as "regDN <extension>,MINET_<model>" —
     * regDN being the registered directory number, i.e. the extension a user would
     * actually quote. That is the single most useful fact about the endpoint and it
     * was previously only ever rendered as part of a label.
     *
     * MINET is Mitel's MiNet protocol, so the vendor is implied by the prefix; the
     * model is expanded to "Mitel 6920" for display rather than left as MINET_6920.
     *
     * @return array{extension: ?string, model: ?string}
     */
    private function parseEndpointIdentity(?string $sysName): array
    {
        if ($sysName === null || trim($sysName) === '') {
            return ['extension' => null, 'model' => null];
        }

        if (! preg_match('/\bregDN\s+(\d+)\s*(?:,\s*MINET[_-]?(\w+))?/i', $sysName, $m)) {
            return ['extension' => null, 'model' => null];
        }

        return [
            'extension' => $m[1],
            'model' => isset($m[2]) && $m[2] !== '' ? 'Mitel '.$m[2] : null,
        ];
    }

    private function resolveRemote(string $sysName, ?string $mgmtAddr = null): ?Device
    {
        $name = trim($sysName);
        if ($name === '' && $mgmtAddr === null) {
            return null;
        }
        ...
        $exact = Device::whereRaw('LOWER(name) = ?', [strtolower($name)])->first()
        ...
        // The advertised management address, if we monitor something at it. Stronger
        // evidence than any name comparison.
        if ($mgmtAddr !== null && ($byAddr = Device::where('ip_address', $mgmtAddr)->first()) !== null) {
            return $byAddr;
        }

        // An appliance commonly advertises the bare site name while the device is
        // recorded with a role suffix (AL0001-SC208 vs AL0001-SC208_SDW). Accept that
        // only when exactly ONE device matches — a guess between two is worse than no
        // link, because the link is what an operator clicks through to.
        if ($short !== '') {
            $candidates = Device::where('name', 'like', $short.'_%')
