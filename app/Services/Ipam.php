...
class Ipam
{
    ...
    public function detail(string $cidr, ?int $siteId = null): array
    {
        ...
        $arp = ArpEntry::query()
            ...
            ->with('device:id,name')
            ->get(['ip', 'mac', 'site_id', 'device_id', 'interface', 'state', 'first_seen_at', 'last_seen_at'])
        ...
        // Every sighting of these MACs, newest first, so the freshest wins the keyBy
        // below rather than whichever row the database happened to return.
        $allSightings = MacAddress::with(['device:id,name,site_id', 'deviceInterface:id,if_name,status'])
            ->whereIn('mac', $macList)
            ->orderByDesc('last_seen_at')
            ->get();

        // A MAC ROAMS. 80:45:DD:BB:26:7D has been learned at six sites on MASSEY-WIFI —
        // #114, #087, #033, #001, #118, #052 — and an unscoped lookup named whichever
        // one came back first, so #024's page reported a host on FL0078-SC114SWA001
        // ge-0/0/46: a switch two hundred miles away, presented as the port it plugs
        // into. Only a sighting at THIS site can be this site's port.
        // unique() before keyBy(): keyBy keeps the LAST row for a key, which off a
        // newest-first ordering is the OLDEST sighting — an endpoint that moved desks
        // would have been reported at the port it left.
        $vendors = ($siteId
            ? $allSightings->filter(fn ($m) => optional($m->device)->site_id === $siteId)
            : $allSightings)->unique('mac')->keyBy('mac');

        // Where else the same MAC has been seen. Reported rather than dropped: a
        // laptop that was on this LAN and is now at another branch is worth knowing
        // about, and it is an answer, not a port.
        $elsewhere = [];
        if ($siteId) {
            foreach ($allSightings as $m) {
                if (optional($m->device)->site_id !== $siteId && $m->device !== null) {
                    $elsewhere[$m->mac] ??= ['site_id' => $m->device->site_id, 'device' => $m->device->name,
                        'port' => optional($m->deviceInterface)->if_name, 'last_seen_at' => $m->last_seen_at];
                }
            }
        }

        // What the endpoint says about itself over LLDP: hostname, and for a Mitel
        // handset the extension and model it registers with. Scoped for the same
        // reason — a neighbour advertised at another site describes that site.
        ...
        foreach ($byIp as $ip => $entries) {
            ...
            // POSITIVE evidence it left: a later walk of that switch did not return the
            // MAC at all. That is a different claim from "we have not confirmed it
            // lately", and the two were rendering identically — the switch at #024 has
            // no record of 18:4A:53:CE:49:E4 and the page still printed ge-0/0/41.
            $portAbsentSince = $fdb?->absent_since;
            ...
            $rows[] = [
                ...
                // What the gateway itself says about the entry. Almost everything on a
                // Linux neighbour table reads `stale` — a cached mapping nobody has
                // revalidated — and calling that "confirmed" is the whole complaint.
                'neighbour_state' => $entries->pluck('state')->filter()->unique()->values()->all(),
                'confirmed_now' => $entries->contains(fn ($e) => $e->state === 'reachable'),
                ...
                // When the forwarding table stopped listing it. Null means it is still
                // listed, or that nothing has polled since.
                'switch_absent_since' => $portAbsentSince,
                'switch_port_down' => $portDown,
                // Sightings of this MAC at OTHER sites. Never the switch/port columns
                // above — those are a claim about where it is now, at this site.
                'seen_elsewhere' => $elsewhere[$macs->first()] ?? null,
                // What it says it is.
    ...
    public function space(string $supernet = self::SUPERNET): array
    {
        ...
        // Blocks an operator has claimed for a site that does not exist yet. These hold
        // no addresses by definition, so occupancy can never reveal them — without this
        // the planner would keep offering space that is already spoken for.
        $held = $this->heldBlocks($prefix);
        ...
        for ($i = 0; $i <= 255; $i++) {
            ...
            $blocks[] = [
                ...
                // A held block that turns out to have live hosts in it is NOT quietly
                // shown as held. Either the hold was written over occupied space or
                // somebody deployed into a block that was spoken for; both are worth
                // someone's attention, and silence here is how the collision ships.
                'state' => match (true) {
                    $reserved => 'reserved',
                    $hold !== null && $occupied => 'conflict',
                    $hold !== null => 'planned',
                    $occupied => $n / 254 >= 0.7 ? 'busy' : 'used',
                    default => 'free',
                },
                'reservation' => $hold === null ? null : [
                    'id' => $hold->id,
                    'cidr' => $hold->cidr,
                    'label' => $hold->label,
                    'holder' => $hold->holder,
                    'planned_for' => optional($hold->planned_for)->toDateString(),
                    'overdue' => $hold->overdue,
                ],
            ];
        }
        ...
        return [
            ...
            'summary' => [
                ...
                // Held for a site not yet built, and held over something already live.
                'planned' => count(array_filter($blocks, fn ($b) => $b['state'] === 'planned')),
                'conflict' => count(array_filter($blocks, fn ($b) => $b['state'] === 'conflict')),
    ...
    /**
     * Which /24s of a supernet are held for sites that have not been built.
     *
     * A hold is stored as a block, not an address, so it has to be expanded to the
     * octets it covers: a /24 is one, a /23 is two, a /22 is four. Anything longer
     * than a /24 still claims the whole /24 it sits in — consistent with how a single
     * recorded address already occupies its block, and the safe direction to err,
     * since the cost of over-claiming is one unused /24 and the cost of under-claiming
     * is two sites addressed the same.
     *
     * @return array<int, SubnetReservation> keyed by third octet
     */
    private function heldBlocks(string $prefix): array
    {
        $out = [];

        foreach (SubnetReservation::held()->with('site:id,name')->get() as $hold) {
            $net = self::parseCidr($hold->cidr);
            if ($net === null || ! str_starts_with($hold->cidr, $prefix)) {
                continue;
            }

            $start = self::thirdOctet($net['base'], $prefix);
            if ($start === null) {
                continue;
            }

            // How many /24s this prefix spans. A /25 and longer sits inside one.
            $span = $net['prefix'] >= 24 ? 1 : 1 << (24 - $net['prefix']);

            for ($o = $start; $o < $start + $span && $o <= 255; $o++) {
                // First hold wins, so an overlap cannot silently rewrite an older
                // claim. The controller refuses overlaps outright; this is the
                // belt-and-braces for rows that predate it.
                $out[$o] ??= $hold;
            }
        }
    ...
    /**
     * Circuit-type labels the newer EdgeConnect appliances put on a WAN port.
     *
     * Those appliances label an uplink by its circuit type — BB for broadband, DIA for
     * dedicated internet access — and the label reaches SNMP as an interface of its own
     * named `,BB` or `,DIA`. On this fleet `,BB` is wan0 and `,DIA` is wan1.
     *
     * The plain wan0/wan1 interfaces are still there and still resolve — 254 of 255
     * circuits map to a WAN port by name — so these are additional, not a truncation of
     * them. Matched as given rather than reconstructed into an interface name this code
     * would only be guessing at.
     */
    private const WAN_LABELS = ['bb', 'dia', 'mpls', 'lte'];
    ...
    public static function isWanInterface(?string $name): bool
    {
        ...
        if (preg_match('/^(wan\d*|internet\d*|ppp\d*|dsl\d*|cable\d*|outside)\b/', $n)) {
            return true;
        }

        // `,BB` on its own, and `wan0,BB` if the full name ever reaches us intact.
        foreach (self::WAN_LABELS as $label) {
            if ($n === ','.$label || str_ends_with($n, ','.$label)) {
                return true;
            }
        }
