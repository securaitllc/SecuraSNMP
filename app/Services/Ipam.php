...
class Ipam
{
    ...
    /**
     * A gateway holds occasional ARP for addresses in ANOTHER site's range — traffic
     * crossing the SD-WAN fabric leaves a trace. One such entry does not make that site
     * an owner of the range: #001 held a single address out of 10.200.14.0/24 and the
     * range was being labelled "#001 North Orlando" when it belongs to #014 Lake Mary,
     * which holds 77 of them plus both its devices.
     *
     * A site owns a range if it has a DEVICE addressed inside it, or if its share of
     * the addresses is a material fraction of the largest holder's. Genuine co-location
     * clears this easily (10.200.2.0/24 splits 61/56); a stray does not.
     */
    private const OWNER_MIN_SHARE = 0.15;

    /** …and never fewer than this many addresses, so tiny ranges behave sanely. */
    private const OWNER_MIN_ADDRESSES = 3;
    ...
    private function lanRanges(array $siteIds): array
    {
        ...
        foreach ($byNet as $cidr => $perSite) {
            // Only a CONTESTED range needs an owner decided. When one site holds it
            // there is nothing to resolve, and applying a floor there would discard a
            // small but perfectly real range.
            if (count($perSite) > 1) {
                $counts = array_map(fn ($d) => count($d['ips'] ?? []), $perSite);
                $top = max($counts ?: [0]);
                $floor = max(self::OWNER_MIN_ADDRESSES, (int) ceil($top * self::OWNER_MIN_SHARE));

                $perSite = array_filter(
                    $perSite,
                    fn ($data, $sid) => isset($deviceOwners[$cidr][$sid]) || count($data['ips'] ?? []) >= $floor,
