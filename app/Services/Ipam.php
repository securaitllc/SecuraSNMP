...
class Ipam
{
    ...
    /**
     * Above this many addresses, one MAC's ARP sightings stop being evidence of hosts.
     *
     * One MAC covering several addresses is normal and worth reporting: a firewall
     * proxy-ARPs for its NAT pools, and at HQ a FortiGate answers for .194, .200 and
     * .213. That MAC resolves to a device we can SEE holding one of those addresses,
     * so it stays exempt however many it covers — the cap is not about the count
     * alone.
     *
     * What the cap catches is an endpoint we cannot place answering for most of a
     * range. SC198 has a single Dell workstation MAC bound to roughly 150 of the 254
     * addresses in 10.200.77.0/24; those read as 150 occupied addresses and drove the
     * range to 80% "filling up" against 53 real endpoints. Whether the cause is ARP
     * spoofing, a NAC responder or a host with proxy_arp left on is a question for the
     * network — but in none of those cases is the sighting evidence that something
     * lives at the address, so it must not be counted as though it were.
     */
    ...
    private function lanRanges(array $siteIds): array
    {
        ...
        $knownIps = Device::whereNotNull('ip_address')->pluck('ip_address')
            ->merge(InterfaceAddress::pluck('ip'))
            ->flip()->all();
        ...
        foreach ($byNet as $cidr => $perSite) {
            ...
            foreach ($perSite as $sid => $data) {
                ...
                // A MAC we cannot place, answering for more of the range than any one
                // host plausibly holds, is not occupancy. SC198 read 80% full on one
                // Dell workstation MAC bound to ~150 addresses; the confirmed endpoint
                // count was 53. Those addresses are reported separately, never as free
                // and never as used, because neither is known to be true.
                $unplaceable = [];
                $byMac = [];
                foreach ($data['mac'] ?? [] as $ip => $mac) {
                    $byMac[$mac][$ip] = true;
                }
                foreach ($byMac as $ips) {
                    if (count($ips) > self::MAX_ADDRESSES_PER_MAC && ! array_intersect_key($ips, $knownIps)) {
                        $unplaceable += $ips;
                    }
                }

                $everSeen = count(array_diff_key($data['ips'] ?? [], $unplaceable));
                $live = count(array_diff_key($data['fresh'] ?? [], $unplaceable));
                $unverified = count($unplaceable);
                $pct = (int) round($live / 254 * 100);
                ...
                if ($unverified > 0) {
                    $notes[] = $unverified.' address(es) answered by one unidentified MAC — not counted';
                }
    ...
    public function detail(string $cidr, ?int $siteId = null): array
    {
        ...
        // MACs whose sightings cannot be taken at face value: too many addresses, and
        // no device we can see holding any of them. See MAX_ADDRESSES_PER_MAC.
        $unplaceable = [];
        foreach ($arp->groupBy('mac') as $mac => $entries) {
            $count = $entries->pluck('ip')->unique()->count();
            if ($count > self::MAX_ADDRESSES_PER_MAC && ($macOwners[$mac] ?? null) === null) {
                $unplaceable[$mac] = $count;
            }
        }
        ...
        return [
            ...
            'summary' => [
                ...
                // Answered, but by a MAC blanketing the range — neither free nor known
                // to be in use, and counted as neither.
                ...
                'live' => count(array_filter(
                    $rows,
                    fn ($r) => ! $r['stale'] && ! in_array($r['state'], ['free', 'unverified'], true),
                )),
            ],
        ];
    }
