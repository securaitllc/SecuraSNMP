    ...
    private function lanRanges(array $siteIds): array
    {
        ...
        foreach ($byNet as $cidr => $perSite) {
            ...
            foreach ($perSite as $sid => $data) {
                ...
                // addresses are empty.
                //
                // Massey's fleet carries duplicate Dell MACs: D4:A2:CD:4E:9B:78 alone
                // answers on 97 addresses across 7 sites and 8 switch ports, which no
                // single NIC can do. Every one of those addresses answered ARP inside
                // the last day, so they are occupied by something real — we simply
                // cannot say what. Excluding them from occupancy read a genuinely full
                // /24 as a fifth full, which is how somebody plans a deployment into a
                // subnet that has no room. They count as used, and they are labelled.
                $unplaceable = [];
                $byMac = [];
                ...
                if ($unverified > 0) {
                    $notes[] = $unverified.' address(es) share one MAC — occupied, identity unverified';
                }
    ...
    public function detail(string $cidr, ?int $siteId = null): array
    {
        ...
        return [
            ...
            'summary' => [
                ...
                // Occupied, but by a MAC that answers across the range and across
                // sites — the address is in use, the host behind it is not identifiable.
