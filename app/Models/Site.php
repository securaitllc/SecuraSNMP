...
class Site extends Model
{
    ...
    /**
     * True when Massey owns the property rather than renting it.
     *
     * NOTE: owning the site does NOT mean there is no lease end — Massey-owned
     * locations still carry one (ground lease, sub-lease, or an agreement that
     * still has to be renewed). Occupancy is recorded for context only; it never
     * gates or clears the lease date.
     */
    ...
    /** Days until the lease ends (negative once lapsed); null when not recorded. */
    public function daysToLeaseEnd(): ?int
    {
        return OpsTime::daysUntil($this->lease_end_date);
    }

    /**
     * Lease lifecycle bucket, driven purely by the date — owned and leased sites
     * alike. Deliberately WIDER than the circuit-contract windows (30/60d): a
     * lease decision — renew, extend, or plan a move — is made seasons ahead, and
     * it has to be made before an ISP contract can be signed against it.
     * none (not recorded yet), expired, ≤90d, ≤180d, ok.
     */
    ...
    /** Leases ending within $days (or already lapsed) — owned sites included. */
    public function scopeLeaseEndingWithin($query, int $days)
    {
        return $query->whereNotNull('lease_end_date')
            ->whereDate('lease_end_date', '<=', OpsTime::now()->addDays($days)->toDateString());
    }
}
