...
class PollNextHops extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->pollForever('nexthops', $interval, function () use ($limit): void {
            ...
            // Each per-device subprocess can only close alerts for an appliance it
            // actually reached. One whose SSH is broken never gets that far — and
            // during an SD-WAN swap that is every appliance being replaced — so its
            // alerts would stay open indefinitely. Sweep them here, once per cycle,
            // where no single device's failure can skip it.
