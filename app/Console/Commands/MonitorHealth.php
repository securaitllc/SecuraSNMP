        ...
        $this->pollForever('health', 300, function () use ($poller, $identityPoller): void {
            Device::whereNotNull('snmp_community')
                ...
                ->each(function (Device $device) use ($poller, $identityPoller): void {
                    ...
                    // Beat per device: this sweep does several blocking SNMP walks
                    // per appliance and legitimately runs far longer than the 900s
                    // stale threshold, so without a progress beat the hang-watchdog
                    // kills it mid-sweep and it never reaches the devices at the end.
                    $this->beat();
                });
        });
    }
