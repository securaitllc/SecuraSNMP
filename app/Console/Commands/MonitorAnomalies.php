...
class MonitorAnomalies extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->pollForever('anomaly', $interval, function () use ($detector, $interval): void {
            ...
            // Baselines describing a path nobody uses any more. A deliberate egress
            // change moved every circuit's latency by ~7 ms and opened 34 anomalies
            // that were all correct and could never resolve: the observed value
            // really is above the old baseline, and always will be.
            try {
                (new StaleBaselineReset)->run();
            } catch (Throwable $e) {
                Log::error('stale-baseline reset failed: '.$e->getMessage());
            }
        });
