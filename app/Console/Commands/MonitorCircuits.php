...
class MonitorCircuits extends Command
{
    ...
    private function pingOne(string $ip): ?array
    {
        ...
        $process = new Process(['ping', '-c', '10', '-i', '0.2', '-W', '2', $ip]);
    ...
    private function pingMany(array $ips): array
    {
        ...
        $fill = function () use (&$queue, &$running) {
            while (count($running) < self::PING_CONCURRENCY && $queue) {
                ...
                // -i 0.2: ten probes in ~2s instead of ~10s. At the default 1s interval
                // a single ping consumed two thirds of its own 15s timeout, so any host
                // load pushed the sweep's pings past it — and a killed ping used to read
                // as a down circuit. Same ten probes, same loss resolution, a fraction of
                // the wall-clock exposure.
                $p = new Process(['ping', '-c', '10', '-i', '0.2', '-W', '2', $ip]);
                $p->setTimeout(15);
                try {
                    $p->start();
                    $running[$ip] = $p;
