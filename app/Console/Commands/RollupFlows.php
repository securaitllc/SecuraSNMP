...
class RollupFlows extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Every 2 min (was 10): re-folds the current partial hour frequently so the Flows
        // page's rollup-backed view stays near-live with only a ~2-min raw tail. Cheap —
        // runs on the dedicated flows container, isolated from the SNMP pollers.
        $interval = max(60, (int) env('FLOW_ROLLUP_SECONDS', 120));
        $this->info("Flow rollup started (every {$interval}s).");
        $this->pollForever('flow-rollup', $interval, fn () => $this->pass());
    ...
    public function pass(): void
    {
        ...
        // Fold the current PARTIAL hour too (h=0), not just completed hours, so the
        // rollups stay near-live — the Flows page serves wide windows from rollups plus a
        // tiny raw tail, and folding hour 0 keeps that tail to ~one rollup interval.
        // Idempotent upsert, so re-folding the growing current hour just overwrites it.
        for ($h = 0; $h <= 3; $h++) {
            $this->rollupHour($now->subHours($h)->startOfHour());
        }
        // Fold yesterday into a daily rollup — yesterday on the OPERATOR's clock.
        // A UTC day cut Massey's "yesterday" at 8pm Eastern, so every daily flow
        // figure was four hours of the wrong evening.
        $this->rollupDay(OpsTime::startOfLocalDay(OpsTime::now()->subDay()));
        $this->prune($now);
    }
    ...
    {
        // Raw-flow retention: default 7 DAYS (was 48h). Legacy FLOW_RAW_HOURS still wins if
        // set. Delete in BATCHES — a single DELETE of millions of rows locks/bloats the
        // undo log; chunked deletes keep it online while flows keep ingesting.
        $rawCutoff = getenv('FLOW_RAW_HOURS') !== false
            ? $now->subHours((int) env('FLOW_RAW_HOURS'))
            : $now->subDays((int) env('FLOW_RAW_DAYS', 7));
        do {
            $deleted = FlowRecord::where('recorded_at', '<', $rawCutoff)->limit(10000)->delete();
        } while ($deleted >= 10000);
