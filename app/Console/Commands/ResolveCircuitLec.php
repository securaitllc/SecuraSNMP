    ...
    public function handle(LecResolver $resolver): int
    {
        ...
        foreach ($circuits as $c) {
            ...
            if ($lec !== null && in_array($conf, $writable, true)) {
                ...
                if ($existing !== '' && $existing !== $lec && ! $force) {
                    // Never swallow this. A hand-typed LEC that the evidence contradicts
                    // is worse than a blank one: somebody will phone that carrier about a
                    // modem they do not own. Surface it and let a human settle it.
                    $action = "CONFLICT: recorded '{$existing}'";
                    $conflicts[] = [$c->circuit_id, $existing, $lec, $r['ip'], implode(',', $r['signals'])];
        ...
        if ($conflicts !== []) {
            $this->newLine();
            $this->error('The evidence CONTRADICTS a LEC that was entered by hand — these were left untouched:');
            $this->table(['Circuit', 'Recorded', 'Evidence says', 'IP', 'Signals'], $conflicts);
            $this->warn('Settle each of these before calling a carrier; --force would overwrite the recorded value.');
        }
        if ($needsReview > 0) {
