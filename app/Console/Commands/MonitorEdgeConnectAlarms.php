        ...
        $this->pollForever('ec-alarms', $interval, function () use ($poller): void {
            Device::where('vendor', 'silverpeak')
                ...
                ->each(function (Device $device) use ($poller): void {
                    ...
                    // Beat after EACH appliance. This is the SOLE alarm source and it
                    // sweeps the whole Silver Peak fleet sequentially (7 snmpwalks each).
                    // A few SNMP-degraded appliances (98% mem drops responses — normal on
                    // this gear) used to push the sweep past the supervisor's stale-
                    // heartbeat kill line; it was killed and restarted FROM THE TOP,
                    // re-hitting the same slow devices first, so appliances late in the
                    // ordering never had their alarms raised OR cleared. Beating per device
                    // keeps a slow-but-progressing sweep alive so every appliance is
                    // reconciled each pass (each walk is hard-capped at 30s ≪ the kill line).
                    $this->beat();
                });

            // The alarms just reconciled name their remote ends. If any names an
            // appliance we don't know, one was renamed on the console — queue it for
            // an immediate hostname re-read instead of waiting out the 6-hour
            // refresh, during which its tunnel symptoms cannot be attributed to it.
            try {
                (new HostnameRecheck)->run();
            } catch (\Throwable $e) {
                Log::error('Hostname recheck failed: '.$e->getMessage());
            }
        });
    }
