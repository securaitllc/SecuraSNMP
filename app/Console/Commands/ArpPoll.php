 ...
 *
 * One bounded snmpwalk per gateway per cycle; scoped to edgeconnect + firewall devices
 * (the L3 gateways that hold ARP for the subnets they route — a FortiGate serving DHCP for
 * a VLAN, e.g. VLAN 730 Verkada at HQ, is the only place those hosts' ARP lives).
 */
class ArpPoll extends Command
{
    ...
    protected $description = 'Polls SD-WAN edge + firewall ARP tables (IP↔MAC) so the MAC tool resolves endpoints by IP and site.';
    ...
    public function handle(): int
    {
        ...
        if ($this->option('once')) {
            $devices = Device::whereIn('role', ['edgeconnect', 'firewall'])->whereNotNull('snmp_version')->get();
            ...
            foreach ($devices as $device) {
                try {
                    ...
                    $errors++;
                    $this->error("  {$device->name} ({$device->ip_address}): {$e->getMessage()}");
                }
            }
            $after = \App\Models\ArpEntry::count();
            $this->info("Swept {$devices->count()} gateway(s) [edges + firewalls], {$errors} error(s). arp_entries: {$before} → {$after}.");
