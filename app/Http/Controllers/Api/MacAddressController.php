...
class MacAddressController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $rows = MacAddress::query()
            ...
            ->limit(self::CAP + 1) // +1 to detect truncation
            ->get()
            // Freshest-first, so keeping the first row per (device, mac) drops any stale
            // VLAN-label forks left behind before the poller collapsed them (MacPoller).
            ->unique(fn (MacAddress $m) => $m->device_id.'|'.$m->mac)
            ->values();
