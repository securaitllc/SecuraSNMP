...
class NoraController extends Controller
{
    ...
    public function send(Request $request, Circuit $circuit, NoraComposer $composer): JsonResponse
    {
        ...
        $messageId = '<nodus-'.Str::uuid().'@'.(parse_url((string) config('app.url'), PHP_URL_HOST) ?: 'nodus.local').'>';

        Mail::to(config('nora.address'))->send(new NoraRequestMail($subject, $body, $messageId));
