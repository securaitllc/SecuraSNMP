...
class SearchController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        // actually quotes on a bridge call, and legacy_ticket_number is matched too
        // so a number issued before the sequential scheme still resolves — someone
        // may be holding it on a dispatch note.
        $alarms = DeviceAlarm::with('device')
            ->where(fn ($q) => $q->where('alarm_id', 'like', $like)
                ->orWhere('ticket_number', 'like', $like)
                ->orWhere('legacy_ticket_number', 'like', $like))
