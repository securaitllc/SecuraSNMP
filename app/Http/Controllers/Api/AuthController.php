...
class AuthController extends Controller
{
    ...
    /** A valid current TOTP, or an unused recovery code (which is then consumed). */
    private function twoFactorPasses(User $user, string $code): bool
    {
        if (\App\Support\TwoFactor::verify((string) $user->two_factor_secret, $code)) {
            return true;
        }
        ...
        foreach ($codes as $i => $stored) {
            if (hash_equals(strtoupper($stored), $target)) {
                unset($codes[$i]);
    ...
    private function publicUser(User $user): array
    {
        return [
            ...
            'mfa_enforced' => (bool) config('mfa.enforced'),
        ];
    }
}
