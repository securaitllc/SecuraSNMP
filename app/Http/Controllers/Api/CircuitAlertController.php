...
class CircuitAlertController extends Controller
{
    ...
    public function dispatch(Request $request, Circuit $circuit): JsonResponse
    {
        ...
        // Keep the circuit-level copy in step — it is what the circuits page, the
        // dashboard and the wallboard read, and it outlives this outage.
        $circuit->update([
            'dispatch_at' => $at,
            'dispatch_end_at' => $end,
            'dispatch_note' => $at ? ($data['note'] ?: null) : null,
        ]);
