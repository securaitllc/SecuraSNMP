...
class TopologyController extends Controller
{
    ...
    private function buildSite(Site $site): array
    {
        ...
        // Busiest-link utilisation per device-backed node, so the click-to-trace
        // path can show a per-hop load bar. One grouped query — no N+1. CASE-max
        // (not GREATEST) so SQLite dev and MySQL prod agree.
        $deviceIds = collect($nodes)->pluck('device_id')->filter()->unique()->values();
        if ($deviceIds->isNotEmpty()) {
            $util = \App\Models\DeviceInterface::whereIn('device_id', $deviceIds)
                ->where('status', 'up')
                ->selectRaw('device_id, MAX(CASE WHEN in_util_pct > out_util_pct THEN in_util_pct ELSE out_util_pct END) as u')
                ->groupBy('device_id')
                ->pluck('u', 'device_id');
            foreach ($nodes as &$n) {
                if (! empty($n['device_id']) && $util->get($n['device_id']) !== null) {
                    $n['util'] = (int) round((float) $util[$n['device_id']]);
                }
            }
            unset($n);
        }
