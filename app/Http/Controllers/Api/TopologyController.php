...
class TopologyController extends Controller
{
    ...
    private function buildSite(Site $site): array
    {
        ...
        $edgeIdsForUplink = $edgeDevices->pluck('id')->all();
        $hasResolvedUplink = fn ($sid) => isset($switchUplinkPort[$sid])
            || $swUnitById[$sid]->contains(fn ($m) => $m->lldpNeighbors->contains(fn ($nb) => in_array($nb->remote_device_id, $edgeIdsForUplink, true)));
        $isOrphanComp = fn (array $comp) => count($comp) === 1 && ! $hasResolvedUplink($comp[0]);
