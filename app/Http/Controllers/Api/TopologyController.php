...
class TopologyController extends Controller
{
    ...
    public function organization(): JsonResponse
    {
        ...
        return response()->json(Cache::remember('topology.organization', 15, function () {
            return ['sites' => $this->buildOrganization()];
        }));
    }

    /** @return Collection<int, array> */
    private function buildOrganization(): Collection
    {
        ...
        return Site::with('hubs:id')->orderBy('name')->get()->map(function (Site $site) use ($inferredHubs) {
            ...
            $incident = $t['incident'];
            $nonUp = collect($t['nodes'])->filter(fn ($n) => $n['status'] !== 'up');
            $degraded = $nonUp->isNotEmpty();
            // Every device that is not up is inside an active maintenance window. The
            // site is NOT healthy — it is being worked on, which is a third answer, and
            // the org grid says so rather than picking red or green.
            $allPlanned = $degraded && $nonUp->every(fn ($n) => ($n['in_maintenance'] ?? false));
            ...
            return [
                ...
                'state' => $allPlanned ? 'maint' : ($incident['active'] ? 'crit' : ($degraded ? 'warn' : 'up')),
    ...
    private function buildSite(Site $site): array
    {
        $site->load([
            ...
            'circuits.latestAlert',
            'sharedCircuits.ispProvider:id,support_phone',
            'sharedCircuits.site:id,name',
            'sharedCircuits.latestAlert',
            'devices.tunnels',
            'devices.tunnels.latestAlert',
        ...
        foreach ($circuits as $c) {
            ...
            // Down, but the NOC already cleared the outage: not an incident any more,
            // and not silently healthy either — the node says so on its face.
            $cCleared = $c->status === 'down' && $this->outageCleared($c);
            $nodes[] = [
                'id' => "isp-{$c->id}",
                'type' => 'cloud',
                'label' => $c->isp_name,
                'sub' => $c->circuit_id.($isShared ? ' · shared' : '').($cCleared ? ' · cleared' : ''),
                'status' => $this->circuitLive($c) ? 'down' : 'up',
                'cleared' => $cCleared ?: null,
                'cleared_note' => $cCleared ? 'Outage cleared by the NOC — the monitor still reads this circuit down.' : null,
                'col' => 0,
                'ip' => $c->monitored_ip,
        ...
        // A drawn link is an INFERENCE, not a measurement — nothing probes the cable
        // between two devices. It may only be called up while BOTH ends are still
        // answering. #045 hovered the LAN link from an appliance that had been
        // unreachable for hours and read "Reachable", because the status was the
        // literal string 'up' written at construction time.
        $unreachableById = $site->devices->mapWithKeys(fn ($d) => [$d->id => $d->isDown()])->all();
        $linkStatus = function (?int ...$deviceIds) use ($unreachableById): string {
            foreach ($deviceIds as $id) {
                if ($id !== null && ($unreachableById[$id] ?? false)) {
                    // Nothing probes a cable: this link is Measurement::INFERRED, so
                    // with an end that will not answer it is unknown, not up.
                    return Measurement::state('up', false);
                }
            }
        ...
        foreach ($edgeDevices as $d) {
            ...
            $sshDownTunnels = $d->tunnels->filter(fn ($t) => $t->status === 'down' && ! $this->outageCleared($t))->count();
            ...
            if ($d->nextHops->isNotEmpty()) {
                foreach ($d->nextHops as $nh) {
                    ...
                    $nodes[] = [
                        ...
                        'device_id' => $d->id,
                        // The appliance port this gateway is reached through. Carried so
                        // the hover card can report THAT link rather than the whole box.
        ...
        foreach ($edgeGroups as $gkey => $members) {
            ...
            foreach ($members as $m) {
                if ($m->id !== $activeId) {
                    $edges[] = ['from' => "ec-{$activeId}", 'to' => "ec-{$m->id}", 'label' => 'HA', 'status' => $linkStatus($activeId, $m->id), 'ha' => true];
                }
            }
        }
        ...
        // A device inside an active maintenance window is being worked on. Its node
        // keeps its real status — the box IS down, and painting it up is the
        // false-healthy failure this map has been fixed for twice — but it carries the
        // reason, so the operator reads "planned" instead of chasing a site outage.
        // Stamped in one pass over every device node (edge, switch, firewall, HA
        // member) rather than in each builder, which is how the surfaces drifted apart
        // in the first place.
        $maintenanceDeviceIds = Maintenance::deviceIds();
        foreach ($nodes as $i => $n) {
            if (! isset($n['device_id'])) {
                continue;
            }
            $covered = array_filter(
                $n['ha_member_ids'] ?? [$n['device_id']],
                fn ($mid) => isset($maintenanceDeviceIds[$mid]),
            );
            $nodes[$i]['in_maintenance'] = $covered !== [];
        }
        ...
        foreach ($site->devices as $d) {
            foreach ($d->lldpNeighbors as $nb) {
                ...
                $edges[] = ['from' => $from, 'to' => $to, 'label' => $label, 'status' => $linkStatus($d->id, $nb->remote_device_id), 'lldp' => true, 'stp_blocked' => $blocked];
        ...
        foreach ($circuits as $c) {
            $cloudId = "isp-{$c->id}";
            $circuitDown = $this->circuitLive($c);
        ...
        if ($edgeDev) {
            ...
            foreach ($components as $comp) {
                ...
                $edges[] = ['from' => "ec-{$edgeDev->id}", 'to' => "sw-{$core}", 'label' => $label, 'status' => $linkStatus($edgeDev->id, $core)];
            }
            ...
            // Where the underlay is already on the drawing (isp -> next-hop ->
            // appliance), that second ec->isp edge is the SAME relationship drawn a
            // second time: it skips the next-hop tier, sweeps back across the whole
            // canvas, and drops its label on top of the other one — #045 rendered
            // "SD-WAN overlaySD-WAN overlay" as two arcs crossing every other link.
            // Mark the path that is already drawn instead of drawing a rival one.
            $edges = self::foldOverlayIntoUnderlay($edges);
        }
        ...
        foreach ($components as $comp) {
            ...
            foreach ($fwUnits as $fwUnit) {
                ...
                $edges[] = ['from' => "sw-{$core}", 'to' => "fw-{$fwId}", 'label' => 'trunk', 'status' => $linkStatus($core, $fwId)];
            }
        }
        ...
        if ($deviceIds->isNotEmpty()) {
            $stats = DeviceInterface::whereIn('device_id', $deviceIds)
                ->selectRaw("device_id,
                ...
                ->get()->keyBy('device_id');
            foreach ($nodes as &$n) {
                // A next-hop is ONE link on the appliance, not the appliance. It carries
                // the device's id so the map can trace it, and that used to be enough to
                // stamp it with the whole box's roll-up: every gateway on #887 showed the
                // same discard total and a "busiest link" belonging to some other port.
                // Its own link stats are attached below.
                if (($n['type'] ?? '') === 'nexthop') {
                    continue;
                }
        ...
        return [
            ...
            // Only positions saved under the CURRENT layout: coordinates from the old
            // left-to-right canvas would drag every node into the wrong place.
            'positions' => TopologyPosition::where('site_id', $site->id)
                ->where('layout', TopologyPosition::LAYOUT)
    ...
    public function savePositions(Request $request, Site $site): JsonResponse
    {
        ...
        foreach ($data['positions'] as $nodeId => $pos) {
            ...
            TopologyPosition::updateOrCreate(
                ['site_id' => $site->id, 'node_id' => (string) $nodeId],
                ['layout' => TopologyPosition::LAYOUT, 'x' => (float) $pos['x'], 'y' => (float) $pos['y']],
            );
        }
    ...
    {
        TopologyPosition::where('site_id', $site->id)->delete();
    ...
    private function healthOf(Device $d): ?array
    {
        ...
        // Health is polled over SNMP. A device that stopped answering keeps the last
        // numbers it gave, and a 9% CPU bar beside "unreachable" reads as a healthy
        // appliance. See Measurement.
        return Measurement::stamp([
            'cpu_pct' => $h->cpu_pct,
            'mem_pct' => $h->mem_pct,
            'temperature_c' => $h->temperature_c,
            'uptime_seconds' => $h->uptime_seconds,
        ], ! $d->isDown(), Measurement::SNMP, $h->updated_at, 'last reading — the device is not answering');
    }
     ...
     *
     * @return array<int, Collection<int, Device>>
     */
    ...
    /**
     * Has this circuit's / tunnel's outage already been dispositioned by the NOC?
     *
     * The monitor's `status` column and the alert row answer two different questions.
     * The column is the last measurement; the alert row is the incident's lifecycle.
     * Clearing an outage from the UI closes the row and never touches the column, and
     * both openers are EDGE-triggered — CircuitMonitor opens on `$wasUp && ! $isUp`,
     * SshVerifier on `$wasUp && down`. So a circuit or tunnel cleared while it was
     * still down keeps a `down` column with no alert behind it and no way to ever open
     * a new one, because the up→down edge never comes round again.
     *
     * Topology read that column alone, so those sites stayed red and "Impacted" on the
     * map while the Alarms page showed nothing open — two views permanently
     * disagreeing, with no row left for the NOC to act on.
     *
     * The column still decides what is DOWN. The alert row decides whether that down is
     * a LIVE incident:
     *   - an OPEN alert   → live incident. The normal case.
     *   - a CLOSED alert  → the NOC dispositioned it. It stops driving the incident and
     *     the site's impacted state — but it is not redrawn as healthy either: the node
     *     carries `cleared`, so the map says "cleared · monitor still reads down"
     *     instead of quietly going green.
     *   - NO alert row    → unknown lifecycle (seeded, imported, or an alert the poller
     *     never opened). Absence of a signal is never a good signal: keep trusting the
     *     column and treat it as live.
     *
     * Next-hops are deliberately NOT covered. NextHopPoller is level-triggered
     * (`status === 'down' && ! $open` opens one), so a cleared next-hop alert reopens on
     * the next sweep by itself — there is no stuck state to fix, and suppressing one
     * would only hide a real fault for the length of a poll.
     *
     * @param  Circuit|Tunnel  $obj  with `latestAlert` eager-loaded
     */
    private function outageCleared($obj): bool
    {
        $latest = $obj->latestAlert;

        return $latest !== null && $latest->ended_at !== null;
    }

    /** A circuit that is down AND still an open incident (see outageCleared). */
    private function circuitLive(Circuit $c): bool
    {
        return $c->status === 'down' && ! $this->outageCleared($c);
    }
    ...
    /**
     * @param  Collection<int, Device>  $unit
     */
    ...
    {
        // The per-hub table is SSH-polled. An appliance that is not answering cannot
        // refresh it, so its rows are last-known values of unknown age — #045 showed
        // "AZURE 5 up · FL0001-HQ 10 up" on a box that had been unreachable for hours,
        // directly under the words "appliance unreachable".
        $measured = ! $d->isDown();

        $hubs = $d->tunnels->whereNotNull('hub')->groupBy('hub')->map(fn ($ts, $hub) => Measurement::stamp([
            'hub' => (string) $hub,
            'total' => $ts->count(),
            'down' => $ts->filter(fn ($t) => $t->status === 'down' && ! $this->outageCleared($t))->count(),
        ], $measured, Measurement::SSH, $ts->max('updated_at'), 'last SSH sweep — the appliance is not answering'))->keyBy('hub')->all();

        foreach ($snmpDownHubs as $hub => $down) {
            $known = $hubs[$hub] ?? ['hub' => (string) $hub, 'total' => $down, 'down' => 0];
            // An SNMP alarm IS a live measurement, so a hub named by one is known
            // even while the SSH table is frozen.
            $hubs[$hub] = Measurement::stamp([
                'hub' => $known['hub'],
                // A hub the SSH table has never seen still has at least the tunnels the
                // alarms name.
                'total' => max($known['total'], $down),
                'down' => max($known['down'], $down),
            ], true, Measurement::SNMP, now());
        }
    ...
    /**
     * Fold each "SD-WAN overlay" edge onto the underlay path already being drawn.
     *
     * The overlay genuinely rides every circuit, and when there is no next-hop node
     * the direct appliance-to-ISP arc is the only way to show it — those are kept.
     * But when isp -> nh -> ec is on the map, the arc adds no information and costs
     * a canvas-crossing curve plus a duplicate label, so the nh -> ec edge carries
     * the overlay flag and the arc is dropped.
     *
     * @param  list<array<string, mixed>>  $edges
     * @return list<array<string, mixed>>
     */
    private static function foldOverlayIntoUnderlay(array $edges): array
    {
        $ispToNh = [];
        $nhToEc = [];
        foreach ($edges as $i => $e) {
            if (str_starts_with((string) $e['from'], 'isp-') && str_starts_with((string) $e['to'], 'nh-')) {
                $ispToNh[$e['from']][] = $e['to'];
            }
            if (str_starts_with((string) $e['from'], 'nh-') && str_starts_with((string) $e['to'], 'ec-')) {
                $nhToEc[$e['from']] = $i;
            }
        }
        ...
        $drop = [];
        foreach ($edges as $i => $e) {
            if (! ($e['overlay'] ?? false) || ! str_starts_with((string) $e['to'], 'isp-')) {
                continue;
            }
            foreach ($ispToNh[$e['to']] ?? [] as $nh) {
                if (isset($nhToEc[$nh])) {
                    // NOT 'overlay' — that flag picks the drawing, an arc that bulges
                    // out past the tier. Setting it here turned every next-hop link
                    // into a sweeping curve with its label flung ~100px clear of the
                    // wire. This edge merely CARRIES the overlay; it is still the
                    // straight underlay run and must be drawn as one.
                    $edges[$nhToEc[$nh]]['carries_overlay'] = true;
                    $drop[$i] = true;
                }
            }
        }

        return array_values(array_diff_key($edges, $drop));
    }
    ...
    private function downTunnelCount(?Device $d): int
    {
        ...
        $sshDown = $d->tunnels->filter(fn ($t) => $t->status === 'down' && ! $this->outageCleared($t))->count();
    ...
    /**
     * Per-LINK stats for the next-hop markers: the load on the appliance port that
     * gateway is reached through.
     *
     * #887 is what this is for. Every next-hop on an appliance was being stamped with
     * the device-wide interface roll-up, so each gateway reported the same discard and
     * error totals, and "busiest link" was whatever the appliance's hottest port
     * happened to be — never the WAN the operator was pointing at. No traffic figure
     * was shown at all, which is the one thing you actually want when a circuit alarms.
     *
     * Matched exactly on the port name, the same way a circuit resolves its WAN
     * (CircuitBandwidth::resolveInterface): `if_name` or the canonical `if_canonical_name`.
     * A guess would be worse than nothing here — attributing another port's load to this
     * gateway is a wrong reading, not a missing one.
     *
     * When the port cannot be found, or has not been polled recently, the payload says
     * so rather than showing zeros. A quiet link and an unmeasured one look identical in
     * a number and must not look identical on screen.
     *
     * @param  array<int, array<string, mixed>>  $nodes
     */
    private function attachLinkStats(array &$nodes): void
    {
        $wanted = [];   // device id => [lowercased port name, ...]
        foreach ($nodes as $n) {
            $name = strtolower(trim((string) ($n['interface'] ?? '')));
            if (($n['type'] ?? '') === 'nexthop' && $name !== '' && ! empty($n['device_id'])) {
                $wanted[$n['device_id']][$name] = true;
            }
        }
        if ($wanted === []) {
            return;
        ...
        $names = collect($wanted)->flatMap(fn ($set) => array_keys($set))->unique()->values()->all();
        $ports = DeviceInterface::whereIn('device_id', array_keys($wanted))
            ->where(fn ($q) => $q->whereIn(DB::raw('LOWER(if_name)'), $names)
                ->orWhereIn(DB::raw('LOWER(if_canonical_name)'), $names))
            ->get(['device_id', 'if_name', 'if_canonical_name', 'status', 'admin_status', 'speed_bps',
                'in_util_pct', 'out_util_pct', 'in_errors_delta', 'out_errors_delta',
                'in_discards_delta', 'out_discards_delta', 'last_polled_at']);

        $byKey = [];
        foreach ($ports as $port) {
            foreach ([$port->if_canonical_name, $port->if_name] as $candidate) {
                $key = $port->device_id.'|'.strtolower(trim((string) $candidate));
                // The canonical name wins: it is the port's own ifName, while if_name may
                // be an operator label that another row also carries.
                $byKey[$key] ??= $port;
            }
        }

        // The interface poller runs every POLL_INTERFACE_SECONDS (default 300). Three
        // missed sweeps and the counters are history, not a reading.
        $fresh = now()->subSeconds(max(900, 3 * (int) config('monitoring.interface_interval', 300)));
        ...
        foreach ($nodes as &$n) {
            ...
            $name = strtolower(trim((string) ($n['interface'] ?? '')));
            $port = $name === '' ? null : ($byKey[$n['device_id'].'|'.$name] ?? null);
            $measured = $port !== null && $port->last_polled_at !== null && $port->last_polled_at->gte($fresh);

            $why = $port === null
                ? ($name === '' ? 'the appliance does not name a port for this gateway' : "no port named {$name} on this appliance")
                : ($port->last_polled_at === null ? 'this port has never been polled' : 'this port has not been polled recently');

            $in = (float) ($port->in_util_pct ?? 0);
            $out = (float) ($port->out_util_pct ?? 0);
            $speed = (int) ($port->speed_bps ?? 0);

            $n['link'] = Measurement::stamp([
                'if_name' => $port?->if_canonical_name ?: ($port?->if_name ?: ($n['interface'] ?? null)),
                'speed_bps' => $measured && $speed > 0 ? $speed : null,
                'in_util_pct' => $measured ? round($in, 1) : null,
                'out_util_pct' => $measured ? round($out, 1) : null,
                // Load in bits/sec, derived from the utilisation the poller recorded
                // against the port speed. Null without a speed — a percentage of an
                // unknown line rate is not a traffic figure.
                'in_bps' => $measured && $speed > 0 ? (int) round($speed * $in / 100) : null,
                'out_bps' => $measured && $speed > 0 ? (int) round($speed * $out / 100) : null,
                'errors' => $measured ? (int) $port->in_errors_delta + (int) $port->out_errors_delta : null,
                'discards' => $measured ? (int) $port->in_discards_delta + (int) $port->out_discards_delta : null,
                'port_status' => $measured ? Measurement::state((string) $port->status, true) : 'unknown',
            ], $measured, Measurement::SNMP, $port?->last_polled_at, $why);
        }
        unset($n);
    }
    ...
    {
        $downCircuit = ($circuits ?? $site->circuits)->first(fn ($c) => $this->circuitLive($c));
        ...
        $downCircuitCount = $allCircuits->filter(fn ($c) => $this->circuitLive($c))->count();
        ...
        $upNextHops = $edgeDev ? $edgeDev->nextHops
            ...
            ->count() : 0;
        // Both signals above are SSH-polled off the appliance, and SSH cannot refresh
        // what will not answer: when the appliance goes unreachable its tunnel and
        // next-hop tables simply FREEZE at their last values, every one of them "up".
        // Site #045 went completely dark with both Lumen circuits down and the
        // topology reported "running on backup WAN, redundancy lost" off those frozen
        // rows — the exact inversion of the rule this app is built on. A device that
        // is not answering confirms nothing.
        $edgeUnreachable = $edgeDev !== null && $edgeDev->isDown();

        // And a tunnel has to ride a circuit. When every circuit we actually monitor
        // at the site is down, there is no path left for one to be up on. Muted
        // circuits are excluded on purpose: a paused circuit's status is not evidence
        // of anything, in either direction.
        $monitoredCircuits = $allCircuits->where('monitoring_enabled', true);
        $everyCircuitDown = $monitoredCircuits->isNotEmpty()
            && $monitoredCircuits->every(fn ($c) => $c->status === 'down');

        $hasWorkingWan = ! $edgeUnreachable
            && ! $everyCircuitDown
            && ($upTunnels > 0 || $upNextHops > 0);
        ...
        $layers = [
            ...
            // Both of these are read off the appliance over SSH. If it is not
            // answering they are last-known values of unknown age, so they are
            // reported as UNMEASURED — never as a green tick, and never as a red
            // cross either. We do not know; saying so is the whole point.
            'next_hop' => [
                'state' => Measurement::state($nextHopDown ? 'down' : 'up', ! $edgeUnreachable),
                'source' => Measurement::SSH,
                'label' => $edgeUnreachable
                    ? 'next-hop not measured — appliance unreachable'
                    : ($nextHopDown ? ($downNhIps !== '' ? "next-hop {$downNhIps} unreachable" : 'next-hop unreachable') : 'next-hop reachable'),
            ],
            'tunnels' => [
                'state' => Measurement::state($downTunnels > 0 ? ($upTunnels > 0 ? 'degraded' : 'down') : 'up', ! $edgeUnreachable),
                'source' => Measurement::SSH,
                'label' => $edgeUnreachable
                    ? 'tunnels not measured — appliance unreachable'
                    : ($downTunnels > 0 ? "{$downTunnels}".($tunnelTotal ? "/{$tunnelTotal}" : '').' SD-WAN tunnels down' : 'SD-WAN tunnels up'),
            ],
        ...
        if ($downCircuit && ($nextHopDown || $downTunnels > 0 || ! $hasWorkingWan)) {
            $rootEdge = null;
            if ($edgeDev) {
