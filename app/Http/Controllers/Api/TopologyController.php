...
class TopologyController extends Controller
{
    ...
    private function healthOf(Device $d): ?array
    {
        ...
        return [
            ...
            // Health is polled over SNMP. A device that stopped answering keeps the
            // last numbers it gave, and a 9% CPU bar beside "unreachable" reads as a
            // healthy appliance. Say when the reading stopped being current.
            'measured' => ! $d->isDown(),
            'as_of' => $h->updated_at,
        ];
    }
    ...
    private function rootCause(Site $site, ?Device $edgeDev, array $edges, ?string $firstNhId = null, $circuits = null): array
    {
        ...
        $layers = [
            ...
            'next_hop' => [
                'state' => $edgeUnreachable ? 'unknown' : ($nextHopDown ? 'down' : 'up'),
            ...
            'tunnels' => [
                'state' => $edgeUnreachable ? 'unknown' : ($downTunnels > 0 ? ($upTunnels > 0 ? 'degraded' : 'down') : 'up'),
                'label' => $edgeUnreachable
