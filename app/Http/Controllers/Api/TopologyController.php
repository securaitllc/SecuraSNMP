...
class TopologyController extends Controller
{
    ...
    private function buildSite(Site $site): array
    {
        ...
        foreach ($edgeDevices as $d) {
            ...
            // The appliance itself is unreachable (device-unreachable alarm) — the whole
            // box is DOWN. Its last-polled tunnel/next-hop data is stale, so it must NOT
            // read "up · 15 tunnels up" (that made a completely dead site — #165, both
            // edge and switch unreachable — look connected on the map). An unreachable
            // appliance can't be passing traffic on any tunnel.
            $edgeDown = $d->isDown();
            $nodes[] = [
                'id' => "ec-{$d->id}",
                'type' => 'edge',
