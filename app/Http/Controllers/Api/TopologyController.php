...
class TopologyController extends Controller
{
    ...
    private function buildSite(Site $site): array
    {
        ...
        // A switch that advertises NO LLDP at all (access switches with LLDP disabled
        // or not yet polled) has zero adjacency, so it lands as its own singleton
        // "cluster". Left alone the code below would wire it to BOTH the SD-WAN and the
        // firewall as if it were a core — the false "everything hangs off the edge/FW"
        // links. Detect these orphans so we can instead attach them to the real fabric
        // core with an explicitly-uncertain (dashed) uplink.
        $switchHasLldp = [];
        foreach ($swUnitById as $sid => $unit) {
            $switchHasLldp[$sid] = $unit->contains(fn ($m) => $m->lldpNeighbors->isNotEmpty());
        }
        $isOrphanComp = fn (array $comp) => count($comp) === 1 && empty($switchHasLldp[$comp[0]]);

        // The fabric's real core: the cluster that uplinks to the SD-WAN (router LLDP),
        // else the first non-orphan cluster's core. Orphans attach here, not to the edge.
        $primaryCore = null;
        ...
        foreach ($components as $comp) {
            ...
            // An orphan switch reads as access hanging off the core, not a peer core.
            if ($isOrphanComp($comp) && $comp[0] !== $primaryCore) {
                $accessIds[$comp[0]] = true;
            }
        }
        if ($accessIds) {
            foreach ($nodes as &$nn) {
    ...
    private function lanNode($unit, string $type, int $col): array
    {
        ...
        // A VC with a dead member is degraded (warn) even if the unit answers.
