...
class TopologyController extends Controller
{
    ...
    private function buildSite(Site $site): array
    {
        ...
        foreach ($edgeDevices as $d) {
            ...
            // Treat the appliance's own WAN alarms (gateway/next-hop unreachable or
            // a WAN interface link-down) as next-hop-down, so the WAN failure shows
            // even when the SSH next-hop poller hasn't caught up.
            $wanDownAlarm = $d->alarms->contains(fn ($a) => (bool) preg_match('/:gw:|:wan\d/i', (string) $a->alarm_id));
            $nextHopDown = $d->nextHopAlerts->isNotEmpty() || $wanDownAlarm;
            ...
            $nodes[] = [
                ...
                'tunnels' => $totalTunnels > 0 ? ($downTunnels > 0 ? "{$downTunnels}/{$totalTunnels} tunnels down" : "{$totalTunnels} tunnels up") : null,
    ...
    private function downTunnelCount(?Device $d): int
    {
        ...
        if ($d->tunnels->isNotEmpty()) {
            return $d->tunnels->where('status', 'down')->count();
        }

        return $d->alarms->filter(fn ($a) => str_contains((string) $a->alarm_id, ':Tunnel'))->count();
    }
    ...
    private function rootCause(Site $site, ?Device $edgeDev, array $edges, ?string $firstNhId = null, $circuits = null): array
    {
        ...
        // signal — the SSH next-hop poller can lag or miss it. A gateway/next-hop
        // alarm (ec:<t>:gw:<ip>) or a WAN interface-link-down alarm (ec:<t>:wanN)
        // means the WAN uplink is down, so the topology must call it the root cause
        // instead of blaming the overlay.
        $gwAlarmIp = '';
        if ($edgeDev) {
        ...
        $wanDownAlarm = $edgeDev && ($gwAlarmIp !== '' || $edgeDev->alarms->contains(
            fn ($a) => (bool) preg_match('/:wan\d/i', (string) $a->alarm_id)));

        $nextHopDown = ($edgeDev && $edgeDev->nextHopAlerts->isNotEmpty()) || $wanDownAlarm;
        $downTunnels = $this->downTunnelCount($edgeDev);
        ...
        $upNextHops = $edgeDev ? $edgeDev->nextHops->where('status', '!=', 'down')->count() : 0;
        ...
        if (($downCircuit || $nextHopDown) && $hasWorkingWan && $edgeDev) {
            ...
            return [
                ...
                'action' => 'A WAN link is down. The SD-WAN is still passing traffic on its backup, but there is no redundancy left — restore this link now; open an ISP ticket if the circuit is at fault.',
                'root_edge' => null,
            ];
        }
        ...
        // Circuit up but a WAN next-hop is unreachable → the local WAN link to the
        // cable modem / ONT (interface + cable) is the first thing to check; if that
        // is clean, it's an ISP problem on the modem / fiber, not the appliance.
        if ($nextHopDown && ! $downCircuit && $edgeDev) {
            ...
            return [
                ...
                'action' => 'Check the WAN interface and physical cable to the cable modem / ONT first. If the interface and cable are good, open an ISP ticket for them to remotely check the cable modem / fiber.',
                'root_edge' => null,
            ];
        }
