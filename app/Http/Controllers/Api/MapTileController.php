...
class MapTileController extends Controller
{
    ...
    /**
     * Cache-busting token for the tile URL — the fingerprint of the active key.
     *
     * Tiles are served `immutable` with a 30-day max-age, and the tile path does not
     * otherwise change when a key is added, so browsers would keep serving their
     * stored WATERMARKED tiles for a month after the key was fixed. Putting this in
     * the query string gives every key its own URL space, so a new key is fetched at
     * once while genuinely unchanged tiles stay cached. Not a secret: it is a
     * truncated hash, and the tile route is public anyway.
     */
    public function version(): Response
    {
        return response()->json(['v' => MapSetting::cacheNamespace()]);
    }
