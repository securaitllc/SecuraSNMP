...
class DeviceLldpController extends Controller
{
    ...
    {
        // Silver Peak EdgeConnect does not expose the LLDP-MIB neighbor table over SNMP
        // (an snmpwalk returns nothing on every EdgeConnect) — read it over SSH instead,
        // the only vantage that reports its neighbors.
        if ($device->vendor === 'silverpeak') {
            try {
                if (! $device->ip_address || ! $device->effectiveSshUsername() || ! $device->effectiveSshCredential()) {
                    return response()->json(['error' => 'This EdgeConnect needs an SSH credential to read LLDP (it does not expose LLDP over SNMP). Link an SSH credential first.'], 422);
                }
            } catch (DecryptException $e) {
                Log::error("LLDP refresh — SSH credential decrypt failed for device {$device->id}: {$e->getMessage()}");

                return response()->json(['error' => 'This device\'s SSH credential could not be decrypted. Re-link it, then try again.'], 502);
            }

            try {
                $count = \App\Services\LldpSshCollector::forProduction()->poll($device);
            } catch (\Throwable $e) {
                Log::warning("On-demand SSH LLDP refresh failed for device {$device->id}: ".SshError::safe($e->getMessage()));

                return response()->json(['error' => 'Could not read LLDP over SSH — the appliance may be unreachable or the credential invalid.'], 502);
            }

            return response()->json(['message' => $count > 0 ? "Read {$count} LLDP neighbor(s)." : 'No LLDP neighbors reported by the appliance.']);
        }
