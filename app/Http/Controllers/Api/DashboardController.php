...
class DashboardController extends Controller
{
    ...
    private function activeAlerts(): array
    {
        ...
        $allOpenAlarms = DeviceAlarm::whereNull('cleared_at')->with(['device', 'acknowledgedBy:id,name'])->get();
        ...
        foreach ($tunnelAlarms as $alarm) {
            ...
            $alerts[] = [
                ...
                // Trust the severity the poller stored. Hardcoding critical here
                // overrode a correctly-classified warning and inflated the KPI.
                'severity' => $isQuality ? ($alarm->severity ?: 'warning') : 'critical',
    ...
    private function isQualityAlarm(?string $description): bool
    {
        return (bool) preg_match(
            '/\b(latency|jitter|loss|threshold|exceeds)\b/i',
            (string) $description,
        );
    }
