...
{
    /**
     * How recently a device or circuit must have been measured for its verdict to
     * count. Devices are pinged every 30s and circuits every 60s, so this is many
     * cycles of slack — it catches a stopped poller, not a slow one.
     */
    private const MEASURED_WINDOW_MINUTES = 15;
    ...
    public function insights(): JsonResponse
    {
        ...
        // Deliberately OUTSIDE the cache and on the dashboard rather than only on
        // /api/health/pollers: nothing in the SPA reads that endpoint, so a warning
        // placed there is itself a blind spot. Nodus polled this fleet for weeks with
        // no notification channel enabled — every alarm raised, queued, delivered to
        // nobody — and no screen said so. The dashboard is the screen people watch.
        $payload['alerting'] = self::alertingSnapshot();
    ...
    private function buildDashboard(): array
    {
        ...
        // Resolved once and shared by the counts below: a device being worked on is
        // expected to be down, so its alarms are counted apart from the ones that
        // need somebody's attention.
        $maintenanceDeviceIds = Maintenance::deviceIds();

        // Stamped once, for EVERY alert type — the device alarm, but equally the port
        // down, the tunnel, the next-hop. A switch under a thirty-day window raises all
        // of them, and marking only one kind is how the dashboard ended up disagreeing
        // with itself about the same box.
        foreach ($alerts as $i => $a) {
            $alerts[$i]['in_maintenance'] = isset($maintenanceDeviceIds[$a['device_id'] ?? -1]);
        }
        ...
        // Everything below this line answers "what needs somebody's attention". A
        // planned window is not that, so the actionable set drops those alerts — while
        // the LIST built above keeps them, flagged. Suppressing and marking are not the
        // same thing: the alarm is still visible, it just stops shouting.
        $actionable = array_values(array_filter($alerts, fn (array $a) => ! ($a['in_maintenance'] ?? false)));

        // Devices that actually produced a reading inside the window. Bounded by
        // recorded_at so it reads the index rather than scanning the whole metric
        // history, and counted ONCE here rather than inline in the counts block.
        $measuredDevices = DeviceMetricHistory::where(
            'recorded_at', '>=', now()->subMinutes(self::MEASURED_WINDOW_MINUTES)
        )->distinct('device_id')->count('device_id');

        // Roll active-alert counts up to each alert's owning site. Maintenance is kept
        // apart so a site being worked on does not paint the map red — the Sites page
        // and the map show it as under maintenance instead.
        $alertsBySite = [];
        $maintenanceBySite = [];
        foreach ($alerts as $alert) {
            ...
            if ($alert['in_maintenance'] ?? false) {
                $maintenanceBySite[$alert['site_id']] = ($maintenanceBySite[$alert['site_id']] ?? 0) + 1;
            } else {
        ...
        $sites = Site::query()
            ...
            ->orderBy('name')
            ->get()
            ->map(function (Site $site) use ($alertsBySite, $maintenanceBySite) {
                $activeAlertCount = $alertsBySite[$site->id] ?? 0;
                $maintenanceCount = $maintenanceBySite[$site->id] ?? 0;
                ...
                return [
                    ...
                    // Counted, not hidden: "quiet" and "being worked on" are different
                    // sentences and the map must not say the first when it means the second.
                    'maintenance_alert_count' => $maintenanceCount,
                    'health' => $activeAlertCount > 0
                        ? 'critical'
                        : ($maintenanceCount > 0 ? 'maintenance' : 'good'),
                ];
            })
            ->values();
        ...
        return [
            ...
            'counts' => [
                ...
                // Alarms on a device under an active maintenance window are expected
                // and are counted separately, the way circuits_maintenance already
                // works. Not deleted and not hidden — a maintenance device is NOT
                // healthy — but it does not belong in the number an operator scans
                // to decide whether anything needs attention.
                'active_alarms' => DeviceAlarm::whereNull('cleared_at')->get()
                    ->reject(fn ($a) => $this->isTunnelAlarm((string) $a->alarm_id))
                    ->reject(fn ($a) => isset($maintenanceDeviceIds[$a->device_id]))->count(),
                'alarms_maintenance' => DeviceAlarm::whereNull('cleared_at')->get()
                    ->reject(fn ($a) => $this->isTunnelAlarm((string) $a->alarm_id))
                    ->filter(fn ($a) => isset($maintenanceDeviceIds[$a->device_id]))->count(),
                'devices_maintenance' => count($maintenanceDeviceIds),
                ...
                'active_alerts' => count($actionable),
                // The same signals, on devices under an active window. Shown beside the
                // figure above so a small number reads as "3 are planned", never as
                // "nothing is wrong".
                'alerts_maintenance' => count($alerts) - count($actionable),
                ...
                'active_incidents' => count($this->correlate($actionable)),
                ...
                // "Reachable" and "up" are claims, and a claim needs a measurement
                // behind it. Both of these used to be total-minus-known-broken, so a
                // device nobody had pinged and a circuit nobody had probed counted as
                // healthy — the reassuring denominator stayed at 304 of 304 whether
                // the pollers were running or not. On 15 September the circuit poller
                // was being killed every sweep and the fleet still read all-green
                // until the alarms caught up.
                //
                // Anything not measured inside its window is now counted separately
                // and excluded from the healthy figure, so an unmeasured fleet can
                // never render as an agreeing one.
                // Clamped: a device can be unreachable AND unmeasured at once (its
                // alarm outlives the poll that raised it), and subtracting one set
                // from the other must never produce a negative headline figure.
                'devices_reachable' => max(0, $measuredDevices - DeviceAlarm::where('alarm_id', 'device-unreachable')
                    ->whereNull('cleared_at')->distinct('device_id')->count('device_id')),
                'devices_unmeasured' => max(0, Device::count() - $measuredDevices),
                'circuits_total' => Circuit::count(),
                'circuits_up' => Circuit::where('status', 'up')
                    ->where('last_checked_at', '>=', now()->subMinutes(self::MEASURED_WINDOW_MINUTES))
                    ->where(fn ($q) => $q->whereNull('last_measured_ok')->orWhere('last_measured_ok', true))
                    ->count(),
                'circuits_unmeasured' => Circuit::where('monitoring_enabled', true)
                    ->where(fn ($q) => $q->whereNull('last_checked_at')
                        ->orWhere('last_checked_at', '<', now()->subMinutes(self::MEASURED_WINDOW_MINUTES))
                        ->orWhere('last_measured_ok', false))
                    ->count(),
            ...
            'trends' => $this->trends(),
            // What closed since the operator's day began. Real information for a
            // two-person team handing over — and it is what fills the panel when
            // the active list is short, instead of empty space.
    ...
    private function activeAlerts(): array
    {
        ...
        foreach ($downTunnels as $tunnel) {
            ...
            $alerts[] = [
                ...
                'ticket_number' => null,
                // The alert row behind this entry, so the NOC can actually clear it.
                // Without a ref the dashboard filtered these out of the selectable
                // set entirely ("only alerts backed by a DeviceAlarm can be cleared"),
                // and a tunnel whose peer had migrated away — permanently down, never
                // coming back up on that name — had no route to being silenced at all.
                // The clear endpoint and the cleared_manually column both already
                // existed; nothing ever called them.
                'tunnel_alert_ref' => optional($open)->id,
        ...
        //
        // Gateway alarms among them name a next-hop IP. SSH opens its own NextHopAlert
        // on the same IP, so the one fault used to be listed twice with two different
        // wordings and two different severities (#075 wan0: one critical, one warning).
        // SNMP is authoritative, so the SNMP row is the one that stays — and it borrows
        // the interface name from the next-hop table, which SNMP does not carry.
        $gatewayAlarmIps = GatewayAlarm::ipsByDevice($genericAlarms);

        // stillFailing(), not just "open": an alert whose next-hop now reads up — or
        // whose appliance stopped answering SSH, so nothing ever closed it — is a
        // leftover, not an incident. See NextHopAlert::scopeStillFailing.
        $openNextHops = NextHopAlert::stillFailing()->with(['device', 'nextHop'])->get();

        // The SSH row graded itself critical when NO other next-hop was up — it was the
        // only signal that knew the appliance had lost every uplink. The appliance's own
        // alarm grades one gateway and often calls it minor, so dropping the SSH row
        // without its verdict turns "this site is isolated" into a warning. Carry it.
        $isolatedByGateway = [];
        foreach ($openNextHops as $nh) {
            $ip = optional($nh->nextHop)->ip_address ?? optional($nh->device)->next_hop_ip;
            if (! GatewayAlarm::covers($gatewayAlarmIps, $nh->device_id, $ip)) {
                continue;   // not suppressed — it still speaks for itself below
            }
            $othersUp = DeviceNextHop::where('device_id', $nh->device_id)
                ->when($nh->nextHop, fn ($q) => $q->where('id', '!=', $nh->nextHop->id))
                ->where('status', 'up')->exists();
            if (! $othersUp) {
                $isolatedByGateway[$nh->device_id][strtolower(trim((string) $ip))] = true;
            }
        }
        $gatewaySeverity = function (DeviceAlarm $alarm) use ($isolatedByGateway): string {
            $own = $alarm->severity === 'critical' ? 'critical' : 'warning';
            $ip = GatewayAlarm::gatewayIp((string) $alarm->alarm_id);

            return $ip !== null && isset($isolatedByGateway[$alarm->device_id][$ip]) ? 'critical' : $own;
        };
        $gatewayIface = function (DeviceAlarm $alarm): string {
            $ip = GatewayAlarm::gatewayIp((string) $alarm->alarm_id);
            if ($ip === null) {
                return '';
            }
            $nh = optional($alarm->device)->nextHops?->first(
                fn ($n) => strtolower(trim((string) $n->ip_address)) === $ip,
            );

            return $nh && $nh->interface ? " ({$nh->interface})" : '';
        };
        foreach ($genericAlarms as $alarm) {
            ...
            $alerts[] = array_merge([
                ...
                'detail' => $isDown ? $alarm->description : "{$alarm->description}{$gatewayIface($alarm)} · {$alarm->alarm_id}",
                'severity' => $isDown ? 'critical' : $gatewaySeverity($alarm),
                'started_at' => $alarm->first_seen_at,
                'ticket_number' => $alarm->ticket_number,
                'device_id' => $alarm->device_id,
                'device_name' => optional($alarm->device)->name,
        ...
        foreach ($openNextHops as $nextHop) {
            ...
            // SNMP already reports this exact gateway down on this device. One fault,
            // one row — see App\Support\GatewayAlarm.
            if (GatewayAlarm::covers($gatewayAlarmIps, $nextHop->device_id, $ip)) {
                continue;
            }
    ...
    private function makeIncident(array $members): array
    {
        ...
        return [
            ...
            'type' => 'incident',
            // Every signal here is from one device, so the window either covers the
            // incident or it does not. Carried up because the grouped row is what the
            // operator actually reads — an unflagged headline over flagged members
            // reads as an unexplained critical.
            'in_maintenance' => ! collect($members)->contains(fn ($m) => ! ($m['in_maintenance'] ?? false)),
            'title' => $device,
            'subtitle' => $subtitle,
    ...
    private function makeSiteOutage(array $members): array
    {
        ...
        return [
            ...
            'type' => 'incident',
            'is_site_outage' => true,
            // Only when EVERY device in the outage is covered. One uncovered box at a
            // site being worked on is still a real outage and must keep the red.
            'in_maintenance' => ! collect($members)->contains(fn ($m) => ! ($m['in_maintenance'] ?? false)),
     ...
     */
    /**
     * Everything that cleared today, on the operator's calendar, newest first.
     *
     * Five alert tables, one list. Each row says WHERE, WHAT, how long it was
     * open, and — when a person closed it — who. A silent self-recovery and a
     * cleared-by-name row are different facts, so the difference is kept.
     *
     * @return array{count:int, rows:list<array<string,mixed>>}
     */
    private function resolvedToday(): array
    {
        $since = OpsTime::startOfLocalDay();
        $rows = [];

        foreach (DeviceAlarm::whereNotNull('cleared_at')->where('cleared_at', '>=', $since)
            ->with(['device.site:id,name', 'clearedBy:id,name'])->get() as $a) {
            $rows[] = $this->resolvedRow($a->device?->site?->name, (string) $a->description, 'alarm',
                $a->first_seen_at, $a->cleared_at, $a->clearedBy?->name);
        }
        foreach (CircuitAlert::whereNotNull('ended_at')->where('ended_at', '>=', $since)
            ->with(['circuit.site:id,name', 'clearedBy:id,name'])->get() as $a) {
            $rows[] = $this->resolvedRow($a->circuit?->site?->name,
                'Circuit outage · '.($a->circuit?->isp_name ?? 'ISP'), 'circuit',
                $a->started_at, $a->ended_at, $a->clearedBy?->name);
        }
        foreach (InterfaceAlert::whereNotNull('ended_at')->where('ended_at', '>=', $since)
            ->with(['deviceInterface.device.site:id,name', 'clearedBy:id,name'])->get() as $a) {
            $if = $a->deviceInterface;
            $rows[] = $this->resolvedRow($if?->device?->site?->name,
                'Port '.($if?->if_name ?? '?').' on '.($if?->device?->name ?? '?'), 'interface',
                $a->started_at, $a->ended_at, $a->clearedBy?->name);
        }
        foreach (TunnelAlert::whereNotNull('ended_at')->where('ended_at', '>=', $since)
            ->with(['tunnel.device.site:id,name', 'clearedBy:id,name'])->get() as $a) {
            $rows[] = $this->resolvedRow($a->tunnel?->device?->site?->name,
                'Tunnel '.($a->tunnel?->tunnel_name ?? '?'), 'tunnel',
                $a->started_at, $a->ended_at, $a->clearedBy?->name);
        }
        foreach (NextHopAlert::whereNotNull('ended_at')->where('ended_at', '>=', $since)
            ->with(['device.site:id,name', 'nextHop'])->get() as $a) {
            $rows[] = $this->resolvedRow($a->device?->site?->name,
                'Gateway '.($a->nextHop?->ip_address ?? '?').' unreachable', 'next_hop',
                $a->started_at, $a->ended_at, null);
        }

        usort($rows, fn ($x, $y) => strcmp($y['ended_at'], $x['ended_at']));

        return ['count' => count($rows), 'rows' => array_slice($rows, 0, 8)];
    }

    /** @return array<string,mixed> */
    private function resolvedRow(?string $site, string $title, string $kind, $startedAt, $endedAt, ?string $by): array
    {
        ...
        return [
            ...
            'duration_min' => $started ? max(0, (int) $started->diffInMinutes($ended)) : null,
            'by' => $by,
        ];
    }
     ...
     */
    /**
     * 24-hour WAN-only traffic totals.
     *
     * WAN-only deliberately: summing every polled interface counts LAN ports, trunks
     * and uplinks too, so one packet crossing an access port and then an uplink lands
     * in the total twice — a figure that moves with the fleet but means nothing in
     * bytes. The WAN ports are the fleet's actual edge, resolved the same way
     * per-circuit bandwidth resolves them.
     *
     * No panel renders this today; per-circuit bandwidth on the Circuits page is where
     * throughput actually answers a question ("is this site near its contract?").
     * Kept because the payload has always carried it and the wallboard may want it.
     *
     * @return array<string, mixed>
     */
    ...
    /**
     * Can an alarm actually reach a person right now?
     *
     * "Enabled" is not the same as "usable". A channel with an empty destination
     * throws on every send while still counting as configured, and one set to
     * critical-only leaves every warning silent. Both report ready and neither is —
     * the same false-healthy shape this codebase keeps being bitten by, so the check
     * is what the channel can actually deliver, not what it claims.
     *
     * @return array<string, mixed>
     */
    public static function alertingSnapshot(): array
    {
        $enabled = NotificationChannel::where('enabled', true)->get();

        $usable = $enabled->filter(function (NotificationChannel $c): bool {
            $cfg = $c->config ?? [];

            return match ($c->type) {
                'email' => filter_var($cfg['email'] ?? '', FILTER_VALIDATE_EMAIL) !== false,
                'slack', 'teams' => trim((string) ($cfg['webhook_url'] ?? '')) !== '',
                'webhook' => trim((string) ($cfg['url'] ?? '')) !== '',
                default => false,
            };
        });
        ...
        $warning = null;
        if ($enabled->isEmpty()) {
            $warning = 'No notification channel is enabled. Alarms are raised and recorded, but nobody is told — this dashboard is the only way to find out.';
        } elseif ($usable->isEmpty()) {
            $warning = 'Every enabled notification channel is missing its destination, so no alarm can be delivered.';
        } elseif ($usable->every(fn (NotificationChannel $c) => $c->min_severity === 'critical')) {
            $warning = 'Only critical alarms are delivered. Warnings are recorded but nobody is told.';
        }

        return [
            'channels_enabled' => $enabled->count(),
            'channels_usable' => $usable->count(),
            'can_notify' => $usable->isNotEmpty(),
            'warning' => $warning,
        ];
    }
}
