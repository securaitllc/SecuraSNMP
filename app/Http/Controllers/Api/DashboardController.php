...
class DashboardController extends Controller
{
    ...
    private function buildDashboard(): array
    {
        ...
        $maintenanceDeviceIds = $this->devicesUnderMaintenance();
     ...
     */
    /**
     * Devices covered by an active maintenance window, resolved once per request.
     *
     * Circuits have had this since maintenance was added — they carry a
     * circuits_maintenance count and are excluded from alarming outright. Devices
     * never got the same treatment, so a switch put under maintenance for thirty
     * days went on presenting a red DOWN alarm and counting toward active_alarms
     * exactly as before.
     *
     * A window is scoped to a device, to a site, or to the whole fleet, and all
     * three cover the device.
     *
     * @return array<int, bool>
     */
    private function devicesUnderMaintenance(): array
    {
        ...
        // A window with neither a site nor a device covers the whole fleet.
    ...
    private function activeAlerts(): array
    {
        ...
        $maintenanceDevices = $this->devicesUnderMaintenance();
        ...
        foreach ($genericAlarms as $alarm) {
            ...
            // A device under an active maintenance window is being WORKED ON. It is
            // genuinely not reachable, so this is not downgraded to healthy — but it
            // is expected, and it must not sit in the stream as an unexplained
            // critical or drive the active-alarm count.
            $underMaintenance = isset($maintenanceDevices[$alarm->device_id]);
            $alerts[] = array_merge([
                'in_maintenance' => $underMaintenance,
                'key' => "alarm-{$alarm->id}",
                'alarm_ref' => $alarm->id,
                'type' => 'alarm',
