...
class DashboardController extends Controller
{
    public function index(): JsonResponse
    {
        ...
        return response()->json([
            ...
            'counts' => [
                ...
                // The SNMP :Tunnel rollup is counted as a TUNNEL, not a generic alarm.
                'active_alarms' => DeviceAlarm::whereNull('cleared_at')->where('alarm_id', 'not like', 'ec:%:Tunnel')->count(),
    ...
    private function activeAlerts(): array
    {
        ...
        // SNMP 'ec:…:Tunnel' rollup alarms are TUNNEL-down signals, not generic
        // alarms — surface each as a clickable tunnel entry (SNMP is authoritative
        // when the SSH table is stale). Skip devices that already have an SSH-down
        // tunnel above, so we don't double-count the same appliance.
        $sshTunnelDeviceIds = $downTunnels->pluck('device_id')->filter()->unique()->all();
        $snmpTunnelAlarms = DeviceAlarm::whereNull('cleared_at')
            ->where('alarm_id', 'like', 'ec:%:Tunnel')
            ->whereNotIn('device_id', $sshTunnelDeviceIds ?: [0])
            ->with('device')->get();
        foreach ($snmpTunnelAlarms as $alarm) {
            $alerts[] = [
                'key' => "tunnel-snmp-{$alarm->id}",
                'type' => 'tunnel',
                'title' => optional($alarm->device)->name.' · tunnels down',
                'subtitle' => 'Tunnels down',
                'detail' => 'The appliance reports SD-WAN tunnels down (SNMP); per-tunnel SSH detail is pending.',
                'severity' => 'critical',
        ...
        // Generic Orchestrator alarms — EXCLUDING the :Tunnel rollup (shown above as
        // a tunnel), so it's counted once, in the Tunnels KPI not the Alarms KPI.
        $activeAlarms = DeviceAlarm::whereNull('cleared_at')->where('alarm_id', 'not like', 'ec:%:Tunnel')
            ->with(['device', 'acknowledgedBy:id,name'])->get();
