...
class CircuitController extends Controller
{
    ...
    public function bandwidthLive(Circuit $circuit): JsonResponse
    {
        ...
        // Resolved by either name the port answers to (label or real ifName) — the
        // same resolution the row and the chart use, so all three read one port.
        $iface = (new CircuitBandwidth)->resolveInterface($circuit, (string) $bw['wan_interface']);
        $device = $iface ? Device::find($iface->device_id) : null;
    ...
    public function renew(Request $request, Circuit $circuit): JsonResponse
    {
        ...
        // A term runs from the current end while that end is still ahead, else from
        // TODAY on the operator's clock. Using UTC here dated an evening renewal from
        // tomorrow, and called a contract ending today already expired.
        $stillRunning = $previous && $previous->toDateString() >= OpsTime::today();
        $newEnd = isset($data['new_end_date'])
            ? Carbon::parse($data['new_end_date'])
            : ($stillRunning ? $previous->copy() : Carbon::parse(OpsTime::today()))->addMonths((int) $data['term_months']);
    ...
    {
        $data = $request->validate([
            'isp_ticket' => ['nullable', 'string', 'max:100'],
            // What the operator was looking at when they raised it, and the finding it
            // came from. Both optional — a ticket typed from the circuits page has
            // neither, and that is still worth recording.
            'reason' => ['nullable', 'string', 'max:255'],
            'anomaly_id' => ['nullable', 'integer'],
            'note' => ['nullable', 'string', 'max:2000'],
        ]);

        $number = $data['isp_ticket'] ?: null;
        $who = $request->user()?->name;
        $open = $circuit->ispTickets()->open()->latest('opened_at')->first();

        // The number is one overwritable field on the circuit, which is all the live
        // screens need. The trail beside it is what makes the interaction findable
        // afterwards — see App\Models\CircuitIspTicket.
        if ($open && $open->ticket_number !== $number) {
            $open->update(['closed_at' => now(), 'closed_by' => $who]);
            $open = null;
        }

        if ($number !== null && ! $open) {
            $circuit->ispTickets()->create([
        ...
        } elseif ($open && ($data['note'] ?? null)) {
            // Same ticket, more detail. Recorded rather than dropped.
            $open->update(['note' => $data['note']]);
        }

        $circuit->update(['isp_ticket' => $number]);

        return response()->json([
            'isp_ticket' => $circuit->isp_ticket,
            'history' => $this->ticketHistory($circuit),
        ]);
    }

    /**
     * Probe the path now, hop by hop, and keep the result.
     *
     * Analyst+ and throttled: an mtr is ten cycles of probes per hop against a live
     * carrier path, and the poller already captures one automatically whenever a
     * circuit crosses into loss. This is for "trace it again, now, while I am on the
     * phone with them".
     */
    public function trace(Request $request, Circuit $circuit, CircuitPathProber $prober): JsonResponse
    {
        $probe = $prober->probe($circuit, 'manual', $request->user()?->name);

        if ($probe === null) {
            return response()->json([
                'message' => $circuit->monitored_ip
                    ? 'The probe produced nothing usable — the tool returned no hops.'
                    : 'This circuit has no monitored address to trace toward.',
            ], 422);
        }

        return response()->json(self::withHopVerdicts($probe));
    }

    /** Recorded path probes, newest first. Readable by any role. */
    public function traces(Circuit $circuit): JsonResponse
    {
        return response()->json([
            'data' => $circuit->pathProbes()->limit(20)->get()->map(self::withHopVerdicts(...)),
            'tool' => CircuitPathProber::hasMtr() ? 'mtr' : 'traceroute',
        ]);
    }

    /**
     * Tag each hop as real loss or a router declining to answer about itself.
     *
     * Derived on read rather than stored, so probes captured before this existed are
     * explained too — and so the rule lives in exactly one place. A hop losing probes
     * while everything behind it is clean is rate-limiting its own control plane, not
     * dropping traffic.
     */
    private static function withHopVerdicts(CircuitPathProbe $probe): CircuitPathProbe
    {
        $probe->hops = CircuitPathProber::classifyHops($probe->hops ?? []);

        return $probe;
    }

    /**
     * Loss episodes in a window: when each run of loss began, ended, how many polls
     * were lossy and the worst of them. The answer to "since when?" — and to "has this
     * happened before?", which is the carrier's second question.
     */
    public function lossEpisodes(Request $request, Circuit $circuit): JsonResponse
    {
        $days = max(1, min(30, (int) $request->query('days', 7)));

        return response()->json([
            'episodes' => (new CircuitLossEpisodes)->forCircuit($circuit, $days),
            'degraded_since' => optional($circuit->degraded_since)->toIso8601String(),
        ]);
    }

    /** Every ISP ticket raised on this circuit, newest first. */
    public function ispTicketHistory(Circuit $circuit): JsonResponse
    {
        return response()->json(['data' => $this->ticketHistory($circuit)]);
    }

    /** @return array<int, array<string, mixed>> */
    private function ticketHistory(Circuit $circuit): array
    {
        return $circuit->ispTickets()->latest('opened_at')->get()
            ->map(fn ($t) => [
                'id' => $t->id,
