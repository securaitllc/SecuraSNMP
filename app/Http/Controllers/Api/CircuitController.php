...
class CircuitController extends Controller
{
    ...
    private function transportDegraded($circuits): array
    {
        ...
        $alarms = DeviceAlarm::whereNull('cleared_at')->whereIn('device_id', $deviceSite->keys())
            ->get(['device_id', 'alarm_id', 'description'])
            ->filter(fn (DeviceAlarm $a) => AlarmCircuitResolver::isLocalTransport($a->alarm_id, (string) $a->description));
        if ($alarms->isEmpty()) {
            return [];
        }

        $nextHops = DeviceNextHop::whereIn('device_id', $alarms->pluck('device_id')->unique())->get()->groupBy('device_id');
        ...
        foreach ($alarms as $a) {
            ...
            // Never flag a paused circuit — it is intentionally muted.
            if ($c && $c->monitoring_enabled) {
