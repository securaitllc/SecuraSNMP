...
class OsintController extends Controller
{
    ...
    public function domain(Request $request): JsonResponse
    {
        ...
        // Enrich the host IP inline so the operator sees hosting without a second search.
        // Falls back to a subdomain-resolved IP when the apex has no A (wildcard-only domain).
        $firstIp = $result['dns']['a'][0] ?? $result['hosting_ip'] ?? null;
        if ($firstIp) {
