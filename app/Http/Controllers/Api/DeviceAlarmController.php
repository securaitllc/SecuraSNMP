...
class DeviceAlarmController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // A device being worked on is expected to be alarming. The row stays — it is
        // genuinely unreachable — but it is marked, so the list can show it without
        // the red and the operator does not have to remember which outages were
        // planned. See App\Support\Maintenance.
        $maintenance = Maintenance::deviceIds();
        ...
        return response()->json($alarms->map(fn (DeviceAlarm $a) => [
            ...
            'in_maintenance' => isset($maintenance[$a->device_id]),
        ]));
    }
    ...
    public function log(Request $request): JsonResponse
    {
        ...
        // Same rule as index(), and for the same reason: this is the page an operator
        // opens to decide what still needs doing. An alarm on a device inside an active
        // window belongs here — it is real — but it must carry the reason with it.
        $maintenance = Maintenance::deviceIds();
