...
class SiteController extends Controller
{
    ...
    public function overview(Site $site): JsonResponse
    {
        $site->load([
            ...
            'devices.health',
            // if_name is needed to tell a physical port from its logical unit; the open
            // alert is what decides whether the count may be shown as a problem. Without
            // both eager-loaded this silently mis-counts and N+1s.
        ...
        $devices = $site->devices->map(function ($device) {
            // Down = PHYSICAL ports that are down, excluding intentionally admin-shut
            // and suppressed ones. Logical units (ge-0/0/11.0 beside ge-0/0/11) are the
            // same cable reported twice and used to double this figure everywhere.
            $downIfs = $device->interfaces
                ->where('status', 'down')
                ->where('admin_status', 'up')
                ->where('alarm_suppressed', false)
                ->reject(fn ($i) => NetworkInterface::isLogicalUnit($i->if_name));
            $ifDown = $downIfs->count();

            // What actually ALARMED. A port can be down, unsuppressed and still have
            // raised nothing — newly discovered, or a cleared alert. Presenting the
            // down count where an operator expects alarms is how site #196 showed four
            // for one fault. This is the number that may be drawn as a problem.
            $ifAlerting = $device->interfaces
                ->reject(fn ($i) => NetworkInterface::isLogicalUnit($i->if_name))
                ->filter(fn ($i) => $i->alerts->isNotEmpty())
                ->count();
            ...
            return [
                ...
                'interfaces_alerting' => $ifAlerting,
        ...
        return response()->json([
            'summary' => [
                ...
                'interfaces_alerting' => $devices->sum('interfaces_alerting'),
