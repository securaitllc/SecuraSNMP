...
class ReportController extends Controller
{
    ...
    public function catalog(): JsonResponse
    {
        ...
        foreach (ReportService::TYPES as $key => $label) {
            $reports[] = [
                'type' => $key,
                'label' => $label,
                'time_scoped' => ! in_array($key, ['device-inventory', 'circuit-contracts', 'site-leases', 'sites-nearby', 'duplicate-macs'], true), // snapshots, not windows
                'supports_role' => in_array($key, ['device-availability', 'device-inventory'], true),
                // A mileage band, measured from the selected site. The Site filter stops
                // being "narrow to this site" and becomes "measure from this site".
                'supports_radius' => $key === 'sites-nearby',
    ...
    private function filters(Request $request): array
    {
        return array_filter([
            ...
            'min_miles' => $request->query('min_miles'),
            'max_miles' => $request->query('max_miles'),
            'stops_per_run' => $request->query('stops_per_run'),
            'min_sites' => $request->query('min_sites'),
        ], fn ($v) => $v !== null && $v !== '');
    }
}
