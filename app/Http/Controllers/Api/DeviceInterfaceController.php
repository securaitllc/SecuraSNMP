...
class DeviceInterfaceController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        $interfaces = DeviceInterface::query()
            ...
            ->with(['device:id,vendor', 'alerts' => fn ($q) => $q->whereNull('ended_at')->latest('started_at')])
            ->withCount('macAddresses') // learned-endpoint count for the list badge
            ->orderBy('if_index')
            ->get();
     ...
     */
    private function attachHealth(Collection $interfaces): void
    {
        ...
        $agg = DB::table('interface_metric_history')
            ->selectRaw('device_interface_id,'
                ...
                .' SUM(in_multicast_delta) as multicast_recent,'
        ...
        foreach ($interfaces as $if) {
            ...
            $multicast = (int) ($row->multicast_recent ?? 0);

            $health = InterfaceHealth::classify($if, $errors, $discards, $now, $multicast);

            $if->health = $health['status'];
            $if->health_attention = $health['attention'];
            $if->errors_recent = $errors;
            // What is left after multicast is accounted for — the number the pill is
            // actually based on, so the panel and the verdict never disagree.
            $if->discards_recent = max(0, $discards - $multicast);
            $if->multicast_recent = $multicast;
