...
class CircuitMetricController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        // Cap points (nulls kept) so a densely-polled circuit can't freeze the graph.
        return response()->json(\App\Support\MetricDownsampler::cap($metrics));
    }
    ...
    {
        // Plain rows, not models (see DeviceMetricController::summary) — with the
        // recorded_at-led covering index this is an index-only scan.
        ...
        $byCircuit = [];
        foreach ($rows as $row) {
            $byCircuit[$row->circuit_id][] = $row->response_time_ms === null ? null : (float) $row->response_time_ms;
        }
