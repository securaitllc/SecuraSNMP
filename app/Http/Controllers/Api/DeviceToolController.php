 ...
 * injection.
 *
 * A supplied target is only honoured for tools that carry nothing secret. Anything
 * that speaks SNMP is pinned to the device's own address, because the credentials
 * it sends belong to that device and v2c puts the community on the wire in clear.
 */
class DeviceToolController extends Controller
{
    /**
     * Tools that transmit the device's stored SNMP credentials. Their destination is
     * never caller-controlled — see the guard in run().
     */
    private const CREDENTIALLED_TOOLS = ['snmpwalk', 'snmptest'];
    ...
    public function run(Request $request, Device $device, string $tool): JsonResponse
    {
        ...
        // SNMP carries this device's OWN credentials. The v2c community travels in
        // cleartext in every request, so letting the caller name the destination turns
        // a diagnostic into a credential exfiltration channel: point it at a host you
        // control and you have read a secret the API redacts to bullets everywhere
        // else (DeviceResource never returns it, not even to an admin). Ping and
        // traceroute send nothing secret and stay free-target — that is the point of a
        // looking glass — but anything that speaks SNMP is pinned to the device.
        if (in_array($tool, self::CREDENTIALLED_TOOLS, true) && $target !== $device->ip_address) {
            return response()->json([
                'message' => "SNMP tools use this device's stored credentials, so they can only be aimed at the device itself ({$device->ip_address}).",
            ], 422);
        }
    ...
    private function tdr(Request $request, Device $device, ?string $interface): JsonResponse
    {
        ...
        try {
            ...
            //
            // This runs over ONE SSH session using the exec channel: the old code called
            ...
            // "TDR doesn't work". We also can't use the interactive shell here: Junos'
            // CLI does space-completion on a PTY, which mangles a multi-word op command
            // ("request diagnostics tdr…" → "requestdiagnosticstdr…" → syntax error). The
            // exec channel runs each op command non-interactively — no completion, no
            // paging — and returns its output cleanly in a few seconds.
            ...
            // Launch the test. Keep the start output: if Junos rejects the start (e.g.
            // TDR unsupported on this port, or the link is down), the reason shows here
            // instead of a silent, forever "Not Started".
            $startOut = trim((string) $ssh->exec($start));

            $raw = '';
            for ($attempt = 0; $attempt < 8; $attempt++) {
                sleep(3);
                $raw = trim((string) $ssh->exec($show));
        ...
        // The test never got past "Not Started" — surface the start output and a hint,
        // rather than showing a bare "Not Started" that reads as "TDR is broken".
        $stuck = $raw === '' || preg_match('/Test status\s*:\s*Not Started/i', $raw);
        if ($stuck && $verdict === '') {
            // Junos rejects `request diagnostics tdr start` unless the login class has the
            // `maintenance` permission — the account can read `show diagnostics` but not
            // launch the test. Call that out precisely instead of the copper/link guess.
            if (preg_match('/permission denied|not authorized|unauthorized command/i', $startOut)) {
                $hint = "⚠ The switch refused to start the cable test: the SSH account this device "
                    ."uses lacks Junos permission to run diagnostics. Grant its login class the "
                    ."'maintenance' permission (or allow-commands \"^request diagnostics\") on the "
                    ."switch, then retry. Read access works — only starting the test is blocked.";
            } else {
                $hint = "⚠ The test did not start on {$interface}. TDR runs on built-in COPPER (RJ-45) "
                    ."ports; it won't start on an SFP/fiber port, and some switches won't run it while "
                    ."the link is down. Try a copper port with a cable connected.";
            }
            $detail = trim(($startOut !== '' ? "start: {$startOut}\n\n" : '').$raw);
