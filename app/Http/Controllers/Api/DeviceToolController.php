...
class DeviceToolController extends Controller
{
    ...
    private function tdr(Request $request, Device $device, ?string $interface): JsonResponse
    {
        ...
        try {
            ...
            // launches the test, which then runs ~5-7s on the switch. Reading the result
            // in the same breath (the old [start, show] back-to-back) always caught it
            // "Not Started"/mid-run and showed nothing. So: kick it off, then POLL the
            // result until the test reports a terminal state (Passed/Failed or per-pair
            // results), capped so a stuck test can't hang the request.
            SshSession::run($device, [$start]);

            $raw = '';
            for ($attempt = 0; $attempt < 4; $attempt++) {
                sleep(4);
                $raw = trim(SshSession::run($device, [$show])[$show] ?? '');
