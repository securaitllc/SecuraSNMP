...
class DeviceToolController extends Controller
{
    ...
    private function tdr(Request $request, Device $device, ?string $interface): JsonResponse
    {
        ...
        try {
            // SshSession reads each command up to its read-timeout, which gives the
            // start ~seconds to finish before the show is issued.
            $out = SshSession::run($device, [$start, $show]);
        } catch (Throwable $e) {
            return response()->json([
    ...
    private function tdrVerdict(string $raw): string
    {
        ...
        if (preg_match('/Test status\s*:\s*Passed/i', $raw) || stripos($raw, 'Normal') !== false) {
            return '✓ Cabling looks good (all pairs Normal) — if the port still errors/flaps, suspect the SFP/interface or the far end, not the cable.';
        }
