...
class FlowController extends Controller
{
    ...
    {
        $cut = CarbonImmutable::now()->startOfHour();   // rollups < cut, raw >= cut (no overlap)
    ...
    {
        $cut = CarbonImmutable::now()->startOfHour();
        $topApps = $this->rollupApps($from, $to)->pluck('app')->take(6)->all();

        // hourly rollup points (completed hours)
        ...
        if ($from < $cut) {
            FlowRollup::where('group_type', 'app')->where('bucket', 'hour')
                ...
                ->each(function ($r) use (&$points, $topApps) {
                    $name = in_array($r->app, $topApps, true) ? $r->app : 'Other';
                    $ts = CarbonImmutable::parse($r->bucket_start)->getTimestamp() * 1000;
                    $points[$name][$ts] = ($points[$name][$ts] ?? 0) + (int) $r->bytes;
                });
        }
        // current partial hour from raw, as one final point at $cut
        FlowRecord::where('recorded_at', '>=', max($from, $cut))->where('recorded_at', '<=', $to)
            ->selectRaw('app, SUM(bytes) as bytes')->groupBy('app')->get()
            ->each(function ($r) use (&$points, $topApps, $cut) {
                $name = in_array($r->app, $topApps, true) ? $r->app : 'Other';
                $ts = $cut->getTimestamp() * 1000;
                $points[$name][$ts] = ($points[$name][$ts] ?? 0) + (int) $r->bytes;
            });
        ...
        $series = [];
        foreach ($points as $name => $byTs) {
            $data = array_map(fn ($ms) => [$ms, $byTs[$ms] ?? 0], $grid);
    ...
    private function window(Request $request): array
    {
        ...
        $from = $request->query('from')
            ...
            : $to->subHours(max(1, min(168, (int) $request->query('hours', 6))));  // up to 7d (raw retention)
