...
class FlowController extends Controller
{
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        $talkers = (clone $base)
            ->selectRaw('src_ip, dst_ip, MAX(app) as app, MAX(app_category) as app_category, MAX(protocol) as protocol, MAX(dst_port) as dst_port, MAX(device_id) as device_id, SUM(bytes) as bytes, SUM(packets) as packets, COUNT(*) as flows')
        ...
        $totalBytes = (int) (clone $base)->sum('bytes');
        $flows = (int) (clone $base)->count();
        $conversations = (int) DB::query()->fromSub((clone $base)->select('src_ip', 'dst_ip')->distinct(), 't')->count();
        $topApp = $apps->first();

        return response()->json([
            'summary' => [
                'total_bytes' => $totalBytes, 'flows' => $flows, 'conversations' => $conversations,
                'top_app' => $topApp ? ['app' => $topApp->app, 'bytes' => (int) $topApp->bytes] : null,
            ],
