...
class FlowController extends Controller
{
    ...
    public function overview(Request $request): JsonResponse
    {
        ...
        // hit is expensive and concurrent/refresh loads would pile up on the DB. Key on a
        // COARSE time bucket, not the exact to=now() — else every request has a unique key
        // and the cache never hits (which is what made warm loads as slow as cold).
        $bucket = (int) floor(now()->timestamp / 20);
