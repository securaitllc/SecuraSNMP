...
class TwoFactorController extends Controller
{
    ...
    public function confirm(Request $request): JsonResponse
    {
        ...
        if (! TwoFactor::verifyForUser($user, $request->input('code'))) {
            throw ValidationException::withMessages(['code' => 'That code is not valid. Check your authenticator and try again.']);
        }

        // Recovery codes are shown ONCE here, in plaintext, and stored only as
        // hashes — a DB + key leak never yields usable codes.
        $plain = TwoFactor::recoveryCodes((int) config('mfa.recovery_codes', 10));
        $user->update([
    ...
    public function regenerateRecoveryCodes(Request $request): JsonResponse
    {
        ...
        if (! $user->hasTwoFactorEnabled() || ! Hash::check($request->input('password'), $user->password)) {
            throw ValidationException::withMessages(['password' => 'Password is incorrect.']);
        }

        $plain = TwoFactor::recoveryCodes((int) config('mfa.recovery_codes', 10));
        $user->update(['two_factor_recovery_codes' => array_map(fn ($c) => Hash::make($c), $plain)]);
