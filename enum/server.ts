 ...
 *
 * Returns a UNIQUE per-request path (or null when no keys). A shared, fixed path
 * would race: two concurrent lookups with different keys would clobber each other's
 * config between write and subfinder's read (TOCTOU), leaking one caller's key into
 * the other's run. The unique file is passed via `-pc` and deleted after the run.
 */
async function writeProviderConfig(keys: Record<string, string>): Promise<string | null> {
  const lines: string[] = []
  ...
  const path = `/tmp/pc-${randomUUID()}.yaml`
...
async function enumerate(domain: string, keys: Record<string, string>) {
  ...
  const pcPath = await writeProviderConfig(keys)
  // -timeout caps each passive source; -max-time caps subfinder's whole run (minutes),
  // so one slow source can't drag the enum out. Both must return within the app's budget.
  const sfCmd = ['subfinder', '-silent', '-d', domain, '-all', '-timeout', '8', '-max-time', '1']
  if (pcPath) sfCmd.push('-pc', pcPath)
  ...
  try {
    // Hard 15s cap per tool so the sidecar always returns inside the app's budget.
    const [sf, dx] = await Promise.all([
      run(sfCmd, 15000),
      run(['dnsx', '-silent', '-d', domain, '-w', WORDLIST, '-r', RESOLVERS, '-wd', domain], 15000),
    ])
    addLines(sf, 'subfinder')
    addLines(dx, 'dnsx-brute')
  } finally {
    if (pcPath) { try { await unlink(pcPath) } catch { /* best-effort key cleanup */ } }
  }
