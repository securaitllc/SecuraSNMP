...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Config versions (two, so the drift diff has something to show).
        $core = Device::where('name', 'demo-core-sw01')->first();
        if ($core) {
            $base = "set system host-name demo-core-sw01\nset interfaces ge-0/0/0 unit 0 family ethernet-switching vlan members SERVERS\nset vlans SERVERS vlan-id 10";
            DeviceConfig::create(['device_id' => $core->id, 'content' => $base, 'hash' => hash('sha256', $base), 'line_count' => 3, 'captured_at' => now()->subDay()]);
            $changed = str_replace('vlan-id 10', 'vlan-id 20', $base)."\nset system ntp server 10.10.1.1";
            DeviceConfig::create(['device_id' => $core->id, 'content' => $changed, 'hash' => hash('sha256', $changed), 'line_count' => 4, 'captured_at' => now()->subHours(2)]);
        }
        ...
        if ($jaxSw) {
            $endpoints = [
                ['ge-0/0/5', 'JAX-AP-Lobby', 'Juniper Mist AP43', 'ap'],
                ['ge-0/0/6', 'JAX-AP-Warehouse', 'Juniper Mist AP33', 'ap'],
                ['ge-0/0/12', 'Polycom-x50-1042', 'Polycom VVX 450 PoE Phone', 'phone'],
                ['ge-0/0/13', 'Polycom-x50-1043', 'Polycom VVX 450 PoE Phone', 'phone'],
                ['ge-0/0/14', 'Yealink-T54W', 'Yealink SIP-T54W', 'phone'],
            ];
            foreach ($endpoints as [$port, $name, $desc, $type]) {
                LldpNeighbor::create([
