...
use App\Http\Controllers\Api\DeviceHostnameController;
...
use App\Http\Controllers\Api\IpPrefixController;
use App\Http\Controllers\Api\IpReservationController;
...
use App\Http\Controllers\Api\NoraController;
...
use App\Http\Controllers\Api\SubnetReservationController;
...
Route::middleware(['auth:sanctum', 'active'])->group(function () {
    ...
    // Real polled uplink ports, so a circuit's WAN port is chosen, not typed.
    Route::get('/sites/{site}/wan-interfaces', [SiteController::class, 'wanInterfaces']);
    ...
    // Registered before the /devices/{device} wildcard so "hostname-drift" is not
    // read as a device id.
    Route::get('/devices/hostname-drift', [DeviceHostnameController::class, 'drift']);
    Route::post('/devices/adopt-hostnames', [DeviceHostnameController::class, 'adopt'])->middleware('role:admin');
    // The trail of what a device used to be — serial, name, model, OS.
    Route::get('/devices/hardware-changes', [DeviceHostnameController::class, 'changes']);
    ...
    // Put a device under maintenance / take it back out — a NOC action, analyst+.
    Route::post('/devices/{device}/maintenance', [DeviceMaintenanceController::class, 'store'])->middleware('role:analyst');
    Route::delete('/devices/{device}/maintenance', [DeviceMaintenanceController::class, 'destroy'])->middleware('role:analyst');
    ...
    Route::get('/mac-addresses', [MacAddressController::class, 'index']);

    // IPAM — read-only, derived from ARP, the switch FDB, devices and circuit subnets.
    Route::get('/ipam/ranges', [IpamController::class, 'ranges']);
    Route::get('/ipam/range', [IpamController::class, 'detail']);
    Route::get('/ipam/space', [IpamController::class, 'space']);
    Route::get('/ipam/reservations', [IpReservationController::class, 'index']);
    // Operator-recorded subnet masks. ARP cannot report a mask, so where the inferred
    // /24 is wrong (10.11.0.0 is a /23) it is corrected by hand.
    Route::get('/ipam/prefixes', [IpPrefixController::class, 'index']);
    // Whole blocks held for sites that do not exist yet — the only record on this page
    // that describes intent rather than what the network reports.
    Route::get('/ipam/subnet-reservations', [SubnetReservationController::class, 'index']);
    ...
    Route::get('/circuits/{circuit}/bandwidth', [CircuitController::class, 'bandwidth']);
    // One live SNMP counter read for the circuit's live bandwidth graph (no DB write).
    Route::post('/circuits/{circuit}/bandwidth-live', [CircuitController::class, 'bandwidthLive'])->middleware('throttle:60,1');
    ...
    // The trail behind the current number: every ticket raised on this circuit, what
    // it was raised for, by whom, and when it closed. Read-only, so viewer may see it.
    Route::get('/circuits/{circuit}/isp-tickets', [CircuitController::class, 'ispTicketHistory']);
    // Hop-by-hop path probes and loss episodes — what the carrier asks for. Reads
    // are open to every role; tracing on demand is a NOC action and rate-limited.
    Route::get('/circuits/{circuit}/traces', [CircuitController::class, 'traces']);
    Route::get('/circuits/{circuit}/loss-episodes', [CircuitController::class, 'lossEpisodes']);
    // Lumen NORA — email-driven carrier ticketing. Preview/history readable by any
    // role; sending and recording the reply are NOC actions.
    Route::get('/circuits/{circuit}/nora', [NoraController::class, 'history']);
    Route::get('/circuits/{circuit}/nora/preview', [NoraController::class, 'preview']);
    Route::post('/circuits/{circuit}/nora/send', [NoraController::class, 'send'])->middleware(['role:analyst', 'throttle:20,1']);
    Route::post('/circuits/{circuit}/nora/{noraRequest}/ticket', [NoraController::class, 'recordTicket'])->middleware('role:analyst');
    Route::post('/circuits/{circuit}/trace', [CircuitController::class, 'trace'])->middleware(['role:analyst', 'throttle:10,1']);
    ...
    Route::middleware('role:admin')->group(function () {
        // Recording an address by hand is a config action: the NAT pools and VIPs that
        // no protocol reports, without which the space reads as free.
        // Changing a mask re-groups every address inside it and re-bases capacity —
        // configuration, so admin, like the rest of IPAM's CRUD.
        Route::post('/ipam/prefixes', [IpPrefixController::class, 'store']);
        Route::put('/ipam/prefixes/{ipPrefix}', [IpPrefixController::class, 'update']);
        Route::delete('/ipam/prefixes/{ipPrefix}', [IpPrefixController::class, 'destroy']);
        Route::post('/ipam/subnet-reservations', [SubnetReservationController::class, 'store']);
        Route::put('/ipam/subnet-reservations/{subnetReservation}', [SubnetReservationController::class, 'update']);
        Route::post('/ipam/subnet-reservations/{subnetReservation}/release', [SubnetReservationController::class, 'release']);
        Route::delete('/ipam/subnet-reservations/{subnetReservation}', [SubnetReservationController::class, 'destroy']);
        Route::post('/ipam/reservations', [IpReservationController::class, 'store']);
        Route::put('/ipam/reservations/{ipReservation}', [IpReservationController::class, 'update']);
        Route::delete('/ipam/reservations/{ipReservation}', [IpReservationController::class, 'destroy']);
        ...
        // Dashboard basemap provider key (CARTO watermarks keyless tiles).
        Route::get('/map-settings', [MapSettingController::class, 'show']);
        Route::put('/map-settings', [MapSettingController::class, 'update']);
        Route::post('/map-settings/test', [MapSettingController::class, 'test'])->middleware('throttle:10,1');
        Route::post('/map-settings/flush', [MapSettingController::class, 'flush'])->middleware('throttle:6,1');
